...
function private report(situation, bot, position, traversaltarget) {
    ...
    /#
        if (isdefined(level.mit.traversal.var_31e45429b9998458)) {
            level.mit.traversal.var_31e45429b9998458 = level.mit.traversal.var_31e45429b9998458 + 1;
        } else {
            level.mit.traversal.var_31e45429b9998458 = 1;
        }
        function_f2bc01a7b0321f52("disable_movement", endposition, endposition + (0, 0, 100), traversaltarget, level.mit.traversal.var_31e45429b9998458);
    #/
}
...
function private function_a087eafdd1fd738a() {
    /#
        ...
        foreach (k, v in level.mit.traversal.traversalhistory) {
            outputstring = v.size + "<unknown string>" + k + "<unknown string>" + traversalposition[k] + "<unknown string>" + traversaltype[k] + "<unknown string>";
            outputstring = outputstring + string_join(v, "<unknown string>");
            for (i = 0; i < 3 - v.size; i++) {
                outputstring = outputstring + "<unknown string>";
            }
            outputlog("<unknown string>", outputstring);
        }
    #/
}
