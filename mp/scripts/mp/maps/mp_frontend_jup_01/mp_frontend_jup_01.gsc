...
function endlesslobbyfloor() {
    ...
    foreach (clutter in var_d84ba0bfd9897989) {
        clutter linkto(var_e1090eabdb435850);
    }
    foreach (clutter in var_80acb33ee101dd52) {
        clutter linkto(floor2);
    }
    var_ec1eda9f068079f = var_e1090eabdb435850.origin;
    var_295832d2fc0e963c = floor2.origin;
    ...
    while (true) {
        if (initial) {
            ...
            waitframe();
            floor2.origin = var_295832d2fc0e963c + (0, 0, -200);
            waitframe();
            floor2.origin = var_295832d2fc0e963c;
            floor2 show();
            floor2 moveto(var_ec1eda9f068079f, time);
...
function trial_moving_target_think() {
    ...
    if (!isdefined(self.mover_ends)) {
        ...
        self.mover_ends[0].origin = self.mover_ends[0].origin + mover_delta;
        self.mover_ends[1].origin = self.mover_ends[1].origin + mover_delta;
    }
    self.moveforward = 1;
    self.moving = 0;
