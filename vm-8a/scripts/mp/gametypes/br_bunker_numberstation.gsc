...
function printcodechosen(codestruct) {
    ...
    while (true) {
        /#
            printtoscreen2d(500, 400, "<dev string:x1c>", (1, 1, 1), 1);
        #/
        if (!scripts\mp\flags::gameflag("prematch_done")) {
            /#
                printtoscreen2d(600, 400, "<dev string:x2a>", (1, 1, 1), 1);
            #/
        } else {
...
function printcodeentered(codestruct) {
    ...
    while (true) {
        ...
        foreach (team, codegroup in codestruct.codeentered) {
            /#
                printtoscreen2d(500, 430 + teamindex * teamspacing, team + "<dev string:x43>", (1, 1, 1), 1);
            #/
            if (!array_contains(codestruct.successfulteams, team)) {
                ...
                /#
                    printtoscreen2d(700, 430 + teamindex * teamspacing, "<dev string:x6e>", (1, 1, 1), 1);
                #/
            }
            teamindex++;
        }
...
function debug_show2dvotext(text, time) {
    ...
    for (curtime = 0; curtime < time; curtime += 0.05) {
        /#
            printtoscreen2d(500, 350, "<dev string:x83>" + text, (1, 1, 1), 1.5);
        #/
        waitframe();
    }
}
    ...
    function function_138da425d10b4a04() {
        while (true) {
            ...
            if (getdvarint(@"hash_4e1e0a899a9ac76c", 0) == 1) {
                ...
                level notify("<dev string:xa1>");
                level notify("<dev string:xb9>");
            }
            wait 1;
        }
    }
