...
function init() {
    ...
    /#
        ...
        function_8f449f8c11c8ea84("<dev string:x1c>", "<dev string:x27>", [1, 5, 10, 15, 20, 35, 30, 35, 60], 30);
        function_8f449f8c11c8ea84("<dev string:x1c>", "<dev string:x45>", [5, 10, 30, 60, 120, 240, 360, 480, 600], 240);
    #/
    setupdom();
...
function domlocale_createquestlocale(placement) {
    ...
    /#
        domflag.debugtype = "<dev string:x5a>";
    #/
...
function domflag_onbeginuse(credit_player) {
    /#
        function_8ff83bdd5420ae28("<dev string:x5a>");
    #/
...
/#
    ...
    function function_34015dcd2a361807() {
        ...
        if (!isdefined(locale)) {
            strings[0] = "<dev string:x5f>";
            return strings;
        }
        if (!isdefined(locale.domflag)) {
            strings[0] = "<dev string:x69>";
            return strings;
        }
        originstr = scripts\mp\gametypes\br_ai_encounters_util::vartostring(locale.domflag.curorigin);
        strings[strings.size] = "<dev string:x75>" + originstr + "<dev string:x7e>" + distance(level.player.origin, locale.domflag.curorigin);
        teamsstr = "<dev string:x86>" + locale.subscribedinstances.size + "<dev string:x8e>";
        for (i = 0; i < locale.subscribedinstances.size; i++) {
            ...
            if (i < locale.subscribedinstances.size - 1) {
                teamsstr += "<dev string:x91>";
            }
        }
        strings[strings.size] = teamsstr + "<dev string:x93>";
        return strings;
    }
