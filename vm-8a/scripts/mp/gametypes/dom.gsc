...
function main() {
    ...
    if (scripts\cp_mp\utility\game_utility::function_21322da268e71c19() || issubstr(getgametype(), "snipers")) {
...
function flagsetup() {
    /#
        setdevdvarifuninitialized(@"hash_695e8f43f6b03cd4", "<dev string:x1c>");
    #/
    foreach (domflag in level.objectives) {
...
/#
    ...
    // Size: 0x632
    function domdebug() {
        spawnpoints = spawnlogic::getteamspawnpoints("<dev string:x1e>");
        fallbacks = spawnlogic::getteamfallbackspawnpoints("<dev string:x1e>");
        ...
        colors[128] = (1, 0, 1);
        while (true) {
            if (getdvar(@"hash_695e8f43f6b03cd4") != "<dev string:x23>") {
                wait 1;
                continue;
            }
            setdevdvar(@"hash_9d60ff019dc50c22", "<dev string:x23>");
            while (true) {
                if (getdvar(@"hash_695e8f43f6b03cd4") != "<dev string:x23>") {
                    setdevdvar(@"hash_9d60ff019dc50c22", "<dev string:x1c>");
                    break;
                }
                ...
                endpoints = [];
                endpoints[1] = [level.objectives["<dev string:x25>"]];
                endpoints[2] = [level.objectives["<dev string:x28>"]];
                endpoints[4] = [level.objectives["<dev string:x2b>"]];
                endpoints[8] = [level.objectives["<dev string:x2e>"]];
                endpoints[16] = [level.objectives["<dev string:x31>"]];
                endpoints[32] = [level.objectives["<dev string:x25>"], level.objectives["<dev string:x28>"]];
                endpoints[64] = [level.objectives["<dev string:x2b>"], level.objectives["<dev string:x28>"]];
                endpoints[128] = [level.objectives["<dev string:x25>"], level.objectives["<dev string:x2b>"]];
                foreach (spawnpoint in spawnpoints) {
                ...
                foreach (flaglocation in level.objectives) {
                    if (flaglocation == level.bestspawnflag["<dev string:x34>"]) {
                        print3d(flaglocation.trigger.origin + heightoffsetnames, "<dev string:x3b>");
                    }
                    if (flaglocation == level.bestspawnflag["<dev string:x1e>"]) {
                        print3d(flaglocation.trigger.origin + heightoffsetnames, "<dev string:x52>");
                    }
                }
                waitframe();
            }
...
/#
    ...
    function function_95b0959344794f91() {
        ...
        while (true) {
            if (getdvarint(@"hash_46a6b424af6acbc2") != 0) {
                scripts\mp\gamescore::giveteamscoreforobjective("<dev string:x34>", getdvarint(@"hash_46a6b424af6acbc2"), 1);
                break;
            } else if (getdvarint(@"hash_86e14326e43c0115") != 0) {
                scripts\mp\gamescore::giveteamscoreforobjective("<dev string:x1e>", getdvarint(@"hash_86e14326e43c0115"), 1);
                break;
            }
            wait 1;
        }
    ...
    function removedompoint() {
        self endon("<dev string:x67>");
        while (true) {
            if (getdvar(@"hash_6ce167f2cdc8ef7c", "<dev string:x72>") != "<dev string:x72>") {
                flaglabel = getdvar(@"hash_6ce167f2cdc8ef7c", "<dev string:x72>");
                foreach (domflag in level.objectives) {
                    if (isdefined(domflag.objectivekey) && domflag.objectivekey == flaglabel) {
                        domflag scripts\mp\gameobjects::allowuse("<dev string:x73>");
                        domflag.trigger = undefined;
                        domflag notify("<dev string:x78>");
                        domflag.visibleteam = "<dev string:x73>";
                ...
                setdynamicdvar(@"hash_6ce167f2cdc8ef7c", "<dev string:x72>");
            }
            wait 1;
        }
    }
    ...
    function placedompoint() {
        self endon("<dev string:x67>");
        while (true) {
            if (getdvar(@"hash_29d45d6822a1cf6d", "<dev string:x72>") != "<dev string:x72>") {
                flaglabel = getdvar(@"hash_29d45d6822a1cf6d", "<dev string:x72>");
                zone = spawnstruct();
                zone.origin = level.players[0].origin;
                zone.angles = level.players[0].angles;
                trigger = spawn("<dev string:x80>", zone.origin, 0, 120, 128);
                zone.trigger = trigger;
                zone.trigger.script_label = flaglabel;
                zone.ownerteam = "<dev string:x8f>";
                ...
                zone.origin = trace["<dev string:x97>"];
                zone.upangles = vectortoangles(trace["<dev string:xa0>"]);
                zone.forward = anglestoforward(zone.upangles);
                zone.right = anglestoright(zone.upangles);
                zone.visuals[0] = spawn("<dev string:xa7>", zone.origin);
                zone.visuals[0].angles = zone.angles;
                domflag = scripts\mp\gameobjects::createuseobject("<dev string:x8f>", zone.trigger, zone.visuals, (0, 0, 100));
                domflag scripts\mp\gameobjects::allowuse("<dev string:xb4>");
                ...
                domflag scripts\mp\gameobjects::setvisibleteam("<dev string:xba>");
                ...
                domflag.baseeffectpos = trace["<dev string:x97>"];
                upangles = vectortoangles(trace["<dev string:xa0>"]);
                ...
                level.objectives[domflag.objectivekey] = domflag;
                setdynamicdvar(@"hash_29d45d6822a1cf6d", "<dev string:x72>");
            }
            wait 1;
        }
    }
