...
// Size: 0x4bb
function function_ad1b6accbaab404e(node, var_fa9e859ca98dcdbf, keyname, highpriority, var_f1277f2577a53beb, var_d23fa479e1b4cae, overridesearchradius, var_46f10c379a78132a) {
    ...
    /#
        if (doors.size > 2) {
            println("<dev string:x1c>" + node.origin + "<dev string:x77>" + doors.size + "<dev string:x87>" + var_aa298d1ea65e8d98 + "<dev string:x98>" + var_6656dfa185b09b6f);
        }
    #/
    if (isdefined(doors)) {
        foreach (door in doors) {
            if (isdefined(door)) {
                ...
                if (istrue(door.var_67535222a761bdc5) && !istrue(highpriority)) {
                    println("<dev string:xa9>" + door.origin);
                    continue;
                }
...
function function_ed1aea6648bcdc28(scriptablename, keylevel) {
    assertex(isdefined(keylevel), "Key Level is undefined in scriptsmplocksandkeys::getLowerQualityKey");
        ...
        while (true) {
            if (getdvarint(@"hash_add2748481956623", 0) == 1) {
                ...
                foreach (item in lootitems) {
                    ...
                    if (isdefined(item.type) && (issubstr(item.type, "<dev string:x1ba>") || issubstr(item.type, "<dev string:x1c3>"))) {
                ...
                foreach (cache in cachelocs) {
                    ...
                    foreach (item in cacheitems) {
                        if (item == "<dev string:x11f>") {
                            var_e98075a1f0dfce41[var_e98075a1f0dfce41.size] = cache.origin;
                            continue;
                        }
                        if (issubstr(item, "<dev string:x1ba>") || issubstr(item, "<dev string:x1c3>")) {
