...
function setupzonecallouts() {
    ...
    assertex(isdefined(areaid), "Headquarters " + zonetrigger.targetname + " origin needs to be inside a callout area or the callout trigger and zone trigger need a common script_noteworthy");
}
...
function awardcapturepoints() {
    ...
    while (!level.gameended) {
        for (waittime = 0; waittime < seconds; waittime += level.framedurationseconds) {
...
/#
    ...
    function kothdebug() {
        while (true) {
            if (getdvar(@"hash_657c47f657a0023c") != "<dev string:xde>") {
                wait 2;
                continue;
            }
            while (true) {
            ...
            if (getdvarint(@"hash_c000ec9a5a2f4b6c") != 0) {
                ...
                foreach (zone in level.objectives) {
                    ...
                    for (i = 1; i < zone.visuals.size; i++) {
                        ...
                        zone.neutralhardpointfx[zone.neutralhardpointfx.size] = spawnfx(level.hardpointfxid["<dev string:x121>"], zone.visuals[i].baseorigin, fxfwd);
                    ...
                    zone scripts\mp\gameobjects::setvisibleteam("<dev string:x129>");
                }
            }
    ...
    function function_b4c2563f73b02c80() {
        ...
        while (true) {
            if (getdvarint(@"hash_46a6b424af6acbc2") != 0) {
                scripts\mp\gamescore::giveteamscoreforobjective("<dev string:x12d>", getdvarint(@"hash_46a6b424af6acbc2"), 0);
                break;
            } else if (getdvarint(@"hash_86e14326e43c0115") != 0) {
                scripts\mp\gamescore::giveteamscoreforobjective("<dev string:x134>", getdvarint(@"hash_86e14326e43c0115"), 0);
                break;
            }
            wait 1;
        }
