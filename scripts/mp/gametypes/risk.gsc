    ...
    /#
        ...
        while (true) {
            if (getdvar(@"hash_24d9f7f9324e9c61") != "dx_mpa_ustl_announcer_") {
                ...
                if (isdefined(level.tdmzones["C130_path_complete_axis"])) {
                    foreach (objective in level.tdmzones["C130_path_complete_axis"]) {
                        thread drawsphere(objective.origin, 20, 1, (1, 1, 1));
                    }
                }
                if (isdefined(level.tdmzones["iw8_ks_ac130_lp"])) {
                    foreach (objective in level.tdmzones["iw8_ks_ac130_lp"]) {
                        thread drawsphere(objective.origin, 20, 1, (1, 1, 1));
                    }
                }
                if (isdefined(level.tdmzones["C130_path_complete_"])) {
                    foreach (objective in level.tdmzones["C130_path_complete_"]) {
                        thread drawsphere(objective.origin, 20, 1, (1, 1, 1));
                    }
                }
            }
...
function function_e3a2a0c2e544728e(var_a4ab3e1ca5d220c4) {
    /#
        if (var_a4ab3e1ca5d220c4 <= 5) {
            ...
            safekey = "spawn_camera_idle" + var_a4ab3e1ca5d220c4 + "veh9_jltv";
            var_eb7299174b67818d = undefined;
            var_fd81d96696f0490a = undefined;
            if (isdefined(level.spawnselectionlocations[safekey]) && isdefined(level.spawnselectionlocations[safekey]["listenJump()"])) {
                var_eb7299174b67818d = level.spawnselectionlocations[safekey]["listenJump()"].spawnpoints;
            }
            if (isdefined(level.spawnselectionlocations[safekey]) && isdefined(level.spawnselectionlocations[safekey]["parachute()"])) {
                var_fd81d96696f0490a = level.spawnselectionlocations[safekey]["parachute()"].spawnpoints;
            }
