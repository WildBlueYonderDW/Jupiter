...
function function_9846df05d91ac5da() {
    /#
        ...
        if (value && isdefined(level.objectives)) {
            ...
            level.current_objectives[0].trigger notify("jup_military_secure_mode_station_01", player);
        }
    #/
}
...
function function_8ab4263516ea015f(currentobjective, var_b2be2b0ad85ea48) {
    ...
    /#
        if (getdvar(@"hash_ab8c774ce0027d59", "new_location") != "new_location") {
            forcedobjective = getdvar(@"hash_ab8c774ce0027d59", "new_location");
            if (forcedobjective == "objective_opened") {
                model = "any";
            }
            if (forcedobjective == "jup_uav_tower_enter") {
                model = "robj_interactzone";
            }
            if (forcedobjective == "start_defenders") {
                model = "game_end";
            }
            if (forcedobjective == "players") {
                model = "script_model";
            }
...
function onenduse(team, player, result, forcecall, var_930163bad6941f10) {
    ...
    if (result) {
        ...
        /#
            if (function_ec2935d47ad1f5ce()) {
                bombzone scripts/mp/gameobjects::allowuse("helicopter");
            }
        #/
...
function function_b66f9aef33ae7f2d(durationseconds) {
    ...
    while (durationprogress < durationseconds) {
        durationprogress = durationprogress + level.framedurationseconds;
