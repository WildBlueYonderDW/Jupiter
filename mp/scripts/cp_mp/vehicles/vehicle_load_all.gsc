#using script_1d1428a3b7b402b0;
#using script_248ec5040062d3ce;
#using script_261f1b574c15eab1;
#using script_2d3fead1b9e1d7e5;
#using script_2d4b9c58ed0e3d91;
#using script_30fea87d65917e79;
#using script_31629c16ddf56ebc;
#using script_320478327c55986c;
#using script_36967ce8ee2ea745;
#using script_37092055e9712f79;
#using script_3a535b44f9ecc6df;
#using script_46307eda1984556f;
#using script_470bc81def6a6980;
#using script_49314540c657d352;
#using script_4d03c33853d322ec;
#using script_5102b41ef5ab6d7e;
#using script_637cf6d1d76cf43b;
#using script_6388715b7706a618;
#using script_66e0e331f9783820;
#using script_69a61780e1cc21a9;
#using script_6bc507afe7a93a3;
#using script_7abaa04d5f9fc22e;
#using script_f6c4fc63efe1cdd;
#using scripts\common\utility;
#using scripts\engine\utility;
#using scripts\quaked\script_struct_mp_iw9_acv_6x6;
#using scripts\quaked\script_struct_mp_iw9_dirt_bike;
#using scripts\quaked\script_struct_mp_iw9_mrap;
#using scripts\quaked\script_struct_mp_iw9_palfa;

#namespace vehicle_load_all;

// Namespace vehicle_load_all / scripts\cp_mp\vehicles\vehicle_load_all
// Params 0, eflags: 0x0
// Checksum 0x0, Offset: 0x16f
// Size: 0xca
function function_b82404e40cca2415()
{
    level.var_1b4da6b781c9dfbd = 1;
    namespace_8624e3257d13b029::main();
    namespace_d8c2d739478db8bb::main();
    namespace_e6f83f35d32d1a60::main();
    namespace_405c71940c7ec9ea::main();
    namespace_9972b4aee3d673ca::main();
    namespace_15896d40cab0d429::main();
    namespace_f68bb0e7df415505::main();
    namespace_29f407445e2b0d95::main();
    scripts\quaked\script_struct_mp_iw9_palfa::main();
    namespace_65ad5249b19d5d41::main();
    namespace_a8b2611e1c2c0f9c::main();
    namespace_3568ab3cfd523b38::main();
    namespace_a8c01726524388f5::main();
    namespace_b835d284f1d90ddd::main();
    namespace_b18fdd4fa12ac75d::main();
    namespace_a5c3c313b4f6886a::main();
    namespace_8db7d778efdc3d65::main();
    namespace_c597e0c0ef10da36::main();
    namespace_1a5e9e1bbf009981::main();
    namespace_f2d76a94b27a2f46::main();
    namespace_ea79ad160580b4e7::main();
    namespace_b10949259c490766::main();
    scripts\quaked\script_struct_mp_iw9_mrap::main();
    scripts\quaked\script_struct_mp_iw9_acv_6x6::main();
    scripts\quaked\script_struct_mp_iw9_dirt_bike::main();
    namespace_19f9046b14b6bdf2::main();
    namespace_6258b6d1cdc40598::main();
}

