...
/#
    ...
    function function_ac138c84c0432491(params) {
        ...
        if (level.var_b4d422cd8fbde1ad) {
            function_6e7290c8ee4f558b("<dev string:x36>" + self.name + "<dev string:x5a>");
            foreach (s_pap in level.var_28967913115d1fa3) {
                function_b23a59dfb4ca49a1(s_pap.name, "<dev string:x5c>" + s_pap.name + "<dev string:x6d>" + self.name, &function_d1ffd6e50972869b);
            }
            function_fe953f000498048f();
            function_6e7290c8ee4f558b("<dev string:x6f>" + self.name + "<dev string:x5a>");
            foreach (s_pap in level.var_28967913115d1fa3) {
                function_b23a59dfb4ca49a1(s_pap.name, "<dev string:x98>" + s_pap.name + "<dev string:x6d>" + self.name, &function_5b1246d4c3b2bae8);
            }
            function_fe953f000498048f();
        }
    }
...
/#
    ...
    function function_3749f024c16fcc43() {
        ...
        function_a9a864379a098ad6("<dev string:xc2>", "<dev string:xd2>", &function_48d69eea9b7ed244, 1);
        function_a9a864379a098ad6("<dev string:xe2>", "<dev string:xf6>", &function_310eb57e9f489403);
        function_a9a864379a098ad6("<dev string:x10a>", "<dev string:x11d>", &function_6301b4b9186189ec);
        function_a9a864379a098ad6("<dev string:x130>", "<dev string:x145>", &function_9b340159cdb06093);
        function_fe953f000498048f();
        function_6e7290c8ee4f558b("<dev string:x15a>");
        function_a9a864379a098ad6("<dev string:x10a>", "<dev string:x11d>", &function_6301b4b9186189ec);
        function_a9a864379a098ad6("<dev string:x130>", "<dev string:x145>", &function_9b340159cdb06093);
        function_fe953f000498048f();
    }
...
function function_127acf3af63de42e() {
    assertex(isdefined(level.gamemodebundle.packapunchbundle), "Create a packapunch bundle asset to use in this gamemode and add to gamemodebundle");
...
function function_e9a23532607e421b(instance) {
    ...
    if (function_5e8f96f1bc95aacd()) {
        ...
        assertex(self.mdl_pap isscriptable(), "PAP Machine Xmodel needs to point to a valid scriptable!");
        self.mdl_pap.var_2fcb35d230365682 = function_8ffd6120b9e72251(self.mdl_pap.origin);
...
function function_9662c40edf345173(s_pap) {
    ...
    if (istrue(var_57c182cc0f81b65b)) {
        /#
            iprintlnbold(self.name + "<dev string:x19c>" + s_pap.name + "<dev string:x1a4>");
        #/
        ...
        foreach (player in a_players) {
            if (player ismeleeing() && !istrue(player.var_69103c55b4319619) && scripts\engine\utility::within_fov(player.origin, player.angles, self.origin, cos(70))) {
                ...
                if (istrue(player.var_f7d4ae1ceae3077d)) {
                    ...
                    /#
                        iprintlnbold("<dev string:x1b1>" + player.name);
                    #/
                } else {
                    player.var_f7d4ae1ceae3077d = 1;
                    /#
                        iprintlnbold("<dev string:x1d6>" + player.name);
                    #/
                }
            ...
            foreach (s_pap in level.var_28967913115d1fa3) {
                ...
                if (!isdefined(s_pap.s_destination)) {
                    if (isdefined(s_pap.str_destination)) {
                        s_pap.s_destination = getstruct(s_pap.str_destination, "<dev string:x1fc>");
                    } else {
                ...
                print3d(s_pap.s_destination.origin, "<dev string:x207>" + s_pap.str_destination, (0, 1, 0), 1, 0.25);
                print3d(v_position - var_39239d1f90650b * 1, "<dev string:x219>" + s_pap.name, (0, 1, 0), 1, 0.25);
                print3d(v_position - var_39239d1f90650b * 2, "<dev string:x224>" + default_to(s_pap.script_power, "<dev string:x231>"), (0, 1, 0), 1, 0.25);
                print3d(v_position - var_39239d1f90650b * 3, "<dev string:x232>" + ter_op(istrue(s_pap.var_cd756b37f6f0e181), "<dev string:x242>", "<dev string:x246>"), (0, 1, 0), 1, 0.25);
                print3d(v_position - var_39239d1f90650b * 4, "<dev string:x249>" + s_pap.str_destination, (0, 1, 0), 1, 0.25);
                foreach (player in level.players) {
                    if (isdefined(player.s_pap_machine)) {
                        print3d(player.origin, player.name + "<dev string:x257>" + player.s_pap_machine.name, (0, 1, 0), 1, 0.25);
                    }
                }
            }
            waitframe();
