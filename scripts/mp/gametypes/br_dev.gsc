...
function function_c96d7535862e245c(command, args) {
    /#
        ...
        case #"hash_d7404b430bfb6e59":
            firstplayer thread scripts/mp/hud_message::showsplash("<unknown string>");
            break;
        ...
        case #"hash_31f1df2db98384de":
            ...
            yaw = float(args[3]);
            firstplayer notify("<unknown string>", testloc);
            break;
                ...
                if (isdefined(level.var_2bec641aeb4e5980)) {
                    ...
                    for (i = 0; i < var_48094c816bbd08e0; i++) {
                        ...
                        if (objectiveiconid != -1) {
                            ...
                            scripts/mp/objidpoolmanager::objective_add_objective(objectiveiconid, "<unknown string>", origin, foreground);
        ...
        case #"hash_ba72800f31a3607c":
            ...
            functable["<unknown string>"] = &devgivemaxammo;
            functable["<unknown string>"] = &function_bc261c1a1cb59524;
            functable["<unknown string>"] = &function_28dc06904e18e469;
            functable["<unknown string>"] = &function_5377fd507f9ea057;
            functable["<unknown string>"] = &function_c41e8bac1d7f3bc8;
            functable["<unknown string>"] = &function_a2fdd1067da9ff6e;
            functable["<unknown string>"] = &function_26ab41fd72f1c761;
        ...
        case #"hash_1447f46e153fa1b6":
            ...
            functable["<unknown string>"] = &function_559b4cb8f64848b5;
            if (args[0] == "<unknown string>") {
                args = getarraykeys(functable);
            }
            foreach (type in args) {
        ...
        case #"hash_fa11c4f6bd516c22":
            ...
            if (args.size == 3) {
                team = args[1];
                index = int(args[2]);
                level.teamdata[team]["<unknown string>"][index] scripts/mp/gametypes/br_plunder::playerplunderpickup(ammount);
            } else if (args.size == 2) {
                team = args[1];
        ...
        case #"hash_b9253a7e780b5cc4":
            ...
            if (args[0] == "<unknown string>") {
                ...
                println("<unknown string>" + player.name + "<unknown string>" + tokencount);
            }
            break;
        ...
        case #"hash_87532133ab44aca2":
            lootid = undefined;
            platelevel = ter_op(args.size > 1 && isdefined(args[1]), args[1], "<unknown string>");
            if (!isdefined(args) || !isdefined(args[0]) || args[0] == "<unknown string>") {
                lootid = namespace_38b993c4618e76cd::function_fae5e1d3de32d3f7("<unknown string>" + platelevel);
            } else if (issubstr(args[0], "<unknown string>")) {
                lootid = namespace_38b993c4618e76cd::function_fae5e1d3de32d3f7("<unknown string>" + platelevel + "<unknown string>");
            } else if (issubstr(args[0], "<unknown string>")) {
                lootid = namespace_38b993c4618e76cd::function_fae5e1d3de32d3f7("<unknown string>" + platelevel + "<unknown string>");
            } else if (issubstr(args[0], "<unknown string>")) {
                lootid = namespace_38b993c4618e76cd::function_fae5e1d3de32d3f7("<unknown string>" + platelevel + "<unknown string>");
        ...
        case #"hash_a500569760927bb6":
            foreach (player in level.players) {
                ...
                player scripts/mp/menus::handleteamchange("<unknown string>", 3);
                break;
            }
            break;
        ...
        case #"hash_8673ffe7cf06c3b1":
            ...
            guards = function_7eb04ca16d045775("<unknown string>");
            firstguard = guards[0];
        ...
        case #"hash_cf9a6afff924db7d":
            ...
            if (args.size > 0) {
                foreach (arg in args) {
                    ...
                    case 1:
                        ...
                        takeitem = getsubstr(var_a6bf85cbfade3e88, 0, 2) == "<unknown string>";
                        ...
                        if (isdefined(playerkey)) {
                            ...
                            itemval = undefined;
                            if (istrue(takeitem)) {
                                arrkey = "<unknown string>";
                            } else {
                                arrkey = "<unknown string>";
                            }
                            if (istrue(var_47b75bbc48a0809a)) {
                                itemval = "<unknown string>";
            ...
            foreach (playerarr in itemdata) {
                foreach (item in ["<unknown string>", "<unknown string>"]) {
                    player = playerarr["<unknown string>"];
                    isperk = issubstr(item, "<unknown string>");
                    var_4aa12e0ed3f6b745 = issubstr(item, "<unknown string>");
                    if (array_contains(playerarr["<unknown string>"], item)) {
                        if (isperk) {
        ...
        case #"hash_8ffd6bd28ea1c5bc":
            if (args.size > 0) {
                equipref = args[0];
                var_c00e3162499a2e59 = function_94c116c69c037a47(equipref, ["<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>"]);
                ...
                case #"hash_93c71e7b6c0b81d7":
                    ...
                    grenadeweaponbundle = getscriptbundle("<unknown string>", grenadeweaponref);
                    break;
                ...
                if (isdefined(equipmentbundle)) {
                    ...
                    case #"hash_e534fd8ec73eafb4":
                        if (!isdefined(superref)) {
                            ...
                            superbundle = getscriptbundle(hashcat("<unknown string>", superref));
                        }
                        break;
                    }
                }
        ...
        case #"hash_a9ac0a5962cdb952":
            ...
            if (isfriendly) {
                teammates = scripts/mp/utility/teams::getteamdata(firstplayer.team, "<unknown string>");
                if (teammates.size > 1) {
                ...
                foreach (teamname, teamdata in level.teamdata) {
                    ...
                    teammembers = scripts/mp/utility/teams::getteamdata(teamname, "<unknown string>");
            ...
            if (isdefined(airstrikeowner)) {
                ...
                level scripts/common/callbacks::callback("<unknown string>", {streakinfo:streakinfo});
            }
            break;
...
function function_94c116c69c037a47(refname, typearr) {
    /#
        foreach (type in typearr) {
            bundlestr = type + "<unknown string>" + refname;
...
function function_1eaac96384e4a9d7(revivee) {
    /#
        ...
        notifycalled = revivee waittill_any_return_4("<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>");
        if ((notifycalled == "<unknown string>" || notifycalled == "<unknown string>") && isalive(revivee) && isdefined(revivee.revivetimems) && revivee.revivetimems > 0) {
            targetms = 3000;
            framecount = int(targetms / level.frameduration + 0.5);
            revivetimestr = "<unknown string>" + revivee.revivetimems + "<unknown string>";
            printtoscreen2d(800, 300, revivetimestr, (1, 1, 1), 2, framecount);
        }
        if (notifycalled != "<unknown string>") {
...
function function_5718089fc75809c3() {
    /#
        ...
        trace = scripts/engine/trace::_bullet_trace(start, end, 1, self);
        player = undefined;
...
function function_78ec4accff5f158b(player) {
    /#
        ...
        traces = scripts/engine/trace::ray_trace_get_all_results(eye, end);
...
function function_9bb9422480e6aa6d() {
    /#
        ...
        hudtext = function_1ce3276d3ef55a0a("<unknown string>", 0.8);
        hudtext setpoint("<unknown string>", "<unknown string>", 200, 160);
        hudtext setdevtext("<unknown string>");
        for (lastmessage = "<unknown string>"; true; lastmessage = nameinfo) {
            wait(0.1);
            if (!isdefined(level.players) || !level.players.size) {
                continue;
            }
            nameinfo = "<unknown string>";
            function_429110eb5af4a19c();
            foreach (teamname in level.teamswithplayers) {
                ...
                nameinfo = nameinfo + "<unknown string>" + teamname + "<unknown string>" + var_6428e68df37399d3 + "<unknown string>";
            }
...
function function_2729e9ab8a4ec44e() {
    /#
        ...
        hudtext = function_1ce3276d3ef55a0a("<unknown string>", 0.8);
        hudtext setpoint("<unknown string>", "<unknown string>", 20, 160);
        hudtext setdevtext("<unknown string>");
        for (lastmessage = "<unknown string>"; true; lastmessage = nameinfo) {
            waitframe();
            if (!isdefined(level.players) || !level.players.size) {
            ...
            function_429110eb5af4a19c();
            foreach (teamname in level.teamswithplayers) {
                nameinfo = nameinfo + "<unknown string>" + teamname + "<unknown string>";
                foreach (player in getteamarray(teamname)) {
                    ...
                    if (isplayeralive) {
                        nameinfo = nameinfo + "<unknown string>" + playername + "<unknown string>";
                        ...
                        nameinfo = nameinfo + "<unknown string>" + playername + "<unknown string>";
                        namecolor = ter_op(istrue(player.killcam), "<unknown string>", "<unknown string>");
                        spectatingplayer = player getspectatingplayer();
                        if (isdefined(spectatingplayer)) {
                            nameinfo = nameinfo + "<unknown string>" + namecolor + spectatingplayer.name + "<unknown string>";
                        }
                        if (isdefined(level.var_8f337a661bccb9f) && isdefined(level.var_8f337a661bccb9f[player.team])) {
                            nameinfo = nameinfo + "<unknown string>" + level.var_8f337a661bccb9f[player.team] + "<unknown string>";
                        }
                    } else {
                        nameinfo = nameinfo + "<unknown string>" + playername + "<unknown string>";
                    }
                    nameinfo = nameinfo + "<unknown string>";
                }
            }
...
function function_9cbddfdef8f1125b(bunkerindex) {
    /#
        ...
        while (true) {
            message, value = player waittill("<unknown string>");
...
function function_fe99ab4dc900865e() {
    /#
        killtriggers = getentarray("<unknown string>", "<unknown string>");
...
function function_d671c39a7867f243(args) {
    /#
        ...
        org = scripts/engine/trace::sphere_trace(origin + (0, 0, var_e25c6801a01f7fce), origin + (0, 0, var_befd185e0007a7b8), var_e89346b6c5bfd043, undefined, contents)["<unknown string>"];
        checkheight = org[2] - var_fe52faf102ebf57b;
        count = 0;
        countglobal = 0;
        total = pow(2 * resolution / precision, 2) * 1;
        for (x = -1 * resolution; x <= resolution; x = x + precision) {
            host iprintlnbold("<unknown string>" + int(countglobal / total * 100));
            for (y = -1 * resolution; y <= resolution; y = y + precision) {
                countglobal++;
        ...
        host iprintlnbold("<unknown string>" + level.holesa.size + "<unknown string>" + level.var_ca369ce88e799edd.size + "<unknown string>" + level.var_fce03ebb07aa6a47.size);
    #/
}
...
function function_237b71faf9fb62d9(player, contractref) {
    /#
        ...
        if (isdefined(trace["<unknown string>"]) || isalive(trace["<unknown string>"])) {
            namespace_1eb3c4e0e28fac71::function_22671614e8f79513(trace["<unknown string>"], contractref);
            return;
        }
...
function function_9503654ca3380f8a(args) {
    /#
        ...
        if (args.size > 0) {
            ...
            skins = ["<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>"];
...
function function_548e30d6072b85e9(player, amount) {
    /#
        var_affbd0043656d928 = ["<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>", "<unknown string>"];
        ...
        dropinfo = scripts/mp/gametypes/br_pickups::getitemdroporiginandangles(dropstruct, player.origin, player.angles, player);
        scripts/mp/gametypes/br_pickups::spawnpickup("<unknown string>", dropinfo);
