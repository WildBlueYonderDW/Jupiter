...
#namespace spawner_scoring;

// Namespace spawner_scoring / namespace_74cdb67d92596cfe
// Params 0, eflags: 0x0
...
function spawner_critical_factors(spawnpoint, respawn, respawn_ent) {
    ...
    if (isdefined(spawnpoint.script_checkpoint)) {
        checkpoints = strtok(spawnpoint.script_checkpoint, ",");
        foreach (checkpoint in checkpoints) {
            if (scripts\cp\cp_checkpoint::function_cd10e09e02e4eb07(checkpoint)) {
                /#
                    if (print_critical) {
                        add_to_score_message(failed_message, "<dev string:x2f0>" + checkpoint + "<dev string:x315>", undefined, undefined, 1, spawnpoint);
                    }
                #/
                return 0;
            }
    ...
    if (istrue(respawn)) {
        if (critical_factor(&invalid_for_teleport, spawnpoint)) {
            /#
                if (print_critical) {
                    add_to_score_message(failed_message, "<dev string:x320>", undefined, undefined, 1, spawnpoint);
                }
            #/
            return 0;
            ...
            if (istrue(spawnpoint.is_on_platform)) {
                /#
                    if (print_critical) {
                        add_to_score_message(failed_message, "<dev string:x33f>", undefined, undefined, 1, spawnpoint);
                    }
                #/
            } else if (isdefined(spawnpoint.noteleport)) {
                /#
                    if (print_critical) {
                        add_to_score_message(failed_message, "<dev string:x359>", undefined, undefined, 1, spawnpoint);
                    }
                #/
            }
            return 0;
            ...
            /#
                if (print_critical) {
                    add_to_score_message(failed_message, "<dev string:x378>", undefined, undefined, 1, spawnpoint);
                }
            #/
            return 0;
        }
    ...
    if (critical_factor(&is_level_escalation_sufficient, spawnpoint)) {
        /#
            if (print_critical) {
                add_to_score_message(failed_message, "<dev string:x398>", undefined, undefined, 1, spawnpoint);
            }
        #/
        return 0;
    }
    if (critical_factor(&are_weapons_free, spawnpoint)) {
        /#
            if (print_critical) {
                add_to_score_message(failed_message, "<dev string:x3b1>", undefined, undefined, 1, spawnpoint);
            }
        #/
        return 0;
    }
    if (!critical_factor(&avoidtelefrag, spawnpoint)) {
        /#
            if (print_critical) {
                add_to_score_message(failed_message, "<dev string:x3d7>", undefined, undefined, 1, spawnpoint);
            }
        #/
        return 0;
    }
...
function get_best_scoring_target(var_bdf059129bdd23a3) {
    ...
    result = function_9a9cc1b12c2b79b5(scoring_array, var_bdf059129bdd23a3);
    ...
    if (isplayer(score_target)) {
        if (isdefined(level.recondronesupers) && level.recondronesupers.size) {
            ...
            for (i = 0; i < level.recondronesupers.size; i++) {
                if (isdefined(level.recondronesupers[keys[i]].owner) && level.recondronesupers[keys[i]].owner == score_target) {
                    return level.recondronesupers[keys[i]].origin;
                }
            }
        }
        if (isdefined(score_target.velo_forward) && score_target should_use_velo_forward()) {
            return score_target.velo_forward;
        } else {
            forward = anglestoforward(score_target.angles);
            return (score_target.origin + forward * 1024);
        }
        return;
    }
    return score_target.origin;
}
...
/#
    ...
    function function_c0f307cf69beb25e(spawnpoints, validspawnpoints, respawn) {
        ...
        if (!isdefined(level.spawner_model)) {
            level.spawner_model = spawn("<dev string:x43f>", (0, 0, 0));
        }
        ...
        while (true) {
            ...
            if (isdefined(validspawnpoints) && validspawnpoints.size > 0) {
                ...
                if (new_spawner) {
                    ...
                    case #"hash_89eb9062c8daa874":
                        level.spawner_model setmodel("<dev string:x45c>");
                        break;
                    case #"hash_abe4b0e0b6ed2786":
                        level.spawner_model setmodel("<dev string:x495>");
                        break;
                    default:
                        level.spawner_model setmodel("<dev string:x4a4>");
                        break;
                    }
                    if (istrue(last_spawner.var_5c66d2472e564b8f)) {
                        scripts\cp\cp_outline::enable_outline_for_players(level.spawner_model, level.players, "<dev string:x4bb>");
                    } else {
                        scripts\cp\cp_outline::enable_outline_for_players(level.spawner_model, level.players, "<dev string:x4cf>");
                    }
                }
                ...
                self.var_d2a78b73684031d9 = [];
                last_spawner = undefined;
                level.spawner_model setmodel("<dev string:x4e5>");
                level.spawner_model dontinterpolate();
...
// Size: 0xdc
function function_9a9cc1b12c2b79b5(scoring_array, var_bdf059129bdd23a3) {
...
function score_ai_spawns(spawnpoints, respawn, var_c62b3b643f30bddf, respawn_ent, var_bdf059129bdd23a3) {
    ...
    var_9ed7cd9678338a4e = get_score_target_pos(score_target);
                ...
                if (scripts\cp\cp_spawning_util::cp_is_point_in_front(level.players[j].origin) == compare) {
                    if (isdefined(self.script_dist_only)) {
                        if (distancesquared(level.players[j].origin, self.origin) <= int(self.script_dist_only)) {
                            thread namespace_5729d24318b60bcd::disable_spawn_point(self, undefined, group_struct);
                            return false;
                        }
                        continue;
                    }
...
function calculate_ai_cluster_spawner_score(spawnpoint, player_index) {
    ...
    if (!isdefined(scorefactor)) {
        /#
            message = add_to_score_message(failed_message, "<dev string:x4f4>", undefined, undefined, 1, spawnpoint);
        #/
        return false;
    }
    /#
    ...
    if (!isdefined(var_8766534be41302a9)) {
        /#
            message = add_to_score_message(failed_message, "<dev string:x508>", undefined, undefined, 1, spawnpoint);
        #/
        return false;
    }
    /#
...
function calculate_ai_spawner_score(spawnpoint, player_index, debug, spawnpoints) {
    ...
    if (!isdefined(scorefactor)) {
        /#
            message = add_to_score_message(failed_message, "<dev string:x53b>", undefined, undefined, 1, spawnpoint);
        #/
        if (istrue(debug)) {
            scorefactor = -10000;
        } else {
    ...
    if (!isdefined(scorefactor)) {
        /#
            message = add_to_score_message(failed_message, "<dev string:x559>", undefined, undefined, 1, spawnpoint);
        #/
        if (istrue(debug)) {
            scorefactor = -10000;
        } else {
    ...
    if (!isdefined(scorefactor)) {
        /#
            message = add_to_score_message(failed_message, "<dev string:x569>", undefined, undefined, 1, spawnpoint);
        #/
        return false;
    }
    /#
    ...
    if (!isdefined(scorefactor)) {
        /#
            message = add_to_score_message(failed_message, "<dev string:x57b>", undefined, undefined, 1, spawnpoint);
        #/
        if (istrue(debug)) {
            scorefactor = -10000;
        } else {
    ...
    if (!isdefined(scorefactor)) {
        /#
            message = add_to_score_message(failed_message, "<dev string:x592>", undefined, undefined, 1, spawnpoint);
        #/
        if (istrue(debug)) {
            scorefactor = -10000;
        } else {
...
function is_spawner_ideal_distance(spawnpoint, player_index) {
    ...
    if (isdefined(spawnpoint.ideal_dist)) {
        ...
        var_aab0a592cd72d6f5 = p_dist * p_dist;
    }
    ...
    function function_b7d0a51460fc8fb3(valid_spawnpoints, var_c62b3b643f30bddf) {
        ...
        for (i = 0; i < valid_spawnpoints.size; i++) {
            ...
            if (isdefined(result) && !isdefined(result.spawner_debug_model)) {
                spawner_model = spawn("<dev string:x43f>", result.origin);
                spawn_scoring_type = result get_spawn_scoring_type();
                switch (spawn_scoring_type) {
                case #"hash_89eb9062c8daa874":
                    spawner_model setmodel("<dev string:x45c>");
                    break;
                case #"hash_abe4b0e0b6ed2786":
                    spawner_model setmodel("<dev string:x495>");
                    break;
                default:
                    spawner_model setmodel("<dev string:x4a4>");
                    break;
                }
