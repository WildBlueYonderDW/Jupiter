...
function private function_d13d0f1fa5d13556(bountysettings, playerentering) {
    /#
        iprintlnbold("<dev string:x1c>" + self.name + "<dev string:x35>" + playerentering.name);
    #/
}
...
function private function_acd85be28950f9f8(bountysettings, playerexiting) {
    /#
        iprintlnbold("<dev string:x39>" + self.name + "<dev string:x35>" + playerexiting.name);
    #/
}
...
function private function_612c88cb58973b43(bountysettings) {
    /#
        ...
        if (istrue(bountysettings.success)) {
            iprintlnbold("<dev string:x51>" + bountysettings.context);
            return;
        }
        iprintlnbold("<dev string:x6b>" + bountysettings.context);
    #/
}
...
/#
    ...
    function private function_85816e9363c64afc() {
        ...
        level waittill("<dev string:x8f>");
        while (true) {
            command = getdvar(@"hash_a42341ed9ee97770", "<dev string:xa4>");
            ...
            setdvar(@"hash_a42341ed9ee97770", "<dev string:xa4>");
            waitframe();
        }
    }
