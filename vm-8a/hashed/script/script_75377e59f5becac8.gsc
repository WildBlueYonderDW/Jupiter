    ...
    foreach (pack in self.packs) {
        ...
        /#
            if (istrue(var_df94e780c2b3c994)) {
                iprintlnbold("<dev string:x6a>" + pack.anime);
            }
        #/
        if (pack.islooping) {
            function_583f118c4064ca21(level.infilstruct.players);
...
/#
    ...
    function function_b0f7742c6d38dbd1() {
        ...
        entright = -1 * entaxis["<dev string:x78>"];
        entup = entaxis["<dev string:x7e>"];
        foreach (player in level.infilstruct.players) {
            if (!isdefined(player)) {
                continue;
            }
            ...
            println("<dev string:x81>" + slotnumber + "<dev string:x9b>" + offset[0] + "<dev string:x9e>" + offset[1] + "<dev string:x9e>" + offset[2] + "<dev string:xa0>");
            ...
            println("<dev string:xa2>" + slotnumber + "<dev string:xbb>" + cameradistance + "<dev string:xc8>" + var_abdc396b57c7f955 + "<dev string:xa0>");
            ...
            println("<dev string:xd0>" + slotnumber + "<dev string:xec>" + var_629f52c3d96c0cb9 + "<dev string:xf6>" + var_867d9818ce7f2161 + "<dev string:xa0>");
        }
    }
...
/#
    ...
    function private function_710d2b779609907d() {
        ...
        while (true) {
            level.var_ca629cf434263639 = 0;
            while (hostplayer buttonpressed("<dev string:x110>")) {
                waitframe();
            }
            while (!hostplayer buttonpressed("<dev string:x110>")) {
                waitframe();
    ...
    function function_b51365f787d44355() {
        ...
        while (true) {
            while (!hostplayer buttonpressed("<dev string:x11b>")) {
                waitframe();
            }
            hostplayer scripts\cp_mp\utility\player_utility::function_6fb380927695ee76();
            hostplayer scripts\cp_mp\utility\player_utility::function_985b0973f29da4f8("<dev string:x123>");
            ...
            hostplayer scripts\cp_mp\utility\player_utility::function_379bb555405c16bb("<dev string:x123>");
    ...
    function private function_a93a090c732f587b() {
        c130pathstruct = undefined;
        if (scripts\mp\gametypes\br_gametypes::isbrgametypefuncdefined("<dev string:x145>")) {
            c130pathstruct = scripts\mp\gametypes\br_gametypes::runbrgametypefunc("<dev string:x145>");
        } else {
    ...
    function function_faabc6943f92e6ac() {
        ...
        while (function_136f4218b1d719b9()) {
            function_9384c9ed097bf7b9(hostplayer, "<dev string:x110>");
            ...
            if (istrue(level.var_7647df14c1f116f7)) {
                ...
                function_9384c9ed097bf7b9(hostplayer, "<dev string:x15a>");
    ...
    function testdvarloop() {
        ...
        setdevdvarifuninitialized(@"hash_2f5821f46995d1e2", "<dev string:x164>");
