...
function init() {
    ...
    /#
        ...
        function_8f449f8c11c8ea84("waypoint_contested_br", "MP_INGAME_ONLY/OBJ_DEFENDING_CAPS", [1, 5, 10, 15, 20, 35, 30, 35, 60], 30);
        function_8f449f8c11c8ea84("waypoint_contested_br", "<unknown string>", [5, 10, 30, 60, 120, 240, 360, 480, 600], 240);
    #/
    setupdom();
    /#
...
function function_34015dcd2a361807() {
    /#
        ...
        strings[strings.size] = "<unknown string>" + originstr + "<unknown string>" + distance(level.player.origin, locale.domflag.curorigin);
        teamsstr = "<unknown string>" + locale.subscribedinstances.size + "<unknown string>";
        for (i = 0; i < locale.subscribedinstances.size; i++) {
            teamsstr = teamsstr + locale.subscribedinstances[i].team;
            if (i < locale.subscribedinstances.size - 1) {
                teamsstr = teamsstr + "<unknown string>";
            }
        }
        strings[strings.size] = teamsstr + "<unknown string>";
        return strings;
    #/
}
