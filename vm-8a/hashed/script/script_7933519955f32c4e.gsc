...
/#
    ...
    function function_36ebdcea4a9d96ce(infil_anim_type) {
        ...
        var_b37140bfa8ba2461 = spawn("<dev string:x10f>", getdvarvector(@"hash_223915c6339dbc10", (0, 0, 0)));
        var_b37140bfa8ba2461 setmodel("<dev string:x11c>");
        var_b37140bfa8ba2461.cleanme = 1;
        var_b37140bfa8ba2461.innards = spawn("<dev string:x10f>", getdvarvector(@"hash_223915c6339dbc10", (0, 0, 0)));
        var_b37140bfa8ba2461.innards setmodel("<dev string:x144>");
        var_b37140bfa8ba2461.innards linkto(var_b37140bfa8ba2461, "<dev string:x169>");
...
/#
    ...
    function function_ec887b61b1baeba8(host) {
        animstruct = buildac130infilanimstruct(undefined, "<dev string:x10f>");
        models = [animstruct.cameraent, animstruct.staticc130, animstruct.playerpositionents["<dev string:x16a>"], animstruct.playerslot1, animstruct.playerslot2, animstruct.playerslot3, animstruct.playerslot4, animstruct.aidoorchief];
        models = array_removeundefined(models);
        while (function_136f4218b1d719b9()) {
            function_9384c9ed097bf7b9(host, "<dev string:x171>");
            if (!function_136f4218b1d719b9()) {
                break;
            }
            ...
            transitionplayerstoac130cinematic([host], "<dev string:x10f>", animstruct.staticc130);
            thread playac130animinternal(animstruct);
            function_9384c9ed097bf7b9(host, "<dev string:x17c>");
            endanimsequence(host, animstruct, models);
            endac130infilanimsinternal(animstruct);
            transitionplayersoutofac130cinematic([host], "<dev string:x10f>");
    ...
    function endanimsequence(player, animstruct, models) {
        ...
        animstruct notify("<dev string:x186>");
    ...
    function function_710d2b779609907d(player, models, animstruct) {
        animstruct endon("<dev string:x186>");
        while (true) {
            level.var_ca629cf434263639 = 0;
            while (player buttonpressed("<dev string:x171>")) {
                waitframe();
            }
            while (!player buttonpressed("<dev string:x171>")) {
                waitframe();
    ...
    function function_b51365f787d44355(player, animstruct) {
        ...
        while (true) {
            while (!player buttonpressed("<dev string:x190>") || !isdefined(animstruct.cameraentlinktag)) {
                waitframe();
            }
            player scripts\cp_mp\utility\player_utility::function_6fb380927695ee76();
            player scripts\cp_mp\utility\player_utility::function_985b0973f29da4f8("<dev string:x198>");
            player cameraunlink();
            player disablephysicaldepthoffieldscripting();
            player _freezelookcontrols(0);
            ...
            while (player buttonpressed("<dev string:x190>")) {
                waitframe();
            }
            while (!player buttonpressed("<dev string:x190>")) {
                waitframe();
            ...
            player cameralinkto(animstruct.cameraent, animstruct.cameraentlinktag, 1, 1);
            player scripts\cp_mp\utility\player_utility::function_a593971d75d82113();
            player scripts\cp_mp\utility\player_utility::function_379bb555405c16bb("<dev string:x198>");
            player enablephysicaldepthoffieldscripting();
            while (player buttonpressed("<dev string:x190>")) {
                waitframe();
            }
    ...
    function function_77108c1c4c1defef(func, timer, param1, param2, param3, param4, param5, param6, param7, param8, param9, param10, param11, param12) {
        self endon("<dev string:x1ba>");
        self endon("<dev string:x1c5>");
    ...
    function function_b0f7742c6d38dbd1(animstruct) {
        ...
        entright = -1 * entaxis["<dev string:x1cb>"];
        entup = entaxis["<dev string:x1d1>"];
        foreach (player in level.players) {
            slotnumber = player.infilanimindex - 1;
            prop_name = "<dev string:x1d4>" + player.infilanimindex;
            ...
            println("<dev string:x1dc>" + slotnumber + "<dev string:x1f6>" + offset[0] + "<dev string:x1f9>" + offset[1] + "<dev string:x1f9>" + offset[2] + "<dev string:x1fb>");
            ...
            println("<dev string:x1fd>" + slotnumber + "<dev string:x216>" + cameradistance + "<dev string:x223>" + var_abdc396b57c7f955 + "<dev string:x1fb>");
            ...
            println("<dev string:x22b>" + slotnumber + "<dev string:x247>" + var_629f52c3d96c0cb9 + "<dev string:x251>" + var_867d9818ce7f2161 + "<dev string:x1fb>");
        }
    }
