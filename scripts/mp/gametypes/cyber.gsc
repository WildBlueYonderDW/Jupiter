...
function data_center_sfx_loop() {
    ...
    ent stoploopsound("data_center_cyber_lp");
    waitframe();
    ent delete();
}
...
function bombplanted(destroyedobj, team) {
    ...
    playsoundatpos(explosionorigin + (0, 0, 15), "iw9_exp_cyber_emp_close");
...
function bomb_pre_exp_sfx_wait(sfx_org) {
    ...
    playsoundatpos(sfx_org + (0, 0, 15), "iw9_exp_cyber_emp_preexplode");
}
...
/#
    ...
    function function_4df61dc99c28632b() {
        ...
        while (getdvarint(@"hash_d51785ab78729b60", 0) == 1) {
            ...
            rot = randomfloat(360);
            explosioneffect = spawnfx(level._effect["<dev string:x191>"], explosionorigin + (0, 0, 50), (0, 0, 1), (cos(rot), sin(rot), 0));
            triggerfx(explosioneffect);
            playrumbleonposition("<dev string:x1a3>", explosionorigin);
            earthquake(0.6, 1.5, explosionorigin, 10000);
            level thread empjamandrumbleclients(1);
            if (game["<dev string:x1b7>"]) {
                exploderindex = 200;
            } else {
                exploderindex = 201;
            }
