...
function playerwelcomesplashes(data) {
    ...
    #/
    level thread scripts/mp/utility/teams::calculatenumteamswithplayers();
    ...
    while (true) {
        ...
        if (timeremaining <= 0 && !var_26482caeea97c1f9) {
            ...
            if (isplayer(var_39e8f33513c716cd)) {
                var_39e8f33513c716cd scripts/cp_mp/killstreaks/nuke::tryusenuke();
                wait(10);
...
function function_34e46d26df79fdad() {
    ...
    scripts/mp/gametypes/br_analytics::function_9d458fb3a5d1e86a(scripts/mp/gametypes/br_plunder::getteamplunder(self.team) * 10, game["teamPlacements"][self.team], totalcash, self.var_db548fc480c33eac[0], self.var_db548fc480c33eac[1], self.var_db548fc480c33eac[2], self.var_db548fc480c33eac[3], self.var_db548fc480c33eac[4]);
    if (istrue(level.var_db813db0fa91a0ce)) {
        ...
        level.var_e224ccaf8102eea7[team].cashtotal = level.var_e224ccaf8102eea7[team].cashtotal + totalcash;
        ...
        level.var_e224ccaf8102eea7[team].contracttotal = level.var_e224ccaf8102eea7[team].contracttotal + self.var_db548fc480c33eac[1];
        ...
        level.var_e224ccaf8102eea7[team].ondeathtotal = level.var_e224ccaf8102eea7[team].ondeathtotal + self.var_db548fc480c33eac[3];
        level.var_e224ccaf8102eea7[team].purchasetotal = level.var_e224ccaf8102eea7[team].purchasetotal + self.var_db548fc480c33eac[4];
...
function hackclientomnvarclamp(omnvar, val) {
    ...
    /#
        assertex(val == valclamp, "plunderRankUpdate() passed invalid omnvar value of: " + val);
    #/
...
function plunderrankupdate() {
    ...
    while (true) {
        ...
        foreach (team in level.teamnamelist) {
            ...
            if (pockets < 0) {
                /#
                    println("spawned_player" + plundercount + "gamestate_75_perc_enemy" + bankedplunder + "ui_br_timed_feature_end_time" + pockets);
                #/
                pockets = 0;
            }
...
function playerdropplunderondeath(dropstruct, attacker) {
    ...
    if (istrue(self.isdisconnecting)) {
        ...
        if (istrue(self.wasexecuted)) {
            ...
            if (scripts/mp/gametypes/br_publicevents::ispubliceventoftypeactive(12)) {
                executionmod = executionmod * level.br_pe_data[12].var_ee5fb771f83add5e;
            }
            finalmod = finalmod * executionmod;
        }
        if (istrue(level.bmoovertime) && (!isdefined(attacker) || self != attacker)) {
            finalmod = finalmod * level.overtimecashmultiplier;
        }
        var_b64f283113c99581 = var_b64f283113c99581 * finalmod;
        var_b64f283113c99581 = var_b64f283113c99581 + bonuscash;
...
function updatelootleadermarks(playerscores, playersorted, playerlookup, teamsorted) {
    ...
    for (i = 0; i < level.lootleadermarkcount; i++) {
        ...
        if (isdefined(level.lootleaders[i])) {
            ...
            showtoplayers = scripts/mp/utility/teams::getenemyplayers(instance.targetplayer.team, 0);
            foreach (player in showtoplayers) {
                instance function_cfd53c8f6878014f(player);
            }
            var_1e502b02d01cf98d = scripts/mp/utility/teams::getfriendlyplayers(instance.targetplayer.team, 0);
...
function packextrascore0(data) {
    ...
    if (isdefined(self.missionparticipation)) {
        missionvalue = missionvalue + self.missionparticipation;
    }
    if (missionvalue > 15) {
        missionvalue = 15;
    }
            ...
            if (getdvarint(@"hash_73759683c80b1a3d", 0) != 0) {
                ...
                foreach (player in level.players) {
                    if (isalive(player) && player != host && player.team != host.team) {
                        ...
                        level.teamdata[targetteam]["ui_nuke_end_milliseconds"] = int(wintarget - 100);
                        break;
                    }
                }
                wait(15);
                level.teamdata[targetteam]["ui_nuke_end_milliseconds"] = int(wintarget);
            }
...
function printallteamsplundercount() {
    /#
        wait(1);
        wintarget = getwincost();
        teamsorted = tablesort(game["Team Plunder Total: { Team: "], "75pf_grav_bmgm");
        for (i = 0; i < teamsorted.size - 1; i++) {
            if (isdefined(level.teamdata[teamsorted[i]]["ui_nuke_end_milliseconds"])) {
                teamplunder = scripts/mp/gametypes/br_plunder::getteamplunder(teamsorted[i]) * 10;
                println("gamestate_90_perc" + teamsorted[i] + "ldls_grav_bmgm" + teamplunder);
            }
        }
    #/
}
