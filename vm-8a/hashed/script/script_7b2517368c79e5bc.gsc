...
function function_d59d110ccf0f5b8b(dropbundle, dropcount, droporigin, dropangles, var_14c85b39207719b8, var_57266f5ca6feebd7, payload1, payload2) {
    ...
    /#
        if (!isscriptabledefined(scriptablename)) {
            assertmsg("<dev string:x6b>" + dropbundle.ref + "<dev string:x9f>" + function_3c8848a3a11b2553(scriptablename) + "<dev string:xc3>");
            return undefined;
        }
    #/
...
function private function_754c0f10a7b580ad(listbundlename) {
    ...
    foreach (itementry in itemlistbundle.itementries) {
        ...
        if (error) {
            switch (error) {
            case 1: 
                println("<dev string:xdd>" + listbundlename + "<dev string:xfd>");
                break;
            case 2: 
    ...
    foreach (itemlistentry in itemlistbundle.itemlistentries) {
        ...
        if (error) {
            switch (error) {
            case 1: 
                println("<dev string:xdd>" + listbundlename + "<dev string:x11b>");
                break;
            case 2: 
...
function private function_a6275c9ae7ae1bda(itembundlename) {
    ...
    if (isdefined(itembundle.scriptable)) {
        if (!isdefined(level.commonitem.var_ec08d293dcc1ea4f[itembundle.scriptable])) {
            ...
            /#
                print("<dev string:x13e>" + level.commonitem.var_ec08d293dcc1ea4f[itembundle.scriptable] + "<dev string:x179>" + itembundlename + "<dev string:x181>" + function_3c8848a3a11b2553(itembundle.scriptable) + "<dev string:x18d>");
            #/
        }
    }
    if (isdefined(itembundle.ref)) {
        if (!isdefined(level.commonitem.var_970a8dd2fe7c510b[itembundle.ref])) {
            ...
            /#
                print("<dev string:x13e>" + level.commonitem.var_970a8dd2fe7c510b[itembundle.ref] + "<dev string:x179>" + itembundlename + "<dev string:x181>" + itembundle.ref + "<dev string:x1d3>");
            #/
        }
    }
    return 0;
...
function function_1749413ee00b69f(scriptablename) {
    if (!isdefined(scriptablename) || !function_f0c15201c9f7e1db(scriptablename)) {
        /#
            if (!isdefined(scriptablename)) {
                assertmsg("<dev string:x20b>");
            } else if (isstring(scriptablename)) {
                assertmsg("<dev string:x261>" + scriptablename + "<dev string:x2ac>");
            } else if (function_73e183e02834309d(scriptablename)) {
                assertmsg("<dev string:x261>" + function_3c8848a3a11b2553(scriptablename) + "<dev string:x2ac>");
            } else {
                assertmsg("<dev string:x2af>");
            }
        #/
        return undefined;
    }
    ...
    itembundlename = function_2cdc55ba39d97d70(scriptablename);
    if (!isdefined(itembundlename)) {
        assertmsg("<dev string:x331>" + scriptablename + "<dev string:x371>");
        return undefined;
    }
    itembundle = getscriptbundle("itemspawnentry:" + itembundlename);
    if (!isdefined(itembundle)) {
        assertmsg("<dev string:x398>" + itembundlename + "<dev string:x371>");
        return undefined;
    }
    return itembundle;
}
...
function function_f4a303d532b044a7(lootid) {
    if (!isint(lootid)) {
        assertmsg("<dev string:x3db>" + (isdefined(lootid) ? lootid : "<dev string:x40b>") + "<dev string:x415>");
        return undefined;
    }
    itembundlename = function_fc925a153c7fd55c(lootid);
    if (!isdefined(itembundlename) || itembundlename == "") {
...
function function_49b86047c8a8b228(dropstruct, baseorigin, baseangles, ignoreent, var_3ace5ac9c7d6fa44, var_688e4da4cb16116d) {
    ...
    if (true) {
        /#
            if (var_551904951118b7b9) {
                println("<dev string:x4af>" + dropstruct.dropidx + "<dev string:x4be>" + baseorigin + "<dev string:x4c6>" + dropdata.outorigin + "<dev string:x4cb>" + dropdata.dist);
            }
        #/
    ...
    /#
        if (var_551904951118b7b9) {
            println("<dev string:x4af>" + dropstruct.dropidx + "<dev string:x4d5>" + dropdata.outorigin + "<dev string:x4e5>" + dropdata.outangles);
        }
    #/
    dropstruct.dropidx++;
...
function private function_9b539449b8809a8(dropstruct, dropdata, baseorigin, ignoreent, tracecontents, var_551904951118b7b9) {
    ...
    /#
        if (var_551904951118b7b9) {
            function_d2d7b153b9d3889e(dropstruct.dropidx, "<dev string:x4e7>", tracestart, traceend, traceresults);
        }
    #/
...
function private function_1bf026a5b9fdae23(dropstruct, dropdata, ignoreent, tracecontents, var_551904951118b7b9) {
    ...
    /#
        if (var_551904951118b7b9) {
            function_d2d7b153b9d3889e(dropstruct.dropidx, "<dev string:x4ea>", tracestart + (0, 1, 0), traceend + (0, 1, 0), traceresults);
        }
    #/
    if (traceresults["fraction"] < 1) {
...
function private function_2b84f1379888e314(dropstruct, dropdata, ignoreent, tracecontents, groundfloat, var_69f3e94ac253c6ab, var_551904951118b7b9) {
    ...
    /#
        if (var_551904951118b7b9) {
            function_d2d7b153b9d3889e(dropstruct.dropidx, "<dev string:x4ed>", tracestart, traceend, traceresults);
        }
    #/
    if (traceresults["fraction"] < 1) {
        ...
        if (isdefined(dropdata.groundentity) && (istrue(dropdata.groundentity.isdestroyed) || istrue(dropdata.groundentity.burningdown))) {
            ...
            /#
                if (var_551904951118b7b9) {
                    function_d2d7b153b9d3889e(dropstruct.dropidx, "<dev string:x4f0>", dropdata.outorigin, traceend, traceresults);
                }
            #/
            if (traceresults["fraction"] < 1) {
                dropdata.outorigin = traceresults["position"] + (0, 0, groundfloat);
                ...
                /#
                    if (var_551904951118b7b9) {
                        println("<dev string:x4af>" + dropstruct.dropidx + "<dev string:x4f4>");
                    }
                #/
...
/#
    ...
    function function_d2d7b153b9d3889e(index, name, start, end, traceresults) {
        ...
        if (traceresults["<dev string:x500>"] < 1) {
            line(start, traceresults["<dev string:x509>"], (1, 0, 0), 1, 0, var_a16891a5162df351);
            if (isdefined(traceresults["<dev string:x512>"])) {
                selectedentity = traceresults["<dev string:x512>"];
                println("<dev string:x4af>" + index + "<dev string:x519>" + name + "<dev string:x51c>" + traceresults["<dev string:x500>"] + "<dev string:x522>" + traceresults["<dev string:x509>"] + "<dev string:x527>" + utility::default_to(selectedentity.name, "<dev string:x40b>"));
            } else {
                println("<dev string:x4af>" + index + "<dev string:x519>" + name + "<dev string:x51c>" + traceresults["<dev string:x500>"] + "<dev string:x522>" + traceresults["<dev string:x509>"]);
            }
            return;
        }
        println("<dev string:x4af>" + index + "<dev string:x519>" + name + "<dev string:x52d>");
...
/#
    ...
    function function_b270e989412a5f67() {
        devgui::function_6e7290c8ee4f558b("<dev string:x536>");
        devgui::function_a9a864379a098ad6("<dev string:x547>", "<dev string:x559>", &draw_items, 1);
        devgui::function_a9a864379a098ad6("<dev string:x571>", "<dev string:x591>", &function_237a0fdd6c243aca);
        devgui::function_b23a59dfb4ca49a1("<dev string:x5b1>", "<dev string:x5cb>", &function_dbbe2312a3bd6d8a);
        devgui::function_b23a59dfb4ca49a1("<dev string:x5ed>", "<dev string:x608>", &function_dbbe2312a3bd6d8a);
    ...
    function function_112c64fc568b602f(var_9ec97bdad14a0a5b) {
        ...
        item_bundle = function_50f8a8f765bad85e("<dev string:x62b>" + var_9ec97bdad14a0a5b, [#"type", #"subtype", #"scriptable"]);
        var_1c769e6e3f9771fc = "<dev string:x63b>";
        if (isdefined(item_bundle.type) && item_bundle.type != "<dev string:x63b>") {
            var_1c769e6e3f9771fc = item_bundle.type + "<dev string:x63c>";
        }
        var_1712f2a4cc83caf8 = "<dev string:x63b>";
        if (isdefined(item_bundle.subtype) && item_bundle.subtype != "<dev string:x63b>") {
            var_1712f2a4cc83caf8 = item_bundle.subtype + "<dev string:x63c>";
        }
        if (!isdefined(item_bundle.scriptable)) {
            return;
        }
        devgui::function_b23a59dfb4ca49a1("<dev string:x63e>" + var_1c769e6e3f9771fc + var_1712f2a4cc83caf8 + var_9ec97bdad14a0a5b, "<dev string:x65a>" + var_9ec97bdad14a0a5b, &function_f8162124bbb6ece3, 0, 0);
    }
    ...
    function private draw_items() {
        while (true) {
            items = namespace_7c5bb630aad381e::function_f8ecd6e02634e682("<dev string:x666>");
            if (isdefined(items)) {
    ...
    function private function_237a0fdd6c243aca() {
        ...
        iprintlnbold(level.var_f8a5820fe8fff0fd ? "<dev string:x673>" : "<dev string:x691>");
    }
    ...
    function function_f8162124bbb6ece3(params) {
        item_name = params[0];
        payload = params[1];
        if (isdefined(item_name) && item_name != "<dev string:x63b>") {
            if (isdefined(payload)) {
            ...
            foreach (var_b58ae7444b8d7baa, var_a59089e939dc706f in level.commonitem.var_970a8dd2fe7c510b) {
                if (item_name == var_a59089e939dc706f || item_name == var_b58ae7444b8d7baa) {
                    itembundle = getscriptbundle("<dev string:x62b>" + var_a59089e939dc706f);
            ...
            itembundle = getscriptbundle("<dev string:x62b>" + item_name);
            if (isdefined(itembundle)) {
                function_c465d27f3f6066b4(itembundle, function_963151d62c29fc95(), undefined, 0, 0, payload);
                return;
            }
            assertmsg("<dev string:x6b0>" + item_name + "<dev string:x6e9>");
        }
    }
    ...
    function function_dbbe2312a3bd6d8a(params) {
        enabled = params[0] == "<dev string:x73e>";
        if (level.players.size > 0) {
            player = level.players[0];
            ...
            if (isdefined(result["<dev string:x509>"]) && isdefined(result["<dev string:x500>"]) && result["<dev string:x500>"] < 1) {
                cylinder(result["<dev string:x509>"], result["<dev string:x509>"] + (0, 0, 1), 32, (0, 1, 0), 0, 10);
                scriptables = getscriptablearrayinradius(undefined, undefined, result["<dev string:x509>"], 32);
                scriptables = utility::array_combine(scriptables, getlootscriptablearrayinradius(undefined, undefined, result["<dev string:x509>"], 32));
