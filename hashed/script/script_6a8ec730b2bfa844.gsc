...
function private function_5a45eb93b9fc8909() {
    ...
    #/
    /#
        assert(isdefined(level.gamemodebundle.factionmissiontable), "Current gamemode requires a faction mission table to be specified");
    #/
    level.var_37cbce6ebdd87f7f = getscriptbundle("factionmissiontable:" + level.gamemodebundle.factionmissiontable);
    /#
        assert(isdefined(level.var_37cbce6ebdd87f7f), "Faction mission table " + level.gamemodebundle.factionmissiontable + " not found");
    #/
    /#
        assert(isdefined(level.var_37cbce6ebdd87f7f.var_847dc5a98178ad30), "Challenges CSV needs to be defined in faction mission table " + level.gamemodebundle.factionmissiontable);
    #/
    ...
    foreach (var_555f50621039c357 in level.var_37cbce6ebdd87f7f.factionmissiontable) {
        ...
        /#
            assert(isdefined(factionmissionasset), "Faction mission asset " + var_555f50621039c357.factionmission + " not found");
        #/
        /#
            assert(isdefined(factionmissionasset.ref), "Faction mission asset " + var_555f50621039c357.factionmission + " is missing a ref value. This should match the achievement name ref of the faction mission root achievement.");
        #/
        row = tablelookuprownum(level.var_37cbce6ebdd87f7f.var_847dc5a98178ad30, 1, factionmissionasset.ref);
        /#
            assertex(isdefined(row) && row >= 0, "Unable to find mission ref " + factionmissionasset.ref + " in " + level.var_37cbce6ebdd87f7f.var_847dc5a98178ad30);
        #/
...
function function_d57e0f306b88dd37() {
    ...
    foreach (lootspawn in spawns) {
        ...
        /#
            assertmsg("Mission item " + lootspawn.targetname + " was not able to be spawned!");
        #/
    }
...
function function_545dc7a8e9dc2b05() {
    if (!isdefined(level.var_25fd54c3e76497cc)) {
        ...
        #/
        /#
            assert(isdefined(level.gamemodebundle.var_25fd54c3e76497cc), "Current gamemode requires activeFactionMissionsStatsKey to be specified");
        #/
...
function onexfil(player, inventory, backpackinventory, clipammo, cash, var_3064fa24dbbd5bbf) {
    ...
    operatorfull = namespace_aead94004cf4c147::function_b13e35608b336d65(player) >= 9 && is_equal(player.var_bed158a6dfac230d, 3) && is_equal(player.gasmasktype, "brloot_equip_gasmask_durable") && player scripts/mp/gametypes/br_public::hasselfrevivetoken() && isdefined(scripts/mp/killstreaks/killstreaks::getkillstreakinslot(1));
...
function function_b0537ee3a491b4dd(player, contents, cash) {
    ...
    player.var_e7faf08f327b0fb5 = player.var_e7faf08f327b0fb5 + scripts/mp/gametypes/dead_drop::function_160a29300ce1e14(totalcashvalue, player);
}
...
function function_baeceec743d9a737(player, items, cash, method, isexfil) {
    ...
    if (!namespace_aead94004cf4c147::function_c4f1bd7a01a307fa(player)) {
        ...
        totalcashvalue = totalcashvalue + namespace_a8b2b88699fc40fb::function_baf5e17d7033a223(player);
    }
    if (istrue(isexfil)) {
...
function function_59fe5e0d7d5e7947(player, fortress) {
    ...
    if (namespace_9823ee6035594d67::function_4ec4f39a2e1745f2(fortress)) {
        if (!isdefined(fortress.name)) {
            /#
                assertmsg("Fortress at " + scripts/common/utility::function_d5ad0ebc9d8935b6(fortress.origin) + " doesn't have a defined name!");
            #/
            return;
        }
...
function function_7184abfe5941a710(player, cashspent) {
    ...
    purchasestruct.cashspent = purchasestruct.cashspent + cashspent;
...
function function_635937c689ec374(camera) {
    ...
    raycast = scripts/engine/trace::ray_trace(camera.origin, level.var_39f292381a9d3bb8.origin, camera);
...
function function_7868d975ef389aba() {
    ...
    while (true) {
        ent = cavetrigger waittill("trigger");
        if (!ent scripts/cp_mp/vehicles/vehicle::isvehicle()) {
            continue;
        }
...
function private function_fffd723ff4050d8e(instance, part, state, player, var_a5b2c541413aa895, usestring) {
    if (state == "usable") {
        ...
        while (buttontime < level.demoprops.planttime) {
            if (instance.cancel || !scripts/mp/utility/player::function_ad443bbcdcf37b85(player)) {
                instance.planting = 0;
...
function private function_2f274301f86d64e0(params) {
    ...
    oldtrain = undefined;
    while (true) {
        newtrain = scripts/cp_mp/utility/train_utility::function_31156831afc882ad();
...
function function_a7fb9d5548d2455a() {
    ...
    foreach (struct in lasers) {
        ...
        trace = scripts/engine/trace::ray_trace(sentry_origin, newpos, turret);
        end = trace["position"];
...
function function_a7614ce40d9dbe48() {
    ...
    wait(2);
    var_4a5d9d7667048bf3 = scripts/engine/trace::create_character_contents();
    while (true) {
        ...
        trace = scripts/engine/trace::ray_trace(start, self.aimlocation);
        end = trace["position"];
        gooddistsq = lengthsquared(start - end);
        trace = scripts/engine/trace::ray_trace(start, end, [], var_4a5d9d7667048bf3);
...
function private function_414cf87bc60bb468() {
    ...
    while (true) {
        weapontaken = player waittill("weapon_taken");
...
function function_ec7e73044204b1cf(instance, part, state, player, var_a5b2c541413aa895, usestring) {
    ...
    if (state == "usable") {
        ...
        buttontime = 0;
        while (buttontime < 5) {
            if (node.cancel || !scripts/mp/utility/player::function_ad443bbcdcf37b85(player)) {
                node.planting = 0;
    ...
    if (state == "defusable") {
        ...
        buttontime = 0;
        while (buttontime < 5) {
            if (node.cancel || !scripts/mp/utility/player::function_ad443bbcdcf37b85(player) || isdefined(player.var_2ca0dac193d83c1) && player.var_2ca0dac193d83c1) {
                node.defusing = 0;
                namespace_5ffce5a43b2d1ec2::function_db896f4486ca187(player, bomb);
