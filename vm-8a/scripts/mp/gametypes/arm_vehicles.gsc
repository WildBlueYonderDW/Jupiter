...
function init_groundwarvehicles() {
    ...
    if (false) {
        foreach (loc in level.vehiclespawnlocs) {
            thread drawline(loc.origin, loc.origin + (0, 0, 1500), 1000, (1, 0, 0));
            /#
                print3d(loc.origin, loc.refname + "<dev string:x3a>" + loc.origin, (1, 0, 0), 1, 1, 100000, 1);
            #/
        }
    }
    level.vehiclespawnlocs = array_randomize(level.vehiclespawnlocs);
...
function function_67578d114d517147() {
    ...
    foreach (loc in data.vehiclespawns) {
        ...
        if (false) {
            thread drawline(loc.origin, loc.origin + (0, 0, 1500), 1000, (1, 0, 0));
            /#
                print3d(loc.origin, "<dev string:x3e>" + "<dev string:x3a>" + loc.origin, (1, 0, 0), 1, 1, 100000, 1);
            #/
        }
        wait 1;
    }
...
function init_rallyvehicles() {
    ...
    foreach (loc in var_13b8accbd9c63733) {
        ...
        if (false) {
            thread drawline(loc.origin, loc.origin + (0, 0, 1500), 1000, (1, 0, 0));
            /#
                print3d(loc.origin, ref + "<dev string:x3a>" + loc.origin, (1, 0, 0), 1, 1, 100000, 1);
            #/
        }
        /#
            issharedfuncdefined("<dev string:x1c>", "<dev string:x2a>", 1);
    ...
    foreach (loc in var_13b8accbd9c63733) {
        ...
        if (false) {
            thread drawline(loc.origin, loc.origin + (0, 0, 1500), 1000, (1, 0, 0));
            /#
                print3d(loc.origin, "<dev string:x4a>" + "<dev string:x3a>" + loc.origin, (1, 0, 0), 1, 1, 100000, 1);
            #/
        }
        team = ter_op(loc.script_team == "axis", "axis", "allies");
        var_da3668523170ea04 = undefined;
    ...
    foreach (loc in function_b08e7e3a0b14f76f("veh9_palfa")) {
        ...
        if (false) {
            thread drawline(loc.origin, loc.origin + (0, 0, 1500), 1000, (1, 0, 0));
            /#
                print3d(loc.origin, "<dev string:x5b>" + "<dev string:x3a>" + loc.origin, (1, 0, 0), 1, 1, 100000, 1);
            #/
        }
        team = ter_op(loc.script_team == "axis", "axis", "allies");
        var_da3668523170ea04 = undefined;
