    ...
    if (isdefined(level.system_funcs[str_name])) {
        /#
            if (!(level.system_funcs[str_name].flags & 1)) {
                system_error("<unknown string>" + (function_d03495fe6418377b(str_name) ? function_3c8848a3a11b2553(str_name) : str_name) + "<unknown string>");
            }
        #/
        return;
    }
    ...
    if (isdefined(func.reqs)) {
        if (isarray(func.reqs)) {
            foreach (req in func.reqs) {
                /#
                    if (!isdefined(req)) {
                        system_error("<unknown string>" + req + "<unknown string>");
                        func.flags = func.flags | 8;
                        continue;
                    }
                #/
        ...
        /#
            if (!isdefined(level.system_funcs[func.reqs])) {
                system_error("<unknown string>" + (function_d03495fe6418377b(func.reqs) ? function_3c8848a3a11b2553(func.reqs) : func.reqs) + "<unknown string>");
                func.flags = func.flags | 8;
                return;
            }
        #/
