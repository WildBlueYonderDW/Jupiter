...
/#
    ...
    function function_709f4a6846f9f8e6(instance, testteam) {
        playersonteam = getteamdata(testteam, "<dev string:x1c>");
        useplayer = undefined;
        foreach (player in playersonteam) {
            if (isbot(player)) {
                useplayer = player;
        ...
        wait 2;
        namespace_e1bfa87a2a692d3c::function_a22ac2645c342456(instance, "<dev string:x24>", "<dev string:x37>", useplayer, 1, undefined);
    }
    ...
    function function_db0e41781abde2e8() {
        ...
        scripts\cp_mp\utility\game_utility::function_6b6b6273f8180522("<dev string:x49>", self.origin, 1000);
        scripts\cp_mp\utility\game_utility::function_6988310081de7b45();
        while (true) {
            print3d(self.origin, "<dev string:x54>", (1, 1, 1), 1, 1, 1, 1);
            waitframe();
        }
    }
