    ...
    if (function_efab78b72d131d76(player)) {
        ...
        if (isdefined(weaponammotype)) {
            ...
            if (weaponstockammo > 1023) {
                assertmsg("updateUIAmmo has too large of an omnvar value, weaponStockAmmo: " + weaponstockammo + ", weapon slot basename" + var_9ac5e72784815708.basename);
            }
...
function function_e37ca8120dc19c6a() {
    /#
        if (issubstr(level.mapname, "<dev string:x1c>") || level.mapname == "<dev string:x29>") {
            return true;
        }
    #/
...
function private function_d87a9518235b9682(index, lootid, quantity, var_f3990d8529de3a54, lootedquantity) {
    ...
    /#
        println(self.name + "<dev string:x33>" + index + "<dev string:x58>" + lootid + "<dev string:x58>" + quantity + "<dev string:x5b>");
        println("<dev string:x5e>" + index + "<dev string:x60>" + lootid + "<dev string:x66>" + ref + "<dev string:x69>" + quantity);
    #/
...
function function_5bf048439a846222(oldequipment) {
    return oldequipment == "equip_c4" || oldequipment == "equip_throwing_knife" || oldequipment == "equip_claymore" || oldequipment == "equip_at_mine" || oldequipment == "equip_throwing_knife_fire";
}
...
// Size: 0x94
function array_sort(array, compare_func) {
...
function function_a40546ed395d06c8(ammoarray) {
    return array_sort(ammoarray, &function_6f3fb8a752376eb2);
}
...
function function_cf1bef64e94f2d1a(itemtype, itemindex, var_a548a6c15ceb9e8c, var_616ab931ecba446b, getsingle) {
    ...
    case 4: 
        ...
        return [lootid, ammocount];
    case 6: 
        if (!scripts\cp_mp\gasmask::hasgasmask(self) || istrue(self.gasmaskswapinprogress)) {
            if (var_a548a6c15ceb9e8c) {
                return [0, 0];
            } else {
                return undefined;
...
/#
    ...
    function printinventory() {
        for (i = 0; i < 50; i++) {
            lootid = self getplayerdata(level.var_5d69837cf4db0407, "<dev string:x6d>", i, "<dev string:x7a>");
            quantity = self getplayerdata(level.var_5d69837cf4db0407, "<dev string:x6d>", i, "<dev string:x81>");
            ref = namespace_38b993c4618e76cd::function_793e8a72cedb8ef3(lootid);
            println("<dev string:x5e>" + i + "<dev string:x60>" + lootid + "<dev string:x66>" + ref + "<dev string:x69>" + quantity);
        }
    }
