...
function function_e7f383d618470607(triggervolume) {
    ...
    self endon("death");
    while (true) {
        ent = triggervolume waittill("trigger");
        if (isplayer(ent)) {
...
function private function_f6b236a9e4b842ea(traptrigger) {
    ...
    self endon("death");
    while (true) {
        ent = traptrigger waittill("trigger");
        if (isplayer(ent)) {
...
function function_a9f5229ecf5cca14() {
    ...
    /#
        assertex(istrue(foundgoalvolume), "ERROR - Could not find a valid goal volume for All For One within the activity instance data!");
    #/
}
