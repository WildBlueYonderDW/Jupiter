...
function function_401ab3dde070adde() {
    ...
    foreach (zoneindex in level.var_d295f7b9a28a5a53) {
        ...
        /#
            println("airport_hotzone_01" + zoneindex + "observatory_hotzone_03" + zonename + "mtntwn_hotzone_01" + zonebounds.midpoint);
        #/
        fortress = undefined;
        poiname = undefined;
    ...
    foreach (teamdata in crate.trackedTeams) {
        ...
        if (isdefined(teamname) && teamdata.teamname != teamname && !istrue(level.var_25e8b4afe5e06169)) {
            if (istrue(validsnapshot) && teamdata.snapshotentcount != teamdata.var_bc0c909d40ee5b6f) {
                teamdata.snapshotentcount = teamdata.snapshotentcount + 1;
                foreach (teammember in teammembers) {
...
function function_9a8bd260dcce74be() {
    ...
    foreach (cache in var_b098d8a727cf5053) {
        ...
        if (level.var_7e293d1e9b987629 > 0) {
            ...
            for (var_dfeaae7539235b0d = 0; var_dfeaae7539235b0d < maxchecks; var_dfeaae7539235b0d = var_dfeaae7539235b0d + 1) {
...
function function_bc8c42a438ada1fc(duration, fortress, uavtype) {
    ...
    for (timepassed = 0; timepassed < duration && isdefined(fortress) && istrue(fortress.captured); timepassed = timepassed + var_6c3716013ea4f716) {
        if (!isdefined(team)) {
            break;
        }
...
function function_c3bc424d680ab470(fortress, point) {
    ...
    largesthalfsize = largesthalfsize * largesthalfsize;
...
function function_de1bd18e889bf4cf() {
    /#
        ...
        while (!scripts/mp/flags::gameflagexists("moderncity_hotzone_11")) {
            waitframe();
        }
        scripts/mp/flags::gameflagwait("moderncity_hotzone_11");
        while (true) {
    ...
    /#
        ...
        if (level.var_5d89e32b0b1d5e91) {
            ...
            if (level.buildingslist.size <= 0) {
                /#
                    assertex(isdefined(level.buildingslist), " name must be included in stronghold tier arrays and the CSV Hotzone");
                #/
                    ...
                    foreach (building in level.buildingslist.var_b7e45754fc79e080) {
                        ...
                        if (istrue(building.var_5dfdfd9369ac6229) && building.targetname == "mtntwn_hotzone_04") {
                            ...
                            if (isdefined(building.details)) {
                                ...
                                debugstar(building.origin + (0, 0, building.buildingnumber * 15 + 2500), (0.9, 0.45, 0.16), 1, "elite", 8);
                            }
                            continue;
                        }
                        if (building.targetname == "mtntwn_hotzone_04") {
                            ...
                            debugstar(building.origin + (0, 0, building.buildingnumber * 15 + 1500), (0, 0.53, 1), 1, "script_model", 8);
                        }
                    }
                }
                waitframe();
...
function function_5edce2f3bff589aa() {
    /#
        ...
        foreach (building in level.buildingslist.var_b7e45754fc79e080) {
            ...
            building.details = building.targetname + "trigger" + building.buildingnumber + "script_vehicle" + building.origin + "script_vehicle" + building.script_noteworthy;
            println(building.targetname + "trigger" + buildingnumber + "script_vehicle" + building.origin + "script_vehicle" + building.script_noteworthy);
            buildingnumber++;
        }
        println(": Spawning Blacksite Fortress for a total of ");
...
function function_a2d65f1e8991513f(hotzonename, isblacksite) {
    /#
        var_b6f0e58e86c6362c = "<unknown string>" + hotzonename + "<unknown string>";
        if (isblacksite) {
