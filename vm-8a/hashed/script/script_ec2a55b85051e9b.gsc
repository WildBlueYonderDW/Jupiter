...
function gas_watchtriggerenter(damage, tickcooldown) {
    ...
    self endon("death");
    while (true) {
        enteringentity = self waittill("trigger");
...
function gas_onexittrigger(triggerid, triggertype) {
    ...
    if (isdefined(triggertype) && function_cdab109851736293(triggertype) && !function_ca84aec5683c61bd(triggertype)) {
        ...
        /#
            function_a3b978f0d3e37d36("<dev string:x1e>" + triggertype);
        #/
    }
}
...
function private function_415ef5b618aed907(damage, interval, var_5fd212d893bd2ef9, triggertype) {
    ...
    /#
        function_a3b978f0d3e37d36("<dev string:x47>" + triggertype);
    #/
    return true;
}
...
function private function_6ee3c7d8ed420d0b(agent) {
    ...
    /#
        if (int(getunarchiveddebugdvar(@"hash_213776149170b77d", "<dev string:x1c>"))) {
            warlordparameters = namespace_ad1f398af6f48cb1::function_9acf953fd6bf2e46("<dev string:x72>");
    ...
    if (isdefined(self) && isdefined(self.ob) && isdefined(self.ob.headicon)) {
        /#
            if (int(getunarchiveddebugdvar(@"hash_54fc92ada753c66a", "<dev string:x1c>"))) {
                print3d(self.origin, "<dev string:x7a>" + showicon + "<dev string:x81>" + gettime(), (1, 1, 0), 1, 1, 30);
            }
        #/
...
function function_157fa9f486d53b6e() {
    ...
    /#
        if (int(getunarchiveddebugdvar(@"hash_54fc92ada753c66a", "<dev string:x1c>"))) {
            print3d(self.origin, "<dev string:x86>", (1, 0, 1), 1, 1, 30, 1);
        }
    #/
}
...
/#
    ...
    function private function_b8f06ed708934a74() {
        ...
        scale = 1;
        line_y = 20;
        printtoscreen2d(offset_x, offset_y, "<dev string:x94>", (0, 1, 1), scale);
        offset_y += line_y;
        if (isdefined(localplayer.gasmaskhealth) && isdefined(localplayer.gasmaskmaxhealth)) {
            printtoscreen2d(offset_x, offset_y, "<dev string:xb1>" + localplayer.gasmaskhealth + "<dev string:xcb>" + localplayer.gasmaskmaxhealth, (0, 1, 1), scale);
            offset_y += line_y;
        }
        if (localplayer function_6ef47ee9171ed1b9()) {
            foreach (id, gastrigger in localplayer.gastriggerstouching) {
                if (isdefined(gastrigger)) {
                    triggertypetext = default_to(gastrigger.triggertype, "<dev string:xcf>");
                    printtoscreen2d(offset_x, offset_y, "<dev string:xd3>" + id + "<dev string:xd8>" + triggertypetext, (0, 1, 0), scale);
                    offset_y += line_y;
                    continue;
                }
                printtoscreen2d(offset_x, offset_y, "<dev string:xe6>", (1, 0, 0), scale);
                offset_y += line_y;
            }
            return;
        }
        printtoscreen2d(offset_x, offset_y, "<dev string:xcf>", (0, 1, 0), scale);
        offset_y += line_y;
    }
    ...
    function private function_1278077fcd4b2924() {
        ...
        self endon("<dev string:x103>");
        while (self.var_e4ba42c30663bec8 > 0) {
            dvar = int(getunarchiveddebugdvar(@"hash_54fc92ada753c66a", "<dev string:x1c>"));
            if (dvar) {
                text = ter_op(val::get("<dev string:x112>"), "<dev string:x121>", "<dev string:x129>");
