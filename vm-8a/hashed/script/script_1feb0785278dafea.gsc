...
function init() {
    ...
    /#
        ...
        function_a2fe753948998c89("<dev string:x32>", "<dev string:x44>");
        function_a2fe753948998c89("<dev string:x72>", "<dev string:x87>");
...
function revive_player(reviver) {
    ...
    level endon("game_ended");
    for (n_timeout = 8; scripts\mp\utility\player::isinlaststand(self) && !istrue(self.var_2fb534abd9eb93ca); n_timeout -= level.framedurationseconds) {
        if (n_timeout <= 0) {
            break;
        }
        waitframe();
