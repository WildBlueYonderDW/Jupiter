...
function targetlead_enterstate(statename) {
    ...
    /#
        function_7ea27fe25f2f18e5("<dev string:x4b>" + statename + "<dev string:x68>");
    #/
...
/#
    ...
    function function_7ea27fe25f2f18e5(text) {
        if (getdvar(@"hash_22ff48dd25613942") == "<dev string:x6b>") {
            iprintln(text);
        }
    }
    ...
    function function_6eee6cec92ff2a91(point, color, radius) {
        if (getdvar(@"hash_b56d344ee60e317f") != "<dev string:x6b>") {
            return;
        }
        if (isdefined(radius)) {
