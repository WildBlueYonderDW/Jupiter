...
function function_378db8dafafcc467(params) {
    ...
    foreach (player in container.playeropen) {
        ...
        player.var_4e300028cc830f21[self.varianttag] = player.var_4e300028cc830f21[self.varianttag] + var_e94b4ab20232f72a;
...
function function_93c37ea2b2745734(player, spawn_struct, var_afe6f9511d9e590b) {
    ...
    if (isalive(player)) {
        ...
        if (isalive(hound)) {
            ...
            hound function_90d0e1682e0a56b6(&function_5a4c982f4f3ea648);
...
function function_a5e16566a91e9488(params) {
    if (isplayer(params.player) && isdefined(params.superinfo)) {
        if (params.superinfo.staticdata.ref == "super_healing_aura") {
            if (isdefined(self.dog_companion) && isalive(self.dog_companion)) {
                self.dog_companion.health = int(min(self.dog_companion.maxhealth, self.dog_companion.health + int(self.dog_companion.maxhealth * 0.25)));
            }
        }
    }
}
...
function function_5a4c982f4f3ea648(params) {
    ...
    if (istrue(self.var_873d30685528d962) && isdefined(params.einflictor) && isdefined(params.einflictor.subclass)) {
        ...
        case #"hash_f297af39454fdc7b":
            if (isdefined(params.einflictor.difficulty)) {
                if (params.einflictor.difficulty == "difficulty_hard") {
                    new_damage = params.idamage / getdvarint(@"hash_19f601a669904907", 9);
                    break;
                } else if (params.einflictor.difficulty == "difficulty_darkaether") {
                    new_damage = params.idamage / getdvarint(@"hash_fc0768453a9a74f8", 10);
                    break;
                }
            }
            new_damage = params.idamage / getdvarint(@"hash_668db753ddaf765d", 8);
            break;
        case #"hash_c1b612499d7bd4a":
        case #"hash_9bc2060c1da7a187":
            if (isdefined(params.einflictor.difficulty)) {
                if (params.einflictor.difficulty == "difficulty_normal") {
                    new_damage = params.idamage / getdvarint(@"hash_b4610111fb80129c", 47);
                    break;
                } else if (params.einflictor.difficulty == "difficulty_hard") {
                    new_damage = params.idamage / getdvarint(@"hash_eb3a663fd08af2f6", 75);
                    break;
                } else if (params.einflictor.difficulty == "difficulty_darkaether") {
                    new_damage = params.idamage / getdvarint(@"hash_b230d798af093bd9", 225);
                    break;
                }
            }
            new_damage = params.idamage / getdvarint(@"hash_d51d8ff4c7a18a7e", 4);
            break;
        case #"hash_3e4a9c816939eeef":
            if (params.einflictor.difficulty == "difficulty_normal") {
                new_damage = params.idamage / getdvarint(@"hash_1cd10fbdf9c981f7", 9.5);
                break;
            } else if (params.einflictor.difficulty == "difficulty_hard") {
                new_damage = params.idamage / getdvarint(@"hash_dfbab5c015170e79", 25);
                break;
            } else if (params.einflictor.difficulty == "difficulty_darkaether") {
                new_damage = params.idamage / getdvarint(@"hash_9f1f885e10de8422", 75);
                break;
            }
            new_damage = params.idamage;
            break;
        case #"hash_60cbd0daf82a39c9":
            if (params.einflictor.difficulty == "difficulty_hard") {
                new_damage = params.idamage * getdvarint(@"hash_484a740918c1779f", 1.75);
                break;
            } else if (params.einflictor.difficulty == "difficulty_darkaether") {
                new_damage = params.idamage * getdvarint(@"hash_58ac5aa693e8a4a0", 1.8);
                break;
            }
            new_damage = params.idamage / getdvarint(@"hash_221a1e5bc26defb7", 1.5);
            break;
        case #"hash_c1fa8721b046b258":
            if (params.einflictor.difficulty == "difficulty_hard") {
                new_damage = params.idamage * getdvarint(@"hash_e789fd5fcf5627f2", 2);
                break;
            } else if (params.einflictor.difficulty == "difficulty_darkaether") {
                new_damage = params.idamage * getdvarint(@"hash_c51eb5fde1ac20dd", 2);
                break;
            }
    ...
    if (isdefined(params.einflictor) && isdefined(params.einflictor.difficulty)) {
        if (params.einflictor.difficulty == "difficulty_hard") {
            ...
            case #"hash_7bb2cd766703d463":
                new_damage = min(new_damage, getdvarint(@"hash_2b5abe643f197c3d", 125));
                break;
            case #"hash_af83e47edfa8900a":
                new_damage = min(new_damage, getdvarint(@"hash_2b5abb643f1975a4", 200));
                break;
            case #"hash_5343b465e56ec9a4":
                new_damage = min(new_damage, getdvarint(@"hash_2b5abc643f1977d7", 275));
                break;
            case #"hash_651f76c0ad6741ec":
                new_damage = min(new_damage, getdvarint(@"hash_2b5abc643f1977d7", 375));
                break;
            }
        } else if (params.einflictor.difficulty == "difficulty_darkaether") {
            switch (self.difficulty) {
            case #"hash_5343b465e56ec9a4":
                new_damage = min(new_damage, getdvarint(@"hash_b05f8c5ecfeb4531", 250));
                break;
            case #"hash_651f76c0ad6741ec":
                new_damage = min(new_damage, getdvarint(@"hash_b05f8d5ecfeb4764", 300));
                break;
            default:
                break;
            }
            if (!istrue(var_eff1dd1bc375195c)) {
                if (new_damage > getdvarint(@"hash_435ce2df9e3d9a9a", 2500)) {
                    new_damage = getdvarint(@"hash_435ce2df9e3d9a9a", 2500);
                }
            }
        }
    }
...
function private function_fb60235e11c3193a(var_d2dc53d5b262c23a, player) {
    ...
    if (isdefined(player)) {
        ...
        if (isdefined(item)) {
            ...
            thread namespace_446fc987a980892f::playevent("quest_s1_rift_gate_unlock_object_acquired", a_squad, 1.5);
        }
    }
}
