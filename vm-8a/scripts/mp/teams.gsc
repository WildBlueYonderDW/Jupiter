    ...
    if (isdefined(projectscriptbundle) && isdefined(projectscriptbundle.parachutelist)) {
        ...
        if (isdefined(var_150ada5de2119f78) && isdefined(var_150ada5de2119f78.parachute_list)) {
            foreach (parachute in var_150ada5de2119f78.parachute_list) {
                ...
                assertex(!isdefined(level.var_8439ac7b613822b0[lootid]), "Duplicate Loot ID found for multiple parachute data assets " + lootid + "with " + parachute.parachutedata);
                if (isdefined(ref)) {
                ...
                assertmsg("ParachuteData asset: " + parachute.parachutedata + "does not have a parachute reference");
            }
        }
    }
}
    ...
    if (isdefined(projectscriptbundle) && isdefined(projectscriptbundle.contraillist)) {
        ...
        if (isdefined(var_94e82efc0c1e2431) && isdefined(var_94e82efc0c1e2431.contrail_list)) {
            foreach (contrail in var_94e82efc0c1e2431.contrail_list) {
                ...
                assertex(!isdefined(level.contraillist[lootid]), "Duplicate Loot ID found for multiple contrail data assets " + lootid + "with " + contrail.contraildata);
                if (isdefined(ref)) {
                ...
                assertmsg("ContrailData asset: " + contrail.contraildata + "does not have a parachute reference");
            }
        }
    }
}
...
function function_e7fe5b7d4b92813c() {
    /#
        if (isai(self)) {
            ...
            if (isdefined(var_1c309d4424389fba) && var_1c309d4424389fba != "<dev string:x1c>") {
                var_16ec163f5e8303eb = strtok(var_1c309d4424389fba, "<dev string:x1d>");
    ...
    if (isdefined(self.operatorcustomization) && isdefined(self.operatorcustomization.operatorref)) {
        ...
        if (scripts\cp_mp\utility\game_utility::isinfectedgametype() && team == "axis") {
            ...
            if (!namespace_465d40bb08a5337a::function_54240a5fc41f6590(operatorref)) {
                println("<dev string:x1f>" + operatorref + "<dev string:x39>");
                operatorref = namespace_465d40bb08a5337a::function_9242a55834c7ead8(var_e9a5f25420e74f74);
            }
        }
    }
...
/#
    ...
    function function_5933f55517e82c57() {
        ...
        level endon("<dev string:x69>");
        for (;;) {
            foreach (player in level.players) {
                player getplayerdata("<dev string:x7a>", "<dev string:x81>", "<dev string:x8f>", "<dev string:x9b>");
            }
            wait 0.05;
        }
    }
...
function private function_8075b157830461e0() {
    if (getdvarint(@"hash_11e4f85d7c35874e", 0) && isdefined(level.players) && level.players.size > 0) {
        ...
        if (var_5af9fdb2229b2cec > level.framedurationseconds) {
            ...
            println("<dev string:x9e>" + 1 + "<dev string:xc9>" + var_5af9fdb2229b2cec + "<dev string:xe3>" + level.players.size + "<dev string:xf3>" + gettime() + "<dev string:xf7>");
            return true;
        }
        if (isdefined(level.var_fa5689aa69fdbd8a)) {
        ...
        println("<dev string:x195>" + var_363dfdcaf05d82e0 + "<dev string:x1cf>" + level.framedurationseconds + "<dev string:x1fe>" + level.players.size);
    }
    return false;
}
...
function getjointeampermissions(team) {
    ...
    println("<dev string:x217>" + team + "<dev string:x23f>");
    if (self ishost()) {
        wait 1.5;
    }
...
function printcustomization() {
    ...
    /#
        print3d(self.origin + (0, 0, 40), "<dev string:x2a2>" + var_1ceef035c3f3046d[0], (1, 1, 1), 1, 0.5);
    #/
    /#
        print3d(self.origin + (0, 0, 60), "<dev string:x2a9>" + var_1ceef035c3f3046d[1], (1, 1, 1), 1, 0.5);
    #/
}
    ...
    while (true) {
        if (isdefined(level.teamdata)) {
            ...
            foreach (team in level.teamnamelist) {
                linecount = 1;
                /#
                    printtoscreen2d(startx, starty + stepy * linecount, "<dev string:x2b0>" + team, (1, 1, 1), 1);
                #/
                linecount++;
