...
/#
    ...
    function function_c623b8e6e4b0a90(tag, size, duration, blendtimesec) {
        ...
        self notify("<dev string:x4e0>" + tag);
        self endon("<dev string:x4e0>" + tag);
        starttime = gettime();
        ...
        while (true) {
            ...
            if (tag != "<dev string:x1c>") {
                ...
                print3d(origin + (0, 0, 1), "<dev string:x4f2>", (1, 1, 1), 1, 0.15, 1, 1);
            }
    ...
    function function_baa34fb5707b7466(var_214d924f4e137244, event, origin, angles) {
        ...
        level notify("<dev string:x500>");
        level endon("<dev string:x500>");
        ...
        foreach (eventlist in level.var_fb96551e5ea54b9) {
            ...
            for (i = 0; i < eventlist.events.size; i++) {
                ...
                if (!isdefined(drawnorigin) || drawnorigin != event.origin || var_1b36f968431506c3 != event.angles) {
                    ...
                    line(event.origin, event.origin + axis["<dev string:x513>"] * 10, (1, 0, 0), 1, 0, duration);
                    line(event.origin, event.origin + axis["<dev string:x51b>"] * 10, (0, 1, 0), 1, 0, duration);
                    line(event.origin, event.origin + axis["<dev string:x521>"] * 10, (0, 0, 1), 1, 0, duration);
                }
                ...
                println("<dev string:x524>" + event.time + "<dev string:x532>" + event.event + "<dev string:x535>" + objectname);
            }
        }
    ...
    function function_d4cfde3fa9af7e5a(shotindex) {
        ...
        if (getdvarint(@"hash_f98fc73c88850645", 0)) {
            ...
            player.var_43996b501c46de7f[0].alignx = "<dev string:x51b>";
            player.var_43996b501c46de7f[0].aligny = "<dev string:x539>";
            player.var_43996b501c46de7f[0].vertalign = "<dev string:x539>";
            player.var_43996b501c46de7f[0].horzalign = "<dev string:x51b>";
            player.var_43996b501c46de7f[0].font = "<dev string:x540>";
            ...
            player.var_43996b501c46de7f[1].alignx = "<dev string:x51b>";
            player.var_43996b501c46de7f[1].aligny = "<dev string:x539>";
            player.var_43996b501c46de7f[1].vertalign = "<dev string:x539>";
            player.var_43996b501c46de7f[1].horzalign = "<dev string:x51b>";
            player.var_43996b501c46de7f[1].font = "<dev string:x540>";
            ...
            if (isdefined(sceneroot.var_2f23edb971f368a6)) {
                shottext = "<dev string:x54b>" + str_shot_name + "<dev string:x552>" + sceneroot.var_2f23edb971f368a6;
            } else {
                shottext = "<dev string:x54b>" + str_shot_name;
            }
    ...
    function function_ffae6ec92028ce08(shotindexes, duration, cameras) {
        ...
        if (getdvarint(@"hash_f98fc73c88850645", 0)) {
            ...
            foreach (arrayindex, shotindex in shotindexes) {
                hudelem = newhudelem();
                str_shot_name = default_to(sceneroot.scenedata.scenescriptbundle.shots[shotindex].variant_object.name, "<dev string:x1c>");
                hudelem.alignx = "<dev string:x51b>";
                hudelem.aligny = "<dev string:x539>";
                hudelem.vertalign = "<dev string:x539>";
                hudelem.horzalign = "<dev string:x51b>";
                hudelem.font = "<dev string:x540>";
                hudelem.x = -25;
                hudelem.y = y;
                hudelem.fontscale = 1;
                ...
                if (isdefined(cameras) && isdefined(cameras[arrayindex])) {
                    ...
                    cameratext = "<dev string:x55a>" + int(camerapos[0]) + "<dev string:x88>" + int(camerapos[1]) + "<dev string:x88>" + int(camerapos[2]) + "<dev string:x55d>";
                }
                hudelem setdevtext(cameratext + "<dev string:x560>" + str_scene_name + "<dev string:x56d>" + str_shot_name);
                y -= 20;
                player.var_7c8fce4c59afb3ae[player.var_7c8fce4c59afb3ae.size] = hudelem;
            }
        }
        if (duration >= 0 && isdefined(player)) {
            player function_f9a207358d70923c(duration, "<dev string:x571>", &function_7fb7bdf37cd47759);
        }
    }
