    ...
    foreach (team in self.teams) {
        ...
        if (var_da1e4a8f39765732) {
            ...
            foreach (player in players) {
                ...
                player scripts\cp_mp\challenges::oncontractend(self.type.index, 1, 1, self.startingpoi, self.type.ref, var_497e8b601a6f12f6);
            }
        ...
        foreach (player in players) {
            player challenges::oncontractend(self.type.index, 0, 1, self.startingpoi, self.type.ref, 0);
        }
        if (isdefined(var_752106078fe9b8fc)) {
            ...
            foreach (var_c92902ad553e3854 in var_da60ac7aae847d8) {
                var_c92902ad553e3854 challenges::oncontractend(self.type.index, 1, 2, self.startingpoi, self.type.ref, 0);
            }
        }
    }
    if (!istrue(self.var_132f5385c2f23a96)) {
...
function function_6ddd1eecbe562acf(groupref, team, rewardposition, contract) {
    if (isdefined(groupref) && groupref != "") {
        ...
        if (scripts\mp\gametypes\br_gametypes::isbrgametypefuncdefined("giveContractBonusRewards")) {
            scripts\mp\gametypes\br_gametypes::function_d72afdccedfc9b07("giveContractBonusRewards", team, contract, rewards);
        }
        return rewards;
    }
}
...
// Size: 0xbc
function createquestobjicon(iconref, state, var_38116998df9814d4, contract) {
    ...
    if (self.objectiveiconid != -1) {
        ...
        function_32c0b4da728d15f0(self.objectiveiconid, contract);
...
// Size: 0x46
function function_32c0b4da728d15f0(objectiveiconid, contract) {
    var_d98cb8fab25129a8 = 4;
    if (br_gametypes::isbrgametypefuncdefined("getSpecialObjectiveBackgroundType")) {
        var_d98cb8fab25129a8 = br_gametypes::runbrgametypefunc("getSpecialObjectiveBackgroundType", contract);
    }
    namespace_6c8a837ec98fe0b8::update_objective_setbackground(objectiveiconid, var_d98cb8fab25129a8);
}
...
function function_5606274c76b4a2df(time) {
    ...
    currenttime = gettime();
    elapsed = currenttime - self.missionstarttime;
    newtime = self.totaltime * 1000 - elapsed + time * 1000;
    foreach (player in teams::getteamdata(self.teams[0], "players")) {
        player setclientomnvar("ui_br_objective_duration_time", newtime);
    }
    function_51d3a2b4cde79545();
}
...
function function_ab19a58fc4be9ce2() {
    ...
    while (level.contractmanager.activetablets.size > 0) {
        ...
        if (!var_db3ef040d8f07322 || !isreallyalive(player)) {
            continue;
        }
