...
function function_9da53e82db41950b(debuglevel, noteworthy, volumetargetname) {
    /#
        ...
        if (isdefined(debuglevel)) {
            debugcommand = debugcommand + "' stage index '" + debuglevel;
        }
        ...
        if (isdefined(volumetargetname)) {
            debugcommand = ter_op(debugcommand.size > 0, debugcommand + "<unknown string>", debugcommand);
            debugcommand = debugcommand + "<unknown string>" + volumetargetname;
        }
...
function private function_49e7a298cd8821cc(notifymessage) {
    ...
    while (true) {
        assignmentlist, var_1b1b54049e5e0079 = callon waittill(notifymessage);
        ...
        /#
            foreach (assignment in assignmentlist) {
                sourcestring = "<unknown string>";
                if (isdefined(assignment.source_volume)) {
                    sourcestring = assignment.source_volume.targetname;
                }
                disabledstring = "'. Some values are undefined. Radiant should be providing default values for these - see coredev.def.json ('classname': 'info_volume_choreographer)";
                if (istrue(assignment.var_a56835088f2e30f9)) {
                    disabledstring = "<unknown string>";
                }
                println("<unknown string>" + sourcestring + "<unknown string>" + disabledstring);
            }
        #/
    }
}
...
function private function_91250540a257b285(notifymessage) {
    ...
    level endon("game_ended");
    while (true) {
        entitylist, var_1b1b54049e5e0079 = callon waittill(notifymessage);
        if (isdefined(var_1b1b54049e5e0079)) {
            if (var_1b1b54049e5e0079 != self.name) {
                continue;
            }
...
function private function_2125e0704a40354b(notifymessage) {
    ...
    level endon("game_ended");
    while (true) {
        var_1b1b54049e5e0079 = callon waittill(notifymessage);
        if (isdefined(var_1b1b54049e5e0079)) {
            if (var_1b1b54049e5e0079 != self.name) {
                continue;
            }
