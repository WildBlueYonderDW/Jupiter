    ...
    function debugrespawn() {
        ...
        while (true) {
            ...
            if (cmd != "<dev string:x1c>") {
                ...
                } else if (cmd == "<dev string:x42>") {
                    host = scripts\mp\gamelogic::gethostplayer();
                    bot = getbot(host);
                    if (isdefined(bot) && isdefined(host.respawnent)) {
                        host.respawnent notify("<dev string:x49>", bot);
                    }
                } else if (cmd == "<dev string:x51>") {
                    host = scripts\mp\gamelogic::gethostplayer();
                    if (isdefined(host.respawnent)) {
                        host.respawnent notify("<dev string:x59>", 1);
                    }
                } else if (cmd == "<dev string:x69>") {
                    host = scripts\mp\gamelogic::gethostplayer();
                    bot = getbot(host);
                    if (isdefined(host.respawnent) && istrue(bot.carrying)) {
                        thread dropbody(host.respawnent, bot, bot.holdingbodyhud, host.team);
                    }
                } else if (cmd == "<dev string:x6e>") {
                    host = scripts\mp\gamelogic::gethostplayer();
                    botfriend = getbot(host, 0, 1);
                    botenemy = getbot(host, 1);
                    botfriend.donotmodifydamage = 1;
                    botfriend dodamage(150, botenemy.origin, botenemy, botenemy, "<dev string:x73>");
                    botfriend.donotmodifydamage = undefined;
                } else if (cmd == "<dev string:x7d>") {
                    host = scripts\mp\gamelogic::gethostplayer();
                    botenemy = getbot(host, 1);
                    var_5b1c712938e3abf = makeweapon("<dev string:x86>");
                    host.donotmodifydamage = 1;
                    host dodamage(150, host.origin, botenemy, botenemy, "<dev string:x73>", var_5b1c712938e3abf);
