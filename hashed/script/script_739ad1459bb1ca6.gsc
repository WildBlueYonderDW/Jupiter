...
// Namespace br_train_quest / namespace_27a72657caad3672
// Params 0, eflags: 0x0
...
function private function_3c51ee611773518c() {
    ...
    while (true) {
        ...
        case #"hash_562e51cba903292e":
            ...
            self.using_progress = self.using_progress + level.framedurationseconds;
...
function private function_857dfc800d57bf4(player) {
    ...
    self endon("changed_player_using");
    while (true) {
        damage, attacker, direction_vec, point, meansofdeath, modelname, tagname, partname, idflags, objweapon, origin, angles, normal, inflictor = player waittill("damage");
        if (self.player_using == player) {
            self notify("cracking_fail");
        }
    }
