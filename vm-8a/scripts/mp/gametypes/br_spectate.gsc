...
function debugforcesre1(player, candidate, playerschecked) {
    ...
    foreach (p in playerschecked) {
        var_396a43e6475305fe += " - player: " + p.name + ", eliminated: " + istrue(p.br_iseliminated);
        count++;
        if (count > maxplayers) {
            break;
        }
...
function debugforcesre2(player, playertospectate) {
    ...
    if (isdefined(playertospectate) && isdefined(playertospectate.name)) {
        var_396a43e6475305fe += ", playerToSpectate: " + playertospectate.name + ", alive: " + isalive(playertospectate) + ", eliminated: " + istrue(playertospectate.br_iseliminated) + ", gulag: " + istrue(playertospectate.gulag);
    }
    scripts\mp\utility\script::demoforcesre(var_396a43e6475305fe);
    waitframe();
    if (isdefined(playertospectate) && isdefined(playertospectate.name)) {
