...
function init() {
    ...
    /#
        bundle = level.streakglobals.streakbundles["playKillstreakDeployDialog"];
        if (isdefined(bundle)) {
...
function hoverjet_fireonairtarget() {
    ...
    while (isdefined(self) && isdefined(self.bestairtarget)) {
        ...
        while (targettimeout > 0 && isdefined(self.bestairtarget) && !hoverjet_airtargetiswithinview(self.bestairtarget)) {
            targettimeout = targettimeout - level.framedurationseconds;
            waitframe();
        }
        if (isdefined(self.bestairtarget)) {
...
function function_380d84b0fdfca800(owner, jet) {
    /#
        ...
        while (true) {
            ...
            scalar = (anglesforward[0] * 300, anglesforward[1] * 300, anglesforward[2]);
            print3d(owner.origin + scalar, "<unknown string>" + distance(self.origin, ownereyepos), self.debugcolor);
...
function function_3f69aefa7b90fb4a() {
    /#
        ...
        self.owner notifyonplayercommand("<unknown string>", "<unknown string>");
        bombweaponname = "<unknown string>";
        bomblifetime = 10;
        for (sideoffset = 100; true; sideoffset = sideoffset * -1) {
            airstrikedebugon = getdvarint(@"hash_920f0586653a26d4", 0);
            if (!istrue(airstrikedebugon)) {
                waitframe();
                continue;
