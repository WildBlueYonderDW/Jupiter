...
function spawnaccesscards() {
    ...
    for (i = 0; i < var_f4dd331cd2cb26f1 * 3; i += 3) {
...
function showdebugresult(resulttext) {
    for (timeshown = 0; timeshown < 5; timeshown += 0.05) {
        /#
...
function debug_printcode(codestruct) {
    ...
    while (true) {
        /#
            printtoscreen2d(500, 100, "<dev string:x1c>", (1, 1, 1), 1);
        #/
        if (!scripts\mp\flags::gameflag("prematch_done")) {
            /#
                printtoscreen2d(600, 100, "<dev string:x35>", (1, 1, 1), 1);
            #/
        } else {
...
function debug_show2dvotext(text, time) {
    ...
    for (curtime = 0; curtime < time; curtime += 0.05) {
        /#
            printtoscreen2d(500, 350, "<dev string:x4e>" + text, (1, 1, 1), 1.5);
        #/
        waitframe();
    }
}
    ...
    function function_138da425d10b4a04() {
        while (true) {
            if (getdvarint(@"hash_b816c5e95360de9e", 0) == 1) {
                if (!istrue(level.stadium_puzzle.boardroomopen)) {
                    if (level.stadium_puzzle.hints["<dev string:x56>"].size > 0) {
                        foreach (keypad in level.stadium_puzzle.hints["<dev string:x56>"]) {
                    ...
                    scripts\engine\utility::exploder("<dev string:x6f>");
                }
