...
function function_39e1cd8a0fce827a(position, dimensions, rot, shape) {
    ...
    for (i = 0; i < 3; i++) {
        if (!isdefined(level.exclusionzones) || !isdefined(level.exclusionzones[i])) {
            level.exclusionzones[i] = function_7d635301465bbed1(position, dimensions, rot, shape);
            return i;
        }
    }
...
function function_de57b9616f575a63(position, var_72759fb8bd38aa1e) {
    ...
    for (i = 0; i < 3; i++) {
        if (isdefined(level.exclusionzones[i])) {
            if (level.exclusionzones[i] function_91bf6ad362e62084(position, var_72759fb8bd38aa1e)) {
                return i;
            }
        }
    }
...
/#
    ...
    function function_a4c336601037a54c(var_72759fb8bd38aa1e) {
        ...
        string = "<dev string:x2f>";
        for (i = 0; i < 3; i++) {
            if (isdefined(level.exclusionzones[i])) {
                string = string + i + "<dev string:x45>" + (level.exclusionzones[i] function_91bf6ad362e62084(position, var_72759fb8bd38aa1e) ? "<dev string:x48>" : "<dev string:x50>");
            }
        }
        string = string + "<dev string:x59>" + function_de57b9616f575a63(position, var_72759fb8bd38aa1e);
        iprintlnbold(string);
    }
