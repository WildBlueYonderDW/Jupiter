...
function on_bot_killed(einflictor, attacker, idamage, smeansofdeath, sweapon, vdir, shitloc, psoffsettime, deathanimduration, killid) {
    ...
    /#
        if (isdefined(level.testdummy) && level.testdummy == self && isdefined(self.menu) && isdefined(self.firstdamagetime)) {
            ...
            self.menu["<dev string:x3fb>"] setvalue(function_c89ed1840c8d0f0f(ttk));
...
/#
    ...
    function function_85449d9e699cd4cd() {
        self notify("<dev string:x3ff>");
        self endon("<dev string:x3ff>");
        self endon("<dev string:x40f>");
        level endon("<dev string:x1c>");
        while (true) {
            if (getdvarint(@"hash_c2de8ca6dc8512c1")) {
                wait 0.05;
    ...
    if (isdefined(crate.boxtype)) {
        ...
        /#
            if (isdefined(level.bot_can_use_box_by_type) && !isdefined(level.bot_can_use_box_by_type[crate.boxtype])) {
                assertmsg("<dev string:x423>" + crate.boxtype + "<dev string:x430>");
                return false;
            }
        #/
...
function monitor_smoke_grenades() {
    ...
    while (true) {
        grenade, objweapon = level waittill("smoke");
...
function handle_smoke() {
    ...
    explosion_location = self waittill("explode");
    ...
    function function_361fab20102e6557() {
        ...
        while (true) {
            ...
            if (var_c2985b5ae5fee336) {
                ...
                player = level waittill("<dev string:x44c>");
            ...
            if (var_e9b4a948c50db028) {
                ...
                level thread [[ level.bot_funcs["<dev string:x45b>"] ]](1, team, undefined, undefined, undefined, undefined, fixedspawndata);
            }
            waitframe();
        }
    }
    ...
    function function_c71139dd1c600b64() {
        ...
        while (true) {
            ...
            if (var_80037671e3f2ed57) {
                ...
                case 1: 
                    var_9369cf0d229589 = "<dev string:x466>";
                ...
                foreach (bot in level.var_6d4b0d324eef7492) {
                    ...
                    if (var_9369cf0d229589 == "<dev string:x466>" || var_9369cf0d229589 == bot.team) {
...
/#
        ...
        while (true) {
            ...
            if (var_a074ce85487e9da0) {
                ...
                if (var_a074ce85487e9da0 == 1) {
                    ...
                    spawnorigin = results["<dev string:x46a>"];
                    spawnangles = results["<dev string:x471>"];
                }
                ...
                fixedspawndata.origin = spawnorigin;
                fixedspawndata.angles = spawnangles;
                level thread [[ level.bot_funcs["<dev string:x45b>"] ]](1, team, &function_8af1863f5e7ade4a, undefined, undefined, undefined, fixedspawndata);
            }
            waitframe();
        }
    }
    ...
    function function_b09b8c00320b91e(eattacker, idamage, smeansofdeath, objweapon, einflictor, shitloc, vpoint, var_1da1a66b5c6a06a7) {
        ...
        weap_token = strtok(getcompleteweaponname(objweapon), "<dev string:x478>");
        weap_name = "<dev string:x38>";
        foreach (tok in weap_token) {
            weap_name += tok + "<dev string:x47a>";
        }
        if (isdefined(self.armorhealth)) {
            self.menu["<dev string:x47c>"] setvalue(self.armorhealth);
        }
        self.menu["<dev string:x482>"] setvalue(self.health);
        self.menu["<dev string:x489>"] setvalue(idamage + var_1da1a66b5c6a06a7);
        self.menu["<dev string:x490>"] setvalue(self.var_b0ab2dca24bfe60a);
        self.menu["<dev string:x49c>"] setvalue(self.var_2fc19f3a438f03a5);
        self.menu["<dev string:x4a1>"] setvalue(range);
        self.menu["<dev string:x4a7>"] setdevtext(shitloc);
        self.menu["<dev string:x4ae>"] setdevtext(weap_name);
    }
    ...
    function function_5723f098aabd9836(host) {
        ...
        placementmodel = spawn("<dev string:x4b5>", (0, 0, 0));
        placementmodel setmodel("<dev string:x4c2>");
        for (;;) {
            waitframe();
            spawnorigin = function_b771795f421ef637(host);
            if (!isdefined(spawnorigin)) {
            ...
            if (host usebuttonpressed()) {
                results = [];
                results["<dev string:x46a>"] = spawnorigin;
                results["<dev string:x471>"] = spawnangles;
    ...
    function function_e53f5c23f0a7a78f(host) {
        ...
        y = 145;
        scale = 0.9;
        self.hud = host function_829a24c34a7fda26("<dev string:x4d2>", x, y, scale, 0.6, 10);
        menu = [];
        menux = x + 5;
        if (isdefined(host.armorhealth)) {
            menu[menu.size] = host function_829a24c34a7fda26("<dev string:x4dd>", menux, y + 10 * (menu.size + 1), scale);
        }
        menu[menu.size] = host function_829a24c34a7fda26("<dev string:x4e5>", menux, y + 10 * (menu.size + 1), scale);
        menu[menu.size] = host function_829a24c34a7fda26("<dev string:x4ee>", menux, y + 10 * (menu.size + 1), scale);
        menu[menu.size] = host function_829a24c34a7fda26("<dev string:x4f7>", menux, y + 10 * (menu.size + 1), scale);
        menu[menu.size] = host function_829a24c34a7fda26("<dev string:x506>", menux, y + 10 * (menu.size + 1), scale);
        menu[menu.size] = host function_829a24c34a7fda26("<dev string:x517>", menux, y + 10 * (menu.size + 1), scale);
        menu[menu.size] = host function_829a24c34a7fda26("<dev string:x526>", menux, y + 10 * (menu.size + 1), scale);
        menu[menu.size] = host function_829a24c34a7fda26("<dev string:x52e>", menux, y + 10 * (menu.size + 1), scale);
        menu[menu.size] = host function_829a24c34a7fda26("<dev string:x53d>", menux, y + 10 * (menu.size + 1), scale);
        menux = x + 85;
        indexoffset = menu.size;
        if (isdefined(host.armorhealth)) {
            menu["<dev string:x47c>"] = host function_829a24c34a7fda26("<dev string:x38>", menux, y + 10 * (menu.size + 1 - indexoffset), scale);
        }
        menu["<dev string:x482>"] = host function_829a24c34a7fda26("<dev string:x38>", menux, y + 10 * (menu.size + 1 - indexoffset), scale);
        menu["<dev string:x489>"] = host function_829a24c34a7fda26("<dev string:x38>", menux, y + 10 * (menu.size + 1 - indexoffset), scale);
        menu["<dev string:x490>"] = host function_829a24c34a7fda26("<dev string:x38>", menux, y + 10 * (menu.size + 1 - indexoffset), scale);
        menu["<dev string:x49c>"] = host function_829a24c34a7fda26("<dev string:x38>", menux, y + 10 * (menu.size + 1 - indexoffset), scale);
        menu["<dev string:x3fb>"] = host function_829a24c34a7fda26("<dev string:x38>", menux, y + 10 * (menu.size + 1 - indexoffset), scale);
        menu["<dev string:x4a1>"] = host function_829a24c34a7fda26("<dev string:x38>", menux, y + 10 * (menu.size + 1 - indexoffset), scale);
        menu["<dev string:x4a7>"] = host function_829a24c34a7fda26("<dev string:x38>", menux, y + 10 * (menu.size + 1 - indexoffset), scale);
        menu["<dev string:x4ae>"] = host function_829a24c34a7fda26("<dev string:x38>", menux, y + 10 * (menu.size + 1 - indexoffset), scale);
        self.menu = menu;
    }
    ...
    function function_829a24c34a7fda26(text, x, y, scale, alpha, sort) {
        ...
        hudelem.elemtype = "<dev string:x546>";
        hudelem.alpha = alpha;
