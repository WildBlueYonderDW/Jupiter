...
function molotov_register_scriptable(shareddata) {
    ...
    println("<dev string:x1c>" + leveldata.scriptables.size);
...
function molotov_register_trigger(trigger) {
    ...
    println("<dev string:x52>" + self getentitynumber() + "<dev string:x7b>" + leveldata.triggers.size);
...
function molotov_delete_trigger() {
    ...
    println("<dev string:x89>" + self getentitynumber() + "<dev string:x7b>" + leveldata.triggers.size);
    self delete();
}
...
function molotov_delete_pool_by_id(id, immediate) {
    ...
    if (isdefined(scriptable)) {
        if (!istrue(immediate)) {
            ...
            println("<dev string:xb2>" + leveldata.scriptables.size);
            return;
        }
        scriptable thread molotov_delete_scriptable();
        println("<dev string:xf5>" + leveldata.scriptables.size);
    }
    trigger = leveldata.triggers[id];
    if (isdefined(trigger)) {
...
function molotov_branch_register_cast(casttype, var_8e87ebe279cdfcfb, hitposition) {
    ...
    /#
        ...
        if (isdefined(var_8e87ebe279cdfcfb)) {
            ...
            case 0: 
                debugmsg = "<dev string:x137>";
                break;
            case 1: 
                debugmsg = "<dev string:x143>";
                break;
            case 2: 
                debugmsg = "<dev string:x14c>";
                break;
            case 3: 
                debugmsg = "<dev string:x15c>";
                break;
            }
            ...
            switch (casttype) {
            case 0: 
                debugmsg = "<dev string:x163>";
                break;
            case 1: 
                debugmsg = "<dev string:x16b>";
                break;
            case 2: 
                debugmsg = "<dev string:x175>";
                break;
            }
        }
