...
/#
    ...
    function function_f539d5d6dc0cc4a7() {
        ...
        self.debug_trig = spawn("<dev string:x40>", self.origin, 0, 48, 48);
        self.debug_trig enablelinkto();
        self.debug_trig linkto(self);
        cylinder(self.origin, self.origin + (0, 0, 48), 48, (1, 1, 0), 0, 100);
        while (true) {
            ent = self.debug_trig waittill("<dev string:x4f>");
            assert(isdefined(ent));
            self notify("<dev string:x57>", [ent]);
            print3d(self.origin, "<dev string:x67>", (1, 1, 0), 1, 0.5, 100, 1);
        }
    }
