        ...
        if (level.loot_choppers.size < var_e1ce21e5b4d1163f) {
            ...
            for (i = 0; i < var_6eebc4632b89c8c8; i++) {
                ...
                if (!isdefined(patrolzone)) {
                    /#
                        iprintlnbold("<dev string:x1c>" + quadid + "<dev string:x61>");
                    #/
                    wait 1;
                }
                if (i == 0) {
...
/#
    ...
    function function_fb7c809b0f8f7639() {
        ...
        while (true) {
            ...
            var_477413d8c0675495 = getdvarint(@"hash_38568daa586b1fe3", 6000);
            if (level.mapname == "<dev string:xb4>") {
                var_477413d8c0675495 = 1000;
            }
            quadid++;
            if (quadid > 4) {
            ...
            patrolzone = lootchopper_findunoccupiedpatrolzone(level.loot_chopper_spawns["<dev string:xc4>" + quadid], var_477413d8c0675495);
            if (!isdefined(patrolzone)) {
                iprintlnbold("<dev string:x1c>" + quadid + "<dev string:x61>");
                wait 1;
            }
            showsplashtoall("<dev string:xca>");
            lootchopper_spawn(patrolzone);
