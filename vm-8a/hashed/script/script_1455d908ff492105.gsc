...
function function_2bd5e2012f429ae3(turret) {
    ...
    if (istrue(turret.isplaced) && !istrue(turret.owner.ksempd) && !istrue(utility::isusingremote()) && !self isswimming()) {
        ...
        /#
            function_45456bb78fa8eb2d("<dev string:x1c>");
        #/
        function_baa645fe416db1d5(turret);
        /#
            function_45456bb78fa8eb2d("<dev string:x35>");
        #/
        ...
        /#
            ...
            function_45456bb78fa8eb2d("<dev string:x53>");
        #/
...
function function_a920f20104777b97(turret, var_de4dbbca9f355800) {
    ...
    if (hasendedearly) {
        /#
            function_45456bb78fa8eb2d("<dev string:x69>");
        #/
        function_178aa80b91f36bf4();
...
function function_3bb43305bf56bfec(var_630af5192659dd3b, player) {
    /#
        self.owner endon("<dev string:xbc>");
    #/
    self endon("kill_turret");
    self endon("carried");
...
/#
    ...
    function function_1223f321ae6ff9a5(var_630af5192659dd3b) {
        ...
        level endon("<dev string:x85>");
        self.turrettarget = spawn("<dev string:xe7>", self gettagorigin("<dev string:xf4>") + anglestoforward(self gettagangles("<dev string:xf4>") * 300));
        ...
        manualmode = "<dev string:xfe>";
        turretmode = sentrymode;
        notifyon = 0;
        while (true) {
            if (getdvarint(@"hash_c9bd691ce96079ac", 0) == 1) {
                if (!istrue(notifyon)) {
                    self.owner notifyonplayercommand("<dev string:xbc>", "<dev string:x10c>");
                    notifyon = 1;
                }
                self.owner waittill("<dev string:xbc>");
                if (turretmode != manualmode) {
                    ...
                    endpos = undefined;
                    if (isdefined(trace["<dev string:x119>"]) && trace["<dev string:x119>"] != "<dev string:x121>") {
                        endpos = trace["<dev string:x12e>"];
                    }
                    if (isdefined(endpos)) {
            ...
            if (turretmode != sentrymode) {
                self.owner notifyonplayercommandremove("<dev string:xbc>", "<dev string:x10c>");
                function_c151e4a17d929b78();
                turretmode = sentrymode;
                self setmode(turretmode);
                thread function_3bb43305bf56bfec();
    ...
    function function_ee5cbfd60227d0b8(firepos) {
        ...
        starttag = "<dev string:x14a>";
        if (self.streakinfo.streakname == "<dev string:x15b>") {
            starttag = "<dev string:x166>";
        }
        endtag = "<dev string:xf4>";
        wait 1;
        while (true) {
    ...
    function function_c151e4a17d929b78() {
        ...
        thread function_b7ffbe66f2c448fc();
        self.turrettarget.targeton = 0;
        self.owner notify("<dev string:x137>");
    }
    ...
    function function_8ffe5560e2df8a72(turret) {
        turret endon("<dev string:xd3>");
        turret endon("<dev string:xdf>");
        self endon("<dev string:x9b>");
        level endon("<dev string:x85>");
        while (true) {
    ...
    function function_818d215a79eecafe(turret) {
        ...
        self endon("<dev string:x9b>");
        level endon("<dev string:x85>");
        turret waittill("<dev string:x9b>");
        self delete();
    }
...
/#
    ...
    function function_45456bb78fa8eb2d(printmsg) {
        if (getdvarint(@"hash_2da6e96d161f55bc", 0) == 1) {
            print("<dev string:x179>" + printmsg);
        }
    }
