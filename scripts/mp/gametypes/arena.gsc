    ...
    foreach (entry in level.teamnamelist) {
        ...
        if (oneLeft) {
            ...
            foreach (player in players) {
                if (!isalive(player)) {
                    teammatelives = teammatelives + player.pers["lives"];
                }
            }
            if (teammatelives == 0) {
...
function setupweapons() {
    ...
    foreach (loc in locs) {
        /#
            debugspawnweapon = getdvar(@"hash_e7ea6273ed7f950f", "camo_l_02");
            if (debugspawnweapon != "camo_l_02") {
...
function getgungameloadoutomnvarindex(player, winningteam) {
    ...
    if (isgungameloadouts()) {
        ...
        if (isdefined(winningteam) && winningteam == player.pers["team"]) {
            playerloadoutindex = playerloadoutindex + 1;
        }
    } else {
        playerloadoutindex = game["roundsWon"][getotherteam(player.pers["team"])[0]];
...
function private function_c452035f58a6ad0e(player, timeinterval) {
    /#
        ...
        for (i = 0; i < game["loadoutKillstreak2"].size; i++) {
            ...
            player scripts/mp/class::giveloadout(player.team, "offhand_2h_wm_grenade_semtex_v0");
            wait(timeinterval);
        }
        player notify("camo_b_06");
...
function private function_d70a085b24e94e4b(player, weaponname, timeinterval) {
    /#
        ...
        thread function_a554b9ddc3d15ec5(player, "camo_f_14", primaryweaponobject);
