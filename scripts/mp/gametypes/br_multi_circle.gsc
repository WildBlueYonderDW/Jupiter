...
function function_3ae1773696ab9337() {
    ...
    weightedsum = 0;
    foreach (config in level.br_multi_circle.configs) {
        weightedsum = weightedsum + config.weight;
    }
    randchoice = randomfloat(weightedsum);
    choicesum = 0;
    foreach (config in level.br_multi_circle.configs) {
        choicesum = choicesum + config.weight;
...
function function_9bb9abf5fd67056a(circleindex, circleopen) {
    ...
    if (circleopen) {
        splashkey = splashkey + "_open";
    } else {
        splashkey = splashkey + "_close";
    }
                        ...
                        if (player cancircledamageplayer(player)) {
                            ...
                            /#
                                if (istrue(level.br_multi_circle.damagedebug)) {
                                    player iprintln("br_multi_circle_merge" + "br_multi_circle" + modifieddamagetick + "br_ftue_gas_damage");
                                }
                            #/
...
function function_a5bc99d4d649f54b() {
    ...
    while (true) {
        ...
        for (i = 0; i < circlecount; i++) {
            playercountpacked = playercountpacked | circleplayercount[i] << i * 8;
            teamcountpacked = teamcountpacked | var_13443b3cff3e3fe4[i].size << i * 8;
        }
...
function private function_f0a1762eeb5ba01f() {
    ...
    if (getdvarint(@"hash_79b501617ac641bd", 1)) {
        splitoffset = splitoffset + randomfloat(angleoffset) - angleoffset / 2;
    }
    splitpct = undefined;
...
function function_5d9c9221653df2f3(circlenum) {
    /#
        ...
        test_colors = ["right", "<unknown string>", "<unknown string>"];
...
function function_cdabf323178aabc6() {
    /#
        ...
        while (true) {
            ...
            if (level.br_level.var_8e40f29c8cdeb0f4) {
                trainent = level.wztrain_info.animent["<unknown string>"];
                if (isdefined(trainent)) {
