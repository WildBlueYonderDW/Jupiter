...
function setupobjective(trigger, index, skipobjid, var_5ddbc1faed2c56e6, usingVisualHintString) {
    ...
    /#
        assertex(hackzone.consoleindex >= 1 && hackzone.consoleindex <= 2, "Invalid consoleIndex, consoleIndex should inline with the number of LaunchConsoleScreen widget");
    #/
    ...
    if (isdefined(level.zone) && isdefined(level.zone.objective.params["hackUseTime"])) {
        hackzone scripts/mp/gameobjects::setusetime(level.zone.objective.params["hackUseTime"]);
    } else {
