...
function private function_3554b56e0cccd80f(attractor) {
    /#
        ...
        range_text = ter_op(attractor.range > 0, "<unknown string>" + attractor.range, "<unknown string>");
        collision_ent = default_to(attractor.var_eb287a759525f0bd, attractor.source);
        health_text = ter_op(isdefined(collision_ent.health), "<unknown string>" + string(default_to(collision_ent.health, 0)), "<unknown string>");
        record3dtext("<unknown string>" + string(attractor.priority) + range_text, attractor.source.origin + (0, 0, 32), color);
        if (attractor.is_attackable) {
            record3dtext("<unknown string>" + health_text, attractor.source.origin + (0, 0, 24), color);
        ...
        if (getdvarint(@"hash_a83f59607205c9c0", 0) <= 0) {
            print3d(attractor.source.origin + (0, 0, 32), "<unknown string>" + string(attractor.priority) + range_text, color);
            if (attractor.is_attackable) {
                print3d(attractor.source.origin + (0, 0, 24), "<unknown string>" + health_text, color);
            }
            if (attractor.range > 0) {
