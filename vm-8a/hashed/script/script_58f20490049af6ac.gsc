...
function function_60951b84c58915ab(testpoint, var_ff696ac4914036ee) {
    ...
    for (multicirclecount = 0; multicirclecount < circleinfo.circles.size; multicirclecount++) {
        ...
        for (i = 0; i < circlecenters.size - 1; i++) {
            ...
            currenttime += level.br_level.br_circledelaytimes[i];
            ...
            currenttime += level.br_level.br_circleclosetimes[i];
        }
    }
...
function function_aa86aebc45c3ebca(var_dfd8829dea17d4f5) {
    ...
    while (true) {
        ...
        /#
            debugplayer iprintlnbold("<dev string:x3b>" + level.var_779cd9f4c4324154 + "<dev string:x50>" + level.var_da89dff7e3009777[level.var_779cd9f4c4324154].name + "<dev string:x68>");
        #/
...
/#
    ...
    function savecircles() {
        ...
        for (i = 0; i < level.br_level.br_circlecenters.size; i++) {
            ...
            println("<dev string:xb8>" + i + "<dev string:xda>" + origin + "<dev string:xe0>");
            waitframe();
        }
        println("<dev string:xb7>");
        for (i = 0; i < level.br_level.br_circleradii.size; i++) {
            radius = level.br_level.br_circleradii[i];
            println("<dev string:xe2>" + i + "<dev string:xda>" + radius + "<dev string:xe0>");
            waitframe();
        }
        println("<dev string:xb7>");
        multicirclecount = function_d987886bb9de9137();
        for (i = 0; i < multicirclecount; i++) {
            for (j = 0; j < level.br_level.br_circlecenters.size; j++) {
                ...
                println("<dev string:x102>" + i + "<dev string:x122>" + j + "<dev string:xda>" + origin + "<dev string:xe0>");
                waitframe();
            }
        }
        println("<dev string:xb7>");
