...
function getdroplocationnearcurrentcircle(radiusbuffer) {
    ...
    if (isdefined(level.juggdroplocations) && level.juggdroplocations.size > 0) {
        ...
        /#
            ...
            if (level.mapname == "<dev string:x18c>") {
...
/#
    ...
    function function_59ed9e94de8f9c42() {
        ...
        while (true) {
            ...
            showsplashtoall("<dev string:x1a7>");
    ...
    function function_fdcc5bb7ff13a7d1() {
        ...
        while (true) {
            ...
            if (isdefined(trace) && trace["<dev string:x1bc>"] != "<dev string:x1c4>") {
                dropcircle = scripts\mp\gametypes\br_jugg_common::createjuggdroplocation(trace["<dev string:x1d1>"]);
                dropcircle.beacon = spawn("<dev string:x1da>", dropcircle.origin);
                dropcircle.beacon setmodel("<dev string:x1e7>");
                dropcircle.beacon setscriptablepartstate("<dev string:x1fb>", "<dev string:x20c>", 0);
                dropcircle function_6b6b6273f8180522("<dev string:x20f>", dropcircle.origin, 5000);
                dropcircle function_6988310081de7b45();
            }
            if (isdefined(dropcircle)) {
                crate = scripts\cp_mp\killstreaks\airdrop::dropbrc130airdropcrate(dropcircle.origin + (0, 0, 5000), dropcircle.origin, (0, 0, 0), "<dev string:x21d>", "<dev string:x236>");
                if (isdefined(crate)) {
                    crate.dropcircle = dropcircle;
                    level.c130successfulairdrops[level.c130successfulairdrops.size] = crate;
                    triggerobject = scripts\cp_mp\killstreaks\airdrop::gettriggerobject(crate);
