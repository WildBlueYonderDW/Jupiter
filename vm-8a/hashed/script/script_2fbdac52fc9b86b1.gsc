...
function private run_step(quest, current_step, parent_step) {
    ...
    /#
        iprintlnbold(quest.name + "<dev string:x1c>" + current_step.path + "<dev string:x1e>");
    #/
    ...
    if (quest.var_368d67245b06a283) {
        /#
            iprintlnbold(quest.name + "<dev string:x29>");
        #/
        return;
    }
...
/#
    ...
    function private function_93119194066b1e22(quest_name) {
        function_6e7290c8ee4f558b("<dev string:x96>" + quest_name + "<dev string:x1c>");
        function_b23a59dfb4ca49a1("<dev string:xa0>", "<dev string:xab>" + quest_name, &function_f7d7be1a5c4f120f);
    ...
    function private function_6e2f110fb0d51295(quest_name, step) {
        devgui_path = "<dev string:x96>" + quest_name + "<dev string:x1c>";
        step_tree = strtok(step.path, "<dev string:x1c>");
        ...
        for (tree_depth = 0; tree_depth < step_tree.size; tree_depth++) {
            ...
            devgui_path += "<dev string:xb7>" + child_step.index + "<dev string:xb9>" + child_step.name + "<dev string:x1c>";
            parent_step = child_step;
        }
        function_6e7290c8ee4f558b(devgui_path);
        function_b23a59dfb4ca49a1("<dev string:xbc>", "<dev string:xc9>" + quest_name + "<dev string:xd2>" + step.path, &function_962580ef2ed6fd8c);
        function_b23a59dfb4ca49a1("<dev string:xd4>", "<dev string:xe2>" + quest_name + "<dev string:xd2>" + step.path, &function_acdda71fab4ed5bc);
        function_fe953f000498048f();
    }
    ...
    function function_4d1eb7b9a161037f(quest_name, step_path) {
        devgui_path = "<dev string:x96>" + quest_name + "<dev string:x1c>";
        step_tree = strtok(step_path, "<dev string:x1c>");
        parent_step = level._quest[quest_name];
        for (tree_depth = 0; tree_depth < step_tree.size; tree_depth++) {
            child_step = undefined;
            foreach (child in parent_step.steps) {
    ...
    function complete_step(step) {
        ...
        step.b_skipped = 1;
        step.completed = 1;
        step.quest.owner notify(step.quest.name + "<dev string:x1c>" + step.path + "<dev string:xec>");
    ...
    function function_acdda71fab4ed5bc(params) {
        quest_name = params[0];
        step_path = params[1];
        assertex(isdefined(level._quest) && isdefined(level._quest[quest_name]), "<dev string:xf7>" + quest_name + "<dev string:x10b>");
        quest = level._quest[quest_name];
        if (!istrue(quest.started)) {
            iprintlnbold("<dev string:x126>" + quest_name + "<dev string:x132>");
            return;
        }
        ...
        for (tree_depth = 0; tree_depth < step_tree.size; tree_depth++) {
            ...
            iprintlnbold(step_tree[tree_depth] + "<dev string:x14e>" + parent_step.path + "<dev string:x16b>" + quest_name + "<dev string:x17c>");
            return;
        }
        assert(isdefined(target_step));
        if (parent_step != quest) {
            if (array_contains(parent_step.completed_steps, target_step.name)) {
                iprintlnbold("<dev string:x17f>" + target_step.path + "<dev string:x18a>" + quest_name + "<dev string:x17c>");
            } else {
