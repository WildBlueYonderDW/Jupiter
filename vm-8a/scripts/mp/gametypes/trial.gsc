...
function trial_mission_data_init() {
    ...
    if (missionid == 0) {
        ...
        if (fallbackid != "") {
            println("<dev string:x1c>" + fallbackid + "<dev string:x53>" + getdvar(@"hash_687fb8f9b7a23245"));
...
function trial_weapon_spawn() {
    ...
    /#
        strid = tablelookup(trial_fetch_mission_table(), 0, level.trial["<dev string:xa9>"], 1);
        if (strid == "<dev string:xb3>") {
