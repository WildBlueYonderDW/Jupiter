...
function function_158897145d2efb26(player, objweapon) {
    self endon("race_end");
    for (timeelapsed = 0; player hasweapon(objweapon) && timeelapsed < 5; timeelapsed = timeelapsed + level.framedurationseconds) {
        waitframe();
    }
...
function dropweaponfordeathlaunch(item, weapongroup, damage, angles) {
    ...
    launchorigin = launchorigin + anglestoforward(weaponangles) * var_eac2b0512d0b1c55;
    launchorigin = launchorigin + anglestoright(weaponangles) * var_8282854ec42283a2;
