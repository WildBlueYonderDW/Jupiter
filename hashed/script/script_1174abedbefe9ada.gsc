...
function private function_dbf56756677d1ae1() {
    ...
    if (isdefined(level.var_2cdf30478fa435ff)) {
        ...
        for (i = 0; i < 40; i++) {
            ...
            raworigin = raworigin + (randomfloat(level.radiation.distancevalues["origin"][0]) - level.radiation.distancevalues["spread"], randomfloat(level.radiation.distancevalues["origin"][1]) - level.radiation.distancevalues["spread"], 0);
            ...
            var_c72adf2f44963edf = distance2d((0, 0, 0), raworigin);
            if (var_c72adf2f44963edf < level.radiation.distancevalues["maxDist"]) {
                continue;
            }
...
function radiation_startspread() {
    ...
    namespace_446fc987a980892f::playevent("radiation_start_spread", level.players);
    ...
    foreach (player in level.radiation.var_378d3b538b839d2c) {
        if (isdefined(player) && isdefined(player.origin) && function_1aa28ad2c858d3d7(player.origin)) {
            ...
            if (scripts/cp_mp/gasmask::hasgasmask(player)) {
                namespace_446fc987a980892f::playevent("radiation_approach_with_gas_mask", [player]);
            } else {
                namespace_446fc987a980892f::playevent("radiation_approach", [player]);
            }
            continue;
        }
        if (!isdefined(player)) {
...
function function_9bc063861b9330f1(damage) {
    ...
    level.radiation.bonusdamage = level.radiation.bonusdamage + int(damage);
}
...
function private function_131e36da68551999() {
    level endon("game_ended");
    while (true) {
        if (istrue(level.var_289df80e1ded586f) && (istrue(level.radiation.spreading) || istrue(level.radiation.var_9957c36d26b92420))) {
