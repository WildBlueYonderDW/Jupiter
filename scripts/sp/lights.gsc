...
function light_debug_thread() {
    ...
    lights = array_combine(getentarray("light_omni", "classname"), lights);
    while (true) {
        var_eef67125f5fbd62 = 0;
        foreach (light in lights) {
            if (isdefined(light)) {
                light light_debug_draw();
                continue;
            }
            var_eef67125f5fbd62 = 1;
        }
        if (var_eef67125f5fbd62) {
...
function strobelight(var_6cedf2c7344e7ae2, var_6cedf3c7344e7d15, period, var_c32ada7be8e0120) {
    ...
    while (true) {
        ...
        self setlightintensity(var_6cedf2c7344e7ae2 + (var_6cedf3c7344e7d15 - var_6cedf2c7344e7ae2) * var_12d4a3234a7f0591);
        wait 0.05;
        time += 0.05;
        if (time > period) {
            time -= period;
...
// Size: 0x3f
function changelightcolorto(var_d63d6df0a89ecacb, totaltime, acceltime, deceltime) {
    ...
    thread changelightcolortoworkerthread(var_d63d6df0a89ecacb, totaltime, acceltime, deceltime);
}
...
// Size: 0x129
function changelightcolortoworkerthread(var_d63d6df0a89ecacb, totaltime, acceltime, deceltime) {
    startcolor = self getlightcolor();
    timefactor = 1 / (totaltime * 2 - acceltime + deceltime);
    time = 0;
    if (time < acceltime) {
        var_85138ed12b40fd22 = timefactor / acceltime;
        while (time < acceltime) {
            fraction = var_85138ed12b40fd22 * time * time;
            self setlightcolor(vectorlerp(startcolor, var_d63d6df0a89ecacb, fraction));
            wait 0.05;
            time += 0.05;
        }
    }
    ...
    time = totaltime - time;
    if (time > 0) {
        var_85138ed12b40fd22 = timefactor / deceltime;
        while (time > 0) {
            fraction = 1 - var_85138ed12b40fd22 * time * time;
            self setlightcolor(vectorlerp(startcolor, var_d63d6df0a89ecacb, fraction));
            wait 0.05;
            time -= 0.05;
        }
    }
    self setlightcolor(var_d63d6df0a89ecacb);
}
...
function set_sun_shadow_params(duration) {
    var_5d31c6f56fa7130e = getdvarint(@"sm_sunEnable", 1);
    var_ff6f85478474b8d5 = getdvarfloat(@"hash_8d3379c8854f3c37", 1);
    sunsamplesizenear = getdvarfloat(@"sm_sunSampleSizeNear", 0.25);
    var_e080de7b131639aa = getdvarfloat(@"hash_485afd6362225fb0", 1);
    if (isdefined(self.script_sunenable)) {
        var_5d31c6f56fa7130e = self.script_sunenable;
    }
    if (isdefined(self.script_sunshadowscale)) {
        var_ff6f85478474b8d5 = self.script_sunshadowscale;
    }
    ...
    if (isdefined(self.script_qualityspotshadow)) {
        var_e080de7b131639aa = self.script_qualityspotshadow;
    }
    var_ce005b1cc11ac933 = getdvarint(@"sm_sunEnable", 1);
    var_f75f0121b866fc52 = getdvarfloat(@"hash_8d3379c8854f3c37", 1);
    var_d1df8ca895b4a0af = getdvarint(@"hash_485afd6362225fb0", 1);
    setsaveddvar(@"sm_sunEnable", var_5d31c6f56fa7130e);
    setsaveddvar(@"hash_8d3379c8854f3c37", var_ff6f85478474b8d5);
    setsaveddvar(@"hash_485afd6362225fb0", var_e080de7b131639aa);
