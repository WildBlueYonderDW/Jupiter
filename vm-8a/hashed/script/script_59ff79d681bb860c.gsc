...
/#
    ...
    function adddevguicmd(command) {
        finalcommand = "<dev string:x250>";
        alltokens = strtok(command, "<dev string:x251>");
        tokensnumber = 0;
        foreach (tok in alltokens) {
            token = strtok(tok, "<dev string:x253>");
            count = 1;
            size = token.size;
            foreach (new_tok in token) {
                if (count < size) {
                    finalcommand = finalcommand + new_tok + "<dev string:x253>";
                } else {
    ...
    function function_a47f77af11ab531d() {
        ...
        for (showoutline = 0; true; showoutline = !getdvarint(@"hash_283524741d876bcd", 0)) {
                ...
                if (showoutline > 0) {
                    foreach (player in level.players) {
                        ...
                        if (isdefined(player.enemy)) {
                            player hudoutlineenable("<dev string:x260>");
                            continue;
                        }
                        player hudoutlineenable("<dev string:x274>");
                    }
                } else {
                    foreach (player in level.players) {
                        if (!isbot(player)) {
            ...
            msg = level waittill_any_in_array_or_timeout(["<dev string:x28a>"], 1);
            if (msg == "<dev string:x28a>") {
            }
        }
    }
    ...
    function function_d3d9d9ce8bb3754d() {
        ...
        while (true) {
            ...
            foreach (bot in level.players) {
                ...
                if (drawteam) {
                    bots = scripts\mp\utility\teams::getteamdata(bot.team, "<dev string:x299>");
                ...
                screenx = 600;
                screeny = 700;
                printstring = "<dev string:x2a6>" + bot.team + "<dev string:x2ad>" + bot.name;
                ...
                if (isdefined(bot.status) && bot.status.size > 0) {
                    printstring = "<dev string:x2b5>" + bot.status;
                    if (isdefined(bot.mind_personality)) {
                        printstring += "<dev string:x2be>" + bot function_d72fc34d742ef919();
                    }
                    printstring += "<dev string:x2be>" + bot function_21fe18b1e1e83a2a();
                    print3d(printlocation, printstring, var_de5a5558ba4d830a, 1, scale, 1);
                    printlocation += (0, 0, linespacing);
                    if (var_89bee820769f600d) {
                        printtoscreen2d(screenx, screeny, printstring, var_c89cc0d4a5adb6d5, 1.5);
                ...
                if (isdefined(bot.mind)) {
                    ...
                    if (var_577f45571c039a54.size > 0) {
                        print3d(printlocation, "<dev string:x2c2>" + var_577f45571c039a54, var_de5a5558ba4d830a, 1, scale, 1);
                        if (var_89bee820769f600d) {
                            printtoscreen2d(screenx, screeny, "<dev string:x2c2>" + var_577f45571c039a54, var_c89cc0d4a5adb6d5, 1.5);
                        }
                    }
                    printlocation += (0, 0, linespacing);
                    if (var_89bee820769f600d) {
                    ...
                    if (actions.size > 0) {
                        print3d(printlocation, "<dev string:x2ce>" + actions, var_de5a5558ba4d830a, 1, scale, 1);
                        printlocation += (0, 0, linespacing);
                        if (var_89bee820769f600d) {
                            printtoscreen2d(screenx, screeny, "<dev string:x2ce>" + actions, var_c89cc0d4a5adb6d5, 1.5);
                            screeny += var_76bf76544f2ff143;
                        }
                    }
                    lasterror = bot namespace_56b9cc3deb6e3f5e::function_7ec4108a3d7a4091();
                    if (isdefined(lasterror)) {
                        printstring = "<dev string:x2d8>" + lasterror;
                        print3d(printlocation, printstring, var_de5a5558ba4d830a, 1, scale, 1);
                        printlocation += (0, 0, linespacing);
                        if (var_89bee820769f600d) {
                            printtoscreen2d(screenx, screeny, printstring, var_c89cc0d4a5adb6d5, 1.5);
    ...
    function function_d72fc34d742ef919() {
        ...
        case #"hash_7038dec66d8275be": 
            return "<dev string:x2ed>";
        case #"hash_6191aaef9f922f96": 
            return "<dev string:x2f6>";
        case #"hash_2a3c28f3f1f0f8a": 
            return "<dev string:x303>";
        default: 
            assertmsg("<dev string:x305>");
            return "<dev string:x323>";
        }
    }
    ...
    function function_21fe18b1e1e83a2a() {
        ...
        case #"hash_29f66d99d48ef101": 
            return "<dev string:x32d>";
        case #"hash_284b2545dbaa82f7": 
            return "<dev string:x337>";
        case #"hash_bd9217f8be6084ee": 
            return "<dev string:x342>";
        case #"hash_b66ed8ef2f4f67f4": 
            return "<dev string:x34c>";
        default: 
            assertmsg("<dev string:x34e>");
            return "<dev string:x323>";
        }
    }
