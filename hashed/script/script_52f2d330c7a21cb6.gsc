...
function private function_a826442ad2416fe0(dmgstruct) {
    ...
    if (dmgstruct.shitloc == "soft") {
        ...
        if (isdefined(dmgstruct.eattacker)) {
            ...
            distance_perc = 0;
            var_54327d15562bd3a1 = var_54327d15562bd3a1 * (1 - clamp(distance_perc, 0, 1) * 0.8);
        }
        ...
        /#
            if (function_41e6ad038b005511()) {
                ...
                print3d(projected_pos, "<unknown string>" + var_8a3284c7a40fe130 + "<unknown string>" + var_54327d15562bd3a1, projected_color);
                print3d(projected_pos + (0, 0, 20), "<unknown string>" + var_52729c4793f5c462 + "<unknown string>" + var_6b6ac322c0340936, projected_color);
            }
        #/
    ...
    /#
        if (function_41e6ad038b005511()) {
            if (isdefined(dmgstruct.vpoint) && isdefined(dmgstruct.vdir)) {
                ...
                function_ad6307798215e0be("<unknown string>" + dmgstruct.shitloc, dmgstruct.vpoint);
            }
        }
    #/
    ...
    /#
        ...
        while (true) {
            ...
            foreach (metabone in var_46a3e181e35f46ab.metabones) {
                if (!array_contains(["<unknown string>", "<unknown string>", "<unknown string>"], metabone.name)) {
                    continue;
                }
                foreach (tag in metabone.tags) {
                    ...
                    tag_below = "<unknown string>" + (tag_num - 1 < 10 ? "<unknown string>" : "<unknown string>") + tag_num - 1;
                    tag_above = "<unknown string>" + (tag_num + 1 < 10 ? "<unknown string>" : "<unknown string>") + tag_num + 1;
                }
            }
            waitframe();
        }
...
function private draw_tag(tagname) {
    /#
        ...
        function_ad6307798215e0be(tagname + "<unknown string>" + tag_angles[0] + "<unknown string>" + tag_angles[1] + "<unknown string>" + tag_angles[2], tag_origin);
    #/
}
