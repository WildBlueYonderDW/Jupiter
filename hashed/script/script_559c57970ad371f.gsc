...
function private function_f034cfbc42d3956f(state) {
    /#
        state = default_to(state, "<unknown string>");
        color = undefined;
        /#
...
function private function_806b0eaacf7b76e9(bossinstance) {
    /#
        /#
            assertex(isstruct(bossinstance), "<unknown string>");
        #/
...
function function_d9799cc056122ed7(boss, name, state) {
    /#
        name = function_17495f409b110fff(default_to(name, "<unknown string>"), 12, 1);
        health = function_17495f409b110fff(boss.health + "<unknown string>", 10, 1);
        state = function_17495f409b110fff(default_to(state, "<unknown string>"), 10, 0);
    ...
    /#
        ...
        if (isdefined(bossinstance) && isdefined(bossinstance.boss)) {
            ...
            if (isdefined(bossinstance.wheelson)) {
                ...
                text = function_d9799cc056122ed7(wheelson, "<unknown string>", state);
                color = function_f034cfbc42d3956f(state);
                function_17d9f3785bf1042(x, y, text, color, 1, index);
                index = index + 1;
            }
...
function function_d974aec0560c2d96(boss, name, state) {
    /#
        ...
        name = default_to(name, "<unknown string>");
        state = default_to(state, "<unknown string>");
...
function function_ebd16f5b10596ff0(bossdetails, bossinstance) {
    /#
        if (isdefined(bossinstance) && isdefined(bossinstance.boss)) {
            ...
            if (isagent(boss)) {
                health = boss.health + "<unknown string>";
                name = default_to(bossdetails.name, "<unknown string>");
                ...
                if (bossdetails.name == "<unknown string>") {
                    health = boss.health + "<unknown string>";
                    name = default_to(bossdetails.name, "<unknown string>");
                    text = function_d974aec0560c2d96(boss, bossdetails.name, state);
                    color = function_f034cfbc42d3956f(state);
                    function_520b0777b699630b(boss.origin, boss.angles, text, color);
                } else if (bossdetails.name == "<unknown string>") {
                    health = boss.health + "<unknown string>";
                    name = default_to(bossdetails.name, "<unknown string>");
                    text = function_d974aec0560c2d96(boss, bossdetails.name, state);
                    color = function_f034cfbc42d3956f(state);
...
function function_b9d60dcb9d82b507() {
    /#
        bossdebug = getdvar(@"hash_a32ed8b3f923aa2d", "<unknown string>");
        var_b78030b29a1c9dae = bossdebug != "<unknown string>" && bossdebug != "<unknown string>";
        var_6e5f901a7b46c1e8 = istrue(var_b78030b29a1c9dae) && bossdebug != "<unknown string>" && bossdebug != "<unknown string>" && bossdebug != "<unknown string>";
