...
function evaluatescore(bot) {
    ...
    foreach (event in soundevents) {
        ...
        /#
            if (!isai(event.owner) && getdvar(@"hash_e39c6d129db1722") == "<dev string:x20>") {
                continue;
            }
        #/
...
function actionprocess(bot) {
    ...
    if (result == "bad_path") {
        println("<dev string:x2e>" + self.decision.name + "<dev string:x45>" + targetpos + "<dev string:x5b>" + bot.origin);
        return result;
    }
}
