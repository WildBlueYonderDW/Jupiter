...
/#
    ...
    function function_7ffa02c6592145d(lifeid, player) {
        ...
        startpos = player.origin + upvector + forward * backdist * -1;
        targetpos = player.origin + forward * targetdist;
        rocket = scripts\cp_mp\utility\weapon_utility::_magicbullet(makeweapon("<dev string:x1c>"), startpos, targetpos, player);
        if (!isdefined(rocket)) {
            return;
        }
        rocket thread handledamage();
        rocket.lifeid = lifeid;
        rocket.type = "<dev string:x38>";
        player cameralinkto(rocket, "<dev string:x3f>");
