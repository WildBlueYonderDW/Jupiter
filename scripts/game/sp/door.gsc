        ...
        foreach (struct in structs) {
            if (isdefined(struct.script_noteworthy)) {
                ...
                case #"hash_3377515bbda6817c": 
                    if (!isdefined(var_f851b6fab0cd01bf)) {
                        break;
                    }
                    ...
                    break;
                case #"hash_fa1e80f6bd5b8e72": 
                    if (!isdefined(var_ef82e2721f3c62b7)) {
                        break;
                    }
                    if (!isdefined(struct.radius)) {
                        struct.radius = 2.5;
...
function snake_cam_logic() {
    ...
    var_d436ea03d794b0b2 = anglestoup(self.angles * -1);
    nvg_was_on = 0;
    while (true) {
...
function handle_cam_enemy_caret() {
    var_d65a6191eef579bc = spawn_tag_origin();
    level.player playsound("support_drone_targeting");
    playfxontag(getfx("enemy_marker"), var_d65a6191eef579bc, "tag_origin");
    cam_enemy_caret_follow_target_til_cleanup(var_d65a6191eef579bc);
    var_d65a6191eef579bc delete();
}
...
// Size: 0x72
function cam_enemy_caret_follow_target_til_cleanup(var_d65a6191eef579bc) {
    self endon("death");
    while (level.player ent_flag("using_snakecam") && ent_flag("snake_cam_marked")) {
        org = self gettagorigin("j_head");
        var_d65a6191eef579bc.origin = org + (0, 0, 18);
        wait 0.05;
    }
}
...
function c4_on_door() {
    ...
    var_c5ea00180ec58e52 = self.breachpoints[0];
    level.player playgestureviewmodel("ges_equip_frag_throw");
    wait 0.5;
    self.door.c4 = spawn("script_model", var_c5ea00180ec58e52.origin);
    self.door.c4.angles = var_c5ea00180ec58e52.angles;
...
function c4_monitor_dmg() {
    ...
    while (true) {
        unused, attacker, unused, unused, type = self.door.c4 waittill("damage");
