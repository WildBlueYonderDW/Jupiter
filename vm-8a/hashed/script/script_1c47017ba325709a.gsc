            ...
            if (isdefined(childnodes)) {
                foreach (childnode in childnodes) {
                    if (array_contains(allnodes, childnode)) {
                        assertmsg("Found a loop in the node graph in: " + spawnnode.poi);
                        continue;
                    }
...
function function_f260b1cef943f1ab(vehicle_type, numriders, var_ec862c2c00c7ab5d, origin, poiname, priority, category, nationality) {
    ...
    if (!var_644d90de7685f75 && !namespace_bfef6903bca5845d::function_359f24553bdcca92(numriders, priority)) {
        /#
            msg = "<dev string:x1c>" + numriders + "<dev string:x45>";
            println(msg);
        #/
        return undefined;
    }
...
/#
    ...
    function function_9b7d54ce89689d91() {
        while (true) {
            ...
            var_c017436249a4749d = namespace_43bf74a2ee9234c0::function_a5b0665e0a814be6(getdvarint(@"hash_59530b43705e341f", 0), ["<dev string:x70>", "<dev string:x73>", "<dev string:x77>", "<dev string:x7f>"], level.players[0].origin, "<dev string:x83>", "<dev string:x88>", "<dev string:x97>", undefined, undefined, 1, scripts\mp\poi::function_6cc445c02b5effac(level.players[0].origin));
        ...
        while (true) {
            if (getdvarint(@"hash_34414e279fdbc0a1", 0) == 1) {
                if (function_47d356083884f913()) {
                    foreach (poiname, poi in level.poi) {
                        foreach (package in level.poi[poiname]["<dev string:x9d>"].var_e45aa5237f84ce6f) {
                            level thread function_956e403f85aab2f7(package);
                            foreach (spawnnode in package.spawnnodes) {
                                print3d(spawnnode.origin + (0, 0, 128), "<dev string:xb3>" + spawnnode.var_7643b53b09bbf1d5, (1, 1, 1), 1, 3, 20);
                            }
                            foreach (pathnode in package.pathnodes) {
                                if (array_contains(package.spawnnodes, pathnode)) {
                                    continue;
                                }
                                newstring = "<dev string:xbb>";
                                foreach (spawnid in pathnode.var_4a3b8372442841e5) {
                                    newstring = newstring + spawnid + "<dev string:xbc>";
                                }
