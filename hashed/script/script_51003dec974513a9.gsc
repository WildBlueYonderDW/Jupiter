...
function function_d5a08a1079b6f4dd() {
    ...
    for (var_d4f512e2e76b7b9f = 0; var_d4f512e2e76b7b9f < 6; var_d4f512e2e76b7b9f++) {
        self.var_e49062f063f27791[var_d4f512e2e76b7b9f] = spawn("script_model", self.origin);
        self.var_e49062f063f27791[var_d4f512e2e76b7b9f] setmodel(self.var_47399212b3052720.var_e58a65b7a8f5973c.var_e6140b1038dfc126);
        thread utility::delete_on_death(self.var_e49062f063f27791[var_d4f512e2e76b7b9f]);
    }
    self.var_d230d829f6d05081 = [0:"zombie_mangler", 1:"zombie_mimic", 2:"zombie_disciple"];
    self.var_e62a57499d165e15 = [0:"zombie_abom_mega"];
...
function private function_89c75ea3b9bff392() {
    ...
    var_b5483230dd35a2b1 = 1 && (var_6b246e279fd1929f && var_de7821bc51ab43a0 >= 4 || var_de7821bc51ab43a0 >= 7);
    var_e033680c51df22ca = 2;
    if (var_de7821bc51ab43a0 <= 9) {
        var_e033680c51df22ca = 1;
    }
    if (var_b5483230dd35a2b1 && isdefined(self.var_36a5706c4995e3d7)) {
        var_9757146279eed4f8 = 0;
        foreach (ai in self.var_36a5706c4995e3d7) {
            if (isalive(ai) && is_equal(ai.aicategory, "elite")) {
                var_9757146279eed4f8++;
            }
        }
        if (var_9757146279eed4f8 >= 2) {
    ...
    for (var_d4f512e2e76b7b9f = 0; var_d4f512e2e76b7b9f < var_d95356124cc5cc35; var_d4f512e2e76b7b9f++) {
        ...
        self.var_e49062f063f27791[var_d4f512e2e76b7b9f].origin = head_pos;
        self.var_e49062f063f27791[var_d4f512e2e76b7b9f] utility::function_3ab9164ef76940fd("mobile_spawner", "mobile_spawner_on");
        var_69e3e4c4a13b7d1 = 0;
        if (var_b5483230dd35a2b1) {
            var_69e3e4c4a13b7d1 = var_d4f512e2e76b7b9f == 0;
            if (!var_69e3e4c4a13b7d1 && var_de7821bc51ab43a0 >= 14) {
                var_69e3e4c4a13b7d1 = var_d4f512e2e76b7b9f == 1;
            }
        }
        thread function_363dbe4e6642291(self.var_e49062f063f27791[var_d4f512e2e76b7b9f], var_69e3e4c4a13b7d1);
    }
...
function private function_363dbe4e6642291(mobile_spawner, var_69e3e4c4a13b7d1) {
    ...
    var_97d10b46c5a312ae = getdvarfloat(@"hash_df2d63ee68c8172b", 3);
    gravity = getdvarfloat(@"hash_c4ce13813f6f8616", 400);
    var_830761ee87186b54 = forward_vec * var_d04bd7f014726149 + (0, 0, -1) * var_c8fef90150a3a014;
    var_830761ee87186b54 = var_830761ee87186b54 + randomint(20) * utility::random([0:-1, 1:1]) * (0, 0, 1);
    var_830761ee87186b54 = var_830761ee87186b54 + randomint(40) * utility::random([0:-1, 1:1]) * anglestoright(self.angles);
    prev_origin = mobile_spawner.origin;
    var_58124d39e9aa990 = 0;
    while (!var_58124d39e9aa990) {
        var_830761ee87186b54 = var_830761ee87186b54 + ((0, 0, -1) * gravity / 8 - forward_vec * var_97d10b46c5a312ae) * level.framedurationseconds;
        mobile_spawner.origin = mobile_spawner.origin + var_830761ee87186b54;
    ...
    if (isdefined(var_28c911f563f84e4)) {
        ...
        if (isdefined(var_2b1c25f7ca164d0f)) {
            ...
            if (istrue(level.var_4fe0c43951f6ce37)) {
                var_2b1c25f7ca164d0f.var_a0d32443f1df7929 = namespace_1c689dd3986a841c::function_54da428ad4a96f2e("ai_flood_fill_encounter:" + self.var_47399212b3052720.var_e58a65b7a8f5973c.var_3ab85d9d466f24bb, mobile_spawner.origin, 10000, 1, 1);
            }
    ...
    foreach (target in targets) {
        if (target.team != self.team) {
            ...
            var_db488b8a52e21f79 = max((128 - var_d45caa2978502280) / 128, 0);
            var_ec46e8a0004279b0 = math::remap(var_db488b8a52e21f79, 0, 1, 100, 300);
...
function private function_448e957d7afaac9a() {
    self endon("death");
    while (1) {
        if (!isalive(self.var_33a6ac144efb85a6) || distance2dsquared(self.var_33a6ac144efb85a6.origin, self.origin) > squared(2000)) {
            if (!isdefined(self.enemy)) {
                self kill();
            }
        }
...
function private function_4b8806398ad868d9(var_7ffb0443e06ecae) {
    var_33a6ac144efb85a6 = self.var_33a6ac144efb85a6;
    ...
    if (isdefined(var_33a6ac144efb85a6) && isdefined(var_33a6ac144efb85a6.var_1b51c793abb7dbe6)) {
        var_33a6ac144efb85a6.var_1b51c793abb7dbe6--;
    }
...
// Size: 0xc2
function private function_3fcec029f50d7376(requestid, userdata, agent, var_5c0a31a149731954) {
    agent function_3ab9164ef76940fd("mobile_spawner", "mobile_spawner_on");
    agent ent_flag_set("spawned_by_mobile_spawner");
    agent.nocorpse = 1;
    if (isdefined(userdata.var_33a6ac144efb85a6)) {
        params = spawnstruct();
        params.ai = agent;
        userdata.var_33a6ac144efb85a6 callback("spawned_from_deathworm", params);
    }
