...
function init() {
    ...
    if (isdefined(level.gamemodebundle) && isdefined(level.gamemodebundle.var_e682083c33f4331f)) {
        setsaveddvar(@"g_friendlyfireDist", level.gamemodebundle.var_e682083c33f4331f);
    }
...
/#
    ...
    function function_194fb483a3eec865() {
        while (true) {
            amount = self waittill("<dev string:x1c>");
...
/#
    ...
    function function_d04335746de340a9(reason, amount) {
        if (!getdvarint(@"hash_8ae88877360e4699")) {
            return;
        }
        if (!isdefined(reason)) {
...
function firedamagefx() {
    ...
    while (true) {
        ...
        setsaveddvar(@"r_mbRadialOverrideStrength", var_9cb09b5da738e30b);
...
function disabledeathsdoor(instant) {
    ...
    setsaveddvar(@"painVisionLerpOutRate", var_3b0606a6c467a8aa);
    self painvisionoff();
...
function removeradialdistortion(outtime) {
    childthread lerp_saveddvar(@"r_mbRadialOverrideDistortion", 0, outtime);
    childthread lerp_saveddvar(@"r_mbRadialOverrideRadius", 0, outtime);
    childthread lerp_saveddvar(@"r_mbRadialOverrideStrength", 0, outtime);
    childthread lerp_saveddvar(@"r_mbRadialOverridePositionActive", 0, outtime);
}
...
function damagepainvision(origin, factor, type) {
    ...
    setsaveddvar(@"painVisionLerpInRate", var_6c10a60c02c4c15b);
    setsaveddvar(@"painVisionLerpOutRate", var_4efe0369fff10028);
    self painvisionon();
    wait holdtime;
    self painvisionoff();
}
...
function focustimeadjust() {
    ...
    while (true) {
        event = level waittill("objectives_updated_state");
        if (event != "invisible") {
...
function radial_distortion(distortion, strength, outtime, origin) {
    ...
    if (isdefined(origin)) {
        ...
        setsaveddvar(@"r_mbRadialOverridePosition", origin);
    }
    if (isdefined(outtime)) {
