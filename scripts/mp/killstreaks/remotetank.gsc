...
function init() {
    ...
    /#
        bundle = level.streakglobals.streakbundles["_destroyed"];
        if (!isdefined(bundle)) {
...
function function_7f60b3159041388a() {
    ...
    if (!isdefined(tankowner)) {
        return;
    }
    tankowner endon("disconnect");
    tankowner endon("joined_team");
    tankowner endon("joined_spectators");
    tankowner endon("team_kill_punish");
    while (true) {
        tankowner waittill("player_fatal_death");
...
function function_93c80ddc3857d00d(var_ee356ed81b5cb7f4, explosionent) {
    ...
    contentlist = scripts/engine/trace::create_contents(1, 0, 0, 0, 0, 0, 0, 0, 0);
    spheretrace = scripts/engine/trace::sphere_trace_get_all_results(startpos, endpos, 60, ignorelist, contentlist);
    foundtarget = 0;
...
function function_257b6f1cf0249339(entityhitpos, tank) {
    /#
        ...
        anglesforward = vectornormalize(var_2d7a2733c414e4df - tank.mgturret gettagorigin("entering_control"));
        for (framecounter = 0; framecounter < var_7de35540c250a948; framecounter = framecounter + 1) {
            print3d(tankorigin + anglesforward * 200, "veh_wheelson_engine_stop" + var_2d7a2733c414e4df, (1, 0, 0));
            print3d(tankorigin + anglesforward * 200 - (0, 0, 20), "killstreak_veh_camera_mp" + distance2d(var_2d7a2733c414e4df, entityhitpos));
            waitframe();
        }
    #/
}
