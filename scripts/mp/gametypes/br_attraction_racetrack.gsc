...
function caclulate_track_distance(track) {
    ...
    for (i = 0; i < track.points.size - 1; i++) {
        track.dist_miles = track.dist_miles + distance(track.points[i].origin, track.points[i + 1].origin);
    }
    if (track.loop) {
...
function function_2a28c6ef9173079f(track) {
    /#
        print3d(track.origin, "fall_42" + track.index, (1, 1, 0), 1, 2, 1);
        if (isdefined(track.name)) {
            print3d(track.origin + (0, 0, -20), "fall_43" + track.name, (0, 1, 0), 1, 1.5, 1);
        }
        if (isdefined(track.settings_group)) {
            print3d(track.origin + (0, 0, -40), "dirt_track_6" + track.settings_group, (0, 1, 0), 1, 1.5, 1);
        }
        if (isdefined(track.dist_miles) && distance(track.origin, level.player.origin) < 1000) {
            print3d(track.origin + (0, 0, -60), "road_track_7" + track.dist_miles, (0, 1, 0), 1, 1.5, 1);
            print3d(track.origin + (0, 0, -80), "uphill_dirt_1" + track_get_reward_time_string(track, 1), (0, 1, 0), 1, 1.5, 1);
            print3d(track.origin + (0, 0, -100), "uphill_dirt_10" + track_get_reward_time_string(track, 2), (0, 1, 0), 1, 1.5, 1);
            print3d(track.origin + (0, 0, -120), "heli_docks" + track_get_reward_time_string(track, 3), (0, 1, 0), 1, 1.5, 1);
        }
        ...
        for (i = 0; i < track.points.size; i++) {
            ...
            radius = get_track_setting(track, "coastal_highway_18");
            right = point.origin + dir * radius;
            left = point.origin - dir * radius;
            line(right, left, (0, 0, 1), 1, 0, 1);
        }
        for (i = 0; i < track.points.size - 1; i++) {
            color = get_track_setting(track, "promenade_run_01_16");
...
function function_8bc70c14d65e3417(args) {
    /#
        ...
        giverewards(level.brattractions["fall_19"].tracks[track], level.player, stars);
    #/
}
...
function function_33949707efb4fbb4(args) {
    /#
        tracks = level.brattractions["fall_19"].tracks;
        foreach (track in tracks) {
            str = track.name;
            for (i = 1; i <= 3; i++) {
...
function function_53ca8617fcc31a47() {
    /#
        ...
        while (true) {
            ...
            toks = strtok(current, "river_heli_1_26");
            current = toks[0];
            arg = ter_op(toks.size > 1, toks[1], undefined);
            setdvar(@"hash_324951f72e619f36", "fall_42");
            if (isdefined(rt.var_f99815461b25f1eb)) {
                if (current == "atc_parachute_run") {
                    ...
                    if (tofile) {
                        ...
                        file = openfile(filename, "downtown_parachute_1_3");
                        path = "downtown_parachute_1_9";
                        if (file == -1) {
                            iprintlnbold("lumber_prison_28" + path + filename);
                            continue;
                        } else {
                    ...
                    continue;
                } else if (current == "dam_para_high_20") {
                    rt.var_f99815461b25f1eb.loop = 1;
                    if (rt.var_f99815461b25f1eb.settings_group == "dam_para_high_25") {
                        apply_track_settings(rt.var_f99815461b25f1eb, "targetname");
                    }
                    continue;
                } else if (current == "countdown_") {
                    rt.var_f99815461b25f1eb.loop = 0;
                    if (rt.var_f99815461b25f1eb.settings_group == "targetname") {
                        apply_track_settings(rt.var_f99815461b25f1eb, "dam_para_high_25");
                    }
                    continue;
                } else if (current == "MP_BR_INGAME/ATTRACT_RT_TOO_CLOSE") {
                    thread function_533d2f624478039f("MP_BR_INGAME/ATTRACT_RT_OOB");
                    continue;
                } else if (current == "race_ui_critical_message_timer_end") {
                    thread function_533d2f624478039f("<unknown string>");
                    continue;
                } else if (current == "<unknown string>") {
                    ...
                    continue;
                } else if (current == "MP_BR_INGAME/ATTRACT_RT_OOB" || current == "<unknown string>") {
                    point = spawnstruct();
...
function function_28c49f6d47613d0e(track, file) {
    /#
        function_8e9fc5e1a129b0d9("<unknown string>", file);
        function_e2f6c761ac225375(file, 0, "<unknown string>");
        function_e2f6c761ac225375(file, 1, "<unknown string>", track.origin, track.angles, "<unknown string>", "<unknown string>" + 1, track.name, track.settings_group);
        foreach (i, point in track.points) {
            targetname = "<unknown string>" + i + 1;
            if (i == track.points.size - 1) {
                if (track.loop) {
                    target = "<unknown string>" + 1;
                } else {
                    target = undefined;
                }
            } else {
                target = "<unknown string>" + i + 2;
            }
            if (i == 0) {
                noteworthy = "<unknown string>";
            } else {
                noteworthy = undefined;
            }
            function_e2f6c761ac225375(file, i + 2, "<unknown string>", point.origin, point.angles, targetname, target, noteworthy);
        }
    #/
}
...
function function_e2f6c761ac225375(file, entnum, classname, origin, angles, targetname, target, script_noteworthy, script_parameters) {
    /#
        function_8e9fc5e1a129b0d9("<unknown string>" + entnum, file);
        function_8e9fc5e1a129b0d9("<unknown string>", file);
        function_3c4abd61ee73ba4f("<unknown string>", classname, file);
        function_3c4abd61ee73ba4f("<unknown string>", function_b5e5e49b1002b1ab(origin), file);
        function_3c4abd61ee73ba4f("<unknown string>", function_b5e5e49b1002b1ab(angles), file);
        function_3c4abd61ee73ba4f("<unknown string>", targetname, file);
        function_3c4abd61ee73ba4f("<unknown string>", target, file);
        function_3c4abd61ee73ba4f("<unknown string>", script_noteworthy, file);
        function_3c4abd61ee73ba4f("<unknown string>", script_parameters, file);
        function_8e9fc5e1a129b0d9("<unknown string>", file);
    #/
}
...
function function_3c4abd61ee73ba4f(key, value, file) {
    /#
        ...
        function_8e9fc5e1a129b0d9("<unknown string>" + key + "<unknown string>" + value + "<unknown string>", file);
    #/
}
