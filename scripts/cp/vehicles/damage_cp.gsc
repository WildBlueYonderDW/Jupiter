...
function callback_vehicledamage_internal(data) {
    ...
    if (isplayer(data.attacker)) {
        data.attacker namespace_3d3d5211c3029f82::updatedamagefeedback("hitcritical", damageisfatal, undefined, hitmarkertype, undefined, 1);
        namespace_dba31853823d1b3::addattacker(self, data.attacker, data.inflictor, data.objweapon, data.damage, data.point, data.direction_vec, data.hitloc, data.timeoffset, data.meansofdeath);
    }
    ...
    if (damageisfatal) {
        if (isplayer(data.attacker)) {
            if (isdefined(self.riders) && self.riders.size > 0) {
                data.attacker thread namespace_22a50d845d3deb2d::killedenemy(undefined, self, data.objweapon, data.meansofdeath, data.inflictor, 0);
            }
