...
/#
        ...
        if (!isdefined(level.var_87a042f91827f93d)) {
            ...
            if (isdefined(level.secondaryvictory) && isdefined(triggers)) {
                foreach (et in triggers) {
                    ...
                    if (objid != -1) {
                        scripts\mp\objidpoolmanager::objective_add_objective(objid, "<dev string:x72>", et.origin, "<dev string:x7a>");
    ...
    function private function_ddd97b020459c8d1(args) {
        ...
        player = devgetplayer();
        players = level.teamdata[player.team]["<dev string:xfa>"];
...
function function_f1a47f3d1492db51() {
    ...
    if (isdefined(teamdata.var_394aebba88e4e8c2)) {
        /#
            iprintln(player.name + "<dev string:x102>");
        #/
        return false;
    }
    return true;
...
function function_70ee001b7b3a1bd0() {
    ...
    if (!scripts\cp_mp\vehicles\vehicle_tracking::canspawnvehicle()) {
        /#
            iprintln("<dev string:x156>" + player.name + "<dev string:x16e>");
        #/
        return 27;
    }
    if (isdefined(teamdata.var_394aebba88e4e8c2)) {
        /#
            iprintln("<dev string:x156>" + player.name + "<dev string:x187>");
        #/
        return 26;
    }
    ...
    if (!isdefined(trigger)) {
        /#
            iprintln("<dev string:x156>" + player.name + "<dev string:x1a9>");
        #/
        return 18;
    }
    return 2;
...
function startevent(player, trigger) {
    ...
    /#
        iprintln("<dev string:x1be>" + var_394aebba88e4e8c2.name);
    #/
    ...
    /#
        iprintln("<dev string:x1f3>" + var_394aebba88e4e8c2.name + "<dev string:x20c>");
    #/
}
...
function function_363fe741930d5e4b() {
    ...
    wait time;
    /#
        iprintln("<dev string:x216>" + scriptbundle.var_8544cdb39053b6da + "<dev string:x23d>");
    #/
    teamsinrange = [];
    ...
    foreach (team in teamsinrange) {
        /#
            iprintln("<dev string:x23f>" + player.team);
        #/
...
function private function_1e1dd16af9297efd(extractpos, var_831b4ced8bc8fbee, var_532ecf4a03864da7, endingid, var_394aebba88e4e8c2) {
    ...
    /#
        iprintln("<dev string:x259>" + self.name);
    #/
    self.extracted = 1;
    level endon("game_ended");
    self endon("disconnect");
