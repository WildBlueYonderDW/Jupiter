...
function spawnrevivetrigger(victim, attacker, victimnotification, meansofdeath) {
    ...
    /#
        if (!isusingmatchrulesdata()) {
            usetime = scripts\mp\utility\dvars::getwatcheddvar("<dev string:x2e>");
...
/#
    ...
    function selfrevive() {
        ...
        self notify("<dev string:x64>");
        self endon("<dev string:x64>");
        ...
        while (hasrevivetrigger(self)) {
            if (getdvarint(@"hash_d475fd19488b5a6d", 0) == 1) {
                ...
                if (isdefined(self.reviver)) {
                    ...
                    self.reviver botpressbutton("<dev string:x81>", presstime);
                    wait presstime + 0.5;
                ...
                foreach (revivetrigger in level.revivetriggers) {
                    foreach (player in level.players) {
                        if (isbot(player) && isalive(player) && player.team == revivetrigger.ownerteam) {
                            ...
                            player botlookatpoint(drop_to_ground(revivetrigger.deadplayer.origin), 10, "<dev string:x85>");
                            player botsetflag("<dev string:x93>", 1);
                            player botsetflag("<dev string:xa4>", 1);
                            self.reviver = player;
                            break;
                        }
                    }
                    ...
                    if (isdefined(self.reviver)) {
                        ...
                        self.reviver botsetflag("<dev string:xa4>", 0);
                    }
                }
