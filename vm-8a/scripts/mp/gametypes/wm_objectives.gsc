...
function function_2cbdea506661cfc5(label) {
    scripts\mp\gamelogic::setwaypointiconinfo("waypoint_capture" + label, 0, "enemy", %MP_INGAME_ONLY/OBJ_HVT_CAPS, "hud_icon_vehicle_samsite", 0);
    ...
    if (isarray(targetnames)) {
        foreach (targetname in targetnames) {
            if (issubstr(targetname, "flag_begin")) {
                ...
                assertex(flagpoints.size != 0, "Can't find entity with target name: " + targetname);
                continue;
            }
            ...
            if (issubstr(targetname, "capture_zone")) {
                ...
                assertex(isdefined(capturezone), "Can't find entity with target name: " + targetname);
                continue;
            }
        ...
        assertex(flagpoints.size != 0, "Can't find entity with target name: " + targetnames);
    } else if (issubstr(targetnames, "flag_end")) {
        flagzone = getent(targetnames, "targetname");
        assertex(flagzone != 0, "Can't find entity with target name: " + targetnames);
    } else if (issubstr(targetname, "capture_zone")) {
        capturezone = getent(targetname, "targetname");
        assertex(isdefined(capturezone), "Can't find entity with target name: " + targetname);
    } else {
        assertmsg("Invalid target name: " + targetnames);
    }
...
function function_f2a1c180dc67b7f8(vehicle, objidnum) {
    ...
    while (vehicle.progress != 1) {
        ...
        scripts\mp\objidpoolmanager::objective_set_progress(objidnum, progress);
        /#
            print3d(vehicle.origin + (0, 0, 72), "<dev string:x22>" + int(cachedistance) + "<dev string:x2a>" + backnode.var_1d7ebd1bcc23a4e1 + "<dev string:x36>" + distance2d(vehicle.origin, vehicle.pathnodes[var_92bc69459236be07 + 1].origin));
        #/
    }
    function_d507ea0668f3666c(vehicle);
}
...
/#
    ...
    function function_d36257ceb2380e24(vehicle, pathnodes) {
        level endon("<dev string:x3e>");
        vehicle endon("<dev string:x49>");
        while (true) {
            for (i = 0; i < pathnodes.size; i++) {
                print3d(pathnodes[i].origin + (0, 0, 50), "<dev string:x56>" + i + "<dev string:x5c>" + pathnodes[i].var_1d7ebd1bcc23a4e1);
                print3d(pathnodes[i].origin + (0, 0, 36), "<dev string:x5f>" + int(pathnodes[i].progress * 100) + "<dev string:x60>");
            }
            waitframe();
        }
    }
...
// Size: 0x110
function function_8bce9c7e1a380040(objectivekey, objectivestatus) {
    status = "Completed";
    if (isdefined(objectivestatus)) {
        status = objectivestatus;
    }
    if (istrue(level.var_a2e61b8ea443febf[objectivekey])) {
        dlog_recordevent("dlog_event_wm_server_sub_objective_end", ["match_guid", function_94c7ae7049488358(), "map_name", level.script, "objective_type", "" + level.zone.objectivetype, "objective_id", level.zone.name, "sub_objective_time", gettime() / 1000, "sub_objective_key", objectivekey, "round", "" + game["roundsPlayed"] + 1, "status", status]);
