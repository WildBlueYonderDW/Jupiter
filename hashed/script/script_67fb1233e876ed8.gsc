...
function private function_705ab1040a64a8fa() {
    ...
    for (i = 0; i < configlist.size; i++) {
        var_45ffc2a32427f2a9 = var_45ffc2a32427f2a9 + configlist[i].script_struct + " ";
    }
...
function private function_8d6027e6daa80c1d(var_3bdadc2ea30c31ba, spawntype) {
    ...
    /#
        if (isdefined(var_3bdadc2ea30c31ba.debug_color)) {
            colorvec = strtok(var_3bdadc2ea30c31ba.debug_color, " has 0 spawns in ");
        ...
        foreach (poirule in var_3bdadc2ea30c31ba.var_80007572cdd4150f) {
            if (!isdefined(poirule.var_dfe282400b2e9ad7)) {
                ...
                foreach (poi in poirule.poi_group) {
                    group_name = group_name + poi.poi_name + "_";
                }
...
function private function_747ff5f8c0e96da5(weights, spawncount) {
    ...
    foreach (entry in weights) {
        if (isdefined(entry.spawn_weight)) {
            totalweight = totalweight + entry.spawn_weight;
        }
    }
    randomweight = 0;
    if (totalweight > 0) {
    ...
    foreach (entry in weights) {
        if (isdefined(entry.spawn_weight)) {
            randomweight = randomweight - entry.spawn_weight;
            if (randomweight <= 0) {
...
function private function_56b91bfbefdc7456(array) {
    /#
        ...
        for (i = 0; i < var_6c46abea90157067.size; i++) {
            arraystring = arraystring + var_6c46abea90157067[i] + " has 0 spawns in ";
        }
        println("Rule Dist Group Min - " + arraystring);
    #/
}
...
function private debug_watch(dvarstring, funcdebug) {
    /#
        ...
        while (true) {
            debugstate = getdvar(dvarstring, "Group");
            if (debugstate == "Group") {
                waitframe();
                continue;
            }
            setdvar(dvarstring, "Group");
            debug_print("<unknown string>" + function_3c8848a3a11b2553(dvarstring) + "<unknown string>" + debugstate);
...
function private debug_draw(scriptstruct, spawns) {
    /#
        ...
        debug_cleanup();
        iprintlnbold("<unknown string>" + scriptstruct + "<unknown string>");
        if (spawns == undefined) {
            spawns = level.ssc.indexes;
        }
        foreach (entry in spawns) {
...
function private function_fd78bc2f959fe5cc(text, origin, angles, color) {
    /#
        ...
        thread draw_line_for_time(origin, ray, color[0], color[1], color[2], timesec, "<unknown string>" + level.ssc.drawcount);
        thread draw_arrow_time(origin, forwardpos, color, timesec, "<unknown string>" + level.ssc.drawcount);
    #/
}
...
function private function_51c00e624bf397fd(text, origin, color, timeframes) {
    /#
        level endon("<unknown string>" + level.ssc.drawcount);
        while (timeframes > 0) {
...
function private debug_cleanup(drawindex) {
    /#
        ...
        level notify("<unknown string>" + drawindex);
    #/
}
...
function private function_f01b154aa1fe41cb(listref) {
    /#
        if (listref == "<unknown string>") {
            listref = getdvar(@"hash_8e30f7eeea837de6", "Group");
        }
        debug_cleanup();
...
function private function_1c9f19f5e86da1ab(var_ca2c833762e5466c) {
    /#
        ...
        debug_print("<unknown string>" + var_ca2c833762e5466c + " has 0 spawns in " + numtimes + "<unknown string>");
    ...
    /#
        ...
        foreach (index in level.ssc.indexes) {
            ...
            if (array_contains_key(spawncounts, index) && spawncounts[index] > 0) {
                ...
                function_fd78bc2f959fe5cc(index + "<unknown string>" + spawncounts[index] + "<unknown string>", origin, angles, color);
                continue;
            }
