...
function private function_a826442ad2416fe0(dmgstruct) {
    ...
    if (dmgstruct.shitloc == "soft") {
        ...
        tag_positions[var_39033f9289f86348] = tag_positions[var_39033f9289f86348] - tag_forward * getdvarfloat(@"hash_19d1b5077a7fb997", 50);
        ...
        /#
            if (function_41e6ad038b005511()) {
                ...
                print3d(projected_pos, "<dev string:x1c>" + var_8a3284c7a40fe130 + "<dev string:x31>" + var_54327d15562bd3a1, projected_color);
                print3d(projected_pos + (0, 0, 20), "<dev string:x3d>" + var_52729c4793f5c462 + "<dev string:x48>" + var_6b6ac322c0340936, projected_color);
            }
        #/
    ...
    /#
        if (function_41e6ad038b005511()) {
            if (isdefined(dmgstruct.vpoint) && isdefined(dmgstruct.vdir)) {
                ...
                function_ad6307798215e0be("<dev string:x57>" + dmgstruct.shitloc, dmgstruct.vpoint);
            }
        }
    #/
...
/#
    ...
    function private function_f9d648a2240fdc83() {
        function_6e7290c8ee4f558b("<dev string:x5e>");
        function_a9a864379a098ad6("<dev string:x7a>", "<dev string:x8a>", &function_484fb913cc61250e);
    ...
    function private function_1dbaaec5b9b3ec48() {
        self endon("<dev string:xa4>");
        while (true) {
            var_46a3e181e35f46ab = function_97fd256a244527b4(self.metabones.bundle_name);
            foreach (metabone in var_46a3e181e35f46ab.metabones) {
                if (!array_contains(["<dev string:xaa>", "<dev string:xb9>", "<dev string:xc8>"], metabone.name)) {
                    continue;
                }
                foreach (tag in metabone.tags) {
                    draw_tag(tag.tagname);
                    var_eda2dcf6e5705be7 = strtok(tag.tagname, "<dev string:xd4>");
                    tag_num = int(var_eda2dcf6e5705be7[var_eda2dcf6e5705be7.size - 1]);
                    tag_below = "<dev string:xd6>" + (tag_num - 1 < 10 ? "<dev string:xde>" : "<dev string:xe0>") + tag_num - 1;
                    tag_above = "<dev string:xd6>" + (tag_num + 1 < 10 ? "<dev string:xde>" : "<dev string:xe0>") + tag_num + 1;
                }
            }
            waitframe();
        }
    ...
    function private draw_tag(tagname) {
        ...
        function_ad6307798215e0be(tagname + "<dev string:xe1>" + tag_angles[0] + "<dev string:xe5>" + tag_angles[1] + "<dev string:xe5>" + tag_angles[2], tag_origin);
    }
    ...
    function private function_484fb913cc61250e() {
        function_2fb888667001fc39("<dev string:xe7>", @"hash_dda69ad425f2daac");
    }
    ...
    function private function_ad6307798215e0be(text, pos) {
        if (function_41e6ad038b005511()) {
            ...
            print3d(pos, "<dev string:x101>" + text, (1, 0, 0));
        }
    }
