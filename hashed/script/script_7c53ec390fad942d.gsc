...
function testprocess() {
    ...
    function_9e400058ef021b03(self.mit.var_7e94a2c0fbf8691b[1], var_cbef341a3aeef001);
...
function private report(situation, bot, start, target, path, comment) {
    ...
    if (isdefined(start)) {
        reportstring = reportstring + "start:" + start;
    }
    reportstring = reportstring + "; ";
    if (isdefined(target)) {
        reportstring = reportstring + "target:" + target;
    }
    reportstring = reportstring + "; ";
    if (isdefined(path)) {
        pathstr = "path: ";
    ...
    if (isdefined(comment)) {
        reportstring = reportstring + "comment:" + comment;
    }
    outputlog("Connectivity", reportstring);
    /#
        if (isdefined(level.mit.var_db71539d3e4b4723)) {
            level.mit.var_db71539d3e4b4723 = level.mit.var_db71539d3e4b4723 + 1;
        } else {
            level.mit.var_db71539d3e4b4723 = 1;
        }
        function_f2bc01a7b0321f52("<unknown string>", start, start + (0, 0, 100), bot.mit.var_7e94a2c0fbf8691b, level.mit.var_db71539d3e4b4723);
    #/
}
