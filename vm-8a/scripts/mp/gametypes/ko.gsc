...
/#
    ...
    function reinitspawns() {
        scripts\mp\spawnlogic::function_8390bf8c3b74466a("<dev string:x40>");
        scripts\mp\spawnlogic::function_8390bf8c3b74466a("<dev string:x47>");
        spawns = scripts\mp\spawnlogic::getspawnpointarray("<dev string:x50>");
        spawnssecondary = scripts\mp\spawnlogic::getspawnpointarray("<dev string:x5d>");
        scripts\mp\spawnlogic::registerspawnset("<dev string:x40>", spawns);
        scripts\mp\spawnlogic::registerspawnset("<dev string:x47>", spawnssecondary);
    }
...
function function_9962a01f46dd3b58(team) {
    ...
    if (gettimelimit() == 0 && level.var_3a00bb035dbcf5ad <= 0) {
        /#
            ...
            setomnvar("<dev string:x74>", flagHoldTimer);
            setomnvar("<dev string:x81>", 1);
            setomnvar("<dev string:x91>", 0);
        #/
        return;
    }
...
/#
    ...
    function function_bf2918399f1800b1() {
        wait 5;
        ents = getentarray("<dev string:xac>", "<dev string:xb7>");
        foreach (ent in ents) {
            thread scripts\mp\utility\debug::drawsphere(ent.origin, 64, 1000, (0, 1, 0));
        }
        ents = getentarray("<dev string:xc9>", "<dev string:xb7>");
        foreach (ent in ents) {
            thread scripts\mp\utility\debug::drawsphere(ent.origin, 64, 1000, (0, 1, 0));
        }
        structs = getstructarray("<dev string:xac>", "<dev string:xb7>");
        foreach (struct in structs) {
            thread scripts\mp\utility\debug::drawsphere(struct.origin, 64, 1000, (0, 1, 0));
        }
        structs = getstructarray("<dev string:xc9>", "<dev string:xb7>");
        foreach (struct in structs) {
            thread scripts\mp\utility\debug::drawsphere(struct.origin, 64, 1000, (0, 1, 0));
        }
    }
    ...
    function spawncompare(spawnpoint, spawnteam) {
        if (getdvarint(@"hash_b5ceec3e3cb419be") != 0) {
            ...
            println("<dev string:xdc>" + scriptspawnpoint.index + "<dev string:xf4>" + scriptspawnpoint.totalscore);
            if (scriptspawnpoint.index == spawnpoint.index) {
                level.var_2dc7108159dc1da3++;
                iprintlnbold("<dev string:x107>" + level.var_2dc7108159dc1da3 / level.var_3d292942d52b104a * 100 + "<dev string:x123>" + "<dev string:x12e>" + level.var_2dc7108159dc1da3 + "<dev string:x130>" + level.var_3d292942d52b104a);
            } else {
                level.var_a065445ba781ddf6++;
                iprintlnbold("<dev string:x132>" + level.var_2dc7108159dc1da3 / level.var_3d292942d52b104a * 100 + "<dev string:x123>" + "<dev string:x12e>" + level.var_2dc7108159dc1da3 + "<dev string:x130>" + level.var_3d292942d52b104a);
            }
