...
function determinetargetteam(player, var_86a80a97f4672fc8) {
    ...
    while (!isdefined(bestteam)) {
        search_bounds = search_bounds + level.var_7e12f6eb4fcb4ea1.searchincrement;
        ...
        var_29f6d200784f77b3 = var_29f6d200784f77b3 / hunterteamsize;
        foreach (potentialtarget in playerlist) {
            ...
            foreach (teammate in getteamdata(potentialtarget.team, "players")) {
                if (!teammate scripts/mp/gametypes/br_public::isplayeringulag() && isreallyalive(teammate)) {
                    currentteamcenter = currentteamcenter + teammate.origin;
                }
            }
            currentteamcenter = currentteamcenter / targetteamsize;
            ...
            var_76e3e3800077282f = var_76e3e3800077282f / targetteamsize;
...
function function_210be3d9455cfb83(hunter, victim) {
    ...
    for (timepassed = 0; timepassed < pingtime; timepassed = timepassed + var_6c3716013ea4f716) {
...
function gettargetteamkills(targetteam) {
    ...
    foreach (targetplayer in level.teamdata[targetteam]["players"]) {
        targetteamkills = targetteamkills + targetplayer.kills;
    }
...
function gettargetteamscore(targetteam) {
    targetteamscore = 0;
    foreach (targetplayer in level.teamdata[targetteam]["players"]) {
        targetteamscore = targetteamscore + targetplayer.score;
    }
