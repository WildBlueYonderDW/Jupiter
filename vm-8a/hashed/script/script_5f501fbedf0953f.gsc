...
function function_21672171040d6aae() {
    ...
    if (!isdefined(self.s_center)) {
        assertmsg("Armored Convoy - could not find a struct with targetname " + self.varianttag + ", bailing out");
        return;
    }
    ...
    /#
        iprintln("<dev string:x1c>" + self.varianttag);
    #/
            ...
            if (isdefined(vehicle)) {
                ...
                /#
                    ...
                    if (drawdebug && vehicle.stucktime > 0) {
                        print3d(vehicle getcentroid() + (0, 0, 100), "<dev string:x3d>" + vehicle.stucktime, (1, 1, 0), 1, 1, 1, 0);
                    }
                #/
...
function function_1f7a69bd7ab10e0(activityinstance, var_1c1e93b32ed347a7, origin, vehicleindex, vehicle) {
    assertex(vehicleindex >= 0 && vehicleindex < activityinstance.var_a5aa6c6f1908df45.size, "Error - Tried to spawn rider with an invalid vehicle index!");
...
function private checkVehicleNormalizedPosition(splinenodeindex) {
    ...
    while (true) {
        ...
        /#
            print3d(self.origin, "<dev string:x3d>" + splinedistancetraveled / splinelength, (1, 1, 1), 1, 1, duration, 1);
        #/
    }
}
        ...
        /#
            ...
            if (drawdebug) {
                for (index = 0; index < var_4b1a1ec87d5ac862.size; index++) {
                    print3d(var_4b1a1ec87d5ac862[index].origin, "<dev string:x3d>" + var_4b1a1ec87d5ac862[index].veh_speed + "<dev string:x6b>" + self.var_269a59bcee49ce1, (1, 1, 1), 1, 1, 1, 1);
                }
            }
        #/
...
function function_a23b8410030b26d0(vehicleposition) {
    assertex(vehicleposition >= 0 && vehicleposition < self.var_157fa22c13019614.maxvehicles, "Error - invalid vehicle position " + vehicleposition + "!!");
...
/#
    ...
    function private function_f47128cc3cb77caf(targetname) {
        ...
        vehicle thread script_func("<dev string:x6d>");
    ...
    function private function_b61f675503083701(addpath, var_784030f0b32d0f1d) {
        ...
        var_784030f0b32d0f1d = ter_op(isdefined(var_784030f0b32d0f1d), var_784030f0b32d0f1d, 0);
        if (!self hascomponent("<dev string:x7c>")) {
            self addcomponent("<dev string:x7c>");
        }
        if (addpath) {
            if (!self hascomponent("<dev string:x80>")) {
                self addcomponent("<dev string:x80>");
            }
        }
        if (var_784030f0b32d0f1d) {
            if (!self hascomponent("<dev string:x85>")) {
                self addcomponent("<dev string:x85>");
            }
        }
        self function_77320e794d35465a("<dev string:x7c>", "<dev string:x8e>", 0);
        self function_77320e794d35465a("<dev string:x7c>", "<dev string:x99>", 1);
        self function_77320e794d35465a("<dev string:x7c>", "<dev string:xa5>", 1);
        self function_77320e794d35465a("<dev string:x7c>", "<dev string:xb5>", 1);
        self function_77320e794d35465a("<dev string:x7c>", "<dev string:xc9>", self.origin);
        self function_77320e794d35465a("<dev string:x7c>", "<dev string:xd3>", 200);
        self function_77320e794d35465a("<dev string:x7c>", "<dev string:xe1>", 1);
        self function_77320e794d35465a("<dev string:x7c>", "<dev string:xf4>", 1);
        self function_77320e794d35465a("<dev string:x7c>", "<dev string:x10b>", 1);
        self function_77320e794d35465a("<dev string:x7c>", "<dev string:x129>", 1);
        self function_77320e794d35465a("<dev string:x7c>", "<dev string:x13f>", 4);
        self function_77320e794d35465a("<dev string:x7c>", "<dev string:xe1>", 2);
        self function_77320e794d35465a("<dev string:x7c>", "<dev string:x155>", 0.9);
        self function_77320e794d35465a("<dev string:x7c>", "<dev string:x167>", 2);
        self function_77320e794d35465a("<dev string:x80>", "<dev string:x17a>", 300);
        self.stopping = 0;
    }
    ...
    function private function_cd400918a8463816(params) {
        ...
        if (istrue(params[1] == "<dev string:x187>")) {
            ...
            if (isdefined(traceresults)) {
                navmeshposition = getclosestpointonnavmesh(traceresults["<dev string:x189>"], level.var_56c51d0b8c3270ca);
                /#
                    color = (1, 0.65, 0);
                    duration = 200;
                    sphere(traceresults["<dev string:x189>"], 5, color, 0, duration);
                    line(navmeshposition, traceresults["<dev string:x189>"], color, 1, 0, duration);
                #/
                goallocation = navmeshposition;
            }
        }
        vehicleforward = anglestoforward(vehicle.angles);
        var_34bb55fbc59c2e1d = isnavmeshloaded("<dev string:x192>");
        var_a8c9b5fffa6387d = isnavmeshloaded("<dev string:x1a0>");
        navmeshlayer = ter_op(var_34bb55fbc59c2e1d, "<dev string:x192>", ter_op(var_a8c9b5fffa6387d, "<dev string:x1a0>", undefined));
        var_c2202bf0909145b5 = ter_op(istrue(params[0] == "<dev string:x187>"), 2, 1);
        ...
        vehicle function_77320e794d35465a("<dev string:x7c>", "<dev string:x1ad>", var_bfe90410fad38aaa);
        if (isdefined(splinepoints)) {
            if (istrue(params[0] == "<dev string:x187>")) {
                ...
                vehicle function_77320e794d35465a("<dev string:x80>", "<dev string:x1b9>", splineid);
            }
    ...
    function private function_a496584181f051a9(numpassengers) {
        ...
        self.riders = [];
        self.unloadque = [];
        self.unload_group = "<dev string:x203>";
        vehicleforward = anglestoforward(self.angles);
        scripts\common\vehicle_aianim::handle_attached_guys();
        scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_setteam(self, "<dev string:x207>");
        for (seatindex = numpassengers; seatindex >= 0; seatindex--) {
            spawnlocation = self.origin + vehicleforward * seatindex * 100 + (0, 0, 200);
            agent = scripts\mp\mp_agent::spawnnewagentaitype("<dev string:x21b>", spawnlocation, self.angles, "<dev string:x207>", "<dev string:x240>");
            if (isdefined(agent)) {
    ...
    function private function_bc8d978e7b0a418c() {
        function_6e7290c8ee4f558b("<dev string:x250>");
        function_df648211d66cd3dd("<dev string:x260>", "<dev string:x260>");
        function_df648211d66cd3dd("<dev string:x27b>", "<dev string:x27b>");
        function_df648211d66cd3dd("<dev string:x29d>", "<dev string:x29d>");
        function_a9a864379a098ad6("<dev string:x2ca>", "<dev string:x2eb>", &function_76624b2b3e4db74f);
        function_b23a59dfb4ca49a1("<dev string:x30b>", "<dev string:x318>" + "<dev string:x334>", &function_1ab5e4e5878bd5e9);
        function_b23a59dfb4ca49a1("<dev string:x348>", "<dev string:x363>" + "<dev string:x334>", &function_2393c0027c00179a);
        function_b23a59dfb4ca49a1("<dev string:x377>", "<dev string:x398>" + "<dev string:x334>", &function_bbfa6f6f2d68f325);
        function_b23a59dfb4ca49a1("<dev string:x3ba>", "<dev string:x3d5>" + 0 + "<dev string:x3f0>" + 0, &function_cd400918a8463816);
        function_b23a59dfb4ca49a1("<dev string:x3f2>", "<dev string:x3d5>" + 1 + "<dev string:x3f0>" + 0, &function_cd400918a8463816);
        function_b23a59dfb4ca49a1("<dev string:x411>", "<dev string:x3d5>" + 0 + "<dev string:x3f0>" + 1, &function_cd400918a8463816);
