...
function private log_msg(loglevel, thetext) {
    if (loglevel < 5 && isdefined(thetext)) {
        /#
            if (isarray(thetext)) {
                thetext = string_join(thetext, "<unknown string>");
            }
            switch (level.var_6492adee56d19067.logdestination) {
            case 0:
                println(level.var_6492adee56d19067.prefixes[loglevel] + "<unknown string>" + thetext);
                break;
            case 1:
                iprintln(level.var_6492adee56d19067.prefixes[loglevel] + "<unknown string>" + thetext);
                break;
            default:
                /#
                    assertmsg("<unknown string>" + default_to(level.var_6492adee56d19067.logdestination, "<unknown string>"));
                #/
                break;
            }
        #/
