...
function javelin_eyetraceforward() {
    ...
    /#
        if (getdvar(@"hash_b56d344ee60e317f") == "<dev string:x47>") {
            ...
            thread scripts\mp\utility\debug::drawsphere(var_7f74be76ba001a7a["<dev string:x49>"], 5, 20, (1, 0, 1));
            iprintlnbold(distance(origin, var_7f74be76ba001a7a["<dev string:x49>"]));
        }
    #/
    results = [];
...
function javelin_enterstate(statename) {
    ...
    /#
        function_26c6f6de312b721f("<dev string:x52>" + statename + "<dev string:x6b>");
    #/
    ...
    if (enable) {
        ...
        /#
            if (getdvar(@"hash_22ff48dd25613942") == "<dev string:x47>") {
                self setclientomnvar("<dev string:x6e>", 0);
            }
        #/
        return;
    }
