    ...
    /#
        ...
        while (true) {
            if (getdvarint(@"hash_1432c61b2a63cd8e", 0) != 0) {
                ...
                level.objectives["preCapPoints"] onuse(capplayer);
                wait(0.15);
                level.objectives["holdAllTimer"] onuse(capplayer);
            }
    ...
    /#
        ...
        while (true) {
            if (getdvar(@"hash_6ce167f2cdc8ef7c", "waypoint_capture") != "waypoint_capture") {
                flaglabel = getdvar(@"hash_6ce167f2cdc8ef7c", "waypoint_capture");
                foreach (domflag in level.objectives) {
                    if (isdefined(domflag.objectivekey) && domflag.objectivekey == flaglabel) {
                        domflag scripts/mp/gameobjects::allowuse("waypoint_defend");
    ...
    /#
        ...
        while (true) {
            if (getdvar(@"hash_29d45d6822a1cf6d", "waypoint_capture") != "waypoint_capture") {
                flaglabel = getdvar(@"hash_29d45d6822a1cf6d", "waypoint_capture");
                zone = spawnstruct();
                ...
                trace = scripts/engine/trace::_bullet_trace(tracestart, traceend, 0, undefined);
                ...
                domflag = scripts/mp/gameobjects::createuseobject("dom_fallback", zone.trigger, zone.visuals, (0, 0, 100));
                domflag scripts/mp/gameobjects::allowuse("idle");
                domflag scripts/mp/gameobjects::setusetime(level.caprate);
                if (isdefined(trigger.objectivekey)) {
                    domflag.objectivekey = trigger.objectivekey;
                } else {
                ...
                domflag scripts/mp/gameobjects::setobjectivestatusicons("lives", "mp_dom_flag_captured");
                domflag scripts/mp/gameobjects::setvisibleteam("teamCount");
                domflag scripts/mp/gameobjects::cancontestclaim(1);
                domflag.onuse = &onuse;
...
function domdebug() {
    /#
        spawnpoints = scripts/mp/spawnlogic::getteamspawnpoints("mp_promenade_gw");
        ...
        while (true) {
            ...
            while (true) {
                ...
                endpoints = [];
                endpoints[1] = [level.objectives["preCapPoints"]];
                endpoints[2] = [level.objectives["holdAllTimer"]];
                endpoints[4] = [level.objectives["<unknown string>"]];
                endpoints[8] = [level.objectives["<unknown string>"]];
                endpoints[16] = [level.objectives["<unknown string>"]];
                endpoints[32] = [level.objectives["preCapPoints"], level.objectives["holdAllTimer"]];
                endpoints[64] = [level.objectives["<unknown string>"], level.objectives["holdAllTimer"]];
                endpoints[128] = [level.objectives["preCapPoints"], level.objectives["<unknown string>"]];
                ...
                foreach (flaglocation in level.objectives) {
                    if (flaglocation == level.bestspawnflag["<unknown string>"]) {
                        print3d(flaglocation.trigger.origin + heightoffsetnames, "<unknown string>");
                    }
                    if (flaglocation == level.bestspawnflag["mp_promenade_gw"]) {
                        print3d(flaglocation.trigger.origin + heightoffsetnames, "<unknown string>");
                    }
                }
                waitframe();
