...
function function_cbe00caa24da0f0e() {
    if (level.var_319544f76c37154b["axis"] == level.var_319544f76c37154b["allies"]) {
        /#
            debugroundwinner("<dev string:x1c>");
        #/
        return "tie";
    }
...
/#
    ...
    function debugendround() {
        ...
        if (result) {
            ...
            switch (result) {
            case 1: 
                level thread scripts\mp\gamelogic::endgame(level.players[0].team, game["<dev string:x2b>"]["<dev string:x36>"]);
                break;
            case 2: 
                team = undefined;
                if (level.players[0].team == game["<dev string:x48>"]) {
                    team = game["<dev string:x52>"];
                } else {
                    team = game["<dev string:x48>"];
                }
                level thread scripts\mp\gamelogic::endgame(team, game["<dev string:x2b>"]["<dev string:x36>"]);
                break;
            case 3: 
                level thread scripts\mp\gamelogic::endgame("<dev string:x1c>", game["<dev string:x2b>"]["<dev string:x36>"]);
                break;
            }
        }
    }
    ...
    function function_eed7800933a86a6d() {
        level endon("<dev string:x20>");
        var_345653ec713a050e = getentarray("<dev string:x7a>", "<dev string:x8b>");
        var_345652ec713a02db = getentarray("<dev string:x92>", "<dev string:x8b>");
        var_345651ec713a00a8 = getentarray("<dev string:xa3>", "<dev string:x8b>");
        while (true) {
