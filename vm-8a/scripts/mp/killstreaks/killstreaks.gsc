...
function triggerkillstreak(streakitem, slotnumber) {
    ...
    /#
        ...
        assertex(streakinfo.debug_state == "<dev string:x64>" || streakinfo.debug_state == "<dev string:x6e>", streakinfo.streakname + "<dev string:x77>");
    #/
    if (isdefined(slotnumber)) {
...
function onsuccessfulstreakactivation(streakitem, slotnumber) {
    ...
    assertex(streakitem.available == 1, "onSuccessfulStreakActivation() called on a killstreak that is not marked as available.");
        ...
        /#
            for (var_ea2fb2bd569ef975 = 0; var_ea2fb2bd569ef975 < 37; var_ea2fb2bd569ef975++) {
                ...
                if (var_730ccc1b27c04ae8 == streakitem) {
                    assertmsg("<dev string:xb6>" + var_ea2fb2bd569ef975);
                    break;
                }
            }
        #/
...
function slotkillstreak(streakitem, slotnumber) {
    assertex(isdefined(streakitem), "slotKillstreak() passed an undefined streakItem");
    assertex(streakitem.streakname != "none", "slotKillstreak() attempting to slot the \"none\" killstreak.");
    if (slotnumber == 0) {
        if (!pushgimmeslotstreakontostack()) {
            return;
        }
    } else {
        assertex(slotnumber == 1 || slotnumber == 2 || slotnumber == 3 || slotnumber == 4, "slotKillstreak() called on an invalid slot (\"" + slotnumber + "\")");
        /#
            function_4e6ba9d5ec3ea26b();
        #/
    }
...
function setselectedkillstreak(slotnumber) {
    ...
    assertex(slotnumber >= 0 && slotnumber <= 3, "setSelectedKillstreak() called on a stacked killstreak slot.");
    streakitem = getkillstreakinslot(slotnumber);
    assertex(isdefined(streakitem), "setSelectedKillstreak() called on slot " + slotnumber + ", but there is nothing there.");
...
function getkillstreaksetupinfo(streakname) {
    ...
    assertex(isdefined(streaksetup), "getKillstreakSetupInfo() called for a killstreak (\"" + streakname + "\") that hasn't been registered");
    return streaksetup;
}
...
function streakglobals_onkillstreaktriggered(streakinfo) {
    /#
        streakinfo.debug_state = "<dev string:x11f>";
    #/
...
function streakglobals_onkillstreakbeginuse(streakinfo) {
    /#
        assertex(isdefined(streakinfo.debug_state), streakinfo.streakname + "<dev string:x129>");
        assertex(streakinfo.debug_state == "<dev string:x11f>", streakinfo.streakname + "<dev string:x169>");
        streakinfo.debug_state = "<dev string:x64>";
    #/
    owner = streakinfo.owner;
    if (isdefined(streakinfo.mpstreaksysteminfo)) {
...
function streakglobals_onkillstreakfinishuse(params) {
    ...
    /#
        assertex(isdefined(streakinfo.debug_state), streakinfo.streakname + "<dev string:x1ce>");
        assertex(streakinfo.debug_state == "<dev string:x64>", streakinfo.streakname + "<dev string:x20f>");
        streakinfo.debug_state = "<dev string:x6e>";
    #/
    owner = streakinfo.owner;
...
function givestreakpoints(type, killpoints, scorepoints) {
    ...
    if (isvalidevent && !istrue(level.disablepersonalnuke) && (istrue(level.allowkillstreaks) || isdefined(level.personalnukecostoverride))) {
        ...
        if (nukecost > 0) {
            if (var_44c59a12e6465947 >= nukecost) {
...
/#
    ...
    function function_4e6ba9d5ec3ea26b() {
        ...
        var_959f178e714b214d = "<dev string:x273>";
        for (slotnumber = 0; slotnumber <= 3; slotnumber++) {
            ...
            streakcost = streakitem.currentcost;
            assertex(streakcost >= var_fc5890f05240ffaa, "<dev string:x274>" + slotnumber + "<dev string:x294>" + streakitem.streakname + "<dev string:x297>" + var_959f178e714b214d + "<dev string:x2c8>");
            var_fc5890f05240ffaa = streakcost;
            var_959f178e714b214d += streakitem.streakname + "<dev string:x2ca>";
        }
    }
