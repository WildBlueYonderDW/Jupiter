...
function control_onstartgametype() {
    ...
    if (level.overtimeDefenders != 0 && game["roundsWon"][defending_team] == level.winlimit - 1 && game["roundsWon"][attacking_team] == level.winlimit - 1) {
        ...
        bestteam = var_57acadc40b2f0a8[0];
...
function function_d5ddc77707c5c50e(trigger, team, var_5ddbc1faed2c56e6, skipobjid, showoncompass) {
    ...
    if (istrue(var_5ddbc1faed2c56e6)) {
        var_3c2389ba69e5822b = scripts\mp\gametypes\obj_dom::getreservedobjid(objectivekey);
    } else {
    ...
    control_zone = scripts\mp\gameobjects::createuseobject(team, trigger, visuals, var_88e6e0bc0479fa98 - trigger.origin, var_3c2389ba69e5822b, skipobjid, showoncompass);
...
function flagsetup() {
    /#
        setdevdvarifuninitialized(@"hash_695e8f43f6b03cd4", "<dev string:x5b>");
    #/
...
/#
    ...
    function function_8861fae4db7f9c2() {
        spawnpoints = spawnlogic::getteamspawnpoints("<dev string:x5d>");
        fallbacks = spawnlogic::getteamfallbackspawnpoints("<dev string:x5d>");
        ...
        colors[128] = (1, 0, 1);
        while (true) {
            if (getdvar(@"hash_695e8f43f6b03cd4") != "<dev string:x62>") {
                wait 1;
                continue;
            }
            setdevdvar(@"hash_9d60ff019dc50c22", "<dev string:x62>");
            while (true) {
                if (getdvar(@"hash_695e8f43f6b03cd4") != "<dev string:x62>") {
                    setdevdvar(@"hash_9d60ff019dc50c22", "<dev string:x5b>");
                    break;
                }
                ...
                endpoints = [];
                endpoints[1] = [level.objectives["<dev string:x64>"]];
                endpoints[2] = [level.objectives["<dev string:x67>"]];
                endpoints[4] = [level.objectives["<dev string:x6a>"]];
                endpoints[8] = [level.objectives["<dev string:x6d>"]];
                endpoints[16] = [level.objectives["<dev string:x70>"]];
                endpoints[32] = [level.objectives["<dev string:x64>"], level.objectives["<dev string:x67>"]];
                endpoints[64] = [level.objectives["<dev string:x6a>"], level.objectives["<dev string:x67>"]];
                endpoints[128] = [level.objectives["<dev string:x64>"], level.objectives["<dev string:x6a>"]];
                foreach (spawnpoint in spawnpoints) {
                ...
                foreach (flaglocation in level.objectives) {
                    if (flaglocation == level.bestspawnflag["<dev string:x73>"]) {
                        print3d(flaglocation.trigger.origin + heightoffsetnames, "<dev string:x7a>");
                    }
                    if (flaglocation == level.bestspawnflag["<dev string:x5d>"]) {
                        print3d(flaglocation.trigger.origin + heightoffsetnames, "<dev string:x94>");
                    }
                }
                waitframe();
            }
...
/#
    ...
    function function_95b0959344794f91() {
        ...
        while (true) {
            if (getdvarint(@"hash_46a6b424af6acbc2") != 0) {
                scripts\mp\gamescore::giveteamscoreforobjective("<dev string:x73>", getdvarint(@"hash_46a6b424af6acbc2"), 1);
                break;
            } else if (getdvarint(@"hash_86e14326e43c0115") != 0) {
                scripts\mp\gamescore::giveteamscoreforobjective("<dev string:x5d>", getdvarint(@"hash_86e14326e43c0115"), 1);
                break;
            }
            wait 1;
        }
    ...
    function removedompoint() {
        self endon("<dev string:xae>");
        while (true) {
            if (getdvar(@"hash_6ce167f2cdc8ef7c", "<dev string:xb9>") != "<dev string:xb9>") {
                flaglabel = getdvar(@"hash_6ce167f2cdc8ef7c", "<dev string:xb9>");
                foreach (domflag in level.objectives) {
                    if (isdefined(domflag.objectivekey) && domflag.objectivekey == flaglabel) {
                        domflag scripts\mp\gameobjects::allowuse("<dev string:xba>");
                        domflag.trigger = undefined;
                        domflag notify("<dev string:xbf>");
                        domflag.visibleteam = "<dev string:xba>";
                ...
                setdynamicdvar(@"hash_6ce167f2cdc8ef7c", "<dev string:xb9>");
            }
            wait 1;
        }
    }
    ...
    function placedompoint() {
        self endon("<dev string:xae>");
        while (true) {
            if (getdvar(@"hash_29d45d6822a1cf6d", "<dev string:xb9>") != "<dev string:xb9>") {
                flaglabel = getdvar(@"hash_29d45d6822a1cf6d", "<dev string:xb9>");
                zone = spawnstruct();
                zone.origin = level.players[0].origin;
                zone.angles = level.players[0].angles;
                trigger = spawn("<dev string:xc7>", zone.origin, 0, 120, 128);
                zone.trigger = trigger;
                zone.trigger.script_label = flaglabel;
                zone.ownerteam = "<dev string:xd6>";
                ...
                zone.origin = trace["<dev string:xde>"];
                zone.upangles = vectortoangles(trace["<dev string:xe7>"]);
                zone.forward = anglestoforward(zone.upangles);
                zone.right = anglestoright(zone.upangles);
                zone.visuals[0] = spawn("<dev string:xee>", zone.origin);
                zone.visuals[0].angles = zone.angles;
                domflag = scripts\mp\gameobjects::createuseobject("<dev string:xd6>", zone.trigger, zone.visuals, (0, 0, 100));
                domflag scripts\mp\gameobjects::allowuse("<dev string:xfb>");
                ...
                domflag scripts\mp\gameobjects::setvisibleteam("<dev string:x101>");
                ...
                ignoreents = [];
                trace = scripts\engine\trace::ray_trace(tracestart, traceend, ignoreents, contentoverride);
                domflag.baseeffectpos = trace["<dev string:xde>"];
                upangles = vectortoangles(trace["<dev string:xe7>"]);
                ...
                setdynamicdvar(@"hash_29d45d6822a1cf6d", "<dev string:xb9>");
            }
            wait 1;
        }
    }
