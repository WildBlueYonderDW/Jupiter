...
/#
    ...
    function private devgui_init() {
        ...
        devgui::function_a2fe753948998c89("<dev string:x3f>", "<dev string:x44>");
        devgui::function_b23a59dfb4ca49a1("<dev string:x8f>", "<dev string:xb3>", &function_1d3ab9e562b3e3db);
        devgui::function_b23a59dfb4ca49a1("<dev string:xc4>", "<dev string:xe5>", &function_1d3ab9e562b3e3db);
        devgui::function_b23a59dfb4ca49a1("<dev string:xf6>", "<dev string:x10a>", &function_ac26cf14f95df3f8);
        devgui::function_b23a59dfb4ca49a1("<dev string:x117>", "<dev string:x129>", &function_ac26cf14f95df3f8);
        devgui::function_b23a59dfb4ca49a1("<dev string:x136>", "<dev string:x14a>", &function_ac26cf14f95df3f8);
        devgui::function_fe953f000498048f();
        if (game_utility::isbrstylegametype()) {
            devgui::function_6e7290c8ee4f558b("<dev string:x157>");
            devgui::function_a2fe753948998c89("<dev string:x172>", "<dev string:x180>");
            devgui::function_fe953f000498048f();
        }
    }
    ...
    function private function_a685587c650ff936(team) {
        ...
        if (team == 0) {
            ...
            if (!isdefined(player)) {
                iprintlnbold("<dev string:x1c1>");
                return;
            }
        }
        streakinfo = namespace_2b1145f62aa835b8::function_bc5bcad4a3bbd734("<dev string:x1f4>", player);
    ...
    function private function_cf8e244be65c5a69(direction) {
        ...
        if (getdvarint(@"hash_d2398cc34f83ff2b", 1)) {
            ...
            if (isdefined(dir_trace) && dir_trace.size > 0) {
                has_debug = isdefined(dir_trace["<dev string:x21f>"]) + isdefined(dir_trace[dir_trace.size - 1]["<dev string:x229>"]);
                for (i = 0; i < dir_trace.size - has_debug; i++) {
                    potential_output = dir_trace[i];
                    if (i > 1) {
                        var_f5ba189e648743db = dir_trace[i - 1];
                        dist = distance(potential_output["<dev string:x22e>"], var_f5ba189e648743db["<dev string:x22e>"]);
                        if (dist > 512) {
                            break;
                        }
                        if (dist < 64) {
                    ...
                    missile = spawn_missile(potential_output["<dev string:x22e>"], -32, (0, 0, -1024), missile);
                    stuck_to = missile function_e9b8d48cb8dd3d5a();
                    level thread missile_explosion(missile, stuck_to);
                }
            }
    ...
    function private full_rt(direction) {
        ...
        if (trace.size == 1) {
            ...
            if (isstring(key) && key == "<dev string:x21f>") {
                return undefined;
            }
        }
    ...
    function private adjust_trace(trace, end, direction, min_dist, max_dist) {
        ...
        valid_trace = [];
        if (isdefined(trace["<dev string:x21f>"])) {
            debug_data = trace["<dev string:x21f>"];
            trace["<dev string:x21f>"] = undefined;
        }
        if (direction == 1) {
        ...
        for (i = 0; i < trace.size; i++) {
            ...
            if (!isdefined(trace[i + 1])) {
                ...
                ntp = valid_trace[valid_trace.size - 1]["<dev string:x22e>"];
            } else {
                ntp = trace[i + 1]["<dev string:x22e>"];
            }
            dist = abs(distance(ct["<dev string:x22e>"], ntp));
            if (dist == 0) {
            ...
            if (direction == 1 && getdvarint(@"hash_95dca2c14e359be2", 1)) {
                test_trace = trace::ray_trace(ct["<dev string:x22e>"], utility::flat_origin(ct["<dev string:x22e>"]), undefined, undefined, 1, 1, 0);
                if (test_trace["<dev string:x237>"] == "<dev string:x23f>") {
                    continue;
                }
            }
            valid_trace = utility::array_add(valid_trace, ct);
    ...
    function private spawn_missile(origin, offset, velocity, prev_missile) {
        ...
        if (!isdefined(prev_missile)) {
            streakinfo = namespace_2b1145f62aa835b8::function_bc5bcad4a3bbd734("<dev string:x1f4>", self);
        } else {
            streakinfo = prev_missile.streakinfo;
        }
        missile = magicgrenademanual("<dev string:x24c>", var_206dbd116490dd48.sourcepos, var_206dbd116490dd48.initvelocity, 6, self);
        missile setscriptablepartstate("<dev string:x259>", "<dev string:x25f>", 0);
        if (!isdefined(prev_missile)) {
            missile setscriptablepartstate("<dev string:x266>", "<dev string:x25f>", 0);
        }
        if (isdefined(prev_missile)) {
            ...
            minimapid = undefined;
            if (utility::issharedfuncdefined("<dev string:x26d>", "<dev string:x272>")) {
                minimapid = missile [[ utility::getsharedfunc("<dev string:x26d>", "<dev string:x272>") ]]("<dev string:x282>", self.team, undefined, 1, 1);
            }
        }
    ...
    function private function_f6ddf2accdb58432() {
        if (self.classname != "<dev string:x2ac>") {
            namespace_2b1145f62aa835b8::function_6262c3a1ef0924d(0);
        }
        if (isdefined(self.minimapid)) {
            if (utility::issharedfuncdefined("<dev string:x26d>", "<dev string:x2b9>")) {
                [[ utility::getsharedfunc("<dev string:x26d>", "<dev string:x2b9>") ]](self.minimapid);
            }
    ...
    function private function_e9b8d48cb8dd3d5a() {
        impact_info = killstreak_shared::function_51afe751c1e36723("<dev string:x2cb>", "<dev string:x2d9>");
        stuck_to = undefined;
        if (isdefined(impact_info)) {
            if (impact_info.msg == "<dev string:x2cb>") {
                stuck_to = impact_info.param1;
            } else if (impact_info.msg == "<dev string:x2d9>") {
                self.water_impact = 1;
            }
        }
        return stuck_to;
    ...
    function private missile_explosion(missile, stuck_to) {
        ...
        explode_part = istrue(missile.water_impact) ? "<dev string:x2ee>" : "<dev string:x302>";
        explosion_ent setscriptablepartstate(explode_part, "<dev string:x25f>", 0);
