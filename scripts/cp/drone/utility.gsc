...
// Size: 0x190
function deploy_drone(player, var_d121285bbe12fc18) {
    ...
    if (istrue(var_d121285bbe12fc18.play_intro)) {
    ...
    if (!istrue(var_d121285bbe12fc18.no_control)) {
        player val::set("deploy_drone", "weapon_switch", 0);
        return deploy_helper_drone_actual(player, var_d121285bbe12fc18);
    }
    player takeweapon("ks_remote_drone_mp");
    player switchtoweapon(player.pre_drone_weapon);
    player.using_drone = undefined;
...
// Size: 0x138
function deploy_helper_drone_actual(player, var_d121285bbe12fc18) {
    start_pos = player.origin + (0, 0, 150);
    if (!istrue(var_d121285bbe12fc18.detonate_mines)) {
        start_pos = find_safe_spawn(player);
    }
    drone = create_drone(player, start_pos, var_d121285bbe12fc18);
    if (isdefined(drone)) {
        if (player isjumping()) {
        ...
        if (istrue(var_d121285bbe12fc18.send_down)) {
            drone thread send_drone_down(drone, player, var_d121285bbe12fc18);
        }
...
// Size: 0x54
function send_drone_down(drone, player, var_d121285bbe12fc18) {
    wait 0.05;
...
// Size: 0x255
function create_drone(player, start_pos, var_d121285bbe12fc18) {
    drone = spawnhelicopter(player, start_pos, player getplayerangles(), var_d121285bbe12fc18.vehicle_info, var_d121285bbe12fc18.model);
    if (!isdefined(drone)) {
        return;
    }
    drone enableaimassist();
    drone setnodeploy(1);
    drone.speed = var_d121285bbe12fc18.speed;
    drone.accel = var_d121285bbe12fc18.accel;
    drone.angles = player.angles;
    drone.owner = player;
    drone.team = player.team;
    if (isdefined(var_d121285bbe12fc18.self_destruct)) {
        drone.self_destruct = var_d121285bbe12fc18.self_destruct;
    }
    if (isdefined(var_d121285bbe12fc18.detonate_mines)) {
        drone.detonate_mines = var_d121285bbe12fc18.detonate_mines;
    }
    if (isdefined(var_d121285bbe12fc18.mark_ai)) {
        drone.mark_ai = var_d121285bbe12fc18.mark_ai;
    }
    if (isdefined(var_d121285bbe12fc18.mark_vehicles)) {
        drone.mark_vehicles = var_d121285bbe12fc18.mark_vehicles;
    }
    ...
    drone.useobj hide();
    drone thread timeout_monitor(player, drone, var_d121285bbe12fc18);
    drone thread damage_monitor(player, drone, var_d121285bbe12fc18);
    drone thread player_exit_monitor(player, drone);
    if (istrue(var_d121285bbe12fc18.detonate_mines)) {
        drone thread remote_detonation_monitor(player, drone);
    }
    if (isdefined(var_d121285bbe12fc18.use_func)) {
        drone thread [[ var_d121285bbe12fc18.use_func ]](player, drone);
    }
    return drone;
}
...
// Size: 0x65
function timeout_monitor(player, drone, var_d121285bbe12fc18) {
    drone endon("death");
    player setclientomnvar("ui_killstreak_countdown", gettime() + int(var_d121285bbe12fc18.timeout * 1000));
    wait var_d121285bbe12fc18.timeout;
    exit_drone(player, drone);
    drone_explode(drone);
}
...
// Size: 0x1a5
function damage_monitor(player, drone, var_d121285bbe12fc18) {
    drone endon("death");
    drone setcandamage(1);
    drone.health = 999999;
    drone.maxhealth = 999999;
    drone.fake_health = var_d121285bbe12fc18.health;
    original_health = var_d121285bbe12fc18.health;
    player setclientomnvar("ui_killstreak_health", 1);
    while (true) {
        damage, attacker, direction_vec, point, type, modelname, tagname, partname, idflags, weapon = drone waittill("damage");
        drone.health = 999999;
        drone.maxhealth = 999999;
        drone.fake_health -= damage;
        player setclientomnvar("ui_killstreak_health", drone.fake_health / original_health);
        if (isdefined(attacker) && isplayer(attacker)) {
            attacker thread namespace_3d3d5211c3029f82::updatedamagefeedback("standard");
        }
