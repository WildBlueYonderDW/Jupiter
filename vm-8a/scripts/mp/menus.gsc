...
function onteamchangecallback(channel, teamselected) {
    if (!scripts\cp_mp\utility\game_utility::isbrstylegametype()) {
        println("<dev string:x1c>" + self getentitynumber() + "<dev string:x25>" + self.name + "<dev string:x30>" + channel);
...
function debugprintsquads() {
    ...
    while (true) {
        if (isdefined(level.squaddata)) {
            ...
            linecount = 1;
            /#
                printtoscreen2d(startx + 250, starty, "<dev string:x10a>", (1, 1, 1), 1);
            #/
            foreach (team, var_70643c8580524236 in level.squaddata) {
                linecount = 1;
                /#
                    printtoscreen2d(startx, starty + stepy * linecount, "<dev string:x115>" + team, (1, 1, 1), 1);
                #/
                linecount++;
                foreach (squad in var_70643c8580524236) {
                    ...
                    /#
                        printtoscreen2d(startx + 25, starty + stepy * linecount, "<dev string:x11c>" + squad.index, color, 1);
                    #/
                    linecount++;
