...
function init(config) {
    ...
    event_info.circleeventweights = scripts\mp\gametypes\br_publicevents::function_8fc51a5af06412cf(config.circleweights);
    event_info.var_b9b56551e1acfee2 = namespace_c711384b1335919::function_8be9bae8228a91f7("dataheist");
    scripts\mp\gametypes\br_publicevents::function_5b7d8ca93f8705f1(event_info, "dataheist");
    scripts\mp\gametypes\br_publicevents::registerpublicevent(8, event_info);
    ...
    if (isdefined(var_f087e13b64fb79ca) && var_f087e13b64fb79ca != "") {
        ...
        foreach (token in tokens) {
            ...
            if (!isfloat(threshold)) {
                println("<dev string:x1d>" + token + "<dev string:x3a>");
                continue;
            }
...
function function_dce158cd5558c35d() {
    ...
    if (force_points.size > 0) {
        ...
        if (var_ff9e6fbb61b45590.size <= 0) {
            ...
            #/
            level thread scripts\mp\gametypes\br_publicevents::function_2907d01a7d692108(8);
            return;
        }
...
function uplink_start(uplink_point) {
    assertex(isdefined(uplink_point), "<dev string:xb7>");
    level endon("game_ended");
    self endon("event_deactivated");
    uplink_point.used = 1;
    uplink = spawnstruct();
    ...
    assertex(uplink.obj_id != -1, "<dev string:xf6>");
...
function function_3723563cad2eb687(requests, reinforcement_types, params) {
    ...
    assertex(agents.size > 0, "<dev string:x122>");
    waitframe();
    foreach (agent in agents) {
        assertex(isdefined(agent), "<dev string:x153>");
        assertex(!isint(agent), "<dev string:x193>");
...
function function_74428d38b94d6664() {
    ...
    if (!isdefined(agents) || agents.size <= 0) {
        println("<dev string:x1c9>" + self.loc_name);
    } else {
        foreach (agent in agents) {
...
function function_63bdbc18191be5f9(dialog, players, delay) {
    ...
    assertex(isdefined(sound_name), "Error: Dataheist failed to find sound name for dialog: " + dialog);
...
function function_24afcbb2ea0ccb49(event_duration) {
    foreach (point in self.uplink_points) {
        ...
        if (!level.br_circle_disabled) {
            ...
            if (hit_time < event_duration) {
                point.is_valid = 0;
                /#
                    point.status = "<dev string:x220>" + hit_time + "<dev string:x241>";
                #/
                continue;
            }
        }
        /#
            point.status = "<dev string:x24a>";
        #/
    }
    ...
    /#
        dump = dump_new("<dev string:x251>");
        dump function_d10a8a5bcebaedf2("<dev string:x270>" + event_duration + "<dev string:x241>");
        dump function_d10a8a5bcebaedf2("<dev string:x281>");
        good_starts = [];
        foreach (index, point in self.uplink_points) {
            ...
            dump function_d10a8a5bcebaedf2("<dev string:x297>" + index + "<dev string:x299>" + point.group + "<dev string:x29d>" + point.status);
            if (point.is_valid) {
                dump function_8c2574850bb05347("<dev string:x2a9>" + point.valid_neighbors.size);
            }
        }
        keys = getarraykeys(good_starts);
        foreach (key in keys) {
            dump function_d10a8a5bcebaedf2(key + "<dev string:x2be>" + good_starts[key]);
        }
        dump dump_print();
    #/
}
...
function function_671cc803581d0e65(points_string) {
    ...
    foreach (point_string in var_64598ecf49ab5725) {
        ...
        /#
            iprintlnbold("<dev string:x2c2>" + point_string + "<dev string:x2e6>" + self.uplink_points.size - 1);
        #/
    }
    return points;
}
...
/#
    ...
    // Size: 0x120
    function debug_init() {
        scripts\common\devgui::function_6e7290c8ee4f558b("<dev string:x314>");
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x333>", "<dev string:x33f>", &function_db70bae7732f5204);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x355>", "<dev string:x360>", &function_79f09180ee96a5de);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x375>", "<dev string:x385>", &function_ca12a92628328432);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x3a5>", "<dev string:x3b8>", &function_c3d7b176abc6b629);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x3d5>", "<dev string:x3eb>", &function_2a20415ca9baa8f9);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x411>", "<dev string:x425>", &function_9f75279b43f251c6);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x449>", "<dev string:x463>", &function_6eeb59951c1edac6);
        scripts\common\devgui::function_b23a59dfb4ca49a1("<dev string:x483>", "<dev string:x49a>", &function_fd6377203b2db297);
        scripts\common\devgui::function_b23a59dfb4ca49a1("<dev string:x4c7>", "<dev string:x4de>", &function_fd6377203b2db297);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x50b>", "<dev string:x525>", &function_83873605c0cc0f3a);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x549>", "<dev string:x562>", &function_41d60ffda1134555);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x586>", "<dev string:x59a>", &function_7e5217f288b95f5e);
    ...
    function function_db70bae7732f5204() {
        level thread scripts\mp\gametypes\br_publicevents::function_e37ee072d95a7c98(8);
    }
    ...
    function function_ca12a92628328432() {
        spawnscriptable("<dev string:x5be>", level.player.origin, level.player.angles);
    }
    ...
    function function_9c68002c913d40e5() {
        level endon("<dev string:x5f7>");
        level endon("<dev string:x5d5>");
        event_data = level.br_pe_data[8];
        if (!isdefined(event_data)) {
            return;
        }
        ...
        while (true) {
            ...
            foreach (uplink in event_data.uplinks) {
                ...
                print3d(uplink.origin + position_offset - var_f037fc926270b3a2 * 0, "<dev string:x602>" + uplink.origin, color, 1, scale, var_e486b5f3a3268190, 1);
                print3d(uplink.origin + position_offset - var_f037fc926270b3a2 * 1, "<dev string:x60b>" + uplink.angles, color, 1, scale, var_e486b5f3a3268190, 1);
                print3d(uplink.origin + position_offset - var_f037fc926270b3a2 * 2, "<dev string:x614>" + uplink.controlling_team, color, 1, scale, var_e486b5f3a3268190, 1);
                print3d(uplink.origin + position_offset - var_f037fc926270b3a2 * 3, "<dev string:x61b>" + uplink.progress * 100 + "<dev string:x626>", color, 1, scale, var_e486b5f3a3268190, 1);
                if (uplink.num_blockers > 0) {
                    print3d(uplink.origin + position_offset - var_f037fc926270b3a2 * 4, "<dev string:x628>" + uplink.num_blockers + "<dev string:x634>", color, 1, scale, var_e486b5f3a3268190, 1);
                }
            }
            wait print_duration;
        }
    ...
    function function_2634cd6355be1303() {
        ...
        while (true) {
            foreach (point in event_data.uplink_points) {
                ...
                print3d(point.origin + position_offset + var_e8285ce19e9412ce * 3, "<dev string:x661>" + point.id, color, 1, scale, var_e486b5f3a3268190, 1);
                print3d(point.origin + position_offset + var_e8285ce19e9412ce * 2, "<dev string:x673>" + point.group, color, 1, scale, var_e486b5f3a3268190, 1);
                print3d(point.origin + position_offset + var_e8285ce19e9412ce * 1, "<dev string:x67b>" + point.status, color, 1, scale, var_e486b5f3a3268190, 1);
    ...
    function function_2a20415ca9baa8f9() {
        ...
        foreach (uplink in event_data.uplinks) {
            uplink notify("<dev string:x68c>", "<dev string:x696>");
        }
    }
    ...
    function function_9f75279b43f251c6() {
        ...
        foreach (uplink in event_data.uplinks) {
            uplink notify("<dev string:x68c>", team);
        }
    }
    ...
    function function_6eeb59951c1edac6() {
        ...
        foreach (point in event_data.uplink_points) {
            event_data.var_6d21295d793761b = array_add(event_data.var_6d21295d793761b, spawnscriptable("<dev string:x5be>", point.origin, point.angles));
        }
    }
    ...
    // Size: 0x3a
    function dump_print() {
        println("<dev string:x6af>" + self.title + "<dev string:x6c0>");
        println(self.msg);
        println("<dev string:x6d0>");
    }
    ...
    function function_d10a8a5bcebaedf2(text) {
        function_8c2574850bb05347("<dev string:x6f3>" + text);
    }
