...
function getspawnpoint(spawningplayer, spawnteam, spawnpointset, fallbackspawnset, var_a22bad5fe6fbf86d, spawnparams) {
    ...
    /#
        if (scripts\mp\utility\game::getgametype() != "<dev string:x20>" && scripts\mp\utility\game::getgametype() != "<dev string:x23>" && scripts\mp\utility\game::getgametype() != "<dev string:x28>" && scripts\mp\utility\game::getgametype() != "<dev string:x31>" && scripts\mp\utility\game::getgametype() != "<dev string:x36>") {
    ...
    if (isdefined(fallbackspawnset) && getspawnsetsize(fallbackspawnset) > 0 && bestspawnpoint.bucket >= 2) {
        ...
        /#
            if (scripts\mp\utility\game::getgametype() != "<dev string:x20>" && scripts\mp\utility\game::getgametype() != "<dev string:x23>" && scripts\mp\utility\game::getgametype() != "<dev string:x28>" && scripts\mp\utility\game::getgametype() != "<dev string:x31>") {
    ...
    if (level.forcebuddyspawn) {
        ...
        if (isdefined(buddyspawnpoint)) {
            /#
                if (getdvarint(@"hash_f3b498670cb45729", 0) != 0) {
                    if (!isdefined(level.var_952af294bfd228a7["<dev string:x39>"])) {
                        level.var_952af294bfd228a7["<dev string:x39>"] = [];
                    }
                    level.var_952af294bfd228a7["<dev string:x39>"][level.var_952af294bfd228a7["<dev string:x39>"].size] = buddyspawnpoint;
                }
            #/
    ...
    if (bestspawnpoint.bucket >= 2) {
        ...
        if (var_a22bad5fe6fbf86d >= 3 && !var_cf9bd39bcb455a12) {
            ...
            if (isdefined(buddyspawnpoint)) {
                ...
                /#
                    if (getdvarint(@"hash_f3b498670cb45729", 0) != 0) {
                        if (!isdefined(level.var_952af294bfd228a7["<dev string:x4a>"])) {
                            level.var_952af294bfd228a7["<dev string:x4a>"] = [];
                        }
                        level.var_952af294bfd228a7["<dev string:x4a>"][level.var_952af294bfd228a7["<dev string:x4a>"].size] = buddyspawnpoint;
                    }
                #/
                buddyspawnpoint.bucket = 3;
                buddyspawnpoint.usingfallback = usingfallback;
    ...
    if (bestspawnpoint.bucket > var_a22bad5fe6fbf86d) {
        /#
            if (getdvarint(@"hash_f3b498670cb45729", 0) != 0) {
                if (!isdefined(level.var_952af294bfd228a7["<dev string:x5d>"])) {
                    level.var_952af294bfd228a7["<dev string:x5d>"] = 0;
                }
                level.var_952af294bfd228a7["<dev string:x5d>"]++;
            }
        #/
        return undefined;
    }
...
function setactivespawnlogic(logictype, criticalfactortype, var_7d25b12691ee7cc1) {
    ...
    /#
        ...
        level.var_e9d31890c07ffffc = "<dev string:x75>" + newdata + "<dev string:x8a>";
        if (newdata) {
            foreach (set in level.spawnglobals.var_8c5be78fb206a3a6) {
                level.var_e9d31890c07ffffc += "<dev string:x8c>" + function_3c8848a3a11b2553(set.alias) + "<dev string:x8f>" + function_3c8848a3a11b2553(default_to(set.factorbundle, "<dev string:x9f>")) + "<dev string:xa9>" + function_3c8848a3a11b2553(default_to(set.criticalbundle, "<dev string:x9f>")) + "<dev string:xbe>";
            }
        }
        level.var_e9d31890c07ffffc += "<dev string:xc1>" + function_3c8848a3a11b2553(logictype) + "<dev string:xd6>" + default_to(criticalfactortype, "<dev string:x9f>") + "<dev string:xed>" + default_to(var_7d25b12691ee7cc1, "<dev string:x9f>") + "<dev string:xbe>";
        level.var_e9d31890c07ffffc += "<dev string:xff>" + istrue(level.teambased) + "<dev string:x8a>";
    ...
    if (shoulduseprecomputedlos() || generatinglosdata()) {
        ...
        /#
            if (spawnpoint.radiuspathnodes.size <= 0) {
                println("<dev string:x10c>" + spawnpoint.index + "<dev string:x11d>");
            }
        #/
    }
...
function spawnpointupdate() {
    ...
    /#
        setdevdvarifuninitialized(@"hash_3c88de2237eb6f6d", "<dev string:x163>");
    #/
...
function checkttlosdeverrors(viewer, spawnpoint) {
    /#
        ...
        if (!isdefined(viewer.spawnviewpathnodes) || viewer.spawnviewpathnodes.size == 0) {
            var_4f2dbc4104804300 = ter_op(isplayer(viewer), viewer.name, "<dev string:x165>");
            errormessage = "<dev string:x177>" + var_4f2dbc4104804300 + "<dev string:x185>";
        } else if (!isdefined(spawnpoint.radiuspathnodes) || spawnpoint.radiuspathnodes.size == 0) {
            errormessage = "<dev string:x19e>" + spawnpoint.origin;
        }
...
/#
    ...
    function updatedebughud() {
        ...
        while (true) {
            if (getdvarint(@"hash_2889bd5801e0ce5a", 0) != 0) {
                printtoscreen2d(1425, 875, "<dev string:x1d4>", (0, 1, 0), 2);
                if (shoulduseprecomputedlos()) {
                    printtoscreen2d(1425, 900, "<dev string:x1e9>", (0, 1, 0), 2);
                } else {
                    printtoscreen2d(1425, 900, "<dev string:x1f6>", (1, 0, 0), 2);
                }
            }
            foreach (player in level.players) {
                ...
                if (var_6fec14662c9a94b9 != var_6d453b4d602809fa) {
                    ...
                    if (var_6fec14662c9a94b9) {
                        iprintlnbold("<dev string:x206>" + player.origin);
                    }
                }
                if (var_6fec14662c9a94b9) {
                    printtoscreen2d(1425, 925, "<dev string:x22d>", (1, 0, 0), 2);
                }
            }
            waitframe();
        }
    ...
    function function_4c2c5ecea9fd1e73(spawnpoint) {
        println("<dev string:x243>" + spawnpoint.index + "<dev string:x26a>");
        println("<dev string:x284>" + spawnpoint.totalscore);
        println("<dev string:x28e>" + spawnpoint.totalpossiblescore);
        foreach (factorname, score in spawnpoint.debugscoredata) {
            println("<dev string:x298>" + factorname + "<dev string:x29a>" + score + "<dev string:x29e>" + spawnpoint.var_6531ac2ff1a37628[factorname] + "<dev string:x2a1>");
        }
    }
    ...
    function function_d344bbcacf5e4d96(factorref, factorstruct, spawncontext) {
        if (!isdefined(spawncontext.factorparams)) {
            assertmsg("<dev string:x2a3>" + function_3c8848a3a11b2553(factorref) + "<dev string:x2b2>");
        }
        outputparams = [];
        foreach (paramref in factorstruct.paramreflist) {
            ...
            assertex(isdefined(outputparams[paramref]), "<dev string:x2a3>" + factorref + "<dev string:x2f9>" + function_3c8848a3a11b2553(paramref) + "<dev string:x317>");
        }
    }
