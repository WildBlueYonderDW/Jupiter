...
/#
    ...
    function tweakart() {
        ...
        setdvar(@"hash_9cdc5d2317818d34", "<dev string:x1c>");
        setdvar(@"hash_eadbba3bd00553b7", "<dev string:x20>");
        setdvar(@"hash_9650c59d0e87c265", level.dof["<dev string:x22>"]["<dev string:x27>"]["<dev string:x2f>"]);
        setdvar(@"hash_8206b44203cf9e7c", level.dof["<dev string:x22>"]["<dev string:x27>"]["<dev string:x39>"]);
        setdvar(@"hash_caaebb3817b8d9fe", level.dof["<dev string:x22>"]["<dev string:x27>"]["<dev string:x41>"]);
        setdvar(@"hash_6f327889e80c9e6b", level.dof["<dev string:x22>"]["<dev string:x27>"]["<dev string:x4a>"]);
        setdvar(@"hash_ce181f72a71fa42a", level.dof["<dev string:x22>"]["<dev string:x27>"]["<dev string:x51>"]);
        setdvar(@"hash_d37a64659df9605f", level.dof["<dev string:x22>"]["<dev string:x27>"]["<dev string:x5a>"]);
        level.var_c3a123d45d467885 = 1;
        file = undefined;
        function_88bd2443c439782e();
        printed = 0;
        for (;;) {
            while (getdvarint(@"hash_41ecbb9b60af8bcf") == 0) {
                wait 0.05;
            }
            setsaveddvar(@"r_artUseTweaks", 1);
            if (!printed) {
                printed = 1;
                setdvar(@"hash_cd133f0daaf9b3e4", 0);
    ...
    function function_88bd2443c439782e() {
        nearstart = getdvarint(@"hash_9650c59d0e87c265");
        nearend = getdvarint(@"hash_8206b44203cf9e7c");
        nearblur = getdvarfloat(@"hash_ce181f72a71fa42a");
        farstart = getdvarint(@"hash_caaebb3817b8d9fe");
        farend = getdvarint(@"hash_6f327889e80c9e6b");
        farblur = getdvarfloat(@"hash_d37a64659df9605f");
    ...
    function function_934935ccb50fdc01() {
        ...
        if (nearstart >= nearend) {
            nearstart = nearend - 1;
            setdvar(@"hash_9650c59d0e87c265", nearstart);
        }
        if (nearend <= nearstart) {
            nearend = nearstart + 1;
            setdvar(@"hash_8206b44203cf9e7c", nearend);
        }
        if (farstart >= farend) {
            farstart = farend - 1;
            setdvar(@"hash_caaebb3817b8d9fe", farstart);
        }
        if (farend <= farstart) {
            farend = farstart + 1;
            setdvar(@"hash_6f327889e80c9e6b", farend);
        }
        if (farblur >= nearblur) {
            farblur = nearblur - 0.1;
            setdvar(@"hash_d37a64659df9605f", farblur);
        }
        if (farstart <= nearend) {
            farstart = nearend + 1;
            setdvar(@"hash_caaebb3817b8d9fe", farstart);
    ...
    function dumpsettings() {
        if (getdvar(@"hash_eadbba3bd00553b7") == "<dev string:x20>") {
            return;
        }
        var_8c7568850c091a01 = getdvar(@"hash_eadbba3bd00553b7") != "<dev string:x20>";
        setdvar(@"hash_eadbba3bd00553b7", "<dev string:x20>");
...
function dof_update() {
    ...
    while (true) {
        ...
        /#
            if (getdvarint(@"hash_41ecbb9b60af8bcf")) {
                continue;
            }
        #/
