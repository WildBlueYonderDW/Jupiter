...
function onnormaldeath(victim, attacker, lifeid, meansofdeath, objweapon, iskillstreakweapon) {
    ...
    if (game["launchChunkRuleSet"] == 0 || game["launchChunkRuleSet"] == 3) {
        if (isbot(attacker)) {
            if (attacker.kills % 4 == 0 || !isbot(victim)) {
                ...
                game["chunkBotsDifficulty"] = difficulties[difficultyindex];
                /#
                    iprintlnbold("loadoutPrimaryReticle" + attacker.name + "fraction" + difficulties[difficultyindex]);
                #/
            }
        }
        if (isbot(victim)) {
            if (victim.deaths % 2 == 0) {
                ...
                game["chunkBotsDifficulty"] = difficulties[difficultyindex];
                /#
                    iprintlnbold("loadoutPrimaryReticle" + victim.name + "offhand_wm_claymore_held" + difficulties[difficultyindex]);
                #/
            }
        }
    ...
    if (!isbot(victim)) {
        ...
        if (timeremaining > 35000) {
            ...
            game["chunkBotsDifficulty"] = difficulties[difficultyindex];
            /#
                iprintlnbold("MP_INGAME_ONLY/PICKUP_ATMINE" + difficulties[difficultyindex]);
            #/
        }
    }
}
