...
function spawnclient(revivespawn) {
    ...
    println("<dev string:x1c>" + self.name + "<dev string:x4d>" + self.class);
    if (!mayspawn()) {
        waitframe();
...
function waitandspawnclient(revivespawn) {
    ...
    self notify("attempted_spawn");
    println("<dev string:x56>" + self.name);
...
function spawnplayer(fauxspawn, revivespawn) {
    ...
    /#
        if (!getdvarint(@"hash_9c2d59c1962cac50")) {
            assert(level.teambased && (denysystemicteamchoice() || self.sessionteam == self.team) || !level.teambased && self.sessionteam == "<dev string:xa1>");
        }
    #/
    ...
    if (!fauxspawn) {
        ...
        if (istrue(self.liveragdoll)) {
            println("<dev string:xa6>" + gettime() + "<dev string:xa9>" + self.name + "<dev string:xac>" + self.health);
        }
...
function callback_playerdisconnect(reason) {
    ...
    if (isdefined(self.kills) && isdefined(self.deaths)) {
        if (getgametype() == "arena" && 3 > self.timeplayed["total"]) {
            ...
            println("<dev string:x112>" + damage);
            ...
            println("<dev string:x112>" + self.kills + "<dev string:x11a>" + self.deaths + "<dev string:x11e>" + kd);
    ...
    if (spawnpoints.size > 0) {
        ...
        if (isdefined(intermissionspawn)) {
            ...
            if (isdefined(cameraentity)) {
                ...
                assertex(isdefined(cameraentity), "Could not spawn script_model for cameraEntity");
            }
        } else {
            assertex(isdefined(intermissionspawn), "Closest intermission spawn was not found");
        }
    } else {
        assertex(spawnpoints.size, "NO mp_global_intermission SPAWNPOINTS IN MAP");
    }
...
function callback_playerconnect() {
    ...
    println("<dev string:x181>" + self.name + "<dev string:x18d>" + self.pers["<dev string:x1aa>"]);
    ...
    if (getdvarint(@"hash_4ba1427c86b79dc5") != 0) {
        ...
        if (matchmakinggame() && !denysystemicteamchoice() && !connectedbot) {
            ...
            #/
            /#
                println("<dev string:x1b3>" + gettime());
                println("<dev string:x1c7>" + getdvarint(@"hash_ff213c5a6a5e05e4", 0));
                println("<dev string:x1dd>" + istrue(level.multiteambased));
                println("<dev string:x1f4>" + istrue(level.teambased));
                println("<dev string:x206>" + self.sessionteam);
            #/
            assert(getdvarint(@"hash_ff213c5a6a5e05e4", 0) == 1 || istrue(level.multiteambased) || istrue(level.teambased) && isgameplayteam(self.sessionteam) || !istrue(level.teambased) && self.sessionteam == "none" || (scripts\cp_mp\utility\game_utility::isinfectedgametype() || getgametype() == "zm_rb") && self.sessionteam == "none");
        }
    }
    if (firstconnect) {
    ...
    /#
        if (getdvarint(@"hash_a91f736779a57bef")) {
            self.pers["<dev string:x219>"] = "<dev string:x21f>";
            self.class = "<dev string:x21f>";
            if (self.sessionteam != "<dev string:x220>" && self.sessionteam != "<dev string:x22a>") {
                self.pers["<dev string:x234>"] = self.sessionteam;
            }
            self.team = "<dev string:x239>";
        }
    #/
    ...
    if (!isdefined(self.pers["team"])) {
        ...
        if (self iscodcaster()) {
            ...
            /#
                if (getdvarint(@"hash_c90bde85e15978ef", 0) == 1 && !istrue(game["<dev string:x23e>"]) && !matchmakinggame() && !denysystemicteamchoice() && doesmodesupportplayerteamchoice()) {
...
function callback_playermigrated() {
    println("<dev string:x24c>" + self.name + "<dev string:x254>" + gettime());
...
function function_556228e50ff920d9() {
    println("<dev string:x29e>" + self.guid + "<dev string:x2cd>" + self getentitynumber() + "<dev string:x2cd>" + self.name + "<dev string:x2cf>");
...
function function_5c9544ef10cb9e0c(reason) {
    println("<dev string:x2d1>" + self.guid + "<dev string:x2cd>" + self getentitynumber() + "<dev string:x2cd>" + self.name + "<dev string:x2cd>" + reason + "<dev string:x2cf>");
...
function private function_ce68631570796a41() {
    ...
    defaultid = gametype == "resurgence" ? 28423 : 29387;
    var_b9c95d2f33c0d8b = getdvarint(@"hash_4c90ef27a8730b6", defaultid);
...
function private function_408b1b8364d0e722(var_b9c95d2f33c0d8b) {
    ...
    println("<dev string:x306>" + self.name + "<dev string:x33d>" + var_b9c95d2f33c0d8b + "<dev string:x347>" + self.var_d405f79d63134727);
}
...
function private function_fd6c077d98fa4be9(var_8ca33da989ab6666) {
    ...
    println("<dev string:x35a>" + self.name + "<dev string:x33d>" + var_8ca33da989ab6666 + "<dev string:x39b>" + var_31fe93819b433368);
    scripts\cp_mp\challenges::initgameflags(var_31fe93819b433368);
    if (scripts\cp_mp\challenges::function_cfe6dcedf7278543(ae_utility::function_e2ff8f4b4e94f723(#"hash_4a7f024e87253788", #"hash_cbddf4cbba197f26"))) {
        println("<dev string:x3a4>");
        scripts\cp_mp\challenges::function_5b58181f4c00ba56(namespace_649c2fab0fd72b8b::function_e2ff8f4b4e94f723(#"hash_4a7f024e87253788", #"hash_cbddf4cbba197f26"));
    }
}
