...
function playerwelcomesplashes(data) {
    ...
    wait 1;
    /#
        if (level.mapname == "<dev string:x25>" && getdvarint(@"hash_7713e10380b6cb97", 0) == 0) {
...
function plunderrankupdate() {
    ...
    while (true) {
        ...
        foreach (team in level.teamnamelist) {
            ...
            if (pockets < 0) {
                println("<dev string:x35>" + plundercount + "<dev string:x87>" + bankedplunder + "<dev string:x9c>" + pockets);
                pockets = 0;
            }
...
function playerdropplunderondeath(dropstruct, attacker) {
    ...
    if (istrue(self.isdisconnecting)) {
        ...
        if (istrue(self.wasexecuted)) {
            ...
            if (scripts\mp\gametypes\br_publicevents::ispubliceventoftypeactive(12)) {
                executionmod *= level.br_pe_data[12].var_ee5fb771f83add5e;
            }
    ...
    /#
        ...
        println("<dev string:xa7>" + var_47545befc5dedb97 + "<dev string:xc5>" + var_6aee9c9054f09ed5 + "<dev string:xce>" + var_b64f283113c99581 + "<dev string:xda>" + var_73dc67ec76d54557);
    #/
    return true;
}
...
function updatelootleadermarks(playerscores, playersorted, playerlookup, teamsorted) {
    ...
    for (i = 0; i < level.lootleadermarkcount; i++) {
        ...
        if (isdefined(level.lootleaders[i])) {
            ...
            var_1e502b02d01cf98d = scripts\mp\utility\teams::getfriendlyplayers(instance.targetplayer.team, 0);
...
function createquestobjicon(iconref, state, var_38116998df9814d4) {
    ...
    println(self.category + "<dev string:xe3>");
}
    ...
    function devthink() {
        ...
        while (true) {
            ...
            if (getdvarint(@"hash_73759683c80b1a3d", 0) != 0) {
                ...
                host = scripts\mp\gamelogic::gethostplayer();
                targetteam = "<dev string:xff>";
                foreach (player in level.players) {
                    if (isalive(player) && player != host && player.team != host.team) {
                        targetteam = player.team;
                        level.teamdata[targetteam]["<dev string:x100>"] = int(wintarget - 100);
                        break;
                    }
                }
                wait 15;
                level.teamdata[targetteam]["<dev string:x100>"] = int(wintarget);
            }
...
function printallteamsplundercount() {
    /#
        wait 1;
        wintarget = getwincost();
        teamsorted = tablesort(game["<dev string:x111>"], "<dev string:x120>");
        for (i = 0; i < teamsorted.size - 1; i++) {
            if (isdefined(level.teamdata[teamsorted[i]]["<dev string:x100>"])) {
                teamplunder = scripts\mp\gametypes\br_plunder::getteamplunder(teamsorted[i]) * 10;
                println("<dev string:x123>" + teamsorted[i] + "<dev string:x12a>" + teamplunder);
            }
        }
    #/
}
