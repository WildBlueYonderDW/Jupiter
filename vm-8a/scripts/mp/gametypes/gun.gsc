            ...
            if (dovalidate) {
                foreach (class in level.weaponcategories) {
                    foreach (weapon in class) {
                        ...
                        setdvar(@"hash_1ce2eb225cc3a6da", weapon["<dev string:x27>"]);
                        wait 1;
                        adddebugcommand("<dev string:x2e>" + "<dev string:x3e>" + weapon["<dev string:x27>"] + "<dev string:x4a>");
                        wait 2;
                    }
                }
            }
                ...
                foreach (player in level.players) {
                    ...
                    if (rankup) {
                        ...
                        if (newrank >= level.gun_guns.size - 1) {
                            player iprintlnbold("<dev string:x4c>");
                        } else {
                            ...
                            player thread scripts\mp\rank::scorepointspopup(points);
                            player thread scripts\mp\rank::scoreeventpopup(#"gained_gun_rank");
                            player playlocalsound("<dev string:x81>");
                        }
                        setdvar(@"hash_131d7c69641a09e6", 0);
                    } else {
                        newrank = player.gungamegunindex - 1;
                        if (newrank < 0) {
                            player iprintlnbold("<dev string:x96>");
                        } else {
                            player.gungamegunindex--;
                            updategun = 1;
                            player scripts\mp\gamescore::giveplayerscore(#"dropped_gun_rank", 1);
                            player incpersstat("<dev string:xcc>", 1);
                            player scripts\mp\persistence::statsetchild("<dev string:xd5>", "<dev string:xcc>", player.pers["<dev string:xcc>"]);
                            player playlocalsound("<dev string:xdb>");
                            if (isplayer(player)) {
                                player setextrascore1(player.pers["<dev string:xcc>"]);
                            }
                        }
                        setdvar(@"hash_4d0d7b8829a798cf", 0);
                    }
                    if (istrue(updategun)) {
                        player notify("<dev string:xf2>");
                        player thread givenextgun(0);
