...
function isteameliminated(team) {
    alivecount = scripts/mp/utility/teams::getteamdata(team, "aliveCount");
    if (alivecount > 0) {
        return 0;
    }
...
function function_a93242f2e58447b3() {
    ...
    while (true) {
        ...
        foreach (team, v in activeteams) {
            logstring("team:" + team);
            players = scripts/mp/utility/teams::getteamdata(team, "alivePlayers");
    ...
    foreach (player in humanplayers) {
        ...
        if (function_a420e0f6eca5f1c6()) {
            ...
            if (speed < getdvarint(@"hash_3d8ce2c99aa4db2b", 1) && var_ae5632495dfca986 > 0) {
                ...
                #/
                engageinfo.var_96be2fbd0ac6a848 = engageinfo.var_96be2fbd0ac6a848 + penalty * updateinterval;
            }
...
function function_719a071e30a49f1() {
    ...
    if (isdefined(level.br_level.br_circledelaytimes)) {
        for (i = 0; i < level.br_level.br_circledelaytimes.size; i++) {
            ...
            circleduration = circleduration + prewaittime;
        }
    }
    if (isdefined(level.br_level.br_circleclosetimes)) {
        for (i = 0; i < level.br_level.br_circleclosetimes.size; i++) {
            ...
            circleduration = circleduration + closetime;
        }
    }
        ...
        while (true) {
            if (getdvarint(@"hash_a2d26b607a32e157", 0) == 1) {
                        ...
                        if (!player isuibot()) {
                            ...
                            if (player function_d012729eac1e7053()) {
                                ...
                                if (player namespace_c4ef709c036dc4a8::issuspended()) {
                                    debugmsg = debugmsg + "<unknown string>";
                                }
                                if (isdefined(player.recyclereason)) {
                                    debugmsg = debugmsg + "<unknown string>" + player.recyclereason;
                                }
                            } else {
                                debugmsg = "<unknown string>";
                            }
                ...
                while (gettime() < var_89e85c0815c992d6) {
                    printtoscreen2d(500, 100, "<unknown string>" + var_3ce65cfec5ad6979 + "<unknown string>" + dormantbotnum + "<unknown string>" + uibotnum + "<unknown string>" + var_defe7b118dc3891d + "<unknown string>" + totalalivebotnum - dormantbotnum, (1, 0, 0), 2);
                    waitframe();
                }
                continue;
            }
                ...
                foreach (player in level.players) {
                    if (isvalidbot(player)) {
                        if (!scripts/mp/gamelogic::inprematch()) {
                            player namespace_c4ef709c036dc4a8::recycle("<unknown string>");
                            continue;
                        }
                        player namespace_c50b30148766aa59::recycle("<unknown string>");
                    }
                }
            }
            wait(1);
...
function function_da1172b593a41342() {
    /#
        ...
        while (true) {
            function_dc925a47fd3404ba(pauserecycle, @"hash_d95f6cfac74fa872", "<unknown string>");
            pauserecycle = getdvarint(@"hash_d95f6cfac74fa872", 0);
        ...
        while (true) {
            if (getdvarint(@"hash_52f1eee4900df5a4", 0) > 0) {
                ...
                if (gettime() < level.var_de1b021064d0f5bb.debugcountdown) {
                    msg = "<unknown string>" + (level.var_de1b021064d0f5bb.debugcountdown - gettime()) / 1000;
                }
                x = 600;
                y = 900;
                printtoscreen2d(x, 600, "<unknown string>" + getdvarint(@"hash_818c699a5caaee4f", 0) + "<unknown string>" + timeleftsec, (1, 0, 0), 1.5);
                ...
                printtoscreen2d(x + curtimesec / 2, y, "<unknown string>", (1, 1, 1), 1);
                for (i = 0; i < 10; i++) {
                ...
                for (i = 0; i < level.var_de1b021064d0f5bb.points.size; i++) {
                    ...
                    if (debugpoints[i] > 0) {
                        printtoscreen2d(x + point[0] / 2, y - debugpoints[i] * 2, "<unknown string>", (0, 1, 0), 1);
                    }
                }
                for (i = 0; i < circlestarttime.size; i++) {
                    printtoscreen2d(x + circlestarttime[i] / 2, y - 5, "<unknown string>", (1, 1, 1), 1);
                }
                waitframe();
                continue;
            }
