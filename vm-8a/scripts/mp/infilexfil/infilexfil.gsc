...
function infil_player_allow(allow, bypass) {
    ...
    println(self.name + "<dev string:x30>" + allow);
}
    ...
    /#
        if (self ishost() && getdvarint(@"hash_9d06e62e59d8f04a") == 1) {
            ...
            if (var_734654199c0e8131 > 0) {
                level thread [[ level.bot_funcs["<dev string:x5e>"] ]](var_734654199c0e8131, self.team);
            }
            other_team = getotherteam(self.team)[0];
            if (level.requiredplayercount[other_team] > 0) {
                level thread [[ level.bot_funcs["<dev string:x5e>"] ]](level.requiredplayercount[other_team], other_team);
            }
        }
    #/
}
...
function infil_player_rig(animname, var_486db5fa512a3b6b, var_40a4287d8d2e7ef9) {
    ...
    println("<dev string:x69>" + self.name + "<dev string:x93>" + animname + "<dev string:xa0>" + isdefined(self.player_rig));
    ...
    if (isdefined(self)) {
        println("<dev string:xa8>" + self.name + "<dev string:x93>" + animname + "<dev string:xa0>" + isdefined(self.player_rig));
        if (isdefined(var_40a4287d8d2e7ef9) && var_40a4287d8d2e7ef9) {
...
function heli_wait_node(nextpoint, lastpoint, var_cfa3aafb71b08d36) {
    ...
    /#
        if (isdefined(nextpoint.script_flag_set)) {
            self notify("<dev string:x10a>", nextpoint, nextpoint.script_flag_set);
        } else {
            self notify("<dev string:x10a>", nextpoint);
        }
    #/
...
/#
    ...
    function private function_8bce6c2b9dfce133() {
        while (true) {
            if (getdvar(@"hash_c022d48104ba5fe6", "<dev string:x11f>") != "<dev string:x11f>") {
                setdvar(@"hash_c022d48104ba5fe6", "<dev string:x11f>");
                infilname = getdvar(@"hash_c022d48104ba5fe6", "<dev string:x11f>");
                setdvar(@"hash_1f7e93431ff79c39", infilname);
                function_2445244fc2466b13("<dev string:x2b>");
            }
            wait 2;
        }
    }
    ...
    function private function_dea41b0700c91408() {
        while (true) {
            if (getdvar(@"hash_f6c08131f5dddab9", "<dev string:x11f>") != "<dev string:x11f>") {
                setdvar(@"hash_f6c08131f5dddab9", "<dev string:x11f>");
                infilname = getdvar(@"hash_f6c08131f5dddab9", "<dev string:x11f>");
                setdvar(@"hash_4c2539684478a68e", infilname);
                function_2445244fc2466b13("<dev string:x24>");
            }
            wait 2;
        }
    }
    ...
    function private infildebugmode() {
        level notify("<dev string:x120>");
        level endon("<dev string:x13e>");
        ...
        while (true) {
            ...
            } else if (hostplayer buttonpressed("<dev string:x165>")) {
                setdvar(@"hash_7194076ab4888f2b", 0);
                setdvar(@"hash_c653d32c30815d2b", 15);
                setdvar(@"hash_704afe6f6c582ee0", 15);
                iprintlnbold("<dev string:x16f>");
                wait 1;
                map_restart();
            } else if (hostplayer buttonpressed("<dev string:x185>")) {
                level.var_e6fbdd8917189dfb = !level.var_e6fbdd8917189dfb;
                if (level.var_e6fbdd8917189dfb == 1) {
                    iprintlnbold("<dev string:x190>");
                } else {
                    iprintlnbold("<dev string:x1a7>");
                }
                wait 1;
            } else if (hostplayer buttonpressed("<dev string:x1bf>")) {
                level.var_114dd6baa6068929 = !level.var_114dd6baa6068929;
                if (level.var_114dd6baa6068929 == 1) {
                    setdvar(@"hash_f711f210f6ffd355", 1);
                    iprintlnbold("<dev string:x1c7>");
                } else {
    ...
    function private function_dc2f945aa7bdd07a() {
        level notify("<dev string:x13e>");
        iprintlnbold("<dev string:x1fa>");
    }
