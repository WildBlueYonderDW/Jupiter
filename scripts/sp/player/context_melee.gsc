...
function main() {
    ...
    setdvarifuninitialized(@"hash_ac0f71bc1ff1dc42", 0);
...
function check_for_close_victim() {
    ...
    if (!isdefined(enemy)) {
        ...
        if (isdefined(enemy) && distance(level.player.origin, enemy.origin) < getdvarint(@"scripted_melee_range") - 8) {
...
function is_enemy_onscreen(enemy) {
    if (isdefined(level.player worldpointtoscreenpos(enemy gettagorigin("j_head"), getdvarint(@"cg_targetBaseFov")))) {
        return true;
    } else if (isdefined(level.player worldpointtoscreenpos(enemy gettagorigin("j_spinelower"), getdvarint(@"cg_targetBaseFov")))) {
        return true;
    } else if (isdefined(level.player worldpointtoscreenpos(enemy gettagorigin("tag_origin"), getdvarint(@"cg_targetBaseFov")))) {
        return true;
    }
    return false;
}
...
function check_stealth_kill_visible(enemies) {
    ...
    level endon("stop_context_melee");
    if (!getdvarint(@"hash_ac0f71bc1ff1dc42")) {
        return false;
    }
