...
function initgulag() {
    ...
    /#
        scripts\engine\utility::registersharedfunc("<dev string:x1c>", "<dev string:x22>", &runcommand);
    #/
}
...
function spawnlootweapons(lootstructs) {
    ...
    foreach (struct in lootstructs) {
        ...
        if (!isdefined(scriptablename)) {
            println("<dev string:x6c>" + struct.script_noteworthy + "<dev string:x7f>");
            continue;
        }
        ...
        println("<dev string:x9a>" + scriptablename + "<dev string:xc3>");
    }
        ...
        if (istrue(level.br_prematchstarted)) {
            ...
            /#
                if (getdvarint(@"hash_8c0da1f3585f4602", 0) > 0) {
                    print3d(level.gulag.arenas[0].center, "<dev string:xc5>");
                    waitframe();
                    continue;
                }
            #/
        ...
        if (arena.time <= 0) {
            ...
            if (istrue(dokill)) {
                foreach (player in arena.arenaplayers) {
                    ...
                    println("<dev string:x107>" + player.name + "<dev string:x13f>" + player getentitynumber());
...
function initplayerjail(var_5643932de03d949d, skipcinematic) {
    ...
    if (!istrue(self.jailed)) {
        println("<dev string:x14b>" + self.name + "<dev string:x16c>" + self.class);
...
function initplayerarena(arena, skipspawn, loadoutindex, debugreason) {
    ...
    println("<dev string:x175>" + self.name);
...
function gulagvictory(arena, winner, var_5b9a9b8346074883, var_5a5a690371917543, var_df2fbb13c226be75, var_fb551788da0fc531, sponsor, var_bf7470006675932e, freezeplayercontrols, var_565c98bdccd583fb) {
    ...
    println("<dev string:x196>" + winner.name + "<dev string:x1b2>" + reason);
...
function gulagplayerlost(arena, var_40d8983fc5e5c7a5, var_93592d4a5fd91ed1) {
    ...
    println("<dev string:x1be>" + losername);
    br_gulag_lose = function_17ee301cf0b5ba85("br_gulag_lose");
    br_gulag_lose_classic = function_17ee301cf0b5ba85("br_gulag_lose_classic");
    if (getdvarint(@"hash_61e9133b2e499a6c")) {
...
function function_6414fdc0080486d8(optparams) {
    println("<dev string:x1db>" + self.name);
...
function trygulagspawn() {
    println("<dev string:x200>" + self.name + "<dev string:x16c>" + self.class);
    ...
    /#
        if (getdvarint(@"hash_9144df4be848d0b7", 1)) {
            if (isdefined(self.values)) {
                allowmovementvalues = self.values["<dev string:x216>"];
                if (isdefined(allowmovementvalues)) {
                    foreach (value in allowmovementvalues) {
                        if (isdefined(value) && isdefined(value.value) && isdefined(value.str_id)) {
                            function_2e096d17b6286e29(self, "<dev string:x225>" + value.str_id, value.value);
                        }
                    }
                }
            }
...
function function_2e096d17b6286e29(player, debugreason, var_90e74e77d08d5afc) {
    /#
        if (getdvarint(@"hash_9144df4be848d0b7", 1)) {
            logstring("<dev string:x239>" + default_to(player.name, "<dev string:x263>") + "<dev string:x264>" + player.clientid + "<dev string:x26b>" + debugreason + "<dev string:x276>" + var_90e74e77d08d5afc);
        }
    #/
}
...
/#
    ...
    function runcommand(args) {
        ...
        if (command == "<dev string:x297>") {
            debugwarptojail();
            return;
        }
        if (command == "<dev string:x2a1>") {
            debugwarptojailsingle();
            return;
        }
        if (command == "<dev string:x2b2>") {
            debugresetguns();
            return;
        }
        if (command == "<dev string:x2bd>") {
            debugfakeduel();
            return;
        }
        if (command == "<dev string:x2c7>") {
            debugfakewinner();
            return;
        }
        if (command == "<dev string:x2d3>") {
            debugshutdown();
            return;
        }
        if (command == "<dev string:x2dc>") {
            function_389315f9f33826c3();
            setdevdvar(@"hash_57dfa3352864c15b", 0);
            return;
        }
        if (command == "<dev string:x2e1>") {
            foreach (arena in level.gulag.arenas) {
                updateplayeromnvarsallmatches(arena);
            }
        ...
        if (command == "<dev string:x2ea>") {
            thread function_e15c373c57d02f90(args);
            return;
        }
        if (command == "<dev string:x2f7>") {
            thread debugmatches(args);
            return;
        }
        if (command == "<dev string:x305>") {
            host = scripts\mp\gamelogic::gethostplayer();
            host setclientomnvar("<dev string:x315>", 1);
            host playcinematicforplayer("<dev string:x32e>");
            setdvar(@"hash_da200314926d462c", 0);
            return;
        }
        if (command == "<dev string:x345>") {
            host = scripts\mp\gamelogic::gethostplayer();
            host setclientomnvar("<dev string:x315>", 0);
            host stopcinematicforplayer();
            setdvar(@"hash_76bb2f96ffd637c8", 0);
            return;
        }
        if (command == "<dev string:x354>") {
            level.gulag.maxuses = 1;
            foreach (player in level.players) {
                player.gulaguses = 1;
    ...
    function function_70deb58a1637a9d(arenanum) {
        ...
        if (!istrue(arena.isjailbreak)) {
            matchdebug.arenastring setdevtext("<dev string:x374>" + arenanum + "<dev string:x37b>");
        } else {
            matchdebug.arenastring setdevtext("<dev string:x374>" + arenanum + "<dev string:x384>");
        }
        if (!istrue(arena.isjailbreak)) {
            if (arena.arenaplayers.size > 0) {
    ...
    function function_b9192ebe3ccbd06e() {
        ...
        matchdebug = level.gulag.matchdebug;
        matchdebug.arenastring = createserverfontstring("<dev string:x38f>", var_a6fe8f42e76395c);
        matchdebug.arenastring scripts\mp\hud_util::setpoint("<dev string:x397>", "<dev string:x397>", var_a384829c3b11db56, var_ecdabfb8180b91b5 + yline * var_a7d9b923f6845a7a);
        matchdebug.arenastring setdevtext("<dev string:x3a0>");
        matchdebug.arenastring.color = (1, 1, 0);
        matchdebug.arenastring.alpha = 1;
        yline++;
        matchdebug.currentstring = createserverfontstring("<dev string:x38f>", var_a6fe8f42e76395c);
        matchdebug.currentstring scripts\mp\hud_util::setpoint("<dev string:x397>", "<dev string:x397>", var_a384829c3b11db56, var_ecdabfb8180b91b5 + yline * var_a7d9b923f6845a7a);
        matchdebug.currentstring setdevtext("<dev string:x3b0>");
        matchdebug.currentstring.color = (0, 1, 0);
        matchdebug.currentstring.alpha = 1;
        yline++;
        matchdebug.playerstrings = [];
        for (playerindex = 0; playerindex < 2; playerindex++) {
            matchdebug.playerstrings[playerindex] = createserverfontstring("<dev string:x38f>", var_a6fe8f42e76395c);
            matchdebug.playerstrings[playerindex] scripts\mp\hud_util::setpoint("<dev string:x397>", "<dev string:x397>", var_a384829c3b11db56, var_ecdabfb8180b91b5 + yline * var_a7d9b923f6845a7a);
            matchdebug.playerstrings[playerindex] setdevtext("<dev string:x3be>" + playerindex);
            matchdebug.playerstrings[playerindex].alpha = 1;
            yline++;
        }
        matchdebug.matches = [];
        for (matchindex = 0; matchindex < var_896781a8a9b653bf; matchindex++) {
            ...
            match = matchdebug.matches[matchindex];
            match.debugstring = createserverfontstring("<dev string:x38f>", var_a6fe8f42e76395c);
            match.debugstring scripts\mp\hud_util::setpoint("<dev string:x397>", "<dev string:x397>", var_a384829c3b11db56, var_ecdabfb8180b91b5 + yline * var_a7d9b923f6845a7a);
            match.debugstring setdevtext("<dev string:x3c6>" + matchindex + 1);
            match.debugstring.color = (0, 1, 0);
            match.debugstring.alpha = 1;
            yline++;
            match.playerstrings = [];
            for (playerindex = 0; playerindex < 2; playerindex++) {
                match.playerstrings[playerindex] = createserverfontstring("<dev string:x38f>", var_a6fe8f42e76395c);
                match.playerstrings[playerindex] scripts\mp\hud_util::setpoint("<dev string:x397>", "<dev string:x397>", var_a384829c3b11db56, var_ecdabfb8180b91b5 + yline * var_a7d9b923f6845a7a);
                match.playerstrings[playerindex].alpha = 1;
                yline++;
            }
        }
    ...
    function createserverfontstring(font, fontscale) {
        fontelem = newhudelem();
        fontelem.elemtype = "<dev string:x3cd>";
        fontelem.font = font;
    ...
    function function_389315f9f33826c3() {
        foreach (player in level.players) {
            player scripts\mp\equipment::giveequipment("<dev string:x3d2>", "<dev string:x3dd>");
        }
    }
    ...
    function debugfakewinner() {
        ...
        if (isdefined(host)) {
            ...
            if (isalive(host) && !host scripts\mp\gametypes\br_public::isplayerinorgoingtogulag()) {
                host thread scripts\mp\gametypes\br_gulag::playerpulloutofgulagwin(host, "<dev string:x3e5>");
                return;
            }
            host thread scripts\mp\gametypes\br_vip_quest::vip_respawnplayer(host, host, "<dev string:x3e5>");
        }
    }
    ...
    function function_e6bd56ae6d7e0e84() {
        ...
        if (isdefined(host) && !isalive(host)) {
            teammates = level.teamdata[host.team]["<dev string:x3eb>"];
            if (teammates.size > 0) {
                teammate = teammates[0];
                teammate scripts\mp\gametypes\br_pickups::addrespawntoken(1);
                host playergulagautowin("<dev string:x3f8>", teammate, 1, 1);
            }
        }
    }
    ...
    function debugfakeduel() {
        ...
        foreach (entry in level.teamnamelist) {
            foreach (teamplayer in getteamdata(entry, "<dev string:x409>")) {
    ...
    function debugshutdown(type) {
        shutdowngulag("<dev string:x3e5>", 0);
    }
    ...
    function debugwarptojailsingle() {
        ...
        if (isdefined(trace["<dev string:x411>"]) && isplayer(trace["<dev string:x411>"]) && isreallyalive(trace["<dev string:x411>"])) {
            player = trace["<dev string:x411>"];
            player initplayerjail();
        }
    }
    ...
    function debugmatches(args) {
        ...
        if (!istrue(level.gulag.var_e15c373c57d02f90)) {
            ...
            level notify("<dev string:x418>");
            level.gulag.var_e15c373c57d02f90 = undefined;
            return;
        }
        level endon("<dev string:x418>");
        while (true) {
            foreach (arenaindex, arena in level.gulag.arenas) {
                foreach (index, player in arena.arenaplayers) {
                    print3d(player.origin + (0, 0, 50), "<dev string:x263>" + index, (0, 1, 0));
                }
                foreach (index, player in arena.jailedplayers) {
                    print3d(player.origin + (0, 0, 50), "<dev string:x263>" + index);
                }
                print3d(arena.center, "<dev string:x263>" + arenaindex, (1, 1, 0), 1, 2);
            }
