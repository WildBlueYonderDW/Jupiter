...
function function_b23eb6ebe631ccdc(playereyeorigin, playerviewangles) {
    ...
    if (getdvarint(@"hash_ad68aff4b57edc12")) {
        /#
            printtoscreen2d(10, 800 + 90, "<unknown string>" + groupscores[beststruct.groupnum], (1, 1, 1), 1.5);
        #/
    }
...
function function_ac3ec9d38fd2434(playereyepos, playerangles) {
    ...
    foreach (ent in ents) {
        ...
        if (dot > level.dyndof.cosfov) {
            ...
            if (getdvarint(@"hash_ad68aff4b57edc12")) {
                /#
                    print3d(entorigin + (0, 0, -5), "<unknown string>" + score + "<unknown string>" + dot + "<unknown string>", (1, 1, 1), 0.8, 0.2);
                #/
            }
        }
        if (ent.dofscore == 0) {
        ...
        if (ent.dofdist < 2000) {
            ...
            if (getdvarint(@"hash_ad68aff4b57edc12")) {
                /#
                    print3d(entorigin + (0, 0, -10), "<unknown string>" + distscore + "<unknown string>" + ent.dofscore + "<unknown string>", (1, 1, 1), 0.8, 0.2);
                #/
            }
        } else {
            ...
            if (getdvarint(@"hash_ad68aff4b57edc12")) {
                /#
                    print3d(entorigin + (0, 0, -10), "<unknown string>" + distscale + "<unknown string>" + ent.dofscore + "<unknown string>", (1, 1, 1), 0.8, 0.2);
                #/
            }
        }
        if (ent.dofscore > 0) {
            trace = scripts/engine/trace::ray_trace(playereyepos, entorigin, [ent, level.dyndof.player], level.dyndof.contents, 1);
            ...
            if (getdvarint(@"hash_ad68aff4b57edc12")) {
                /#
                    print3d(entorigin + (0, 0, -15), "<unknown string>" + scale + "<unknown string>" + ent.dofscore + "<unknown string>", (1, 1, 1), 0.8, 0.2);
                #/
            }
        }
        ...
        if (var_6e2b9d3e75679206 && (isai(ent) || isdefined(ent.var_5cb5bea2f8851b21)) && ent.dofscore > 0 && ent.dofdot > 0.99) {
            foreach (directent in directents) {
                ...
                if (directent.dofdist < ent.dofdist && function_7d02f8d29555766e(playereyepos, ent.doftraceorigin, directent.origin)) {
                    ...
                    if (getdvarint(@"hash_ad68aff4b57edc12")) {
                        /#
                            print3d(ent.doftraceorigin + (0, 0, -20), "<unknown string>", (1, 1, 1), 0.8, 0.2);
                        #/
                        /#
    ...
    if (isdefined(bestscoreent)) {
        ...
        if (getdvarint(@"hash_ad68aff4b57edc12")) {
            /#
                printtoscreen2d(10, 800 + 60, "<unknown string>" + struct.dofscore, (1, 1, 1), 1.5);
            #/
        }
    }
    return struct;
...
function function_77d9a6389a3305ee(playereyeorigin, playerangles, var_f85ab554f9f088c8, pos) {
    ...
    if (!isdefined(trace) || trace.size == 0) {
        trace = scripts/engine/trace::internal_pack_default_trace(pos);
    } else {
        trace = trace[0];
    }
    return trace;
...
function function_abfbc195c3aac94c(playereyeorigin, highestgroup) {
    ...
    /#
        while (getdvarint(@"hash_ad68aff4b57edc12")) {
            closestpos = level.dyndof.var_1fef56bc059b6f9a.trace["<unknown string>"];
...
function dyndof_trace_target(pos) {
    ...
    if (isdefined(trace) && isdefined(trace[0]) && isdefined(trace[0]["position"])) {
        if (distance(trace[0]["position"], pos) < 8) {
            /#
                if (getdvarint(@"hash_93ca035fa3964d3d", 0)) {
                    print3d(pos_print, "<unknown string>", (1, 0, 1), 1, 0.1, 1, 1);
                    playereye = dyndof_getplayerorigin();
            ...
            /#
                if (getdvarint(@"hash_93ca035fa3964d3d", 0)) {
                    print3d(pos_print, "<unknown string>", (1, 0, 0), 1, 0.1, 1, 1);
                    playereye = dyndof_getplayerorigin();
    ...
    /#
        if (getdvarint(@"hash_93ca035fa3964d3d", 0)) {
            print3d(pos_print, "<unknown string>", (0, 1, 0), 1, 0.1, 1, 1);
            playereye = dyndof_getplayerorigin();
    ...
    /#
        if (getdvarint(@"hash_93ca035fa3964d3d", 0)) {
            while (true) {
                ...
                line(closesttrace["<unknown string>"], dyndof_getplayerorigin() + (0, 0, -1), (1, 1, 0));
                value = math::normalize_value(0, 3000, distance(dyndof_getplayerorigin(), closesttrace["<unknown string>"]));
                value = value * 32;
                sphere(closesttrace["<unknown string>"], value, (1, 1, 0));
                print3d(closesttrace["<unknown string>"], closesttrace["<unknown string>"], (1, 1, 1), 1, 0.1, 1);
                if (isdefined(closesttrace["<unknown string>"]) && isdefined(closesttrace["<unknown string>"].model)) {
                    model = closesttrace["<unknown string>"].model;
                    ...
                    print3d(closesttrace["<unknown string>"] + (0, 0, -1.5), model, (1, 1, 1), 1, 0.1, 1);
                }
                waitframe();
            }
        }
