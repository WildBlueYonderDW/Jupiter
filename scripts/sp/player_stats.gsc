...
// Size: 0x28f
function register_kill(var_c991a0bed398ca91, cause, weaponname, var_87f5148b60eed56d) {
    ...
    if (isdefined(level.skip_pilot_kill_count) && isdefined(var_c991a0bed398ca91.drivingvehicle) && var_c991a0bed398ca91.drivingvehicle) {
        return;
    }
    player.stats["kills"]++;
    if (isdefined(var_c991a0bed398ca91)) {
        if (var_c991a0bed398ca91 was_headshot()) {
            player.stats["headshots"]++;
        }
        if (isdefined(var_c991a0bed398ca91.juggernaut)) {
            player.stats["kills_juggernaut"]++;
        }
        if (isdefined(var_c991a0bed398ca91.issentrygun)) {
            player.stats["kills_sentry"]++;
        }
        if (var_c991a0bed398ca91.code_classname == "script_vehicle") {
            player.stats["kills_vehicle"]++;
            if (isdefined(var_c991a0bed398ca91.riders)) {
                foreach (rider in var_c991a0bed398ca91.riders) {
                    if (isdefined(rider)) {
                        player register_kill(rider, cause, weaponname, var_87f5148b60eed56d);
                    }
                }
            }
        }
