...
function function_d5250ff459d106fe() {
    /#
        ...
        foreach (vehicle in leveldata.debuginstances) {
            ...
            tfl = vehicle.origin;
            tfl = tfl + anglestoup(vehicle.angles) * top;
            tfl = tfl + anglestoforward(vehicle.angles) * front;
            tfl = tfl + anglestoright(vehicle.angles) * -1 * left;
            tfr = vehicle.origin;
            tfr = tfr + anglestoup(vehicle.angles) * top;
            tfr = tfr + anglestoforward(vehicle.angles) * front;
            tfr = tfr + anglestoright(vehicle.angles) * right;
            tbl = vehicle.origin;
            tbl = tbl + anglestoup(vehicle.angles) * top;
            tbl = tbl + anglestoforward(vehicle.angles) * -1 * back;
            tbl = tbl + anglestoright(vehicle.angles) * -1 * left;
            tbr = vehicle.origin;
            tbr = tbr + anglestoup(vehicle.angles) * top;
            tbr = tbr + anglestoforward(vehicle.angles) * -1 * back;
            tbr = tbr + anglestoright(vehicle.angles) * right;
            bfl = vehicle.origin;
            bfl = bfl + anglestoup(vehicle.angles) * -1 * bottom;
            bfl = bfl + anglestoforward(vehicle.angles) * front;
            bfl = bfl + anglestoright(vehicle.angles) * -1 * left;
            bfr = vehicle.origin;
            bfr = bfr + anglestoup(vehicle.angles) * -1 * bottom;
            bfr = bfr + anglestoforward(vehicle.angles) * front;
            bfr = bfr + anglestoright(vehicle.angles) * right;
            bbl = vehicle.origin;
            bbl = bbl + anglestoup(vehicle.angles) * -1 * bottom;
            bbl = bbl + anglestoforward(vehicle.angles) * -1 * back;
            bbl = bbl + anglestoright(vehicle.angles) * -1 * left;
            bbr = vehicle.origin;
            bbr = bbr + anglestoup(vehicle.angles) * -1 * bottom;
            bbr = bbr + anglestoforward(vehicle.angles) * -1 * back;
            bbr = bbr + anglestoright(vehicle.angles) * right;
