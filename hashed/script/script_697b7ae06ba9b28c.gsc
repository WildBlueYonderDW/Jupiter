...
function ondisconnect(player, aeeventname) {
    ...
    disconnectreason = 0;
    playercounts = scripts/mp/teams::countplayers();
...
function function_9502b0bb442142b6(string) {
    /#
        ...
        logstring("<unknown string>" + scripts/mp/gamelogic::getstreamedinplayercount());
...
function function_cc11b7aeffd6b54a(srtype, optionalarg) {
    ...
    bitspacked = 0;
    packedvalue = packedvalue + ((int(abs(player.var_8eda8a21ec98626f)) & maxvalue) << bitspacked);
    bitspacked = bitspacked + 10;
    packedvalue = packedvalue + ((int(var_f6adbcd287921ab6) & maxvalue) << bitspacked);
    bitspacked = bitspacked + 1;
    packedvalue = packedvalue + ((int(abs(var_36f3228e1d630f1b)) & maxvalue) << bitspacked);
    bitspacked = bitspacked + 8;
    packedvalue = packedvalue + ((int(var_1343ab307ce38f3b) & maxvalue) << bitspacked);
    bitspacked = bitspacked + 1;
    packedvalue = packedvalue + ((int(level.var_e461f6b82b97cc19.var_ac0560ffa2ea0ad) & maxvalue) << bitspacked);
    bitspacked = bitspacked + 4;
    packedvalue = packedvalue + ((int(srtype) & maxvalue) << bitspacked);
...
function function_5b4b593ace7e0ac1(squadsremaining, victim) {
    if (isdefined(victim) && scripts/mp/gametypes/br::function_99e3948bfd8a99b8(victim.team) == 0) {
        squadsremaining = squadsremaining + 1;
    }
