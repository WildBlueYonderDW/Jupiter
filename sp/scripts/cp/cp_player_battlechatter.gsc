...
// Namespace namespace_8b0201354228db72 / scripts\cp\cp_player_battlechatter
// Params 0, eflags: 0x0
...
function hurtbadlywait() {
    ...
    if (isdefined(stance) && stance == "prone" && !namespace_d4aab8c9cb8ecb14::isinlaststand(self)) {
...
function addrecentattacker(attacker) {
    ...
    if (self.recentattackers.size > 1) {
        level thread trysaylocalsound(self, #"hash_14f86af7d651b93f");
    }
}
    ...
    while (true) {
        ...
        foreach (player in nearplayers) {
            ...
            if (isdefined(grenadedistancesquared) && grenadedistancesquared < 384) {
                ...
                switch (var_49e6ef3edadd524e) {
                case #"hash_7c03088193266bc4":
                    level thread trysaylocalsound(player, #"hash_ed1c8a7166b894af");
                    break;
                case #"hash_8670ac083666f3a4":
                    level thread trysaylocalsound(player, #"hash_65f21a33fc38c333");
                    break;
                case #"hash_e042760d17966848":
                    level thread trysaylocalsound(player, #"hash_5b8de9f0fcacc509");
                    break;
                case #"hash_e1f6f84e4cd950eb":
                    level thread trysaylocalsound(player, #"hash_83c99ca7c7c2f310");
                    break;
                case #"hash_5d11ac1131cddab1":
                    level thread trysaylocalsound(player, #"hash_d894310067f8aa6f");
                    break;
                case #"hash_c9a436974fe60919":
                    level thread trysaylocalsound(player, #"hash_86cf3380b9ab1d0f");
                    break;
                case #"hash_fa1e80f6bd5b8e72":
                    level thread trysaylocalsound(player, #"hash_9cb65d1bb6a45ff0");
                    break;
                case #"hash_13492848c09971da":
                    level thread trysaylocalsound(player, #"hash_83c99ca7c7c2f310");
                    break;
                case #"hash_a0319e349a692b8f":
                    level thread trysaylocalsound(player, #"hash_5a5ffe8ec30ca285");
                    break;
                case #"hash_8059d93d3568c07":
                    level thread trysaylocalsound(player, #"hash_cd28251747015eb8");
                    break;
                case #"hash_11a03a12f93f62b2":
                    level thread trysaylocalsound(player, #"hash_9e6f5a8035522781");
                    break;
                case #"hash_d9108a0595d7d0b9":
                    level thread trysaylocalsound(player, #"hash_6d48af2a13824eba");
                    break;
                default:
                    if (weaponclass(self.weapon_name) == "rocketlauncher") {
...
function killstreakdestroyed(streakname) {
    ...
    case #"hash_4ad475e6e15635bd":
        ...
        break;
    case #"hash_e477c24ee18bb2f6":
        level thread trysaylocalsound(self, #"hash_1056518f46efab4f");
        break;
    case #"hash_4d5aa76e8707f47b":
        ...
        foreach (player in players) {
            ...
            if (self.operatorcustomization.voice != player.operatorcustomization.voice && distancesquared(pos, player.origin) <= 262144 && !isspeakerinrange(player)) {
                ...
                if (soundexists(var_a7413bbf3983933c) && cointoss()) {
                    var_9373e8da64267c1f = var_a7413bbf3983933c;
                } else {
                    var_9373e8da64267c1f = undefined;
                }
                if (isdefined(var_9373e8da64267c1f)) {
                    player thread dosound(var_9373e8da64267c1f, 0, 1);
                }
                break;
            }
        }
...
function testweaponfiredtolisteners(attacker, objweapon) {
    ...
    foreach (player in var_378d3b538b839d2c) {
        ...
        if (namespace_a0628d6954815ef8::iscacprimaryweapon(objweapon.basename) || namespace_a0628d6954815ef8::iscacsecondaryweapon(objweapon.basename)) {
            intensity = 0;
...
function onsixfriendlytracking() {
    ...
    wait 25;
    while (true) {
        if (!saidtoorecently(#"hash_213c1cf2e5154626")) {
...
function handleHelloDialog() {
    ...
    while (true) {
        ...
        if (gettime() - self.var_d94e306d9220520b < 1000) {
            var_30c7f4fcb853dbb2 = 0;
            foreach (player in level.players) {
                if (self == player) {
                    continue;
                }
                if (var_30c7f4fcb853dbb2) {
                    continue;
                }
                ...
                var_30c7f4fcb853dbb2 = 1;
            }
            if (var_30c7f4fcb853dbb2) {
                level thread trysaylocalsound(self, #"hash_53a437c67f7aaa4a", undefined, 0.6);
                wait 10;
            }
        }
