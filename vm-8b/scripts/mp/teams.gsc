...
function init() {
    ...
    activeteamcount = min(activeteamcount, getdvarint(hashcat(@"hash_d98c82b5a26dc973", getgametype(), "_teamcount"), -1));
    ...
    if (isdefined(self.operatorcustomization) && isdefined(self.operatorcustomization.operatorref)) {
        ...
        if (scripts\cp_mp\utility\game_utility::isinfectedgametype() && team == "axis") {
            ...
            if (!namespace_465d40bb08a5337a::function_54240a5fc41f6590(operatorref)) {
                println("<dev string:x20d>" + operatorref + "<dev string:x22a>");
                operatorref = namespace_465d40bb08a5337a::function_9242a55834c7ead8(var_e9a5f25420e74f74);
            }
        }
    }
...
function getoperatorcustomization() {
    ...
    if (istrue(game["isLaunchChunk"])) {
        ...
        if (body == "" || head == "") {
            assertmsg("<dev string:x25d>" + head + "<dev string:x29f>" + body);
...
/#
    ...
    function function_5933f55517e82c57() {
        ...
        level endon("<dev string:x2ac>");
        for (;;) {
            foreach (player in level.players) {
                player getplayerdata("<dev string:x2c0>", "<dev string:x2ca>", "<dev string:x2db>", "<dev string:x2ea>");
            }
            wait 0.05;
        }
    }
...
function private function_8075b157830461e0() {
    if (getdvarint(@"hash_11e4f85d7c35874e", 0) && isdefined(level.players) && level.players.size > 0) {
        ...
        if (var_5af9fdb2229b2cec > level.framedurationseconds) {
            ...
            println("<dev string:x2f0>" + 1 + "<dev string:x31e>" + var_5af9fdb2229b2cec + "<dev string:x33b>" + level.players.size + "<dev string:x34e>" + gettime() + "<dev string:x355>");
            return true;
        }
        if (isdefined(level.var_fa5689aa69fdbd8a)) {
        ...
        println("<dev string:x3f9>" + var_363dfdcaf05d82e0 + "<dev string:x436>" + level.framedurationseconds + "<dev string:x468>" + level.players.size);
    }
    return false;
}
...
function checkforafk(var_6d2f7985df18659a, var_55d48eee7e287cda, var_bdbf4608f99aedb3) {
    ...
    var_2a8062c05d69fcf1 = default_to(scripts\mp\utility\stats::getpersstat("distTrackingPassedOrResetTime"), 0);
    var_913292631f9d9e2a = function_19300abfbb10928b() - var_2a8062c05d69fcf1;
    var_9af0bd77617d4e0d = 60;
    var_f6dba1911b90d82c = nokillsorassists && var_913292631f9d9e2a > 120;
    var_d7c6b91d300a1023 = default_to(self.var_3681793bfd7b68e, 0) > gettime();
    var_b79528872f8bdde = istrue(scripts\mp\utility\stats::getpersstat("distTrackingPassed")) || var_d7c6b91d300a1023;
    if (scripts\cp_mp\utility\game_utility::isbrstylegametype()) {
    ...
    self.afkstate = 0;
    kickreason = "default";
    if (self function_f771642d3445111f() && self.sessionstate == "playing" && nokillsorassists && !var_b79528872f8bdde) {
        self.afkstate = function_7b76ffd725358204(var_913292631f9d9e2a, var_9af0bd77617d4e0d, var_e817fdf4412ba4ec);
        kickreason = "checkForAFK() TRUE, IsCommandInputStale() - " + self.name + "timeSinceLastPassedDistCheck (" + var_913292631f9d9e2a + ") > afkDistTimeThreshold(" + var_9af0bd77617d4e0d + "), lastDistTrackingPassedTime: " + var_2a8062c05d69fcf1;
    }
    if (self function_9eff7048a5747cc5()) {
        self.afkstate = 2;
        kickreason = "checkForAFK() TRUE, IsCommandInputRepetitive() - " + self.name + "timeSinceLastPassedDistCheck ( " + var_913292631f9d9e2a + " ), lastDistTrackingPassedTime: " + var_2a8062c05d69fcf1;
    }
    ...
    if (isroundbased() && !var_bad61657da03a98d) {
        ...
        if (checkdistancetravelled && !var_b79528872f8bdde) {
            ...
            if (scripts\cp_mp\utility\game_utility::isinfectedgametype()) {
                ...
                if (isdefined(scripts\mp\utility\stats::getpersstat("lastTouchTime"))) {
                    function_7b76ffd725358204(var_913292631f9d9e2a - scripts\mp\utility\stats::getpersstat("lastTouchTime"), 60, var_e817fdf4412ba4ec);
                }
                var_c8238f8ea16268f7 = 2;
                if (isdefined(scripts\mp\utility\stats::getpersstat("lastRamTime"))) {
                    var_c8238f8ea16268f7 = function_7b76ffd725358204(var_913292631f9d9e2a - scripts\mp\utility\stats::getpersstat("lastRamTime"), 60, var_e817fdf4412ba4ec);
                }
                ...
                self.afkstate = max(self.afkstate, var_58dfeb144008c413);
                kickreason = "checkForAFK() TRUE, " + self.name + "timeSinceLastPassedDistCheck ( " + var_913292631f9d9e2a + " ), disTrackingPassed: " + var_b79528872f8bdde;
            }
        }
            ...
            if (nokillsorassists) {
                ...
                if (!var_b79528872f8bdde) {
                    if (scripts\cp_mp\utility\game_utility::isinfectedgametype()) {
                        ...
                        self.afkstate = max(self.afkstate, var_58dfeb144008c413);
                        kickreason = "checkForAFK() TRUE, Non Round Based " + self.name + "( noKillsOrAssists(" + nokillsorassists + ") && !recentDamageState(" + recentdamagestate + ") && timeSinceLastPassedDistCheck(" + var_913292631f9d9e2a + ") >= afkDistTimeThreshold(" + var_9af0bd77617d4e0d + "), lastDistTrackingPassedTime: " + var_2a8062c05d69fcf1;
                    }
                }
            }
        }
...
function getjointeampermissions(team) {
    ...
    println("<dev string:x484>" + team + "<dev string:x4af>");
    if (self ishost()) {
        wait 1.5;
    }
...
function printcustomization() {
    ...
    /#
        print3d(self.origin + (0, 0, 40), "<dev string:x515>" + var_1ceef035c3f3046d[0], (1, 1, 1), 1, 0.5);
    #/
    /#
        print3d(self.origin + (0, 0, 60), "<dev string:x51f>" + var_1ceef035c3f3046d[1], (1, 1, 1), 1, 0.5);
    #/
}
    ...
    while (true) {
        if (isdefined(level.teamdata)) {
            ...
            foreach (team in level.teamnamelist) {
                linecount = 1;
                /#
                    printtoscreen2d(startx, starty + stepy * linecount, "<dev string:x529>" + team, (1, 1, 1), 1);
                #/
                linecount++;
