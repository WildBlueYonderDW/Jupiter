...
/#
    ...
    function function_44f4ade288270b27(var_d8695b9748c307be, flightdir) {
        ...
        range = 300;
        sizemultiplier = 1;
        if (getmatchrulesdata("<dev string:x63>", "<dev string:x70>")) {
            sizemultiplier = getmatchrulesdata("<dev string:x63>", "<dev string:x86>");
            range = 300 * sizemultiplier;
        }
        range += getmatchrulesdata("<dev string:x63>", "<dev string:xa2>");
        var_c86bc952d8cc7c15 = function_2de43934d42594e(var_d8695b9748c307be, flightdir, sizemultiplier);
        foreach (circleorigin in var_c86bc952d8cc7c15) {
            currentindex = self.var_bd0aa6c111497ed3.size;
            self.var_bd0aa6c111497ed3[currentindex] = spawnstruct();
            self.var_bd0aa6c111497ed3[currentindex] function_6b6b6273f8180522("<dev string:xc2>", circleorigin, range);
        }
    }
...
/#
    ...
    function function_9018ff93eb75a769(bombexplodedelay, var_f9f79f4923d00e6e) {
        ...
        level endon("<dev string:xe1>");
        self notify("<dev string:xec>");
        self endon("<dev string:xec>");
        self notifyonplayercommand("<dev string:x107>", "<dev string:x11f>");
        self notifyonplayercommand("<dev string:x107>", "<dev string:x12a>");
        bombweapon = makeweapon("<dev string:x134>");
        while (true) {
            airstrikedebugon = getdvarint(@"hash_920f0586653a26d4", 0);
            if (!istrue(airstrikedebugon)) {
                waitframe();
            ...
            self waittill("<dev string:x107>");
            scripts\cp_mp\utility\shellshock_utility::_shellshock("<dev string:x147>", "<dev string:x154>", 2, 0);
            self setscriptablepartstate("<dev string:x159>", "<dev string:x164>", 0);
            wait bombexplodedelay - 0.1;
            self setscriptablepartstate("<dev string:x159>", "<dev string:x16b>", 0);
        }
    }
