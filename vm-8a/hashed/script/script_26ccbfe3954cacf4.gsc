...
/#
        ...
        while (true) {
            if (getdvarint(@"hash_a2d26b607a32e157", 0) == 1) {
                    ...
                    if (isbot(player) && !player iseliminated()) {
                        ...
                        if (!player isuibot()) {
                            ...
                            if (player function_d012729eac1e7053()) {
                                debugmsg = "<dev string:x3c2>";
                                if (player namespace_c4ef709c036dc4a8::issuspended()) {
                                    debugmsg += "<dev string:x3ca>";
                                }
                                if (isdefined(player.recyclereason)) {
                                    debugmsg += "<dev string:x3d6>" + player.recyclereason;
                                }
                            } else {
                                debugmsg = "<dev string:x3da>";
                            }
                ...
                while (gettime() < var_89e85c0815c992d6) {
                    printtoscreen2d(500, 100, "<dev string:x3e1>" + var_3ce65cfec5ad6979 + "<dev string:x3ff>" + dormantbotnum + "<dev string:x401>" + uibotnum + "<dev string:x403>" + var_defe7b118dc3891d + "<dev string:x416>" + totalalivebotnum - dormantbotnum, (1, 0, 0), 2);
                    waitframe();
                }
                continue;
            }
                ...
                foreach (player in level.players) {
                    if (isvalidbot(player)) {
                        if (!scripts\mp\gamelogic::inprematch()) {
                            player namespace_c4ef709c036dc4a8::recycle("<dev string:x426>");
                            continue;
                        }
                        player namespace_c50b30148766aa59::recycle("<dev string:x426>");
                    }
                }
            }
            wait 1;
    ...
    function function_da1172b593a41342() {
        ...
        while (true) {
            function_dc925a47fd3404ba(pauserecycle, @"hash_d95f6cfac74fa872", "<dev string:x43a>");
            pauserecycle = getdvarint(@"hash_d95f6cfac74fa872", 0);
            function_dc925a47fd3404ba(var_29080d691bb72834, @"hash_25920b075500f084", "<dev string:x447>");
            var_29080d691bb72834 = getdvarint(@"hash_25920b075500f084", 0);
            function_dc925a47fd3404ba(var_24b5eeca7a6109ed, @"hash_70561094fa4e91b5", "<dev string:x456>");
            var_24b5eeca7a6109ed = getdvarint(@"hash_70561094fa4e91b5", 0);
            function_dc925a47fd3404ba(var_7c4e466930c5330e, @"hash_dd9f8b3596ac0b3d", "<dev string:x46c>");
            var_7c4e466930c5330e = getdvarint(@"hash_dd9f8b3596ac0b3d", 0);
            function_dc925a47fd3404ba(var_889f45625d6bc05c, @"hash_aefbe1721e059470", "<dev string:x481>");
            var_889f45625d6bc05c = getdvarint(@"hash_aefbe1721e059470", 0);
            wait 1;
        }
    }
    ...
    function function_dc925a47fd3404ba(dvarvalue, dvarname, hintstring) {
        if (dvarvalue != getdvarint(dvarname, 0)) {
            if (getdvarint(dvarname, 0)) {
                iprintln(hintstring + "<dev string:x4a5>");
                return;
            }
            iprintln(hintstring + "<dev string:x4aa>");
        }
    }
        ...
        while (true) {
            if (getdvarint(@"hash_52f1eee4900df5a4", 0) > 0) {
                ...
                if (gettime() < level.var_de1b021064d0f5bb.debugcountdown) {
                    msg = "<dev string:x4af>" + (level.var_de1b021064d0f5bb.debugcountdown - gettime()) / 1000;
                }
                x = 600;
                y = 900;
                printtoscreen2d(x, 600, "<dev string:x4c0>" + getdvarint(@"hash_818c699a5caaee4f", 0) + "<dev string:x4eb>" + timeleftsec, (1, 0, 0), 1.5);
                printtoscreen2d(x, 650, "<dev string:x4fe>" + point[1] + "<dev string:x504>" + var_8b49e0c7718e511c + "<dev string:x50a>" + msg, (1, 0, 0), 1.5);
                printtoscreen2d(x, 700, "<dev string:x50d>" + function_e3b7c7a52e05ac10(point) + "<dev string:x518>" + function_94f7fcf9188a0a81(point), (1, 0, 0), 1.5);
                printtoscreen2d(x, y, "<dev string:x524>", (1, 1, 1), 1);
                var_ee5ab934f482b124 = scripts\mp\gametypes\br_gulag::gettimetogulagclosed();
                printtoscreen2d(x + var_ee5ab934f482b124 / 2, y, "<dev string:x563>", (1, 1, 1), 1);
                printtoscreen2d(x + curtimesec / 2, y, "<dev string:x565>", (1, 1, 1), 1);
                for (i = 0; i < 10; i++) {
                    printtoscreen2d(x, y - i * 15, "<dev string:x567>", (1, 1, 1), 1);
                }
                printtoscreen2d(x, y - 150, "<dev string:x569>", (1, 1, 1), 1);
                for (i = 0; i < level.var_de1b021064d0f5bb.points.size; i++) {
                    point = level.var_de1b021064d0f5bb.points[i];
                    if (i == level.var_de1b021064d0f5bb.curindex) {
                        printtoscreen2d(x + point[0] / 2, y - point[1] * 2, "<dev string:x56f>", (1, 1, 0), 1);
                    } else {
                        printtoscreen2d(x + point[0] / 2, y - point[1] * 2, "<dev string:x56f>", (1, 0, 0), 1);
                    }
                    if (debugpoints[i] > 0) {
                        printtoscreen2d(x + point[0] / 2, y - debugpoints[i] * 2, "<dev string:x56f>", (0, 1, 0), 1);
                    }
                }
                for (i = 0; i < circlestarttime.size; i++) {
                    printtoscreen2d(x + circlestarttime[i] / 2, y - 5, "<dev string:x567>", (1, 1, 1), 1);
                }
                waitframe();
                continue;
            }
