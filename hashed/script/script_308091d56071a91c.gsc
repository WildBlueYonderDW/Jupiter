...
function private function_b0f35d29939ca41d() {
    /#
        ...
        while (isdefined(walk)) {
            ...
            countshot = countshot + sceneshots;
            countobject = countobject + sceneobjects;
        ...
        if (getdvarint(@"hash_7dda2ca8e979b1b2", 0)) {
            ...
            foreach (root in roots) {
                ...
                strorigin = "<unknown string>" + int(root.sceneroot.origin[0]) + "<unknown string>" + int(root.sceneroot.origin[1]) + "<unknown string>" + int(root.sceneroot.origin[2]) + "<unknown string>";
                strstate = isdefined(state) ? state : "<unknown string>";
                print(root.sceneroot.var_8ac77218f7a34e3e + "<unknown string>" + strstate + "<unknown string>" + strorigin + "<unknown string>" + root.sceneshots + "<unknown string>" + root.sceneobjects);
            }
        }
        setdvar(@"hash_7dda2ca8e979b1b2", 0);
        printtoscreen2d(50, 60, "<unknown string>" + countscene + "<unknown string>" + countshot + "<unknown string>" + countobject + "<unknown string>" + countbundle, (1, 1, 1), 1);
    #/
}
            ...
            foreach (s_scene in level.var_a0b4eb1703be349a) {
                ...
                foreach (ent in a_ents) {
                    ...
                    if (isdefined(str_anim) && isdefined(str_name)) {
                        ...
                        print3d(var_b6e6ba17ec8331f4, "<unknown string>" + ent getentitynumber() + str_type + "<unknown string>" + (isdefined(str_name) ? str_name : "<unknown string>" + ent getentitynumber()), (0.8, 0.8, 0), 0.8, 0.3);
...
function function_ce4b6d4d859ede39(var_214d924f4e137244, event, origin, angles) {
    /#
        ...
        if (filter != "<unknown string>" && !issubstr(tolower(filter), tolower(var_214d924f4e137244.sceneobject function_aeb2b46911a5811d())) && !issubstr(tolower(var_214d924f4e137244.sceneobject function_aeb2b46911a5811d()), tolower(filter)) && !issubstr(tolower(event), tolower(filter))) {
            return;
        }
...
function function_d4cfde3fa9af7e5a(shotindex) {
    /#
        ...
        if (getdvarint(@"hash_f98fc73c88850645", 0)) {
            ...
            str_scene_name = default_to(sceneroot.var_8ac77218f7a34e3e, "<unknown string>");
            str_shot_name = default_to(sceneroot.scenedata.scenescriptbundle.shots[shotindex].variant_object.name, "<unknown string>");
            ...
            if (isdefined(sceneroot.var_2f23edb971f368a6)) {
                shottext = "<unknown string>" + str_shot_name + "<unknown string>" + sceneroot.var_2f23edb971f368a6;
            } else {
                shottext = "<unknown string>" + str_shot_name;
            }
    ...
    /#
        ...
        if (getdvarint(@"hash_f98fc73c88850645", 0)) {
            ...
            foreach (arrayindex, shotindex in shotindexes) {
                ...
                cameratext = "<unknown string>";
                if (isdefined(cameras) && isdefined(cameras[arrayindex])) {
                    camerapos = cameras[arrayindex];
                    cameratext = "<unknown string>" + int(camerapos[0]) + "<unknown string>" + int(camerapos[1]) + "<unknown string>" + int(camerapos[2]) + "<unknown string>";
                }
                hudelem setdevtext(cameratext + "<unknown string>" + str_scene_name + "<unknown string>" + str_shot_name);
                y = y - 20;
