...
function function_e63a2606efcb8d2c(zone) {
    ...
    /#
        var_2354bc413c2092db = ["<dev string:x1c>", "<dev string:x1f>", "<dev string:x22>", "<dev string:x25>", "<dev string:x28>", "<dev string:x2b>", "<dev string:x2e>"];
    #/
...
/#
    ...
    function function_c8a59a91c2a5ea39(zone) {
        if (zone == "<dev string:x31>" && getdvarint(@"hash_562d6f609676f102", 0)) {
            return 0;
        }
    ...
    function function_3f3ae4ca68128b9d(objectives) {
        ...
        foreach (obj in objectives) {
            ...
            if (is_equal(level.objectives[obj].id, "<dev string:x46>")) {
                scripts\mp\gametypes\obj_dom::removeobjective(level.objectives[obj]);
                continue;
            }
            level.objectives[obj] scripts\mp\gameobjects::setvisibleteam("<dev string:x4e>");
            level.objectives[obj] scripts\mp\gameobjects::allowuse("<dev string:x4e>");
        }
    }
...
/#
    ...
    function reinitspawns() {
        ...
        scripts\mp\spawnlogic::function_8390bf8c3b74466a("<dev string:x5a>");
        spawns = scripts\mp\spawnlogic::getspawnpointarray("<dev string:x63>");
        spawnssecondary = scripts\mp\spawnlogic::getspawnpointarray("<dev string:x70>");
        scripts\mp\spawnlogic::registerspawnset("<dev string:x53>", spawns);
        scripts\mp\spawnlogic::registerspawnset("<dev string:x5a>", spawnssecondary);
    }
...
/#
    ...
    function spawncompare(spawnpoint, spawnteam) {
        if (getdvarint(@"hash_b5ceec3e3cb419be") != 0) {
            ...
            println("<dev string:x87>" + scriptspawnpoint.index + "<dev string:x9f>" + scriptspawnpoint.totalscore);
            if (scriptspawnpoint.index == spawnpoint.index) {
                level.var_2dc7108159dc1da3++;
                iprintlnbold("<dev string:xb2>" + level.var_2dc7108159dc1da3 / level.var_3d292942d52b104a * 100 + "<dev string:xce>" + "<dev string:xd9>" + level.var_2dc7108159dc1da3 + "<dev string:xdb>" + level.var_3d292942d52b104a);
            } else {
                level.var_a065445ba781ddf6++;
                iprintlnbold("<dev string:xdd>" + level.var_2dc7108159dc1da3 / level.var_3d292942d52b104a * 100 + "<dev string:xce>" + "<dev string:xd9>" + level.var_2dc7108159dc1da3 + "<dev string:xdb>" + level.var_3d292942d52b104a);
            }
...
/#
    ...
    function function_8469a5927d069db7() {
        ...
        while (true) {
            if (getdvarint(@"hash_46a6b424af6acbc2") != 0) {
                ...
                scripts\mp\gamescore::giveteamscoreforobjective("<dev string:x115>", points, 0);
                break;
                ...
                scripts\mp\gamescore::giveteamscoreforobjective("<dev string:x11c>", points, 0);
                break;
            }
            wait 1;
        }
    ...
    function function_bf2918399f1800b1() {
        wait 5;
        ents = getentarray("<dev string:x121>", "<dev string:x12c>");
        foreach (ent in ents) {
            thread scripts\mp\utility\debug::drawsphere(ent.origin, 64, 1000, (0, 1, 0));
        }
        ents = getentarray("<dev string:x13e>", "<dev string:x12c>");
        foreach (ent in ents) {
            thread scripts\mp\utility\debug::drawsphere(ent.origin, 64, 1000, (0, 1, 0));
        }
        structs = getstructarray("<dev string:x121>", "<dev string:x12c>");
        foreach (struct in structs) {
            thread scripts\mp\utility\debug::drawsphere(struct.origin, 64, 1000, (0, 1, 0));
        }
        structs = getstructarray("<dev string:x13e>", "<dev string:x12c>");
        foreach (struct in structs) {
            thread scripts\mp\utility\debug::drawsphere(struct.origin, 64, 1000, (0, 1, 0));
        }
    }
        ...
        while (true) {
            ...
            if (dvar) {
                ...
                foreach (objective in level.zone.objectives) {
                    ...
                    level.objectives[objective] scripts\mp\gameobjects::setvisibleteam("<dev string:x4e>");
                    if (level.objectives[objective].objectivetype == 1) {
                        level.objectives[objective] scripts\mp\gameobjects::allowuse("<dev string:x4e>");
                    }
                }
                level notify("<dev string:x151>", level.zone.name);
            }
