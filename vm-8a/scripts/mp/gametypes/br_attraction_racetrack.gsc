...
function race_ui_add_critical_message(race, type) {
    ...
    case #"hash_3683ae6c261adfef": 
        ...
        break;
    default: 
        assertmsg("Unknown critical race message type '" + type + "'");
        break;
    }
...
function race_ui_critical_message_timer(race, timeremaining, message) {
    ...
    case #"hash_3683ae6c261adfef": 
        ...
        break;
    default: 
        assertmsg("Unknown critical race message type '" + message.type + "'");
        break;
    }
...
/#
    ...
    function function_1d97ac4a5afd1642() {
        rt = level.brattractions["<dev string:x77>"];
        if (istrue(rt.var_45d5625806911f2c)) {
            rt.var_45d5625806911f2c = 0;
            rt notify("<dev string:x7a>");
            return;
        } else {
            rt endon("<dev string:x7a>");
    ...
    function function_2a28c6ef9173079f(track) {
        print3d(track.origin, "<dev string:x8e>" + track.index, (1, 1, 0), 1, 2, 1);
        if (isdefined(track.name)) {
            print3d(track.origin + (0, 0, -20), "<dev string:x8f>" + track.name, (0, 1, 0), 1, 1.5, 1);
        }
        if (isdefined(track.settings_group)) {
            print3d(track.origin + (0, 0, -40), "<dev string:x96>" + track.settings_group, (0, 1, 0), 1, 1.5, 1);
        }
        if (isdefined(track.dist_miles) && distance(track.origin, level.player.origin) < 1000) {
            print3d(track.origin + (0, 0, -60), "<dev string:xa1>" + track.dist_miles, (0, 1, 0), 1, 1.5, 1);
            print3d(track.origin + (0, 0, -80), "<dev string:xa9>" + track_get_reward_time_string(track, 1), (0, 1, 0), 1, 1.5, 1);
            print3d(track.origin + (0, 0, -100), "<dev string:xb2>" + track_get_reward_time_string(track, 2), (0, 1, 0), 1, 1.5, 1);
            print3d(track.origin + (0, 0, -120), "<dev string:xbb>" + track_get_reward_time_string(track, 3), (0, 1, 0), 1, 1.5, 1);
        }
        if (!track.points.size) {
            return;
        }
        teleport_to_start = get_track_setting(track, "<dev string:xc4>");
        if (teleport_to_start) {
            teleport_origin = track_get_teleport_target(track);
            debugstar(teleport_origin, (1, 1, 0), 1);
        } else {
            launch_sec = get_track_setting(track, "<dev string:xd6>");
            if (launch_sec > 0) {
                gravity = -800;
        ...
        for (i = 0; i < track.points.size; i++) {
            ...
            radius = get_track_setting(track, "<dev string:xeb>");
            right = point.origin + dir * radius;
            left = point.origin - dir * radius;
            line(right, left, (0, 0, 1), 1, 0, 1);
        }
        for (i = 0; i < track.points.size - 1; i++) {
            color = get_track_setting(track, "<dev string:xfd>");
    ...
    function function_8bc70c14d65e3417(args) {
        ...
        giverewards(level.brattractions["<dev string:x77>"].tracks[track], level.player, stars);
    }
    ...
    function function_33949707efb4fbb4(args) {
        tracks = level.brattractions["<dev string:x77>"].tracks;
        foreach (track in tracks) {
            str = track.name;
            for (i = 1; i <= 3; i++) {
    ...
    function function_53ca8617fcc31a47() {
        ...
        setdvarifuninitialized(@"hash_324951f72e619f36", "<dev string:x8e>");
        while (true) {
            waitframe();
            if (isdefined(rt.var_f99815461b25f1eb)) {
            ...
            if (current == "<dev string:x8e>") {
                continue;
            }
            toks = strtok(current, "<dev string:x13b>");
            current = toks[0];
            arg = ter_op(toks.size > 1, toks[1], undefined);
            setdvar(@"hash_324951f72e619f36", "<dev string:x8e>");
            if (isdefined(rt.var_f99815461b25f1eb)) {
                if (current == "<dev string:x13d>") {
                    function_5ec615ca5f0e291b();
                    continue;
                } else if (current == "<dev string:x14a>" || current == "<dev string:x14f>" || current == "<dev string:x15c>" || current == "<dev string:x165>") {
                    tofile = current == "<dev string:x14f>" || current == "<dev string:x165>";
                    mapformat = current == "<dev string:x15c>" || current == "<dev string:x165>";
                    file = undefined;
                    if (tofile) {
                        filename = "<dev string:x176>" + rt.var_f99815461b25f1eb.name;
                        if (mapformat) {
                            filename += "<dev string:x188>";
                        } else {
                            filename += "<dev string:x18d>";
                        }
                        file = openfile(filename, "<dev string:x192>");
                        path = "<dev string:x198>";
                        if (file == -1) {
                            iprintlnbold("<dev string:x1b8>" + path + filename);
                            continue;
                        } else {
                            iprintlnbold("<dev string:x1cf>" + path + filename);
                        }
                    }
                    if (mapformat) {
                    ...
                    continue;
                } else if (current == "<dev string:x1de>") {
                    rt.var_f99815461b25f1eb.loop = 1;
                    if (rt.var_f99815461b25f1eb.settings_group == "<dev string:x1e3>") {
                        apply_track_settings(rt.var_f99815461b25f1eb, "<dev string:x1eb>");
                    }
                    continue;
                } else if (current == "<dev string:x1f8>") {
                    rt.var_f99815461b25f1eb.loop = 0;
                    if (rt.var_f99815461b25f1eb.settings_group == "<dev string:x1eb>") {
                        apply_track_settings(rt.var_f99815461b25f1eb, "<dev string:x1e3>");
                    }
                    continue;
                } else if (current == "<dev string:x201>") {
                    thread function_533d2f624478039f("<dev string:x20e>");
                    continue;
                } else if (current == "<dev string:x212>") {
                    thread function_533d2f624478039f("<dev string:x228>");
                    continue;
                } else if (current == "<dev string:x235>") {
                    level notify("<dev string:x242>");
                    continue;
                } else if (current == "<dev string:x255>") {
                    if (rt.var_f99815461b25f1eb.points.size) {
                        rt.var_f99815461b25f1eb.points[rt.var_f99815461b25f1eb.points.size - 1] = undefined;
                    }
                    continue;
                } else if (current == "<dev string:x25c>") {
                    init_track(rt.var_f99815461b25f1eb);
                    function_5ec615ca5f0e291b();
                    continue;
                } else if (current == "<dev string:x261>") {
                    if (isdefined(arg)) {
                        rt.var_f99815461b25f1eb.name = arg;
                    }
                    continue;
                } else if (current == "<dev string:x266>") {
                    if (isdefined(arg)) {
                        apply_track_settings(rt.var_f99815461b25f1eb, arg);
                    }
                    continue;
                } else if (current == "<dev string:x20e>" || current == "<dev string:x228>") {
                    point = spawnstruct();
    ...
    function function_28c49f6d47613d0e(track, file) {
        function_8e9fc5e1a129b0d9("<dev string:x2a4>", file);
        function_e2f6c761ac225375(file, 0, "<dev string:x2ad>");
        function_e2f6c761ac225375(file, 1, "<dev string:x2b8>", track.origin, track.angles, "<dev string:x2c6>", "<dev string:x2db>" + 1, track.name, track.settings_group);
        foreach (i, point in track.points) {
            targetname = "<dev string:x2db>" + i + 1;
            if (i == track.points.size - 1) {
                if (track.loop) {
                    target = "<dev string:x2db>" + 1;
                } else {
                    target = undefined;
                }
            } else {
                target = "<dev string:x2db>" + i + 2;
            }
            if (i == 0) {
                noteworthy = "<dev string:x2e0>";
            } else {
                noteworthy = undefined;
            }
            function_e2f6c761ac225375(file, i + 2, "<dev string:x2b8>", point.origin, point.angles, targetname, target, noteworthy);
        }
    }
    ...
    function function_ba2c486db215737b(track, file) {
        function_8e9fc5e1a129b0d9("<dev string:x2e6>" + track.name + "<dev string:x30f>" + track.name + "<dev string:x316>", file);
        ln = "<dev string:x31b>";
        ln += track.origin + "<dev string:x331>";
        ln += track.angles + "<dev string:x334>";
        ln += track.settings_group + "<dev string:x316>";
        function_8e9fc5e1a129b0d9(ln, file);
        function_8e9fc5e1a129b0d9("<dev string:x8e>", file);
        foreach (i, point in track.points) {
            ln = "<dev string:x338>";
            if (i == track.points.size - 1) {
                if (track.loop) {
                    ln += "<dev string:x334>" + track.name + "<dev string:x351>";
                }
            } else {
                ln += "<dev string:x334>" + track.name + "<dev string:x355>" + i + 1 + "<dev string:x357>";
            }
            if (i == 0) {
                ln += "<dev string:x359>";
            }
            ...
            function_8e9fc5e1a129b0d9(ln, file);
            ln = "<dev string:x31b>";
            ln += point.origin + "<dev string:x331>";
            ln += point.angles + "<dev string:x363>";
            function_8e9fc5e1a129b0d9(ln, file);
            function_8e9fc5e1a129b0d9("<dev string:x8e>", file);
        }
    }
    ...
    function function_e2f6c761ac225375(file, entnum, classname, origin, angles, targetname, target, script_noteworthy, script_parameters) {
        function_8e9fc5e1a129b0d9("<dev string:x367>" + entnum, file);
        function_8e9fc5e1a129b0d9("<dev string:x36f>", file);
        function_3c4abd61ee73ba4f("<dev string:x371>", classname, file);
        function_3c4abd61ee73ba4f("<dev string:x37b>", function_b5e5e49b1002b1ab(origin), file);
        function_3c4abd61ee73ba4f("<dev string:x382>", function_b5e5e49b1002b1ab(angles), file);
        function_3c4abd61ee73ba4f("<dev string:x389>", targetname, file);
        function_3c4abd61ee73ba4f("<dev string:x394>", target, file);
        function_3c4abd61ee73ba4f("<dev string:x39b>", script_noteworthy, file);
        function_3c4abd61ee73ba4f("<dev string:x3ad>", script_parameters, file);
        function_8e9fc5e1a129b0d9("<dev string:x3bf>", file);
    }
    ...
    function function_3c4abd61ee73ba4f(key, value, file) {
        ...
        function_8e9fc5e1a129b0d9("<dev string:x3c1>" + key + "<dev string:x3c4>" + value + "<dev string:x357>", file);
    }
    ...
    function function_b5e5e49b1002b1ab(xyz) {
        ...
        return xyz[0] + "<dev string:x13b>" + xyz[1] + "<dev string:x13b>" + xyz[2];
    }
