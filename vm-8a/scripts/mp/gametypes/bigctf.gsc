...
/#
    ...
    function removeflag() {
        ...
        while (true) {
            if (getdvar(@"hash_6ce167f2cdc8ef7c", "<dev string:x27>") != "<dev string:x27>") {
                goallabel = getdvar(@"hash_6ce167f2cdc8ef7c", "<dev string:x27>");
                if (goallabel == "<dev string:x28>") {
                    team = "<dev string:x2b>";
                } else {
                    team = "<dev string:x32>";
                }
                if (team == "<dev string:x2b>") {
                    if (game["<dev string:x37>"]) {
                        team = game["<dev string:x45>"];
                    } else {
                        team = game["<dev string:x4f>"];
                    }
                } else if (game["<dev string:x37>"]) {
                    team = game["<dev string:x4f>"];
                } else {
                    team = game["<dev string:x45>"];
                }
                level.teamflags[team].trigger notify("<dev string:x59>");
                level.teamflags[team] scripts\mp\gameobjects::allowuse("<dev string:x69>");
                level.teamflags[team].trigger = undefined;
                level.teamflags[team] notify("<dev string:x6e>");
                level.teamflags[team].visuals[0] delete();
                level.teamflagbases[team] delete();
                level.capzones[team] scripts\mp\gameobjects::allowuse("<dev string:x69>");
                level.capzones[team].trigger = undefined;
                level.capzones[team] notify("<dev string:x6e>");
                foreach (player in level.players) {
                    player player_delete_flag_goal_fx(team);
                }
                level.teamflags[team].visibleteam = "<dev string:x69>";
                level.teamflags[team] scripts\mp\gameobjects::setobjectivestatusicons(undefined, undefined);
                level.capzones[team].visibleteam = "<dev string:x69>";
                level.capzones[team] scripts\mp\gameobjects::setobjectivestatusicons(undefined, undefined);
                level.teamflags[team] = undefined;
                setdynamicdvar(@"hash_6ce167f2cdc8ef7c", "<dev string:x27>");
            }
            wait 1;
        }
    }
...
/#
    ...
    function placeflag() {
        level endon("<dev string:x1c>");
        while (true) {
            if (getdvar(@"hash_29d45d6822a1cf6d", "<dev string:x27>") != "<dev string:x27>") {
                goallabel = getdvar(@"hash_29d45d6822a1cf6d", "<dev string:x27>");
                if (goallabel == "<dev string:x28>") {
                    team = "<dev string:x2b>";
                } else {
                    team = "<dev string:x32>";
                ...
                goal.radius = 30;
                goal.team = team;
                trigger = spawn("<dev string:x76>", goal.origin, 0, 30, 80);
                visuals[0] = spawn("<dev string:x85>", goal.origin);
                visuals[0] setmodel(level.flagmodel[team]);
                newteamflag = scripts\mp\gameobjects::createcarryobject(team, trigger, visuals, (0, 0, 85));
                newteamflag scripts\mp\gameobjects::setteamusetext("<dev string:x92>", %"<dev string:x98>");
                newteamflag scripts\mp\gameobjects::setteamusetext("<dev string:xa9>", %"<dev string:xb2>");
                newteamflag scripts\mp\gameobjects::allowcarry("<dev string:x92>");
                newteamflag scripts\mp\gameobjects::setvisibleteam("<dev string:x69>");
                ...
                newteamflag.baseeffectpos = trace["<dev string:xc4>"];
                upangles = vectortoangles(trace["<dev string:xcd>"]);
                newteamflag.baseeffectforward = anglestoforward(upangles);
                level.teamflagbases[team] = createteamflagbase(team, newteamflag);
                if (team == "<dev string:x2b>") {
                    if (game["<dev string:x37>"]) {
                        level.teamflags[game["<dev string:x45>"]] = newteamflag;
                    } else {
                        level.teamflags[game["<dev string:x4f>"]] = newteamflag;
                    }
                } else if (game["<dev string:x37>"]) {
                    level.teamflags[game["<dev string:x4f>"]] = newteamflag;
                } else {
                ...
                visuals = [];
                trigger = spawn("<dev string:x76>", goal.origin - (0, 0, goal.radius / 2), 0, goal.radius, 80);
                ...
                capzone scripts\mp\gameobjects::allowuse("<dev string:xa9>");
                capzone scripts\mp\gameobjects::setvisibleteam("<dev string:xd4>");
                ...
                if (team == "<dev string:x2b>") {
                    if (game["<dev string:x37>"]) {
                        level.capzones[game["<dev string:x45>"]] = capzone;
                    } else {
                        level.capzones[game["<dev string:x4f>"]] = capzone;
                    }
                } else if (game["<dev string:x37>"]) {
                    level.capzones[game["<dev string:x4f>"]] = capzone;
                } else {
                ...
                setdynamicdvar(@"hash_29d45d6822a1cf6d", "<dev string:x27>");
            }
            wait 1;
        }
    }
...
/#
    ...
    function function_9c513c5e546a4f52() {
        ...
        while (true) {
            if (getdvar(@"hash_24d9f7f9324e9c61") != "<dev string:xd8>") {
                ...
                if (isdefined(level.tdmzones["<dev string:x28>"])) {
                    foreach (objective in level.tdmzones["<dev string:x28>"]) {
                        thread drawsphere(objective.origin, 20, 1, (1, 1, 1));
                    }
                }
                if (isdefined(level.tdmzones["<dev string:xda>"])) {
                    foreach (objective in level.tdmzones["<dev string:xda>"]) {
                        thread drawsphere(objective.origin, 20, 1, (1, 1, 1));
                    }
                }
                if (isdefined(level.tdmzones["<dev string:xdd>"])) {
                    foreach (objective in level.tdmzones["<dev string:xdd>"]) {
                        thread drawsphere(objective.origin, 20, 1, (1, 1, 1));
                    }
                }
            }
    ...
    function function_e3a2a0c2e544728e(var_a4ab3e1ca5d220c4) {
        if (var_a4ab3e1ca5d220c4 <= 5) {
            key = "<dev string:xe0>" + var_a4ab3e1ca5d220c4;
            var_8a33e220c4aaa38b = level.spawnselectionlocations[key]["<dev string:x32>"].spawnpoints;
            var_4c00998026fee890 = level.spawnselectionlocations[key]["<dev string:x2b>"].spawnpoints;
            ...
            safekey = "<dev string:xe0>" + var_a4ab3e1ca5d220c4 + "<dev string:xe9>";
            var_eb7299174b67818d = undefined;
            var_fd81d96696f0490a = undefined;
            if (isdefined(level.spawnselectionlocations[safekey]) && isdefined(level.spawnselectionlocations[safekey]["<dev string:x32>"])) {
                var_eb7299174b67818d = level.spawnselectionlocations[safekey]["<dev string:x32>"].spawnpoints;
            }
            if (isdefined(level.spawnselectionlocations[safekey]) && isdefined(level.spawnselectionlocations[safekey]["<dev string:x2b>"])) {
                var_fd81d96696f0490a = level.spawnselectionlocations[safekey]["<dev string:x2b>"].spawnpoints;
            }
        ...
        if (var_a4ab3e1ca5d220c4 == 6) {
            key = "<dev string:xef>";
            spawns = level.spawnselectionlocations[key]["<dev string:x32>"].spawnpoints;
        ...
        if (var_a4ab3e1ca5d220c4 == 7) {
            key = "<dev string:xfd>";
            spawns = level.spawnselectionlocations[key]["<dev string:x2b>"].spawnpoints;
            foreach (spawnpos in spawns) {
                thread drawsphere(spawnpos.origin, 30, 60, (1, 0, 0));
                vect = anglestoforward(spawnpos.angles) * 30 + spawnpos.origin;
                thread drawline(spawnpos.origin, vect, 60, (0, 1, 0));
