...
/#
    ...
    function debugkillcament(visual) {
        self endon("<dev string:x9a>");
        level endon("<dev string:xa0>");
        visual endon("<dev string:x9a>");
        while (true) {
...
/#
    ...
    function function_8598880f9b913b55() {
        level endon("<dev string:xa0>");
        self notify("<dev string:xab>");
        self endon("<dev string:xab>");
        self endon("<dev string:xb6>");
        wait 5;
    ...
    function function_847f04c317e266d() {
        level endon("<dev string:xa0>");
        self notify("<dev string:xce>");
        self endon("<dev string:xce>");
        self endon("<dev string:xb6>");
        wait 5;
        while (!level.bombplanted) {
            if (getdvarint(@"hash_31674c76da07f46f", 0) == 1) {
                foreach (p in level.players) {
                    if (isreallyalive(p) && p.team == "<dev string:xdc>") {
                        level.cyberemp.safeorigin = level.cyberemp.trigger.origin;
                        level.cyberemp.carrier = p;
                        level.objectives["<dev string:xe3>"] [[ level.objectives["<dev string:xe3>"].onuse ]](p);
            ...
            if (getdvarint(@"hash_c65dbfe96fb1f3c4", 0) == 1) {
                foreach (p in level.players) {
                    if (isreallyalive(p) && p.team == "<dev string:xe3>") {
                        level.cyberemp.safeorigin = level.cyberemp.trigger.origin;
                        level.cyberemp.carrier = p;
                        level.objectives["<dev string:xdc>"] [[ level.objectives["<dev string:xdc>"].onuse ]](p);
    ...
    function function_4df61dc99c28632b() {
        ...
        while (getdvarint(@"hash_d51785ab78729b60", 0) == 1) {
            explosionorigin = level.objectives[team].visuals[0] gettagorigin("<dev string:x104>");
            level.objectives[team].visuals[0] radiusdamage(explosionorigin, 512, 1, 1, undefined, "<dev string:x10f>", "<dev string:x11d>");
            playsoundatpos(explosionorigin, "<dev string:x12a>");
            rot = randomfloat(360);
            explosioneffect = spawnfx(level._effect["<dev string:x142>"], explosionorigin + (0, 0, 50), (0, 0, 1), (cos(rot), sin(rot), 0));
            triggerfx(explosioneffect);
            playrumbleonposition("<dev string:x151>", explosionorigin);
            earthquake(0.6, 1.5, explosionorigin, 10000);
            level thread empjamandrumbleclients(1);
            if (game["<dev string:x162>"]) {
                exploderindex = 200;
            } else {
                exploderindex = 201;
            }
