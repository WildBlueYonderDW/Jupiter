        ...
        if (isdefined(hide_node)) {
            ...
            if (result == "goal") {
                for (i = 0; i < var_724c1907987840a4.size; i++) {
                    ...
                    for (var_eeb18c09526831f4 = 0; var_724c1907987840a4[i].frames_visible < 18 && var_eeb18c09526831f4 < 3.6; var_eeb18c09526831f4 = var_eeb18c09526831f4 + 0.25) {
    ...
    if (bot_is_protecting() || bot_is_bodyguarding()) {
        ...
        /#
            if (isdefined(self getnearestnode()) && !(isdefined(level.var_a4e60ff6b2b0e717) && level.var_a4e60ff6b2b0e717)) {
                /#
                    assertex(entrances.size > 0, "<unknown string>" + self.origin + "<unknown string>");
                #/
            }
        #/
        return entrances;
    ...
    while (true) {
        ...
        foreach (node in self.watch_nodes) {
            if (var_9bc4f8e26f7a0ac4) {
                ...
                node.watch_node_chance[self.entity_number] = node.watch_node_chance[self.entity_number] * gametype_scalar;
            }
    ...
    for (z = 0; z < level.zonecount; z++) {
        ...
        for (i = 0; i < nodes_testing.size; i++) {
            ...
            if (var_a296921504813986) {
                nodes_testing[i].watch_node_chance[self.entity_number] = nodes_testing[i].watch_node_chance[self.entity_number] * scalar;
