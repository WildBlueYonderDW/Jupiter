...
function getteammatesoutofcombat(team) {
    ...
    foreach (player in level.players) {
        ...
        if (!isdefined(player.team) || player.team != team) {
            /#
                scripts\mp\dev::function_fe598e0c02fe84ec(player, "<dev string:x24>");
            #/
            continue;
        }
        if (player == self) {
            /#
                scripts\mp\dev::function_fe598e0c02fe84ec(player, "<dev string:x2f>");
            #/
            continue;
        }
        ...
        if (!player val::get("player_for_spawn_logic")) {
            /#
                scripts\mp\dev::function_fe598e0c02fe84ec(player, "<dev string:x34>");
            #/
            continue;
        }
...
function canplayerbebuddyspawnedon(player) {
    if (player.sessionstate != "playing") {
        /#
            scripts\mp\dev::function_fe598e0c02fe84ec(player, "<dev string:x44>" + player.sessionstate);
        #/
        return false;
    }
    if (!isreallyalive(player)) {
        /#
            scripts\mp\dev::function_fe598e0c02fe84ec(player, "<dev string:x53>");
        #/
        return false;
    }
    if (!player isonground()) {
        /#
            scripts\mp\dev::function_fe598e0c02fe84ec(player, "<dev string:x58>");
        #/
        return false;
    }
    if (player isonladder()) {
        /#
            scripts\mp\dev::function_fe598e0c02fe84ec(player, "<dev string:x60>");
        #/
        return false;
    }
    if (player isflashed()) {
        /#
            scripts\mp\dev::function_fe598e0c02fe84ec(player, "<dev string:x67>");
        #/
        return false;
    }
    if (player.health < player.maxhealth && (!isdefined(player.lastdamagedtime) || gettime() < player.lastdamagedtime + 3000)) {
        /#
            scripts\mp\dev::function_fe598e0c02fe84ec(player, "<dev string:x6f>");
        #/
        return false;
    }
    /#
        scripts\mp\dev::function_fe598e0c02fe84ec(player, "<dev string:x76>");
    #/
    return true;
}
...
function testbuddyspawncriticalfactors(spawnpoint) {
    if (!critical_factor(&function_22b490a6df990a49, spawnpoint)) {
        /#
            scripts\mp\dev::function_41fd2e38468e1538(spawnpoint, "<dev string:x80>");
        #/
        return false;
    }
    if (!critical_factor(&function_5fdeed44e6029789, spawnpoint)) {
        /#
            scripts\mp\dev::function_41fd2e38468e1538(spawnpoint, "<dev string:x89>");
        #/
        return false;
    }
    if (!critical_factor(&function_63d3482ba43b656c, spawnpoint)) {
        /#
            scripts\mp\dev::function_41fd2e38468e1538(spawnpoint, "<dev string:x96>");
        #/
        return false;
    }
    if (!critical_factor(&avoidcarepackages, spawnpoint)) {
        /#
            scripts\mp\dev::function_41fd2e38468e1538(spawnpoint, "<dev string:xa0>");
        #/
        return false;
    }
    if (!critical_factor(&avoidbradleys, spawnpoint)) {
        /#
            scripts\mp\dev::function_41fd2e38468e1538(spawnpoint, "<dev string:xb2>");
        #/
        return false;
    }
    if (!critical_factor(&avoidtelefrag, spawnpoint)) {
        /#
            scripts\mp\dev::function_41fd2e38468e1538(spawnpoint, "<dev string:xc1>");
        #/
        return false;
    }
    if (!critical_factor(&function_4546108b3957a698, spawnpoint)) {
        /#
            scripts\mp\dev::function_41fd2e38468e1538(spawnpoint, "<dev string:xca>");
        #/
        return false;
    }
    /#
        scripts\mp\dev::function_41fd2e38468e1538(spawnpoint, "<dev string:xd5>");
    #/
    return true;
}
...
function getstartspawnpoint_freeforall(spawnpoints) {
    ...
    var_d4d7a55bc1e8f68e = scripts\mp\spawnlogic::getactiveplayerlist();
...
function logbadspawn(typestring, spawningentity) {
    ...
    println("<dev string:xdd>" + typestring + "<dev string:xeb>");
...
function getspawnpoint_legacy(spawnpoints, fallbackspawnpoints, factorparams, var_fcb3e7fed624752a, nobuddyspawns) {
    ...
    /#
        if (isdefined(self) && isplayer(self)) {
            self iprintlnbold("<dev string:xed>");
        }
    #/
        ...
        foreach (lanetrigger in level.spawnglobals.lanetriggers) {
            foreach (otherplayer in level.players) {
                ...
                if (ispointinvolume(otherplayer.origin, lanetrigger)) {
                    ...
                    spawnglobals.occupiedlanemasks[otherplayerteam] = spawnglobals.occupiedlanemasks[otherplayerteam] | lanetrigger.indexflag;
                }
            }
        }
    }
