...
/#
    ...
    function function_5dee7155086e9ada() {
        scripts\common\devgui::function_6e7290c8ee4f558b("<dev string:x21>");
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x43>", "<dev string:x54>", &function_4345f8a9f656b405);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x64>", "<dev string:x7c>", &function_a436c573abc56a6a);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x90>", "<dev string:xa1>", &function_b71dae6abf83d269);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:xb7>", "<dev string:xc6>", &function_87ac4dc3e17d9720);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:xd7>", "<dev string:xe6>", &function_3783df970b21d0b9);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:xfa>", "<dev string:x111>", &function_8f4bec208d29321a);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x122>", "<dev string:x136>", &function_d34f28c4111fcc3);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x144>", "<dev string:x158>", &function_756f0952a1f01e6d);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x166>", "<dev string:x177>", &function_a805454c14fec2d);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x188>", "<dev string:x19a>", &function_64ccd72856415728);
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x1ac>", "<dev string:x1bd>", &function_acc43fa45aaf68be);
...
function private agent_spawngroup(var_56ac4b49cc795bc0) {
    ...
    if (!isdefined(var_6f42354cebcd7f35)) {
        assertmsg("<dev string:x1d2>" + self.origin + "<dev string:x1f2>");
        return;
    }
    ...
    assertex(validspawnpoints.size == var_8357fb6a08a850d5, "<dev string:x20d>" + validspawnpoints.size + "<dev string:x247>" + var_8357fb6a08a850d5);
...
function function_545f0df81e095369(target, pulsate) {
    ...
    if (!isdefined(self.objectiveiconid)) {
        assertmsg("<dev string:x2d0>" + istrue(self.taskcompleted));
        return;
    }
    if (isdefined(target)) {
        if (isvector(target)) {
...
function function_d01cb68a0f2ea6f6() {
    ...
    if (isdefined(castresult["hittype"] != "hittype_none") && isdefined(castresult["position"])) {
        ...
        assertmsg("<dev string:x391>" + self.origin);
    }
...
function function_206405b8c9d63038(type, startnode) {
    ...
    if (var_c09a118d31c10225.size == 0) {
        println("<dev string:x3c3>" + startnode.origin + "<dev string:x40a>" + type);
        return undefined;
    }
    bestnode = undefined;
        ...
        if (level.var_4e85625f13c359ab.var_3b2141e078017f8c) {
            ...
            if (isdefined(vehicle) && vehicle isvehicle() && !vehicle scripts\cp_mp\vehicles\vehicle_damage::vehicle_damage_isburningdown() && istrue(vehicle.islocked)) {
                if (is_equal(attacker.team, self.teams[0])) {
                    /#
                        attacker iprintlnbold("<dev string:x414>");
                    #/
                } else {
                    /#
                        iprintlnbold("<dev string:x440>");
                    #/
                }
...
function function_89e3fc49199d2019() {
    ...
    /#
        if (!istrue(self.tablet.init)) {
            ...
            endcontract(undefined, undefined, undefined, "<dev string:x4a1>");
            return;
        }
    #/
...
function function_d97ac1473f7695b(secondarymode) {
    ...
    if (!isdefined(startnode.script_string)) {
        assertmsg("<dev string:x4ac>" + startnode.origin + "<dev string:x4cf>");
...
/#
    ...
    function function_7ec025a0970cc56c() {
        self endon("<dev string:x59b>");
        debugposoutputs = [];
        while (true) {
            wait 1;
            if (function_29abf5b8a864c5dd(self)) {
                iprintln("<dev string:x5af>");
            } else {
    ...
    function function_317eb7e23a9f9ae9() {
        ...
        foreach (node in nodes) {
            ...
            scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_setteam(vehicle, "<dev string:x5c9>");
    ...
    function function_fd7c18412575bc79() {
        ...
        debugtype[2] = "<dev string:x574>";
        debugtype[3] = "<dev string:x5ce>";
    ...
    function function_84a50610b98c3166() {
        foreach (contract in level.contractmanager.activetablets) {
            if (!is_equal(contract.type, "<dev string:x5d3>")) {
                continue;
            }
            if (!is_equal(contract.task.tasktype, "<dev string:x574>")) {
                continue;
            }
    ...
    function function_a436c573abc56a6a() {
        host = scripts\mp\dev::devfindhost();
        println("<dev string:x5ee>");
        println("<dev string:x60f>");
        println("<dev string:x629>" + level.var_4e85625f13c359ab.enabled);
        println("<dev string:x649>" + level.var_4e85625f13c359ab.time);
        println("<dev string:x669>" + level.var_4e85625f13c359ab.var_cf9f04c2455c9551);
        println("<dev string:x689>" + level.var_4e85625f13c359ab.var_5250e11f76eaf5bb);
        println("<dev string:x6a9>" + level.var_4e85625f13c359ab.var_946b411053737262);
        println("<dev string:x6c9>" + level.var_4e85625f13c359ab.var_a8f585deac0aaa44);
        println("<dev string:x6e9>" + level.var_4e85625f13c359ab.var_623c71246715c68a);
        println("<dev string:x709>" + level.var_4e85625f13c359ab.var_f618c857c91d9adc);
        println("<dev string:x729>" + level.var_4e85625f13c359ab.var_b4862b86b957b504);
        println("<dev string:x749>" + level.var_4e85625f13c359ab.var_1aa0d8c830908356);
        println("<dev string:x769>" + level.var_4e85625f13c359ab.var_dad5abd3a17a252e);
        println("<dev string:x789>" + level.var_4e85625f13c359ab.var_c9a4667d43c3d2b7);
        println("<dev string:x7a9>" + level.var_4e85625f13c359ab.var_74de30492b65dedb);
        println("<dev string:x7c9>" + level.var_4e85625f13c359ab.var_f767533f0cc83c57);
        println("<dev string:x7e9>" + level.var_4e85625f13c359ab.var_4156aa8334394ed5);
        println("<dev string:x809>" + level.var_4e85625f13c359ab.var_3b2141e078017f8c);
        println("<dev string:x5ee>");
    }
    ...
    function function_8f4bec208d29321a() {
        host = scripts\mp\dev::devfindhost();
        level endon("<dev string:x829>");
        host endon("<dev string:x59b>");
        ...
        agent = namespace_bfef6903bca5845d::ai_mp_requestspawnagent(aitype, (0, 0, 0), (0, 0, 0), "<dev string:x834>", "<dev string:x83b>", "<dev string:x845>");
        assert(isdefined(agent) && isagent(agent));
        streakinfo = scripts\cp_mp\utility\killstreak_utility::createstreakinfo("<dev string:x84f>", agent);
    ...
    function function_756f0952a1f01e6d() {
        host = scripts\mp\dev::devfindhost();
        vehicle = function_4944e32d48208083("<dev string:x574>", host.origin, host.angles);
        helicopter = level helicopter_start(host.origin, vehicle, host);
        if (!isdefined(helicopter)) {
            assertmsg("<dev string:x85b>");
        }
    }
