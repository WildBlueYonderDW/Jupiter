...
// Namespace namespace_e1db05e1cbb8d8d5 / scripts\cp\cp_awards
// Params 0, eflags: 0x0
...
function giveaward(ref, objweapon, pointsoverride, xpoverride, immediate, victim, cankillchain, streakinfo, var_ad8c6c5cc50af10b, eventinfo) {
    ...
    if (!namespace_5aac85eab99c40a::function_34294184e90b96c(ref)) {
    ...
    xp = xpoverride;
    if (!isdefined(xp)) {
        xp = namespace_5aac85eab99c40a::getscoreinfoxp(ref);
    }
    if (isdefined(xp) && xp > 0) {
        namespace_5aac85eab99c40a::giverankxp(ref, xp, objweapon);
    }
    if (!isdefined(pointsoverride)) {
        points = namespace_5aac85eab99c40a::getscoreinfovalue(ref);
...
function addawardtoaarlist(ref) {
    ...
    for (listindex = self.aarawards.size - 2; listindex >= insertindex; listindex--) {
        ...
        /#
            if (isdefined(awardinfo.ref) && function_d03495fe6418377b(awardinfo.ref)) {
...
function challengetrackerforjuggkills() {
    ...
    foreach (player in level.players) {
        player thread namespace_6099285b4066f63b::doScoreEvent(#"kill_juggernaut");
    }
