...
function function_100765fe9abe8d6d() {
    ...
    for (i = 0; i < var_4bb47473cc0e8dc1.size; i++) {
        totalweightvalue = totalweightvalue + var_4bb47473cc0e8dc1[i].var_90b63d94c68dade5;
...
function decoy_getfiretype() {
    /#
        dvarfiretype = getdvar(@"hash_621c6f1da92a54d5", "physicsquery_closest");
        if (dvarfiretype != "physicsquery_closest") {
            return dvarfiretype;
        }
    #/
    totalweightvalue = 0;
    ...
    for (i = 0; i < firetypes.size; i++) {
        totalweightvalue = totalweightvalue + firetypes[i].var_f7dff98a85f6c2cc;
        maxweightvalues[i] = totalweightvalue;
    }
    weightvalue = randomint(totalweightvalue);
    for (i = 0; i < maxweightvalues.size; i++) {
...
function decoy_getfireeventimpulse(velocity, firetype, fireangles) {
    ...
    impulse = velocity;
    impulse = impulse + anglestoup(fireangles) * randomfloatrange(var_738862482821aad9, var_a824a799de1219c3);
    impulse = impulse + anglestoforward(fireangles) * randomfloatrange(var_bcfd5ca36fa3b723, var_256cb1561781b97d);
    return impulse;
}
