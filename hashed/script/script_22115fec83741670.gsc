...
function function_d8b2df497e0b97f() {
    gasbundle = namespace_4a3033eafa6fd07::function_bf9c7e9dd30180e3("super_gas_trap");
...
function function_f539d5d6dc0cc4a7() {
    /#
        ...
        self.debug_trig = spawn("<unknown string>", self.origin, 0, 48, 48);
        self.debug_trig enablelinkto();
        self.debug_trig linkto(self);
        cylinder(self.origin, self.origin + (0, 0, 48), 48, (1, 1, 0), 0, 100);
        while (true) {
            ent = self.debug_trig waittill("<unknown string>");
            /#
                assert(isdefined(ent));
            #/
            self notify("<unknown string>", [ent]);
...
function function_779cd9adc64d02e9(triggerid) {
    /#
        assertex(isdefined(triggerid), "gas_trap_gasOnExitTrigger called with an invalid trigger ID.");
    #/
...
function function_e527fb1c56a67ed2(trigger) {
    ...
    if (isdefined(gasbundle)) {
        while (self.var_a8005043799ca23 > 0) {
            ...
            self.var_a8005043799ca23 = self.var_a8005043799ca23 - level.var_3ad99a60ec62068f.dot_interval;
