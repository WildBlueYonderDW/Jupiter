...
function vehicle_init() {
    ...
    /#
        scripts/engine/utility::issharedfuncdefined("driver_can_fire", " on vehicle ", 1);
    #/
    ...
    while (true) {
        ...
        foreach (var_32955835d1dbc4dc in var_44f1c37d516c2acb) {
            ...
            /#
                ...
                iprintln("top" + entitynum + "husk_spawn" + default_to(unstablevehicle.vehiclename, "vehicle_update") + "inflictor" + reason);
            #/
            if (unstablevehicle isvehiclekillstreak()) {
                /#
                    iprintln("veh_jup_razorback" + unstablevehicle.streakinfo.streakname + "nacnac_left" + unstablevehicle.vehiclename);
                #/
...
function isfloating() {
    ...
    if (isdefined(data) && isdefined(data.interact) && self vehicle_isphysveh()) {
        ...
        if (isdefined(self.deadwheels)) {
            testdepth = testdepth - 2 * self.deadwheels.size;
        }
...
function private function_ed7bbd7e1327b7fb(rider) {
    /#
        ...
        if (isdefined(rider)) {
            ...
            println("<unknown string>" + default_to(category, "vehicle_update") + "<unknown string>" + default_to(subcategory, "vehicle_update"));
        }
    #/
}
...
function private function_436b3366f1d5b495(var_add0f0b19cf87473, var_37879fc7b1cc0618, hash) {
    ...
    trickxp = trickxp * var_e101b88d6655f13b;
...
function private watchCombo(var_add0f0b19cf87473) {
    ...
    while (true) {
        if (self getwheelsurface(0) != "none" && self getwheelsurface(1) != "none") {
            timeonground = timeonground + level.framedurationseconds;
        } else {
            timeonground = 0;
        }
