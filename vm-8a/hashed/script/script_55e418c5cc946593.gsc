...
/#
    ...
    function function_d6a5c9019b66d5d3() {
        ...
        yoffset = -80;
        yoffsetincrement = 10;
        var_75599bf3d6abf81f = createfontstring("<dev string:x32>", 1);
        var_75599bf3d6abf81f setpoint("<dev string:x3a>", "<dev string:x3a>", xoffset, yoffset);
        self.var_a1adcd8ba70c9723["<dev string:x3f>"] = var_75599bf3d6abf81f;
        for (i = 1; i <= 4; i++) {
            var_d523d0a459b67453 = createfontstring("<dev string:x32>", 1);
            var_d523d0a459b67453 setpoint("<dev string:x3a>", "<dev string:x3a>", xoffset, yoffset + yoffsetincrement * (i - 1 + 1));
            self.var_a1adcd8ba70c9723["<dev string:x4c>" + i] = var_d523d0a459b67453;
        }
    ...
    function function_e95ed23f86785c82() {
        ...
        if (!isdefined(self.pers["<dev string:x51>"])) {
            return;
        }
        if (!scripts\mp\flags::gameflag("<dev string:x57>")) {
            return;
        }
        ...
        if (isdefined(lootref)) {
            self.var_a1adcd8ba70c9723["<dev string:x3f>"] setdevtext("<dev string:x65>" + lootref);
        } else {
            self.var_a1adcd8ba70c9723["<dev string:x3f>"] setdevtext("<dev string:x71>");
        }
        for (i = 1; i <= 4; i++) {
            perkname = "<dev string:x7e>";
            if (isdefined(perks) && array_contains_key(perks, "<dev string:x4c>" + i)) {
                perkname = perks["<dev string:x4c>" + i];
            }
            if (function_56f3d322e17b9974()) {
                scoreunlock = level.var_767653d4d33e973[i];
                statustext = "<dev string:x7f>";
                if (function_ee52068fbb9b0dec(i)) {
                    statustext = "<dev string:x86>";
                } else if (self.pers["<dev string:x51>"] < scoreunlock && self.pers["<dev string:x51>"] > var_b904c15f8ea08a65) {
                    percentunlocked = round((self.pers["<dev string:x51>"] - var_b904c15f8ea08a65) / (scoreunlock - var_b904c15f8ea08a65) * 100);
                    statustext = percentunlocked + "<dev string:x8f>";
                } else if (self.pers["<dev string:x51>"] >= scoreunlock) {
                    statustext = "<dev string:x86>";
                }
                self.var_a1adcd8ba70c9723["<dev string:x4c>" + i] setdevtext("<dev string:x91>" + i + "<dev string:x97>" + perkname + "<dev string:x9b>" + statustext);
                var_b904c15f8ea08a65 = scoreunlock;
                continue;
            }
            self.var_a1adcd8ba70c9723["<dev string:x4c>" + i] setdevtext("<dev string:x91>" + i + "<dev string:x9f>" + perkname);
        }
    }
