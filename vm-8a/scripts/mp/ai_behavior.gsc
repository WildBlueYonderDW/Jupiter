...
function function_a5117518725da028(agent, reinforceorigin, var_e298cec41261345c, goalradiusoverride, var_21f24aecb313e479, var_3e332762c9138001, rushgroup, var_66db74486a8f804, var_1b3d1c2f22b62206, targetent) {
    ...
    if (isagent(agent) && distance(agent.origin, reinforceorigin) > 12500) {
        assertmsg("behavior_combatRushOrigin: combat rush reinforceOrigin (" + reinforceorigin + ") is too far from where agent is at (" + agent.origin + ")");
        /#
            print3d(agent.origin, "<dev string:x5b>", (1, 0, 0), 1, 5, 60, 1);
...
function function_a0e63fe5c7a71428(agent) {
    ...
    agent endon("death");
    agent endon("alerted_to_player");
    idamage, eattacker = agent waittill("damage");
...
function function_ef3a20cdb3c82b48(node) {
    ...
    if (!istrue(level.global_stealth_broken) && !istrue(self.entered_combat) && isdefined(node.script_animation_type)) {
        ...
        if (isdefined(var_dd1921916c4e99c7)) {
            ...
            /#
                print3d(self.origin + (0, 0, 64), "<dev string:x7a>" + node.script_animation_type, (1, 1, 0), 1, 0.5, 60, 1);
            #/
            self.playing_skit = 1;
...
function behavior_executebehaviorpackage(agent) {
    ...
    case #"hash_10c1fbec939bdcf7": 
        ...
        assertex(distance(agent.origin, bp.reinforceorigin) < 12500, "behavior_executeBehaviorPackage: trying to combat rush with a goal too far away");
...
function function_9bcf6b76ea52ee4c() {
    ...
    level.var_7ce9081bcf4af5a6 = namespace_bfef6903bca5845d::ai_mp_requestspawnagent(aitype, covernode.origin, spawnnode.angles, "high", "reinforcements", "debug");
...
function function_a1d0e5ea1a887436(agent, node, time) {
    ...
    agent endon("death");
    interval = 0.05;
    for (currenttime = 0; !isdefined(time) || currenttime < time; currenttime += interval) {
    ...
    function function_1efa739f9e55871f() {
        ...
        while (true) {
            if (getdvarint(@"hash_b85b66dbdc0853f9", 0) == 1) {
                ...
                agents = namespace_bfef6903bca5845d::function_6214774e8e021663(1, level.players[0].origin, "<dev string:xa5>", "<dev string:xae>", "<dev string:xbd>", undefined, team, poiname, 256, 257);
            ...
            if (getdvarint(@"hash_b85b66dbdc0853f9", 0) == 3) {
                agents = namespace_bfef6903bca5845d::function_353fecd1549f5f42(level.players[0].origin, 50000, level.players[0].team, 100, "<dev string:xcc>", undefined, undefined, 1);
                setdevdvar(@"hash_b85b66dbdc0853f9", 0);
            }
