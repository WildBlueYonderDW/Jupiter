                    ...
                    if (function_24c5a8d31ae262f(player.origin, var_819edacdacb810e4, dangercircleradius)) {
                        if (player cancircledamageplayer(player)) {
                            ...
                            if (isdefined(level.var_aa1aaed4a9b80d4b) && isdefined(level.var_152d5a9c5ac5cdad) && level.br_circle.circleindex <= level.var_aa1aaed4a9b80d4b) {
                                gasmaskdamage = gasmaskdamage * level.var_152d5a9c5ac5cdad;
                            }
...
function function_81652bad17cd28bd(modifieddamagetick) {
    ...
    if (isdefined(player.var_c2c2848ce2585f29)) {
        finalperc = finalperc + player.var_c2c2848ce2585f29;
    }
    modifieddamagetick = modifieddamagetick * finalperc;
        ...
        while (true) {
            ...
            for (x = 0; x < areacount; x++) {
                for (y = 0; y < areacount; y++) {
                    ...
                    print3d((xmin + x * var_d3426ff50f0ff497 + var_d3426ff50f0ff497 / 2, ymin + y * var_d3426ff50f0ff497 + var_b3dd581cafbd5000 / 2, 0), "Minimap radius not defined for circle " + count, color, 1, scale, 1, 1);
                }
            }
            waitframe();
        }
    ...
    /#
        ...
        while (true) {
            foreach (var_3db3628ca5e80be6 in circlesimulations) {
                ...
                for (circleindex = startingcircleindex; circleindex <= endingcircleindex; circleindex = circleindex + 1) {
                    print3d(circlecenters[circleindex], "Minimap radius not defined for circle " + circleindex, color, 1, 25, 1);
...
function function_d45be26e9bfb904(var_50c422b464b3e15d, var_ad4d828d3b1da2e6, var_b7c9fe00120ea96c) {
    ...
    for (circleindex = var_50c422b464b3e15d; circleindex >= var_ad4d828d3b1da2e6; circleindex--) {
        ...
        var_370050a3c3a88a8f = var_370050a3c3a88a8f + movedistances[circleindex];
    }
...
function getmintimetillpointindangercircle(point) {
    ...
    for (i = circleindex + 1; i < pointindex; i++) {
        safetime = safetime + level.br_level.br_circleclosetimes[i];
        safetime = safetime + level.br_level.br_circledelaytimes[i];
    }
    delaytime = level.br_level.br_circledelaytimes[circleindex];
    closetime = level.br_level.br_circleclosetimes[circleindex];
    ...
    if (pointindex > circleindex) {
        ...
        totaltime = delaytime + closetime;
        safetime = safetime + totaltime - timepassed;
    ...
    if (function_d6f728b66bd3966(testindex) && delaytimeremaining > 0) {
        ...
        if (distance2d(var_63471afdb08176f2, point) >= startradius) {
            safetime = safetime + function_3fa958de973c98c6(point, startorigin, var_63471afdb08176f2, startradius, startradius, delaytimeremaining);
            return safetime;
        } else {
            startorigin = var_63471afdb08176f2;
        }
    }
    safetime = safetime + delaytimeremaining;
    safetime = safetime + function_3fa958de973c98c6(point, startorigin, endorigin, startradius, endradius, closetimeremaining);
    return safetime;
}
        ...
        foreach (guid, revive in level.revivetriggers) {
            if (isdefined(revive) && distance2dsquared(center, revive.trigger.origin) > radsqr) {
                ...
                if (isdefined(validclosets) && validclosets.size > 0) {
                    /#
                        println("_scale" + validclosets.size + "] " + validclosets[0].origin);
                    #/
                    revive.victim scripts/mp/teamrevive::relocatetrigger(validclosets[0].origin);
                    continue;
                }
                revive.victim scripts/mp/teamrevive::removetrigger(guid);
            }
        }
    }
}
...
function applymovingcircles() {
    ...
    case 1:
        ...
        for (i = 0; i < var_f59ab661ba766221; i++) {
            delta = delta + circleradiusdeltas[i];
...
function function_e5958529715b6395() {
    /#
        ...
        starty = -32;
        lineheight = 16;
        header = player scripts/mp/hud_util::createfontstring("dmz", 1.2);
        header.alpha = 1;
        header.color = (1, 1, 0);
        header scripts/mp/hud_util::setpoint("<unknown string>", undefined, startx, starty);
        header setdevtext("<unknown string>");
        circleorigin = player scripts/mp/hud_util::createfontstring("dmz", 1.2);
        circleorigin.alpha = 1;
        circleorigin.color = (1, 1, 0);
        circleorigin scripts/mp/hud_util::setpoint("<unknown string>", undefined, startx, starty + lineheight);
        circleradius = player scripts/mp/hud_util::createfontstring("dmz", 1.2);
        circleradius.alpha = 1;
        circleradius.color = (1, 1, 0);
        circleradius scripts/mp/hud_util::setpoint("<unknown string>", undefined, startx, starty + lineheight * 2);
        circletime = player scripts/mp/hud_util::createfontstring("dmz", 1.2);
        circletime.alpha = 1;
        circletime.color = (1, 1, 0);
        circletime scripts/mp/hud_util::setpoint("<unknown string>", undefined, startx, starty + lineheight * 3);
...
function function_7f31d213264baf4d(header, circleorigin, circleradius, circletime) {
    /#
        ...
        while (true) {
            ...
            circletime setdevtext("<unknown string>" + getmintimetillpointindangercircle(level.player.origin));
            waitframe();
        }
    #/
}
...
function function_9eb173d6a8fed181() {
    /#
        ...
        test_colors = ["<unknown string>", "<unknown string>", "<unknown string>"];
        for (spawnents = 1; true; spawnents = 0) {
            ...
            for (circleindex = 0; circleindex < level.br_level.br_circlecenters.size; circleindex++) {
                ...
                if (spawnents) {
                    ...
                    circleent function_6b6b6273f8180522("<unknown string>" + test_colors[circleindex % test_colors.size], center, max(50, radius));
...
function function_8ebb53f2c5ed597d(args) {
    /#
        ...
        if (args.size > 0) {
            ...
            trace = scripts/engine/trace::ray_trace(start, end, host);
            testpt.origin = trace["<unknown string>"];
        }
        spawnents = 1;
        while (true) {
            foreach (area in level.br_badcircleareas) {
            ...
            if (isdefined(testpt)) {
                ...
                if (spawnents) {
                    circleent = spawnstruct();
                    circleent function_6b6b6273f8180522("<unknown string>", testpt.origin, testpt.radius);
                    circleent thread function_c3f32250a5b22066();
                }
            }
            spawnents = 0;
