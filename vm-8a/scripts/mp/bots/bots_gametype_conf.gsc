...
/#
    ...
    function function_1303b6877c472535() {
        ...
        while (true) {
            if (getdvarint(@"hash_9cac85d6351e1df") == 1) {
                if (getdvar(@"hash_242f40f6d99b93e9") == "<dev string:x1c>") {
                    foreach (tag in level.dogtags) {
                        if (tag bot_can_tag_interact_with("<dev string:x1e>") || tag bot_can_tag_interact_with("<dev string:x25>")) {
            ...
            if (getdvar(@"hash_313eed123eed35ac") == "<dev string:x2a>") {
                foreach (tag in level.dogtags) {
                    if (tag bot_can_tag_interact_with("<dev string:x1e>") || tag bot_can_tag_interact_with("<dev string:x25>")) {
...
function bot_watch_new_tags() {
    while (true) {
        newtag = level waittill("new_tag_spawned");
...
function bot_find_best_tag_from_array(tag_array, var_fcb9461bb8ed3b2a) {
    ...
    if (tag_array.size > 0) {
        ...
        foreach (tag_struct in tag_array) {
            var_93b8fc938f87c60e = get_num_allies_getting_tag(tag_struct.tag);
