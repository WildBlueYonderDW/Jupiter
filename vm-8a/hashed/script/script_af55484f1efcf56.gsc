...
function function_e2bbf555c17be641() {
    ...
    if (isdefined(level.var_e121c8daae441150)) {
        ...
        weaponobj, droppeditem = self waittill("weapon_pickup");
        outlinedisable(level.var_e121c8daae441150.outline, self);
    }
    /#
        self iprintln("<dev string:xb4>");
    #/
    flag_set("next_event");
}
...
function function_8ce66a93e6be27b8() {
    ...
    #/
    player = laddertrigger waittill("trigger");
    laddertrigger function_d2da945656356633();
    function_4f223e0dd031befb(2);
    self playsound("mp_jup_training_objective_success");
    /#
        self iprintln("<dev string:x2a7>");
    #/
    flag_set("next_event");
}
...
function function_8aa67e2c556382dc() {
    flag_clear("next_event");
    /#
        self iprintln("<dev string:x614>");
    #/
    wait 3;
    function_7c82d04922ac1bba();
    /#
        self iprintln("<dev string:x63c>");
    #/
    flag_set("next_event");
}
...
/#
    ...
    function spawncompare(spawnpoint, spawnteam) {
        if (getdvarint(@"hash_b5ceec3e3cb419be") != 0) {
            ...
            println("<dev string:x662>" + scriptspawnpoint.index + "<dev string:x67a>" + scriptspawnpoint.totalscore);
            if (scriptspawnpoint.index == spawnpoint.index) {
                level.var_2dc7108159dc1da3++;
                iprintlnbold("<dev string:x68d>" + level.var_2dc7108159dc1da3 / level.var_3d292942d52b104a * 100 + "<dev string:x6a9>" + "<dev string:x6b4>" + level.var_2dc7108159dc1da3 + "<dev string:x6b6>" + level.var_3d292942d52b104a);
            } else {
                level.var_a065445ba781ddf6++;
                iprintlnbold("<dev string:x6b8>" + level.var_2dc7108159dc1da3 / level.var_3d292942d52b104a * 100 + "<dev string:x6a9>" + "<dev string:x6b4>" + level.var_2dc7108159dc1da3 + "<dev string:x6b6>" + level.var_3d292942d52b104a);
            }
...
function function_2e8e7982711d50f2() {
    ...
    data.damage = 0;
    while (true) {
        damage, attacker, direction_vec, point, meansofdeath, modelname, tagname, partname, idflags, weapon, origin, angles, normal, var_13ea67fc910045ad = self waittill("damage");
        hitloc = "torso_upper";
        ...
        scaleddamage = int(floor(damage * function_77011f86063f18b4(data.weapon, hitloc)));
...
function function_396516aef4db32c0(var_caf1c9236bfa0831) {
    ...
    if (cantakedamage) {
        ...
        if (self.health <= 0) {
            ...
            /#
                level.player iprintln("<dev string:x6e5>");
            #/
    ...
    if (self.failhit >= 3) {
        /#
            level.player iprintln("<dev string:x704>");
        #/
...
/#
    ...
    function function_13f93acfa5b5c378() {
        scripts\common\devgui::function_6e7290c8ee4f558b("<dev string:x732>");
        scripts\common\devgui::function_a9a864379a098ad6("<dev string:x748>", "<dev string:x759>", &function_c7926861b25a8d49);
