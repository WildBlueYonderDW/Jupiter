...
function cacheplayeraction(actionbit) {
    ...
    self.analyticslog.cachedactions = self.analyticslog.cachedactions | actionbit;
}
...
function recordbreadcrumbdata() {
    ...
    while (true) {
        ...
        starttime = gettime();
        for (startplayer = 0; startplayer < playercount; startplayer = startplayer + var_db5e61a22ce91ab9) {
...
function buildkilldeathactionvalue() {
    ...
    if (self isjumping()) {
        actionvalue = actionvalue | 4;
    }
    if (isdefined(self.lastshotfiredtime) && gettime() - self.lastshotfiredtime < 500) {
        actionvalue = actionvalue | 8;
    }
    if (self isreloading()) {
        actionvalue = actionvalue | 16;
    }
    return actionvalue;
}
