    ...
    foreach (loc in locs) {
        ...
        /#
            if (getdvarint(@"hash_1af6b48d9e65d84c", 0) == 1) {
                ...
                print3d(struct.origin, "<dev string:x1c>" + struct.origin[0] + "<dev string:x1f>" + struct.origin[1] + "<dev string:x1f>" + struct.origin[2] + "<dev string:x22>", (1, 1, 1), 1, 1, 9999, 1);
...
function function_e28bb63a5e8e8d12() {
    ...
    while (true) {
        ...
        if (getdvarint(@"hash_13fae9c305971d06", 0) == 1) {
            ...
            linecount = 0;
            /#
                printtoscreen2d(startx, starty + stepy * linecount, "<dev string:x24>", (1, 1, 1), 1);
            #/
            linecount++;
            /#
                printtoscreen2d(startx, starty + stepy * linecount, "<dev string:x33>" + level.var_48e03f24b9b5c1a, (1, 1, 1), 1);
            #/
            linecount++;
            /#
                printtoscreen2d(startx, starty + stepy * linecount, "<dev string:x49>" + var_ec823f5f10f705ac, (1, 1, 1), 1);
            #/
            linecount++;
            /#
                printtoscreen2d(startx, starty + stepy * linecount, "<dev string:x5a>" + buffer, (1, 1, 1), 1);
            #/
            linecount++;
            var_12ddf4a9082aaa2a = max(0, (var_3c1586d15c611ae.var_38ef642aede44556 + var_11c5e87250ca0bf9 - currenttime) / 1000);
            /#
                printtoscreen2d(startx, starty + stepy * linecount, "<dev string:x6a>" + var_12ddf4a9082aaa2a, (1, 1, 1), 1);
            #/
            linecount++;
            var_12ddf4a9082aaa2a = max(0, (var_3c1586d15c611ae.var_38ef642aede44556 + var_5f857334b93b7b81 - currenttime) / 1000);
            /#
                printtoscreen2d(startx, starty + stepy * linecount, "<dev string:x77>" + var_12ddf4a9082aaa2a, (1, 1, 1), 1);
            #/
            linecount++;
            /#
                printtoscreen2d(startx, starty + stepy * linecount, "<dev string:x8a>" + var_88f3d778efe40c6e, (1, 1, 1), 1);
            #/
            linecount++;
            /#
                printtoscreen2d(startx, starty + stepy * linecount, "<dev string:xaa>" + var_c732ac2a579c99b1 + "<dev string:xbe>" + var_88f3d778efe40c6e * axisscalar, (1, 1, 1), 1);
            #/
            linecount++;
            /#
                printtoscreen2d(startx, starty + stepy * linecount, "<dev string:xc2>" + var_8c6bcd8c000b8c10 + "<dev string:xbe>" + var_88f3d778efe40c6e * alliesscalar, (1, 1, 1), 1);
            #/
            linecount++;
        }
...
function function_e62dcabb18095abb() {
    while (true) {
        ...
        for (key = 0; key < level.frontlinedata.size; key++) {
            ...
            /#
                print3d(data.origin, "<dev string:xd8>" + key, color, 1, 20, 1, 1);
            #/
            prev = undefined;
            for (laneKey = 0; laneKey < data.lanedata.size; laneKey++) {
                lanedata = data.lanedata[laneKey];
                color = ter_op(level.var_48e03f24b9b5c1a == key, (0, 1, 0), (1, 1, 1));
                /#
                    print3d(lanedata.origin, "<dev string:xe4>" + laneKey, color, 1, 10, 1, 1);
                #/
                foreach (team, locs in lanedata.coverzones) {
                    ...
                    foreach (loc in locs) {
                        ...
                        /#
                            print3d(loc.origin, "<dev string:xeb>" + key + "<dev string:xf6>" + laneKey + "<dev string:xf8>" + team, color, 1, 5, 1, 1);
                        #/
                    }
                }
                foreach (team, loc in lanedata.backvolume) {
                    ...
                    level thread scripts\cp_mp\utility\debug_utility::drawline(lanedata.origin, loc.origin, level.framedurationseconds, color);
                    /#
                        print3d(loc.origin, "<dev string:xfb>" + key + "<dev string:xf6>" + laneKey + "<dev string:xf8>" + team, color, 1, 5, 1, 1);
                    #/
                }
                foreach (team, loc in lanedata.frontvolume) {
                    ...
                    level thread scripts\cp_mp\utility\debug_utility::drawline(lanedata.origin, loc.origin, level.framedurationseconds, color);
                    /#
                        print3d(loc.origin, "<dev string:x107>" + key + "<dev string:xf6>" + laneKey + "<dev string:xf8>" + team, color, 1, 5, 1, 1);
                    #/
                }
    ...
    function function_7bfc015c3d7ccaf8() {
        foreach (frontline in level.frontlinedata) {
            for (i = 0; i < frontline.lanedata.size; i++) {
                frontline.lanedata[i].backvolume["<dev string:x114>"] thread draw_bounds((1, 0, 0));
                frontline.lanedata[i].frontvolume["<dev string:x114>"] thread draw_bounds((1, 0, 0));
                frontline.lanedata[i].backvolume["<dev string:x119>"] thread draw_bounds((0.5, 0.5, 1));
                frontline.lanedata[i].frontvolume["<dev string:x119>"] thread draw_bounds((0.5, 0.5, 1));
            }
        }
    }
    ...
    while (true) {
        ...
        foreach (team in level.teamnamelist) {
            ...
            foreach (laneKey, lanedata in level.agentmanager[team]) {
                linecount = 1;
                /#
                    printtoscreen2d(startx, starty + stepy * linecount, "<dev string:x120>" + laneKey + "<dev string:x12a>", (1, 1, 1), 1);
                #/
                linecount++;
                foreach (index, agent in lanedata.var_e2c31580f508b878) {
                    ...
                    /#
                        printtoscreen2d(startx, starty + stepy * linecount, "<dev string:x12c>" + index + "<dev string:x12e>" + agent getentitynumber(), color, 1);
                    #/
                    linecount++;
                }
                startx += 100;
...
function function_ae56674143e0ec69() {
    ...
    while (true) {
        ...
        /#
            printtoscreen2d(300, 50, "<dev string:x132>" + axiscount, (0, 0, 1), 2.5, 1);
            printtoscreen2d(300, 100, "<dev string:x13f>" + alliescount, (0, 0, 1), 2.5, 1);
