    ...
    if (isdefined(victim.attackers) && victim.attackers.size == 1 && isdefined(victim.guid) && !isdefined(victim.attackers[victim.guid])) {
        /#
            if (!isdefined(victim.attackers[self.guid])) {
                println("<dev string:x2b>" + getcompleteweaponname(objweapon));
                println("<dev string:x34>" + meansofdeath);
                println("<dev string:x43>" + self.guid + "<dev string:x53>" + self.name + "<dev string:x5c>");
                i = 0;
                foreach (key, value in victim.attackers) {
                    println("<dev string:x5e>" + i + "<dev string:x70>" + key + "<dev string:x53>" + value.name + "<dev string:x5c>");
                    i++;
                }
            }
        #/
    ...
    /#
        foreach (event in self.splashpriorityqueue) {
            if (splashallowed && istrue(event.showassplash) && event.alwaysshowsplash) {
                println("<dev string:x78>" + function_3c8848a3a11b2553(event.scoreeventref));
            }
            println("<dev string:x82>" + function_3c8848a3a11b2553(event.scoreeventref));
        }
    #/
    foreach (event in self.splashpriorityqueue) {
