...
function adddeathicon(attacker, entity, dyingplayer, team, timeout) {
    ...
    if (isbr) {
        ...
        if (!isdefined(sessionnumber) || sessionnumber < 0) {
            /#
                iprintln("<dev string:x1c>" + dyingplayer.name + "<dev string:x44>");
            #/
            return;
        }
...
function function_b3ff47edf02118d1(deathiconlocation) {
    ...
    var_fbf7a3d9a735ec16 = player br_public::isplayerinorgoingtogulag() || player br_public::function_8f0e6614368f64c2();
    if (!scripts\mp\utility\player::isreallyalive(player) && !br_public::isrespawningfromtoken(player) && !var_fbf7a3d9a735ec16) {
        return;
    }
    ...
    for (var_ed636cd9ad93833d = 0; !var_ed636cd9ad93833d; var_ed636cd9ad93833d = 1) {
        wait 1;
        if (player isonladder() || player ishanging() || player isonground() || player isswimming() || player isswimunderwater()) {
        }
    }
