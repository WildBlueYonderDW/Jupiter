...
function private function_403e79a58ca142a3(damageinfo, damagefactor) {
    if (!isdefined(damageinfo.damagefactor)) {
        ...
        damageinfo.damagefactor *= damagefactor;
        /#
            damageinfo.debugstring += "<dev string:x1d>";
        #/
    }
    /#
...
function private collision_damage_watcher() {
    ...
    while (true) {
        ...
        if (gettime() - self.borntime < 5000) {
            /#
                collision_debug(position, normal, normalspeed, 0, "<dev string:x1f>");
            #/
            continue;
        }
        if (isdefined(ent) && istrue(ent.iscrossbowbolt)) {
            /#
                collision_debug(position, normal, normalspeed, 0, "<dev string:x28>");
            #/
            continue;
        }
        if (isdefined(ent) && isdefined(ent.helperdronetype) && ent.helperdronetype == "radar_drone_recon") {
            /#
                collision_debug(position, normal, normalspeed, 0, "<dev string:x37>");
            #/
            continue;
        }
        if (istrue(self.var_a1a5d39c25c6d70b)) {
            /#
                collision_debug(position, normal, normalspeed, 0, "<dev string:x49>");
            #/
            continue;
        }
        ...
        /#
            collision_debug(position, normal, normalspeed, dmgval, "<dev string:x5d>" + damagefactor + "<dev string:x5f>" + damageinfo.debugstring + "<dev string:x61>");
        #/
        wait 0.5;
    }
}
...
/#
    ...
    function function_6ae603e37e7a394f(position, normal, normalspeed, damage, debugstring) {
        ...
        self notify("<dev string:x69>" + level.var_569260d6901267c4);
        self endon("<dev string:x69>" + level.var_569260d6901267c4 + 10);
