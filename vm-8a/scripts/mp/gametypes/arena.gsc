...
function initspawns() {
    ...
    if (var_1cf8be03fb3c42de.size > 0 || var_68c73806e24ff493.size > 0) {
        ...
        /#
            validatespawns("<dev string:x52>");
            validatespawns("<dev string:x67>");
        #/
    }
...
function checkshouldallowtradekilltie(winningteam) {
    ...
    foreach (entry in level.teamnamelist) {
        println("<dev string:x7c>" + entry + "<dev string:x82>" + getteamdata(entry, "<dev string:x91>"));
        if (!getteamdata(entry, "aliveCount") && !livescount[entry]) {
            println("<dev string:x9c>" + entry);
...
function private function_ceaf8a81ff311f20(weaponname, loadout) {
    ...
    assertex(isdefined(slotstring), "The specified weapon '" + weaponname + "' does not match any weapon defined in the loadout.");
    ...
    if (scripts\mp\gametypes\arena::israndomloadouts()) {
        /#
            if (getdvarint(@"hash_b665ceb14dabc0ed", -1) != -1) {
                game["<dev string:xe6>"] = array_randomize(game["<dev string:xf9>"]);
                game["<dev string:x107>"] = 0;
                return;
            }
        #/
        if (game["roundsPlayed"] == 0) {
...
function setupweapons() {
    ...
    foreach (loc in locs) {
        /#
            debugspawnweapon = getdvar(@"hash_e7ea6273ed7f950f", "<dev string:x11f>");
            if (debugspawnweapon != "<dev string:x11f>") {
...
function spawnweapon(loc, weaponstruct, var_5f3b26e2789a42c0) {
    ...
    if (isdefined(equipmentinfo)) {
        if (istrue(level.snowballfight)) {
            ...
            /#
                if (is_equal(weapontier, "<dev string:x120>")) {
...
function private getrandomweaponforweapontier(weapontier) {
    if (issubstr(weapontier, "rand")) {
        ...
        assertex(isdefined(tokens) && tokens.size == 2 && tokens[0] == "rand", "Invalid random weapon specifier: " + weapontier);
        weaponcategory = scripts\mp\gametypes\arena::function_f8be37b178efc473(tokens[1]);
        assertex(isdefined(weaponcategory), "Invalid random weapon category \"" + weaponcategory + "\" in weapon specifier: " + weapontier);
...
function function_b7c18ded2347456c() {
    ...
    foreach (weaponcategory in weaponcategories) {
        ...
        assertex(isdefined(var_e1898741919f6aaf), "Unable to find script bundle associated with weapon table: " + weapontablename);
...
function private function_c20500181ea66dc8(classtableentry) {
    ...
    assertex(isdefined(var_344c92bda1a2ddfb), "Unable to find script bundle associated with class table entry: " + classtableentry);
...
/#
    ...
    function private function_ee47e658f5b6314e() {
        ...
        setdvar(@"hash_886f8f08dbf3df5f", "<dev string:x11f>");
        while (true) {
            waitframe();
            dvarvalue = getdvar(@"hash_886f8f08dbf3df5f");
            if (dvarvalue == "<dev string:x11f>") {
                continue;
            }
            setdvar(@"hash_886f8f08dbf3df5f", "<dev string:x11f>");
            tokens = strtok(dvarvalue, "<dev string:x145>");
            ...
            case #"hash_9d9755ce454b0010": 
                if (arglist.size == 0) {
                    player notify("<dev string:x1b6>");
                    player notify("<dev string:x1d8>");
                } else if (arglist.size < 1 || int(arglist[0]) < 1) {
            ...
            case #"hash_4db5d4970492f541": 
                if (arglist.size == 0) {
                    player notify("<dev string:x1d8>");
                } else if (arglist.size < 2 || arglist[0] == "<dev string:x11f>" || int(arglist[1]) < 1) {
                    iprintlnbold("<dev string:x247>");
                } else {
                    thread function_d70a085b24e94e4b(player, arglist[0], int(arglist[1]));
                }
                break;
            default: 
                iprintlnbold("<dev string:x287>" + command);
                break;
            }
        }
    }
    ...
    function private function_c452035f58a6ad0e(player, timeinterval) {
        level endon("<dev string:x13a>");
        player endon("<dev string:x299>");
        player notify("<dev string:x159>");
        player endon("<dev string:x159>");
        if (!isdefined(game["<dev string:xf9>"])) {
            error("<dev string:x2ad>");
            return;
        }
        level.perks_suppressasserts = 1;
        thread function_bac839c909252aeb(player, "<dev string:x159>", player.pers["<dev string:x2e0>"]);
        for (i = 0; i < game["<dev string:xf9>"].size; i++) {
            player.pers["<dev string:x2e0>"] = game["<dev string:xf9>"][i];
            player scripts\mp\class::giveloadout(player.team, "<dev string:x2f0>");
            wait timeinterval;
        }
        player notify("<dev string:x159>");
    ...
    function private function_5dd6bdb4dfced5ba(player, timeinterval) {
        ...
        player endon("<dev string:x299>");
        player notify("<dev string:x1b6>");
        player endon("<dev string:x1b6>");
        if (!isdefined(game["<dev string:x2f9>"])) {
            error("<dev string:x306>");
            return;
        }
        foreach (weapon in game["<dev string:x2f9>"]) {
    ...
    function private function_d70a085b24e94e4b(player, weaponname, timeinterval) {
        ...
        player endon("<dev string:x299>");
        player notify("<dev string:x1d8>");
        player endon("<dev string:x1d8>");
        if (!isdefined(game["<dev string:x2f9>"])) {
            error("<dev string:x354>");
            return;
        }
        weapon = game["<dev string:x2f9>"][weaponname];
        if (!isdefined(weapon)) {
            error("<dev string:x3a5>");
            return;
        }
        ...
        thread function_a554b9ddc3d15ec5(player, "<dev string:x1d8>", primaryweaponobject);
    ...
    function private function_bac839c909252aeb(player, notifystring, loadout) {
        ...
        player.pers["<dev string:x2e0>"] = loadout;
        player scripts\mp\class::giveloadout(player.team, "<dev string:x2f0>");
    }
