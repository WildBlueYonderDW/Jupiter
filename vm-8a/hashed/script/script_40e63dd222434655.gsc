    ...
    while (true) {
        ...
        if (isdefined(level.br_circle) && isnavmeshloaded() && !bot_is_remote_or_linked()) {
            ...
            case #"hash_b276405783bce7ac": 
                if (self isskydiving() && (istrue(self.jumped) || wasprematch)) {
            ...
            case #"hash_f4773cddc9292f61": 
                ...
                break;
            default: 
                assertex(0, "The bot's next status is " + nextstatus + " but we have no logic for it.");
                break;
            }
        }
        wait 0.05;
        ...
        if (isdefined(landingspot)) {
            ...
            /#
                if (isdefined(zoneindex) && isdefined(level.botzones.zones[zoneindex])) {
                    println("<dev string:x62>" + level.botzones.zones[zoneindex].name + "<dev string:x8c>");
                } else {
...
function function_b7799ae6d79c86c9() {
    if (namespace_ca7b90256548aa40::issquadmode()) {
        ...
        if (isdefined(zoneindex)) {
            println("<dev string:x109>", "<dev string:x12d>", self.team, "<dev string:x134>", self.sessionsquadid, "<dev string:x145>", self.name, "<dev string:x14c>", zoneindex);
        } else {
...
function function_e39354ebeca71c56() {
    if (isdefined(level.botzones) && level.botzones.sumweight > 0) {
        /#
            debugzonename = getdvar(@"hash_a2fdccd1d3ae6e9e", "<dev string:x158>");
            if (debugzonename != "<dev string:x158>") {
                foreach (index, zone in level.botzones.zones) {
...
function parachute(isprematch) {
    ...
    while (self isskydiving()) {
        ...
        var_b2bee1aed960f257 = bot_get_angles_to_goal(self, self.desiredlandingspot);
        speed = 1;
        ...
        function_c43469e7a5ae4ba4(self.desiredlandingspot, 0.05);
...
/#
    ...
    function function_fd1265363788e726() {
        self endon("<dev string:x159>");
        level endon("<dev string:x16d>");
        while (true) {
            if (isdefined(self.desiredlandingspot)) {
                ...
                print3d(self.desiredlandingspot, self.name + "<dev string:x17b>", var_b792c387402095b1, 1, 1, 5, 1);
            }
            wait 0.05;
        }
    }
...
function function_ac55a70c78ec6aa5() {
    ...
    if (verticalspeed > 50 && var_1a094e3a2254b084 > 200) {
        ...
        if (var_4cf0e4661298ed8a < var_8105af709f0adc35 && var_4cf0e4661298ed8a > 1 && var_3840f75fd5fc4336 > 0) {
            ...
            /#
                if (getdvarint(@"hash_ba0d232bde107bad") > 0) {
                    println("<dev string:x18b>" + self.entity_number + "<dev string:x191>" + "<dev string:x195>" + "<dev string:x1c1>" + self.desiredlandingspot[0] + "<dev string:x1c7>" + self.desiredlandingspot[1] + "<dev string:x1c7>" + self.desiredlandingspot[2] + "<dev string:x1c9>" + self.origin[0] + "<dev string:x1c7>" + self.origin[1] + "<dev string:x1c7>" + self.origin[2] + "<dev string:x1cf>" + var_4cf0e4661298ed8a + "<dev string:x1cf>" + var_8105af709f0adc35 + "<dev string:x1de>" + var_2127d880e420797f);
                }
            #/
            ...
            if (isdefined(var_365b4b90d1e6a26c)) {
                ...
                /#
                    if (getdvarint(@"hash_ba0d232bde107bad") > 0) {
                        ...
                        debugmsg = "<dev string:x1ed>" + var_365b4b90d1e6a26c[0] + "<dev string:x1c7>" + var_365b4b90d1e6a26c[1] + "<dev string:x1c7>" + var_365b4b90d1e6a26c[2] + "<dev string:x1ef>";
        ...
        while (true) {
            ...
            if (!self isparachuting() && var_d459f6d98bfb8aad < var_966b732f6569a932) {
                ...
                if (var_4e25ddb1d88d3b34) {
                    ...
                    randompoint = self.desiredlandingspot + offset;
                    newpos = function_f8350ab882cc2439(randompoint);
                    if (function_bcb0778bb33e86de(newpos) && scripts\engine\trace::ray_trace_passed(self geteye(), newpos, self)) {
                        self.desiredlandingspot = newpos;
                        self.var_b7c77afce58bcac6 = 2147483647;
                    } else {
                        var_5a131f8f7a5add85 = 1000;
...
// Size: 0xd6
function function_8673b301658f7e88(weaponbasename, rarity) {
    ...
    foreach (fullweaponname, var_6180da62ad446599 in level.br_pickups.br_weapontoscriptable) {
        if (issubstr(fullweaponname, weaponbasename) && issubstr(var_6180da62ad446599, rarity)) {
            return fullweaponname;
        }
    }
    if (scripts\mp\utility\game::getsubgametype() != "aladdin" && !scripts\cp_mp\utility\game_utility::function_6493ec89ae923684()) {
        assertmsg("No weapon can be selected ( " + weaponbasename + ", " + rarity + " ). ");
    }
    return undefined;
}
...
/#
    ...
    function function_c8cc3e3589f34923(table) {
        ...
        var_abb1091bb76019bc["<dev string:x1f1>"] = 0;
        var_abb1091bb76019bc["<dev string:x1f6>"] = 1;
        var_abb1091bb76019bc["<dev string:x1fb>"] = 2;
        var_abb1091bb76019bc["<dev string:x200>"] = 3;
        var_abb1091bb76019bc["<dev string:x205>"] = 4;
        if (isdefined(table)) {
            var_7d958726e11b327 = scripts\mp\class::function_df2933f96d726d71(table);
            for (i = 0; i < var_7d958726e11b327; i++) {
                struct = scripts\mp\class::loadout_getclassstruct();
                struct = scripts\mp\class::function_1d52554d28b51cbc(struct, i, table);
                var_e2154dbde4bd1cac = strtok(struct.loadoutprimary, "<dev string:x20a>");
                var_63cbf0be53790618 = strtok(struct.loadoutsecondary, "<dev string:x20a>");
                ...
                if (!istrue(level.var_a9350d89a2e4d752)) {
                    ...
                    for (row = 0; row < lastrow; row++) {
                        if (tablelookupbyrow(level.brloottablename, row, 0) != "<dev string:x20c>") {
                            continue;
                        }
                        ...
                        if (baseweaponname == var_e2154dbde4bd1cac[0] && rarity == var_abb1091bb76019bc[var_e2154dbde4bd1cac[1]]) {
                            primaryweaponexist = 1;
                        }
                        if (baseweaponname == var_63cbf0be53790618[0] && rarity == var_abb1091bb76019bc[var_63cbf0be53790618[1]]) {
                    ...
                    assertex(primaryweaponexist, "<dev string:x211>" + struct.loadoutprimary + "<dev string:x21a>");
                    assertex(secondaryweaponexist, "<dev string:x211>" + struct.loadoutsecondary + "<dev string:x21a>");
                    assertex(var_9eea23b5420bb92e, "<dev string:x240>" + struct.loadoutequipmentprimary + "<dev string:x21a>");
                    assertex(var_adc7a698896266e, "<dev string:x240>" + struct.loadoutequipmentsecondary + "<dev string:x21a>");
                    continue;
                }
                ...
                assertex(primaryweaponexist, "<dev string:x211>" + struct.loadoutprimary + "<dev string:x24c>");
                assertex(secondaryweaponexist, "<dev string:x211>" + struct.loadoutsecondary + "<dev string:x24c>");
                assertex(var_9eea23b5420bb92e, "<dev string:x240>" + struct.loadoutequipmentprimary + "<dev string:x24c>");
                assertex(var_adc7a698896266e, "<dev string:x240>" + struct.loadoutequipmentsecondary + "<dev string:x24c>");
            }
        }
    }
...
function function_bbc2d6967287dc7(lootitemname, valuetable) {
    ...
    assertex(isdefined(loottype), "The loot name is: " + lootitemname);
    switch (loottype) {
    case #"ammo": 
    ...
    case #"weapon": 
        ...
        if (!isdefined(classname)) {
            ...
            assertmsg(lootitemname + " does not have fullweaponobj.classname");
        } else {
...
function function_5ba1e582607305e4(slot, lootname, valuetable) {
    ...
    if (!isdefined(var_58f7650e8def9a65)) {
        println("<dev string:x28a>" + equippedloot + "<dev string:x299>");
        return 0;
    }
...
function function_6dd2dfeb41384764() {
    assertex(scripts\cp_mp\utility\game_utility::isbrstylegametype(), "MixedSquadSupported() is meant to be called in BR mode!");
    if (!namespace_ca7b90256548aa40::issquadmode()) {
        return 0;
    }
    if (scripts\mp\utility\game::matchmakinggame()) {
...
function nearestplace(places, inputarray, var_330c401bd2801f95, var_3b233cde4adf9cd0, var_bef59d5b603763b0, var_6681cebd53b80d74) {
    ...
    foreach (place in places) {
        ...
        if (isdefined(var_bef59d5b603763b0)) {
            var_7d9a6d00a336393b = namespace_bbc79db4c3949a5c::gettimetilldangerforpoint(place.origin);
...
function function_b5e22620a8778c4a(arena) {
    ...
    foreach (loser in losers) {
        ...
        println("<dev string:x2bb>" + loser.name + "<dev string:x2e1>");
    }
}
