...
function function_b0afa182f3914d4f(uniquename) {
    ...
    assertex(isdefined(ProgressTracker), "Trying to end " + uniquename + " progress tracker but that was not found on this activity.");
    function_2d8ae6d7b07724e2(ProgressTracker);
}
...
function function_9ff369ea020a30a6(uniquename, pausetimer) {
    ProgressTracker = self.var_e3d833b1e5d30b03[uniquename];
    assertex(isdefined(ProgressTracker), "Trying to pause " + uniquename + " progress tracker but that was not found on this activity.");
    assertex(isdefined(ProgressTracker.paused), "Trying to pause " + uniquename + " progress tracker but this is not a time-based tracker.");
