...
function manualturret_watchuse(owner, triggerobj) {
    ...
    while (true) {
        ...
        if (player == owner) {
            for (buttontime = 0; player usebuttonpressed() && buttontime < 0.25; buttontime += level.framedurationseconds) {
                waitframe();
            }
            if (buttontime >= 0.25) {
...
function manualturret_watchpickup(owner) {
    ...
    while (isdefined(self) && isdefined(self.useownerobj)) {
        ...
        for (buttontime = 0; owner usebuttonpressed() && buttontime < 0.25; buttontime += level.framedurationseconds) {
            waitframe();
        }
        if (!isdefined(player)) {
            continue;
